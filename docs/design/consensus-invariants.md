...
## I5 — Deterministic fork-choice among finalized descendants; every consensus-derived quantity is a pure function of hash-covered content; every safety violation is attributable (an honest node is never slashed)

**Statement.** Fork-choice is a **deterministic total order** (**height → head-hash**), evaluated **only over descendants of the latest finalized block.** Both terms are read from the signed block body (`Height` is inside `Hash()`'s unsigned struct; the head hash *is* the body digest), so head selection is a pure function of the committed chain and of nothing a replica holds privately.

**A weight term is deliberately absent** (owner-ratified 2026-09-03, O3 Direction T). silt commits at an intersecting quorum and treats the committed head as final (the I4 ruling, `:109`), so the pre-finality reorg gate admits only forks that CONTAIN the committed head — a set on which any pure, extension-monotone chain-weight function selects the same head as height. The GHOST-family weight term is the fork-choice organ of the commit/final decoupling silt declined to build; retiring it gives up no defense. #357's D2 denial — *two histories cannot both stand* — is delivered by I1's intersecting quorum plus the finality gate, which is stronger than heaviest-chain heal: the losing side cannot commit at all, so there is no second history to reconcile away.

**And every consensus-derived quantity — the committed state transition AND fork choice — is a pure function of (committed pre-state, `Hash()`-covered block content).** Certificates (`Atts`, `PrepareQC`, `CommitRound`) are **validity** inputs, never **transition** or **fork-choice** inputs. Two replicas holding the same committed chain and *different but valid* certificates for its blocks must compute identical accepted state and select the identical head.
...
**Scars:**
- **#357** — among zero-weight ramp forks, `heavier` fell to a **height-blind head-hash tiebreak** → committed blocks dropped by hash luck (non-deterministic-looking, and reorg-of-final). Closed by the **§3 quorum-finality gate** plus the **§1b height preference**, both preserved. The §1a anchor bootstrap weight that shipped alongside them went inert with the era-2 signature change (#432, 2026-08-16) and is retired (O3 Direction T); the scar no longer depends on a weight signal existing.
...
- **Genesis seating (2026-09-04).** `AppendGenesis` seated genesis `Atts` **without verifying their signatures**, and `Atts` sit outside the `Hash()` preimage — so a relaying peer could seat a phantom into `validatorsSeen` (an era-3 root leaf) from zero key material and diverge a fresh-sync victim's committed root. Fix (`D-GENESIS-ATTS-SEATING`): a genesis seats only attestations that verify over its hash, strips the rest, never refuses. The same hash-coverage discipline as the two scars below, at height 0.
- **R0.6 (2026-09-03) — the cross-height `Pruned` slash forgery.** The accountability predicate **quantified over a fact (`Height`) outside the signed message**: `VerifyEquivocation` read the height from a struct field but the signed digest from `Hash()`, which returns the accuser-supplied `Pruned` for the two evidence blocks. Two genuine signatures at two different heights, re-labelled with one height, convicted an honest validator through `Append` — in era 1 and era 2 — and a Byzantine *peer* sufficed, because an honest node queued the forgery on detection. The exhaustive I5 oracle never saw it: it fuzzed one height, never set `Pruned`, and was era-2-only. Fix: evidence hashes are always recomputed from the body and a pruned evidence block is refused (`D-F2-EVIDENCE-RECOMPUTE`).
- **R-BOX-ATTESTS (2026-09-02) — the transition site.** The committed transition wrote `validatorsSeen` from the block's own **uncovered** `Atts`, so the recomputed state root depended on which certificate a replica held. **Code site:** `core/chain/chain.go` — `apply`'s attestation loop; `Hash()`'s unsigned struct, which excludes `Atts`/`PrepareQC`/`CommitRound`.
- **State-root domain separation (R3.1, 2026-09-06).** The floor box's recompute and witness paths rest on
  SMT proof unforgeability against the committed `StateRoot` — a property OUTSIDE I1–I5 (this file does not
  read `StateRoot`; the committed root is attester-signed). The certified argument, its seven scope
  invariants and gates, and the two latent fold-surface defects closed behind gates are recorded in
  [`state-root-domain-separation.md`](state-root-domain-separation.md).
- **The dead fork-choice weight (2026-09-03) — the fork-choice site, and #558 at its THIRD verify site.** `blockWeight` verified attestations against the **bare block hash** (era-1) while every production attestation has signed `consensusSigBytes(phase, round, hash)` since #432. The term was identically **0** on every block any binary mints; shipped fork-choice was silently `height → head-hash` for 18 days. **Retired, not repaired:** a signature-only repair would have made fork-choice a function of the evaluating replica's *live* qualification and bond state (the #357 oscillation mechanism, verbatim), and any correctly built weight is provably redundant inside the finality gate. The rule the scar encodes: **a domain-separation change is a property of the SCHEME — every verifier moves together, or the missed one silently rejects every valid signature and the path it guards goes dead.** **Code site:** `core/chain/chain.go` — `blockWeight` (deleted), `heavier`.

**Governs (code):** `core/chain/chain.go` — `heavier` (height → head-hash), `Reconcile` and its pre-finality gate, `Hash()`'s unsigned struct, `apply`'s attestation loop; slash path `core/chain/equivocation.go` (`CheckEquivocation`, `FindEquivocations` — one body-recomputed hash for both), `core/chain/chain.go` `validateSlashes` (+ `SlashesBytesCap`), `core/credit/credit.go` `SlashEquivocation`.

**Assert (test):** fork-choice is a **pure function** (replay determinism — same inputs → same head on every replica): `TestModelCheck_357_ForkChoiceIsOrderIndependent` (order-independence) plus the certificate-variant oracle (two replicas holding the same committed chain and *different but valid* certificates — a same-round extra precommit; a different-round `(PrepareQC, Atts, CommitRound)` triple — select the identical head **and** compute the identical accepted state: `TestO3T_CertificateVariantNeverRanksHeavier`, `TestO3T_ForkChoiceIsCertificateIndependentWithoutFinality`) plus the fast-path/slow-path equivalence (`appendExtension` and `reconstructFork → Reconcile` yield the same head on the same served window) — `TestO3T_FastSlowPathSameHead`, `core/node` `TestO3T_NodeFastSlowPathSameHead`; **`heavier` reads no field outside `Height` and the head `Hash()`** (the fork-choice purity pin, AST-walked, with a teeth test) — `TestO3T_HeavierReadsOnlyHeightAndHeadHash`, `TestO3T_HeavierPinHasTeeth`; **no attestation is verified in non-test `core/chain` outside `verifyAtt` and the era-1-gated `signedBlock`** (the #558 verifier-inventory pin, with a teeth test) — `TestO3T_VerifierInventoryPin`, `TestO3T_VerifierInventoryPinHasTeeth`, `TestO3T_Era2CertificateAcceptedByEveryEra2Verifier`; and **no honest schedule ever produces a slash** (the accountable-safety oracle — this is the direct catch for #397). **The accountable-safety oracle must vary the DECLARED height away from the SIGNED height, set `Pruned` to {unset, the real hash, another block's hash} on each side, and cover era 1 as well as era 2** (`TestModelCheck_I5_CrossHeightPrunedExtension_{Era1,Era2}`); the write-path gates drive the real `Append` (`core/chain/r06_i5_evidence_recompute_test.go`, `core/node/r06_i5_evidence_recompute_test.go`).

**Literature (B8):** Casper FFG accountable safety / slashing conditions; LMD-GHOST determinism. Tendermint/CometBFT and HotStuff carry **no fork-choice weight**: safety is the locking rule over an intersecting quorum, and the committed log is final, so head selection is the committed tip. Gasper's LMD-GHOST weight exists because Casper FFG finalizes up to two epochs behind the head — a commit/final decoupling silt does not have (I4 ruling), so the weight does not transfer.
