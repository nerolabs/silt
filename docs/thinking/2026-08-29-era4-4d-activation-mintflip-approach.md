...
|---|---|---|
| **Before activation: v4, era-3 freeze holds** | Below H_era4 the chain mints v4; the v4 committed root is byte-identical to era-3 (spine + era-4 scalars absent from the v4 leaf set). | Emit `tagEra4LockedIn`/`tagEra4Height` (or a spine leaf) in `stateRootLeaves` (the era-3 set) instead of `stateRootLeavesV5` → the DISJOINTNESS guard reddens AND a v4 block's root diverges from the era-3 replay. |
