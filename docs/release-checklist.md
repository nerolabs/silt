...
1. Move `CHANGELOG.md`'s `## [Unreleased]` to a dated `## [<version>] — <date>`
   section (this is the moment "unreleased" becomes real) and commit it.
   `website/changelog.html` is gitignored and regenerated by the deploy, so
   there is no page to commit (`D-WEBSITE-HTML-AT-DEPLOY-2026-09-12`).
2. Tag and push:
   ```sh
   git tag v<version>
