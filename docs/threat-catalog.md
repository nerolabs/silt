...
- F3 **Timing/size fingerprinting** ✗ — chunk sizes + timing fingerprint files even encrypted (no padding). **Sharpened 2026-09-09 by `R-SHORT-FINAL-STRIPE`, in two directions, and NEITHER is mitigated here** (silt asserts no size hiding anywhere, so this is a catalog update, not a regression against a claim; a red-team pass is owed on the surface, and inventing a salt without one would be the unreviewed novelty B8 forbids). **(a) The exact plaintext byte length is public, keyless, permanent and unvalidated — and always was.** `ports.Entry.FileSize` (`ports/ports.go:138`) is the exact plaintext byte count. It rides inside `chain.Block.Entries` (`core/chain/chain.go`, cbor key 3), is inside the signing preimage in **both** the pre-era-4 and the era-4 branch of `bodyHash` (`core/chain/chain.go` — `Entries` is in the `unsigned` literal of each branch), replicates to every node, and **no validity rule reads it**: neither `(*Chain).ValidateEntry` (`core/chain/chain.go`) nor its floor-box mirror `v5ValidateEntry` (`core/chain/validate_v5_predicates.go`) mentions it, and the only size check on the read path reads the *manifest's* copy inside the sealed box (`pipeline.Get`'s `counter.n != m.FileSize`). Every registry adapter re-publishes it (`adapters/httpregistry` and `adapters/fileregistry` `entryJSON`, `cmd/silt/ui.go`'s chain-entry row). **The earlier claim that "padding blurred it to 'somewhere in one chunk'" was false**: a holder receives and persists `StorageProof.Root` with the chunk (`(*Node).hostShardLocally`, `core/node/node.go`) and a caretaker holds the root by construction, so both read the exact size off the chain regardless of framing. What `R-SHORT-FINAL-STRIPE` did change is narrower and real — the **stored shard** of a sub-frame object is now `L + 24` bytes instead of `chunkSize + 16`, so the length became legible to a party that measures byte counts *without* holding the root, such as a splicing relay (`peerConn.write`'s 4-byte length prefix and the relay splice, `adapters/tcpnet/tcpnet.go`). For that observer the chain's exact-length column is the **lookup table**, not a duplicate. **Bound — cite this, not the corpus number.** The field is an exact `int64` and sub-frame objects occupy `L ∈ [1, 262135]`, so it partitions that class into at most 262,135 cells: a channel capacity of ≈18 bits. Any corpus of `n` sub-frame objects has an expected singleton fraction ≈ `e^(−n/262135)` under a uniform distribution — **99.3 % at n = 1,779**. A measurement over 1,779 files of the silt working tree, *a software repository's length distribution and not a publish population*, came in **lower** at 79.6 % alone and 94.9 % in a set of ≤ 2, because real file lengths cluster; that corpus is therefore FRIENDLIER than uniform and the conclusion is forced by cardinality in the same direction for any realistic corpus. **The anonymity set of an object under its length is routinely ONE. silt does not try to raise it and does not claim to** — the earlier text named anonymity-set size as the bound and gave no floor, and a bound with no floor reads as a promise of *some* set. **(b) Chunk size is no longer an accidental salt against the F6 confirmation attack.** The same bytes published in convergent mode at 64 KiB, 256 KiB and 1 MiB now yield ONE root, measured (`core/pipeline` `TestConvergentDedupNowSpansTheChunkSize`); before, the padding made the ciphertext — and so the root and the link key — depend on the publisher's `-chunk-size`. Dedup improves by exactly the amount the guesser's job gets easier: a guesser needs the plaintext alone, no longer the plaintext AND the geometry. Bounded to SUB-FRAME objects — a multi-frame object's root still depends on the chunk size, and the same test drives that side.
...
- F8 **Encryption-mode oracle off the sealed-manifest frame length** ✓ — **CLOSED 2026-09-11.** `Mode` sits in the inner secrets box, which exists precisely so a care-link holder cannot read it (`core/manifest/sealed.go`). Because `pipeline.ManifestFrameSize` frames a single-chunk manifest at its true length, the frame length was an affine function of the sealed CBOR — `secretsPart` carries one 34-byte `ChunkSecrets` entry **per data chunk** in convergent mode against one flat 34-byte `FileKey` in private mode, and `crypto.SealBox` expands by a constant 16-byte tag (`crypto.Overhead`, `crypto.SealBox`). A peer with no keys, no care link, no bond and no token read `Entry.ManifestChunks` off the unauthenticated `MsgGetChain` (`core/node/chainrole.go`), fetched that one chunk over the unauthenticated `MsgFetchChunk` (`core/node/node.go`), and recovered the publisher's own secret/not-secret classification of every root on the chain — 341 B private vs 345 B convergent at `FileSize = 1`, the gap widening by 34 B per data chunk. **No prior disclosure covered this.** It was not a size leak and not a traffic observation; it was a **sealed field read by an unauthenticated stranger**, and by `cmd/silt/main.go`'s own framing (private is the default, convergent carries a loud warning) that field is the publisher's statement about whether the content needed protecting. **What closed it:** `manifest.secretsPlainLen` pads the secrets plaintext to a length that is a function of the **public data-shard count alone** before sealing — length-hiding authenticated encryption (Paterson–Ristenpart–Shrimpton, ASIACRYPT 2011), in the shape TLS 1.3 deploys it ([RFC 8446 §5.4](https://www.rfc-editor.org/rfc/rfc8446#section-5.4)). Cost ≈0.013 % of object bytes. Gated by `core/pipeline` `TestRT_SFO_1_ModeIsNotRecoverableFromManifestFrameLength` (seen RED first at 345 vs 341) and `core/manifest` `TestSealedSecretsLengthIsModeIndependent`, which closes it for the care-link holder too. **Two things that do NOT fix it, recorded so they are not re-proposed:** removing or moving the `Mode` field (`(*Manifest).Validate` already makes `ChunkSecrets`/`FileKey` the real discriminator), and re-padding the manifest *frame* (it lifts the oracle only while both modes fit one frame — beyond that `len(Entry.ManifestChunks)` separates them on-chain with no fetch at all).
