...
- F3 **Timing/size fingerprinting** ✗ — chunk sizes + timing fingerprint files even encrypted (no padding). **Sharpened 2026-09-09 by `R-SHORT-FINAL-STRIPE`, in two directions, and NEITHER is mitigated here** (silt asserts no size hiding anywhere, so this is a catalog update, not a regression against a claim; a red-team pass is owed on the surface, and inventing a salt without one would be the unreviewed novelty B8 forbids). **(a) An exact byte-length oracle for sub-frame objects.** An object that fits in one frame is now stored at its true length, so any holder reads its exact size from the stored shard and any caretaker from the layout's `ChunkSize`; before, padding blurred it to "somewhere in one chunk". The class is every object at or below `chunkSize − 9` (262,135 B at the shipped default). **(b) Chunk size is no longer an accidental salt against the F6 confirmation attack.** The same bytes published in convergent mode at 64 KiB, 256 KiB and 1 MiB now yield ONE root, measured (`core/pipeline` `TestConvergentDedupNowSpansTheChunkSize`); before, the padding made the ciphertext — and so the root and the link key — depend on the publisher's `-chunk-size`. Dedup improves by exactly the amount the guesser's job gets easier: a guesser needs the plaintext alone, no longer the plaintext AND the geometry. Bounded to SUB-FRAME objects — a multi-frame object's root still depends on the chunk size, and the same test drives that side.
