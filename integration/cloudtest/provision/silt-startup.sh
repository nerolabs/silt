...
# This was the symptom of: silt originally did a ONE-SHOT bootstrap, so a
# joining node whose -bootstrap target was not yet listening came up with an EMPTY
# routing table and NEVER re-bootstrapped. this gate is now FIXED IN-PRODUCT —
...
# NOTE: with this belt fastened, no flow exercises an empty-table join over the
# wire, so the in-product fix is not yet wire-verified. To drive it, set
# SKIP_BOOTSTRAP_WAIT=1 on a single joining validator and assert the
...
if [ "$${SKIP_BOOTSTRAP_WAIT:-0}" = 1 ]; then
  # Skip the TCP-wait belt so this node joins into an empty routing table and
  # must self-heal via the in-product -bootstrap-retry loop.
  echo "silt-startup: $${NODE} SKIP_BOOTSTRAP_WAIT=1 — no TCP-wait; relying on in-product -bootstrap-retry self-heal"
  BOOTSTRAP_REF=""
fi
