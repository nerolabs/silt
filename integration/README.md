...
| `floor/run.sh` | a validator on the declared floor spec (one core, 2 GiB, 10 GiB): validates and converges under a **kernel-enforced** memory ceiling with no swap (honest load; adversarial input not yet driven), sheds heavy bond proofs below the retention horizon, and restarts from the pruned store. The witness half of the floor-box claim — validating against witnesses without holding the tree, and stalling when no provider is reachable — is reported **UNWIRED**: `chain.NewBox` has no caller outside its own tests and nothing implements `chain.WitnessSource` in production, so there is no process to drive |
