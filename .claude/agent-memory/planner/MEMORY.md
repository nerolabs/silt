...
- [▶▶ SESSION RESUME (2026-08-29, session-13)](session-resume.md) — START HERE: RegCap DONE+MERGED (#635 @ main 5ea1401, N=256 total-count count-cap); 4a=PR #637 prepped/green; then 4b→4c→4d. ★ memory is uncommitted until a periodic PR.
...
- [Delegate reversible step-merges](feedback-delegate-reversible-step-merges.md) — Andrew delegates intermediate step-merges to the Planner post-blind-loop; ratifies only at the irreversible gate.
- [Periodic memory PR](feedback-periodic-memory-pr.md) — seat memory reaches main via a PERIODIC batched memory PR (not per-change, not Andrew direct); route a Bash seat, gather deliberately.
- [★ Billable-run orchestration playbook](billable-run-orchestration-playbook.md) — evergreen fleet-owner, default-PRESERVE, durable sink, re-attach, EXTERNAL watchdog. LEARNING DOC.
- [★ Orchestra leverage & loop health](orchestra-leverage-and-loop-health.md) — aim at silt's bottleneck (tester-depth + research-parallel, NOT builder throughput); seed the scar ledger; the 3 loop-health signals.
- [Standing deep-run loop](standing-deep-run-loop.md) — STOPPED pending isolated-clone redesign (it `git reset --hard`'d the main tree); discover via `integration/cloudtest/local-deep-*/`.
- [Keystone era-3 freeze sequencing](keystone-era3-freeze-sequencing.md) — era-3 freeze CRITICAL PATH; TWO-DEBT-LIST union rule; the same-root bond-reg order-dependence finding + fix.
