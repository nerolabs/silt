...
// The relay-lane prepayment anchor: the PRIMITIVE-tier RED-first gates. (one RSA key, two FDH domains — sound by BNPS
// one-more-inversion), §9, §5 (the domain string is a FORMAT CONSTANT the proof depends on: "pin byte-exact (the
// TestDemandFDHInputBindsTheEpochByteExactly twin); a change is a token-format version, never an edit"). Build shape: §1.1 (domain =
// "silt/blindrelay/fdh/v1", message = uint64BE(issueEpoch) ‖ serial, byte-identical to demandMsg) and §5 step 1 (three one-line
// wrappers over blindD / unblindD / verifyD).
//
// ABLATIONS that must redden: remove the domain string.
//
...
func signRelayAnchorAt(t *testing.T, k *rsa.PrivateKey, e uint64, serial []byte) []byte {
	t.Helper()
	blinded, secret, err := BlindRelayAnchor(rand.Reader, &k.PublicKey, testChain, e, serial)
	if err != nil {
		t.Fatalf("BlindRelayAnchor: %v", err)
	}
	sig, err := UnblindRelayAnchor(&k.PublicKey, testChain, e, serial, mustSign(t, k, blinded), secret)
	if err != nil {
...
// TestRelayAnchorDomainIsPinnedByteExactly is the gate: the FDH input under
...
// 1. the constant is the literal (a rename is a token-format version);
...
// VERIFY path to the byte-exact input, so an implementation that wires blind and verify
...
// VerifyDemand — the four domains pairwise distinct (the precondition).
func TestRelayAnchorDomainIsPinnedByteExactly(t *testing.T) {
	if relayAnchorDomain != relayAnchorDomainLiteral {
		t.Fatalf("relayAnchorDomain = %q, want the format constant %q (a change is a token-format version, never an edit)", relayAnchorDomain, relayAnchorDomainLiteral)
	}
	for _, other := range []struct{ name, v string }{{"publish", fdhDomain}, {"credit", creditDomain}, {"demand", demandDomain}} {
		if relayAnchorDomain == other.v {
			t.Fatalf("relayAnchorDomain collides with the %s domain %q — one signature would verify in two lanes (door (iii))", other.name, other.v)
		}
	}
	...
	want := independentRelayFDH(pub, testChain, epoch, serial)
	bound, berr := chainBoundMsg(testChain, demandMsg(epoch, serial))
	if berr != nil {
		t.Fatal(berr)
	}
	if got := fullDomainHashD(pub, bound, relayAnchorDomain); got.Cmp(want) != 0 {
		t.Fatal("the relay-anchor FDH input is not \"silt/blindrelay/fdh/v2\" ‖ ctr ‖ chainID(32B) ‖ epoch(8B big-endian) ‖ serial — " +
	...
	if !VerifyRelayAnchor(pub, testChain, epoch, serial, rawSig) {
		t.Fatal("VerifyRelayAnchor rejects a raw RSA signature over the independently recomputed " +
			"relay-domain FDH — the verify path is not pinned to chainID ‖ uint64BE(epoch) ‖ serial under \"silt/blindrelay/fdh/v2\"")
	}
	if Verify(pub, serial, rawSig) || VerifyCredit(pub, testChain, serial, rawSig) || VerifyDemand(pub, testChain, epoch, serial, rawSig) {
		t.Fatal("a relay-anchor signature verified as a publish token, a credit, or a demand token — domain separation is broken")
	}
}

// TestRelayAnchorSignatureIsNotADemandSignature is the primitive half of; the
// node-tier half is core/node TestRelayAnchorDomainIsNotADemandToken: under the SAME
// key_E, the SAME epoch and the SAME serial, an anchor signature fails VerifyDemand
// and a demand signature fails VerifyRelayAnchor. This is what makes one 50,000 fee
// unable to pay both the delivery lane and the relay lane (one
// signature, one domain).
...
// TestRelayAnchorSignatureDoesNotVerifyAtAnotherEpoch transfers (b1) to the anchor
// lane. The issue epoch is inside
// the signed message, so an anchor verifies under exactly the pair key_E,
// E) — which is what lets the guard key on (epoch, serial) and expire with the
// keyset, gate.
