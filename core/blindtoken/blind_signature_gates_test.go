...
// Blind-signature gates.
//
// One test per finding, each named for the finding and each driving the primitive
// rather than a proxy. Four degenerate moduli are reproduced
// verbatim as the fixture, because those are the shapes its exploratory spike
...
// ---------------------------------------------------------------------------
// RFC 9474 §4.4 Finalize. Unblind MUST verify before returning.
// ---------------------------------------------------------------------------

func TestUnblindRefusesASignatureThatDoesNotVerify(t *testing.T) {
	priv := testKey(t)
	pub := &priv.PublicKey
...
func TestUnblindDemandRefusesTheWrongEpochAndTheWrongKey(t *testing.T) {
	priv := testKey(t)
	other := testKey(t)
	pub := &priv.PublicKey
	serial, _ := NewSerial(rand.Reader)
...
// ---------------------------------------------------------------------------
// the issuer's private-key operation: canonical input, blinded modexp,
// verify-after-sign. The wire format must be UNCHANGED.
// ---------------------------------------------------------------------------

func TestSignBlindedRoundTripsAndKeepsTheWireFormat(t *testing.T) {
	priv := testKey(t)
	pub := &priv.PublicKey
	for i := 0; i < 16; i++ {
		serial, _ := NewSerial(rand.Reader)
...
func TestSignBlindedRefusesANonCanonicalOrOutOfRangeInput(t *testing.T) {
	priv := testKey(t)
	pub := &priv.PublicKey
	serial, _ := NewSerial(rand.Reader)
...
// TestFuzzBlindedInputsNeverPanicAndNeverForge is the "fuzz of blinded
...
func TestFuzzBlindedInputsNeverPanicAndNeverForge(t *testing.T) {
	priv := testKey(t)
...
func TestVerifyAfterSignRefusesAFaultedModexp(t *testing.T) {
	priv := testKey(t)
	serial, _ := NewSerial(rand.Reader)
...
// ---------------------------------------------------------------------------
// canonical signatures. A valid signature has exactly ONE wire spelling.
// ---------------------------------------------------------------------------

func TestANonCanonicalEncodingOfAValidSignatureIsRefused(t *testing.T) {
	priv := testKey(t)
	pub := &priv.PublicKey
	serial, _ := NewSerial(rand.Reader)
	blinded, secret, err := Blind(rand.Reader, pub, serial)
...
// ---------------------------------------------------------------------------
// RFC 9474 §4.2 step 4, is_coprime(m, n).
//
...
func TestCoprimePredicateRefusesASharedFactor(t *testing.T) {
...
// ---------------------------------------------------------------------------
// ValidatePub hardness. Four degenerate shapes, verbatim.
// ---------------------------------------------------------------------------

func TestValidatePubRefusesTheFourWeakKeyShapes(t *testing.T) {
	honest := testKey(t)
	...
	// 122 distinct 17-bit primes ≈ 2049 bits, the smooth modulus.
	smooth := big.NewInt(1)
...
// TestValidatePubCostBudget lives in validatepub_cost_test.go, behind
...
// TestHardnessRunsAtAdmissionNotOnEveryModexp pins the split. The hardness checks
...
func TestHardnessRunsAtAdmissionNotOnEveryModexp(t *testing.T) {
	priv := testKey(t)
	pub := &priv.PublicKey
	PrewarmValidatePub()
	serial, _ := NewSerial(rand.Reader)
...
// TestShapeStillRefusesTheDegenerateKeysBeforeEveryModexp is the other half: moving
...
func TestShapeStillRefusesTheDegenerateKeysBeforeEveryModexp(t *testing.T) {
	for _, tc := range []struct {
		name string
		key  *rsa.PublicKey
	}{
