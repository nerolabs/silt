...
// The COST measurement, split out behind `//go:build !race`.
//
...
// therefore runs in the NON-race full suite (`go test./.`, which CI runs), and is skipped
// only in the -race job. Nothing else moves: every correctness gate in
// blind_signature_gates_test.go runs under both.
...
// TestValidatePubCostBudget is a RATIO gate, not a wall-clock budget. What must hold is that
// ONE key admission stays within a constant factor of ONE RSA verify ON THE SAME MACHINE:
// 154x measured on an M4 (3.290916 ms / 21.454 µs), 159x on a second machine. Both sides
// are 2048-bit big.Int work in the same process, so a uniform hardware slowdown cancels
// where an absolute millisecond figure does not — the same code costs 3.3 ms on the project
// M4 and 10.5 ms best / 30.3 ms worst on the CI runner, which is the closer analogue of a
// pony-class node. The milliseconds are still LOGGED, so a human reads the real cost; only
// the ASSERTION is hardware-independent.
//
...
//
// THE OTHER DIRECTION IS ALREADY GATED. TestHardnessRunsAtAdmissionNotOnEveryModexp
...
func TestValidatePubCostBudget(t *testing.T) {
