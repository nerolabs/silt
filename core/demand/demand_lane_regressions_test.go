...
// ---------------------------------------------------------------------------
// (CONTROL, held under attack and must keep holding). A signature made for epoch E
// verifies under exactly (key_E, E) and no other epoch, even under one shared key.
// This is the (b1) close the whole expiry argument rests on.
// ---------------------------------------------------------------------------
func TestEpochBindingHoldsUnderOneKey(t *testing.T) {
	k := rtKey(t)
...
// ---------------------------------------------------------------------------
// CLOSED. A consensus commitment attests 32 BYTES — which bytes the
...
// ---------------------------------------------------------------------------
func TestDegenerateKeysAreRefusedEverywhere(t *testing.T) {
	good := rtKey(t)
...
// ---------------------------------------------------------------------------
// RE-HOMED (C1, 2026-09-08). The bank's own spent set retired with the
// v2 flat lane: under an anchor is spent at session OPEN, into the credit ledger's
...
//
// - bounded, refuse-never-evict, expiry-only eviction — core/credit
...
// (F3) — core/credit TestGuardEntryExpiresOnItsOwnIssueEpoch.
//
// ---------------------------------------------------------------------------
// CLOSED, on the session wire. Neither guard bounded the serial's SIZE, and
...
// ---------------------------------------------------------------------------
func TestOversizedSerialIsRefusedAtTheSessionWire(t *testing.T) {
	k := rtKey(t)
	...
	if _, derr := UnmarshalSessionOpen(blob); !errors.Is(derr, ErrSessionBounds) {
		t.Fatalf("BREAK REOPENED: the session decode accepted a %d-byte anchor serial (err=%v)",
			oversized, derr)
	}
