...
//
// RELAY-LANE-per-node-ledger-mint-FIX-DIRECTION- ("NOT closed by the anchor; it
// needs its own bound. Promote [Verifier.walkSteps] from instrumentation to an enforced budget");
// red-team ADVERSARY-relay-lane-session-grant-and-byte-price-2026-09-03.md ("each bogus pay = ~53
// ms relay CPU from a ~48-byte wire message. replayable forever on one open session").
//
...
// `TestAdvanceToClampsToChainLength` (the pre-existing test) nor a bare
...
// EXISTING clamp (payword.go) and is ALREADY GREEN on main —
// see TestAdvanceToClampsToChainLength (payword_test.go). This file
...
// spent, a further claim is refused BEFORE walking." This is what
...
// of full-S bogus walks, each ~S hashes, forever — the live
...
// verifier. This half of this gate is ALREADY GREEN on main
// (payword.go) — see TestAdvanceToClampsToChainLength
// (payword_test.go) for the authoritative ablation-backed test. Kept
// here only so a reader of the interim gates does not have to go hunting for
// which half is already closed.
...
// TestRelayPayAdvanceToCumulativeWalkBudgetEnforced is the substantive
...
// session — the exact CPU-DoS the red-team measured
...
func TestRelayPayAdvanceToCumulativeWalkBudgetEnforced(t *testing.T) {
	...
	if got := v.walkSteps; got != afterFirst {
		t.Fatalf("cumulative walk budget is NOT enforced: a second bogus AdvanceTo on the same session walked AGAIN (walkSteps %d -> %d, session budget S=%d) instead of being refused before walking — Verifier.walkSteps is still instrumentation only, not the enforced per-session budget (unbounded CPU-DoS: red-team measured ~53ms/bogus pay, replayable forever on one open session)", afterFirst, got, S)
	}
}
