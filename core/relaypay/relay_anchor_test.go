...
// The relay-lane prepayment anchor: the WIRE-tier RED-first gates. (k_max "must be DERIVED in code: ⌈MaxChainLength /
// fee⌉ with a test that the wire bound covers it at the shipped fee"), §8 (decode bounds, the F5 shape: "len(Anchors) ≤
// MaxAnchorsPerSession; each Serial ≤ blindtoken.SerialSize (32); each Sig ≤ MaxModulusBits/8 = 1,024 B; Fetcher == 32; Sig == 64.
// Refuse before any map write or modexp"; and "v1 open at a v2 relay: len(Anchors) == 0 ⇒ errRelayNoAnchor" — the v1 payload must still
// DECODE). Build shape: (RelayOpen v2 = Root, S, Funding, Anchors[k], Fetcher, Sig) and §5 step 4.
//
...
// shippedFee is cmd/silt's default --fee (50,000 credits), the face of one
// anchor: face = Fee, an identity with the burn. Pinned as a literal so the
// derivation test is independent of core/credit.
const shippedFee = 50_000

// The reading of the v2 layout (see the stub block).
...
// k_max: at the shipped fee, MaxAnchorsPerSession anchors fund the
...
func TestRelayMaxAnchorsPerSessionCoversTheSessionCeiling(t *testing.T) {
	...
	if got := MaxAnchorsPerSession; got != 1 {
		t.Fatalf("MaxAnchorsPerSession = %d, want 1 = ⌈%d / %d⌉ (the derived DoS/decode bound; one anchor funds the whole session since the re-price)", got, sessionCredits, shippedFee)
	}
	// Pin: the session ceiling is EXACTLY what a
	...
	if MaxSessionBytes != MaxChainLength*RelayIncrementBytes || sessionCredits != ShippedAnchorFace {
		t.Fatalf("the relay-granularity derivation is broken: MaxSessionBytes %d vs S_max·B %d; S_max·credit %d vs face %d", MaxSessionBytes, MaxChainLength*RelayIncrementBytes, sessionCredits, ShippedAnchorFace)
	}
	// S_max and the session ceiling now DERIVE from the face, so the face's own
	// drift detector has to be an INDEPENDENT literal — the derivation cannot catch the
	// face moving. The three values, pinned as numbers.
	if ShippedAnchorFace != 50_000 || MaxChainLength != 50_000 || MaxSessionBytes != 26_214_400_000 {
		t.Fatalf("face %d / S_max %d / MaxSessionBytes %d, want 50,000 / 50,000 / 26,214,400,000 — a moved face re-derives k_max, S_max, the session ceiling, the 64 GiB pin and the guard bound together; re-check before re-pinning", ShippedAnchorFace, MaxChainLength, MaxSessionBytes)
	}
	if RelayIncrementBytes != 524_288 {
		t.Fatalf("RelayIncrementBytes = %d, want 524,288 — the settled 2026-09-06 re-price; a different value re-opens (the 64 GiB pin read on the summed prices, Don't #7 below, T-AR above)", RelayIncrementBytes)
	}
...
// TestRelayOpenDecodeBoundsRefuseOversizedAnchors is decode-bound gate (the F5
// amplifier shape, demand.go): every oversized field is a DECODE error,
// before any map write or modexp. A v1 payload (no v2 fields) must still decode —
// skew fails safe at the node (len(Anchors)==0 ⇒ refused), not at the codec. RED on
// main: UnmarshalRelayOpen has no bounds and ignores the v2 keys.
