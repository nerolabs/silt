...
// The H5-B eclipse cap keyed on the OBSERVED contacted-at address (geth / Bitcoin Core form), at the
// routing-table tier. RED-first the gate.(table half) from: The four build conditions: C-1 one group
// namespace per /24 across classes; C-2 the reserve bounds ALL non-DIRECT entries at K − R; C-3 DIRECT
// is never downgraded; C-4 the class lives with the table entry. Node-tier halves live in
// core/node/address_keying_test.go; transport halves in adapters/tcpnet/class_test.go.
//
...
const (
	...
	r43bCapRelay = 2 // cap_relay, the shadow hypothesis
	r43bReserve  = 4 // R, the shadow hypothesis: K − R = 4 free non-DIRECT slots
)
...
// TestOneObservedGroupIsCappedUnderOn, table half. Eight Sybils that all answered from
// ONE observed /24 are ONE group whatever they declare: at most cap_direct of them enter
// a K=8 bucket under `on`, and the six honest peers from six distinct /24s that arrive
// AFTER them are all admitted. The inverse is pinned: under `off` the same surround
// fills the bucket (so the defence is the address rule, nothing else). The
// declared-label half of (ten distinct -domain labels) is core/node/dht_domain0_test.go
// TestLabelledSybilsDefeatTheDomainCap_OPEN_BREAK, rewritten to assert the defence.
func TestOneObservedGroupIsCappedUnderOn(t *testing.T) {
	...
	if s > r43bCapDir {
		t.Fatalf("RED: %d of 8 Sybils from ONE observed /24 admitted into a K=8 bucket under on (cap_direct=%d) — the eclipse still costs one /24", s, r43bCapDir)
	}
	if h != 6 {
		t.Fatalf("RED: %d of 6 honest peers (six distinct observed /24s) admitted after a one-/24 surround; want 6 (the bucket holds %d Sybils + 6 honest = 8)", h, s)
	}
	if s, h := run(AddressCapOff); s != 8 || h != 0 {
		t.Fatalf("inverse pin: under off the one-/24 surround should fill the bucket (8 Sybils, 0 honest); got %d/%d — something other than the address rule is capping", s, h)
	}
}

// TestIntroducedIDsAreChargedToTheIntroducer, table half (Bitcoin Core's srcgroup rule).
// Ids learned from a FindNodeReply and never contacted enter as UNVERIFIED in the
// INTRODUCER's group: one reply seeds at most cap uncontacted entries per bucket; the
// entry carries the introducer's group; on first contact it re-keys to its own observed
// group.
func TestIntroducedIDsAreChargedToTheIntroducer(t *testing.T) {
	...
	if k := tab.kept(xs); k > r43bCapDir {
		t.Fatalf("RED: %d of 8 never-contacted ids from ONE reply admitted into a bucket (want ≤ %d per introducer group) — a replier can seed a bucket with uncontacted ids for free", k, r43bCapDir)
	} else if k == 0 {
		t.Fatalf("fixture: 0 introduced ids admitted — the charge must admit up to the cap, not refuse")
	}
	var kept []ports.NodeID
	for _, x := range xs {
		if tab.has(x) {
			classOf(t, tab, x, ports.ClassUnverified, 0x11, "RED (introduced entry)")
			kept = append(kept, x)
		}
	}
	...
	if k := tab.kept(ys); k > r43bCapDir || k == 0 {
		t.Fatalf("RED: second introducer group admitted %d uncontacted ids (want 1..%d)", k, r43bCapDir)
	}
	// First contact re-keys the entry to its OWN observed group.
	o.direct(kept[0], 0x33)
	tab.Observe(kept[0])
	classOf(t, tab, kept[0], ports.ClassDirect, 0x33, "RED (re-key on contact)")
	...
	tab.Observe(refused)
	if !tab.has(refused) {
		t.Fatalf("RED: an id refused as uncontacted (introducer group full) was not admitted when it later answered from its own /24 (0x44, unsaturated)")
	}
	classOf(t, tab, refused, ports.ClassDirect, 0x44, "RED (late contact)")
}

// TestRelayGroupIsOneNamespaceAcrossClasses: RELAYED is its own
...
func TestRelayGroupIsOneNamespaceAcrossClasses(t *testing.T) {
	t.Run("N Sybils behind one relay hold at most cap_relay", func(t *testing.T) {
		...
		if k := tab.kept(xs); k > r43bCapRelay {
			t.Fatalf("RED: %d of 8 peers reached through ONE relay admitted into a bucket (cap_relay=%d) — a relay's address is N free groups", k, r43bCapRelay)
		}
		for _, x := range xs {
			if tab.has(x) {
				classOf(t, tab, x, ports.ClassRelayed, 0x7E1A, "RED (relayed entry)")
			}
		}
	})
	t.Run("a /24 hosting a relay plus a direct Sybil yields the composed bound, never cap_direct+cap_relay", func(t *testing.T) {
		...
		if inGroup >= capDir+capRelay {
			t.Fatalf("RED (C-1): the /24 0x24 holds %d entries in one bucket (relay + direct Sybil + %d relayed clients) = cap_direct+cap_relay — the group namespace is split by class, so a relay /24 costs half", inGroup, tab.kept(clients))
		}
		if inGroup > max(capDir, capRelay) {
			t.Fatalf("RED (C-1): the /24 0x24 holds %d entries in one bucket; the composed bound is max(cap_direct, cap_relay)=%d", inGroup, max(capDir, capRelay))
		}
		if d := tab.kept(ids[:2]); d > capDir {
			t.Fatalf("RED (C-1): %d DIRECT entries of /24 0x24 in one bucket (cap_direct=%d)", d, capDir)
		}
	})
	t.Run("table-wide per-group cap", func(t *testing.T) {
		...
		if k := tab.kept(spread); k > 10 {
			t.Fatalf("RED: one /24 holds %d entries across the table (want ≤ 10, geth's bucketIPLimit-style table cap) — a single /24 can still own a large share of the whole table", k)
		}
		other := idInBucket(rng, self, 240)
		o.direct(other, 0x5B)
		tab.Observe(other)
		if !tab.has(other) {
			t.Fatalf("RED: a peer from a fresh /24 was refused after another /24 hit the table-wide cap — the table cap is per GROUP, not global")
		}
	})
}

// TestReserveBoundsAllNonDirectEntries: Sybils spread behind M ≥ 8 honest
// relays (free) hold at most K − R slots per bucket; the remaining R slots admit
// DIRECT peers only; seeds / -persistent-peers are exempt from every cap; the reserve
// cannot be configured below K/2.
func TestReserveBoundsAllNonDirectEntries(t *testing.T) {
	...
	t.Run("relayed across 8 relays ≤ K−R; DIRECT still admitted", func(t *testing.T) {
		...
		if k := tab.kept(xs); k > r43bK-r43bReserve {
			t.Fatalf("RED (C-2): %d relayed entries in one bucket via 8 distinct relays (want ≤ K−R = %d) — honest relays' address diversity is borrowable for free", k, r43bK-r43bReserve)
		}
		...
		if k := tab.kept(ds); k != 4 {
			t.Fatalf("RED: %d of 4 DIRECT peers (distinct /24s) admitted into the reserve after the relayed pool filled its share; want 4", k)
		}
	})
	t.Run("the reserve counts relayed + unverified + unclassified together", func(t *testing.T) {
		...
		if k := tab.kept(ids); k > r43bK-r43bReserve {
			t.Fatalf("RED (C-2): %d non-DIRECT entries (relayed ∪ unverified ∪ unclassified) in one bucket; want ≤ K−R = %d — the reserve bounds only one of the three classes", k, r43bK-r43bReserve)
		}
	})
	t.Run("seeds and static peers are exempt from every cap", func(t *testing.T) {
		...
		if !tab.has(seed) {
			t.Fatalf("RED (C-2 exemption): a configured seed reached through a relay was refused because the reserve was full — seeds and -persistent-peers are operator-typed and exempt from every cap")
		}
		static := ids[5] // an unclassified static peer
		tab.ObserveStatic(static)
		if !tab.has(static) {
			t.Fatalf("RED (C-2 exemption): an unclassified -persistent-peers entry was refused")
		}
	})
	t.Run("R below K/2 is refused or clamped, never applied", func(t *testing.T) {
		...
		if applied {
			return // refused by panic: acceptable (a security parameter outside its range)
		}
		xs := idsInBucket(rng, self, 200, 8)
		for i, x := range xs {
		...
		if k := tab.kept(xs); k > r43bK/2 {
			t.Fatalf("RED: reserve=2 (< K/2) was APPLIED — %d non-DIRECT entries in one bucket (want ≤ K−K/2 = %d, the clamp) — the adversary owns a bucket MAJORITY for free via honest relays", k, r43bK/2)
		}
	})
}

// TestDirectIsNeverDowngraded: A peer classified DIRECT, later reached only
// through a relay into a SATURATED relayed class, keeps its DIRECT entry oscillation: a
// public node re-dials a pony through the relay after the pony's outbound conn dies; a
// later DIRECT conversation at a new /24 re-keys it.
func TestDirectIsNeverDowngraded(t *testing.T) {
	...
	o.direct(pony, 0xA1)
	tab.Observe(pony)
	classOf(t, tab, pony, ports.ClassDirect, 0xA1, "fixture")

	o.relayed(pony, 0x900) // the transport now reaches it through relay 0x900 (saturated)
	tab.Observe(pony)
	if !tab.has(pony) {
		t.Fatalf("RED (C-3): a DIRECT peer later reached through a relay into a saturated relayed class was EVICTED — the re-check downgraded it and fired")
	}
	classOf(t, tab, pony, ports.ClassDirect, 0xA1, "RED (C-3: DIRECT downgraded to the relay's class)")

	o.direct(pony, 0xA2) // a later DIRECT conversation at a different /24
	tab.Observe(pony)
	classOf(t, tab, pony, ports.ClassDirect, 0xA2, "RED (a later DIRECT conn at a new /24 must re-key)")
}

// r43bEvent is one step of the random trace.
...
// TestShadowChangesNoAdmissionAndCountsEveryRefusal. Over random swarms the same event
// trace is fed to three tables (off, shadow, on) kept in lock-step: after every event
// shadow's buckets equal off's; whenever `on` diverges from off it does so by exactly the
// event's id and shadow recorded exactly one new would-refuse cell under the configured
// (R, cap_relay); when `on` does not diverge, shadow recorded nothing (or a bucket-full
// short-circuit). The refused id is then removed from off/shadow so the three stay
// synchronised and every later decision is compared on identical state. The grid subtest
// pins that shadow evaluates R ∈ {4,6,8} × cap_relay ∈ {2,4} (series A).
func TestShadowChangesNoAdmissionAndCountsEveryRefusal(t *testing.T) {
	for seed := int64(1); seed <= 12; seed++ {
		...
		for step := 0; step < 600; step++ {
			...
			if len(sOff) != len(sSh) {
				t.Fatalf("RED (seed %d, step %d, %s): shadow holds %d entries, off holds %d — shadow changed an admission", seed, step, ev, len(sSh), len(sOff))
			}
			for id, b := range sOff {
				if sSh[id] != b {
					t.Fatalf("RED (seed %d, step %d, %s): %s is in off's bucket %d but not shadow's — shadow changed an admission", seed, step, ev, id.String()[:8], b)
				}
			}
			...
			if len(onOnly) != 0 {
				t.Fatalf("RED (seed %d, step %d, %s): on holds %d entries off does not — on admitted something off refused", seed, step, ev, len(onOnly))
			}
			switch len(offOnly) {
			case 0:
				if delta != 0 {
					...
					if full < r43bK || delta != 1 {
						t.Fatalf("RED (seed %d, step %d, %s): shadow recorded %d would-refuse cell(s) but on made no refusal (bucket holds %d) — the counter over-reports", seed, step, ev, delta, full)
					}
				}
			case 1:
				if offOnly[0] != ev.id {
					t.Fatalf("RED (seed %d, step %d, %s): on diverged by %s, not by the event's id — a refusal touched a different entry", seed, step, ev, offOnly[0].String()[:8])
				}
				if delta != 1 {
					t.Fatalf("RED (seed %d, step %d, %s): on refused the event's id but shadow recorded %d new would-refuse cell(s) under (R=%d, cap_relay=%d); want exactly 1", seed, step, ev, delta, r43bReserve, r43bCapRelay)
				}
				off.Remove(ev.id) // resynchronise so the next decision is compared on identical state
				shadow.Remove(ev.id)
			default:
				t.Fatalf("RED (seed %d, step %d, %s): on diverged from off by %d entries in one step", seed, step, ev, len(offOnly))
			}
		}
		t.Logf("seed %d: %d would-refuse cells recorded under (R=%d, cap_relay=%d), %d entries tabled", seed, r43bShadowCount(shadow, r43bReserve, r43bCapRelay), r43bReserve, r43bCapRelay, off.Size())
		if r43bShadowCount(shadow, r43bReserve, r43bCapRelay) == 0 {
			t.Fatalf("fixture (seed %d): the trace produced no would-be refusal at all — the differential proved nothing; widen the trace", seed)
		}
	}
	...
	t.Run("shadow evaluates the (R, cap_relay) grid", func(t *testing.T) {
		...
		for k, v := range want {
			if got[k] != v {
				t.Fatalf("RED (series A grid): would-refuse[bucket %d, relayed, R=%d, cap_relay=%d] = %d, want %d — shadow does not evaluate the (R, cap_relay) grid the reserve must be set from", k.Bucket, k.Reserve, k.CapRelay, got[k], v)
			}
		}
		if tab.Size() != 14 {
			t.Fatalf("RED: shadow admitted %d of 14 (want all 14: shadow never refuses)", tab.Size())
		}
	})
}

// TestRecheckIsANarrowingAndClassLossIsInert: An UNVERIFIED entry whose first
// classification lands in a saturated group is removed; into a non-saturated group it
// stays, re-keyed; a re-check never adds; and CLASS LOSS (the classifier no longer knows a
// tabled id) changes nothing — never a re-check, never an eviction, even with the reserve
// full.
func TestRecheckIsANarrowingAndClassLossIsInert(t *testing.T) {
	...
	if !tab.has(x1) || !tab.has(x2) {
		t.Fatalf("fixture: the two introduced ids were not admitted (introducer group 0x11 has room)")
	}

	o.direct(x1, 0x5) // first classification: its true group is saturated
	tab.Observe(x1)
	if tab.has(x1) {
		t.Fatalf("RED: an UNVERIFIED entry whose first classification lands in a SATURATED group (0x5 holds cap_direct) stayed tabled — the re-check is not applied (the admission rule applied late)")
	}
	size := tab.Size()
	o.direct(x2, 0x6) // non-saturated
	tab.Observe(x2)
	if !tab.has(x2) {
		t.Fatalf("RED: an UNVERIFIED entry re-checked into a NON-saturated group was removed — the re-check is wider than a narrowing")
	}
	classOf(t, tab, x2, ports.ClassDirect, 0x6, "RED (re-key)")
	if tab.Size() != size {
		t.Fatalf("RED: table size %d → %d across a re-check; a re-check never adds", size, tab.Size())
	}
	...
	tab.Observe(x2)
	if !tab.has(x2) {
		t.Fatalf("RED (C-4): a tabled DIRECT entry was EVICTED when the classifier lost its class — class loss fired the re-check (Finding-C flood ⇒ eviction weapon)")
	}
	classOf(t, tab, x2, ports.ClassDirect, 0x6, "RED (C-4: class loss must be inert; the class lives with the entry)")
}

// TestGroupZeroIsExemptFromTheGroupCap, table half. Loopback / link-local conversations
// classify to group 0 (exempt): an on-host swarm's table fills to K exactly as before.
// (The RFC1918-is-CLASSIFIED half is adapters/tcpnet/class_test.go.)
func TestGroupZeroIsExemptFromTheGroupCap(t *testing.T) {
	...
	if k := tab.kept(ids); k != r43bK {
		t.Fatalf("RED: %d of 8 loopback (group 0) peers admitted under on; want 8 — an on-host e2e swarm's tables shrink", k)
	}
}

// TestTelemetrySeriesBAndEFromTheTable — the per-node halves of series B (relay
// fan-in) and E (per-bucket group-density census), read straight from the entries'
// stored classes. Series A is ShadowRefusals.
func TestTelemetrySeriesBAndEFromTheTable(t *testing.T) {
...
// TestDirectIsNeverDowngradedIntoAnUnsaturatedRelayClass covers the case the
// saturated fixture above does not reach: a re-key of a contacted
// entry is applied only when the new key is admissible, so with the relay class
// SATURATED that rule alone keeps the pony DIRECT even without. C-3 is "never
...
// and the re-check evicts it oscillation.
func TestDirectIsNeverDowngradedIntoAnUnsaturatedRelayClass(t *testing.T) {
	...
	tab.Observe(pony)
	classOf(t, tab, pony, ports.ClassDirect, 0xA1, "RED (C-3: DIRECT downgraded into an unsaturated relay class)")
}
