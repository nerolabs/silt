...
// The two substantive gates of the research (
// §7): the fold BINDS every delete sibling (digest == SHA-256(preimage)) before
// seeding the library's node store a proof whose NonMembershipLeafData does not begin
// 0x00 is refused BEFORE VerifyProof, because the library enforces the prefix with panic and
// core/ has no recover. Each carries its positive control: the raw library call panics on the
// witness, and the fold accepts the unbound sibling with the check removed.
...
// TestUnboundDeleteSiblingStallsTheFold: a delete op whose DeleteSiblings carries a
...
func TestUnboundDeleteSiblingStallsTheFold(t *testing.T) {
...
// TestMalformedLeafPrefixIsRefusedNotPanicked: the 33-byte witness — a proof whose
...
func TestMalformedLeafPrefixIsRefusedNotPanicked(t *testing.T) {
	var root ports.Hash
	root[0] = 0x5a
	...
	if !panicked {
		t.Fatalf("control: the raw library VerifyProof did not panic on a 0x01-prefixed NonMembershipLeafData — if the library now returns an error instead, this gate's premise changed; re-read  §4.7")
	}
...
// TestMalformedSiblingDataIsRefusedNotPanicked: the panic also reaches through SiblingData
// — validateBasic hashes it unbounded when SideNodes is non-empty. Three shapes panic the
// raw library (controls captured with recover): an empty SiblingData, and a 0x02
...
func TestMalformedSiblingDataIsRefusedNotPanicked(t *testing.T) {
	var root ports.Hash
	root[0] = 0x5a
	key := Key("a\x00", []byte("absent"))
	...
	for name, sib := range shapes {
		...
		if !panicked {
			t.Fatalf("control (%s): the raw library did not panic on this SiblingData shape — the gate's premise changed; re-read ", name)
		}
...
// TestForgedExtensionSiblingDoesNotBind is the false-ACCEPTANCE direction of for the
// extension arm: an HONEST extension sibling, taken from a real trie's ProveWithSiblings,
// binds; the same preimage with a mutated child digest, or mutated bounds, does not. Without
// this, weakening the arm to "accept any 0x02 preimage under any digest" left the whole suite
// green.
func TestForgedExtensionSiblingDoesNotBind(t *testing.T) {
