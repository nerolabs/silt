...
// ADVERSARY — the sub-frame size oracle: the single-shard reconstruction half, and
// the parity-distinctness defect.
//
// The idiom used for the live defect below (a PIN with TESTABLE TEETH plus a MECHANISM arm) is
// documented in full at the head of subframe_manifest_oracle_test.go. this gate is RED on main
// today and its disposition is not settled — the red-team routed it to the research and the
// research and explicitly declined to price it — so it is pinned, not asserted.
//
// The column-key half lives in core/node (it needs colKey / placementKey).
...
// TestPinRedensWhenTheSixthParityBecomesDistinct, but only to exercise the predicate.)
func rtSubFrameLayout(t *testing.T, size int, mode crypto.Mode) (*manifest.Layout, rtObject) {
	...
	if len(l.Chunks) != 1 {
		t.Fatalf("fixture precondition failed: a %d-byte object produced %d data chunks, want 1. These gates "+
			...
			"sub-frame boundary moved (pipeline.DataFrameSize) and need re-deriving, not "+
...
// ══════════════════════════════════════════════════════════════════════════════ (MED, NEW) —
// every one of a sub-frame object's seven shards alone reconstructs it.
//
...
// today and a change to it is a change to the durability model, which is the.
// ══════════════════════════════════════════════════════════════════════════════

func TestEverySubFrameShardAloneReconstructs(t *testing.T) {
	...
	// columns 10.15 (core/node/column.go columnAt: data leaf i -> i%k; parity leaf q ->
	// k+q%(n-k)).
	leaves := l.Leaves()
	...
	for i, col := range cols {
		...
		if err := erasure.ReconstructStripe(p, shards, 1); err != nil {
			t.Fatalf("holding only the shard at column %d, reconstruction failed: %v. The red-team "+
				"measured 7 of 7 stored shards individually reconstructing a sub-frame object. If this is now an "+
				"error the stripe geometry changed; re-derive together.", col, err)
		}
		if !bytes.Equal(shards[0], wantData) {
			t.Fatalf("holding only column %d, reconstruction produced the wrong data shard", col)
		}
		recovered++
	}
	if recovered != len(cols) {
		t.Fatalf("%d of %d shards individually reconstructed, want all", recovered, len(cols))
	}
	...
	// specifically, which is why the gate sits next to it.
	shards := make([][]byte, p.N)
	shards[0] = bytes.Repeat([]byte{1}, 64)
	if err := erasure.ReconstructStripe(p, shards, 2); err == nil {
		t.Fatal("bound arm: one shard reconstructed a TWO-data-shard stripe. Single-shard sufficiency " +
			"is supposed to be specific to realData=1; if it is now general, the erasure layer changed and the " +
			"durability accounting in credit.RarestShardMultiplier is wrong by more than says.")
	}
}

// ══════════════════════════════════════════════════════════════════════════════
// (LOW-MED, NEW) — a sub-frame object's "six parity shards" are five.
//
...
// ("the and the, not mine") and no disposition has been ruled. The pin is GREEN today
// because it asserts the defect is still present; see the idiom note in
// subframe_manifest_oracle_test.go.
// ══════════════════════════════════════════════════════════════════════════════
...
// TestPinRedensWhenTheSixthParityBecomesDistinct.
func rtSFO4Pin(declared, distinct int, dupes []string) string {
	if distinct == declared {
		return fmt.Sprintf(
			"PIN IS RED — A ONE-DATA-SHARD STRIPE NOW HAS %d DISTINCT PARITY SHARDS, WHICH IS THE FIX.\n"+
				"  At 00082b8 it had %d of %d: columns 12 and 14 were byte-identical, so manifest.Leaves\n"+
				...
				"       TestEverySubFrameShardAloneReconstructs still reflects the intended geometry.\n"+
				"    4. Replace this pin with the positive assertion.",
			distinct, declared-1, declared)
	}
	if distinct != declared-1 {
		return fmt.Sprintf(
			"PIN IS RED — THE DEFECT MOVED: %d distinct parity shards of %d declared, pinned at %d.\n"+
	...
	if len(dupes) != 1 || dupes[0] != "col12==col14" {
		return fmt.Sprintf(
			"PIN IS RED — the collision is still one, but it MOVED: %v, pinned at [col12==col14].\n"+
...
func TestSingleDataShardStripeHasSixDistinctShards_PINNED_DEFECT(t *testing.T) {
	p := erasure.DefaultParams
	...
	// (b) THE CONSEQUENCE, asserted where it actually bites: a REPEATED HASH in the
	// committed leaf set. Pinning only the byte-equality would miss a change that kept the
	// bytes equal but stopped them sharing a leaf (or vice versa). This arm is what ties the
	// erasure defect to the durability accounting that reads manifest.Leaves.
	leaves := l.Leaves()
	...
	if repeated != 1 {
		t.Fatalf("PIN IS RED (consequence arm) — the committed leaf set has %d repeated hashes, pinned "+
	...
	// (c) THE CONTROL — and it is a control, not a pin. Stripes with 2.k real data shards
	// have all six parity distinct, so the defect is specific to realData==1. Without this
	// arm the pin would also be green if EVERY stripe collapsed, which is a far worse defect
	// wearing the same number. A non-uniform data population per stripe, so a wrong reducer
	// (say, one that compares lengths instead of bytes) cannot look right.
	for realData := 2; realData <= p.K; realData++ {
		...
		if d != p.ParityShards() {
			t.Fatalf("control arm: a stripe with %d real data shards has %d distinct parity of %d (%v). "+
...
// TestPinRedensWhenTheSixthParityBecomesDistinct is the TEETH. It exercises
// rtSFO4Pin directly with the post-fix, worsened, and moved-collision inputs.
func TestPinRedensWhenTheSixthParityBecomesDistinct(t *testing.T) {
