...
// THE SUB-FRAME SIZE ORACLE — a confirmed adversarial break, kept as a permanent
// regression gate.
//
// This file carries three of them: the keyless encryption-mode oracle, the total
// chunk-size salt collapse, and Entry.FileSize being the exact byte count. The stripe
// half is in subframe_stripe_oracle_test.go; the column half is in core/node.
//
// ────────────────────────────────────────────────────────────────────────────────────────
// THE IDIOM — A DEFECT PIN WITH TESTABLE TEETH, and why it is this and not a plain assert.
// ────────────────────────────────────────────────────────────────────────────────────────
//
// These are LIVE DEFECTS on main, and their disposition is still open: privacy is a
// Part-0 corner, and closing one of these changes a published claim. So a gate here cannot
// assert that the property holds — it does not — and a failing test cannot merge.
//
...
// failure text. GREEN today. It goes RED the moment the
...
// (name the settled corner you are buying):
// - core/chain/issuerkey_rollout_gate_test.go: gateFStampPin is a factored predicate whose
// teeth are asserted by TestStampRaiseIsAHardFailure. That factoring exists because
...
// ══════════════════════════════════════════════════════════════════════════════
// A keyless encryption-mode oracle. CLOSED.
//
...
// recorded as The defect PIN that shipped in is retired here and replaced by the straight
// assertion; its teeth are kept and re-pointed.
// ══════════════════════════════════════════════════════════════════════════════

// rtSFO1Sizes carries both measurements: `framed`, the post-fix length both modes now share,
// and `wasConv`/`wasPriv`, byte-for-byte what recorded. The pre-fix pair is kept because a
// gate that no longer remembers the defect cannot say how far it moved, and because
// TestGateRedensWhenTheModesSeparate feeds it back in.
...
// themselves testable (TestGateRedensWhenTheModesSeparate), the gateFStampPin
...
func rtSFO1ModeHidden(L, conv, priv int) string {
	...
	return fmt.Sprintf(
		"IS BACK — THE SEALED-MANIFEST FRAME LENGTH PUBLISHES THE ENCRYPTION MODE AGAIN.\n"+
...
// TestModeIsNotRecoverableFromManifestFrameLength is the gate. It replaces the
// PINNED_DEFECT that shipped before the fix.
//
// SEEN RED FIRST on the pre-fix tree — a green gate with no demonstrated red is
// decoration:
//
//	THE ORACLE IS BACK — at FileSize=1 the manifest frames at convergent=345 private=341 (delta 4).
func TestModeIsNotRecoverableFromManifestFrameLength(t *testing.T) {
	cs := pipeline.DefaultChunkSize

	// (a) THE PROPERTY, at the eight sizes the oracle was measured at.
	for _, tc := range rtSFO1Sizes {
		...
		if conv != tc.framed {
			t.Fatalf("DRIFT — a %d-byte object now frames at %d bytes in both modes; this gate pins %d.\n"+
	...
	// and 429 is the population classified 429/429 correctly, so this arm is
	...
	// bought the wrong way. Re-padding the manifest frame is explicitly
	...
	for _, L := range []int{1, 1024, 262136, 262137, 600000, 2097152} {
		...
		if len(bc) != len(bp) {
			t.Fatalf("IS BACK AT THE ROOT — the SEALED BLOB for a %d-byte object is %d bytes convergent "+
	...
	if got, want := rtManifestFrameBytes(t, 1024, crypto.Convergent, cs, 0, erasure.Params{}),
		...
		t.Fatalf("GATE IS MEASURING THE WRONG THING — the manifest frame is %d bytes where the sealed "+
			...
			"the partial fix the rule refuses; re-read its §9 before trusting this gate.",
			got, chunk.HeaderSize, want)
	}
}

// TestModeIsNotRecoverableAtAnyErasureGeometry is the geometry arm, inverted by the
...
func TestModeIsNotRecoverableAtAnyErasureGeometry(t *testing.T) {
	cs := pipeline.DefaultChunkSize
...
// TestGateRedensWhenTheModesSeparate is the TEETH, carried over from the retired
...
func TestGateRedensWhenTheModesSeparate(t *testing.T) {
	for _, tc := range rtSFO1Sizes {
		if msg := rtSFO1ModeHidden(tc.L, tc.wasConv, tc.wasPriv); msg == "" {
			t.Fatalf("rtSFO1ModeHidden stayed silent on the ORIGINAL  measurement at FileSize=%d "+
				"(convergent=%d, private=%d, as recorded at 00082b8) — the gate cannot detect the defect it "+
				"exists to prevent, so it is decoration", tc.L, tc.wasConv, tc.wasPriv)
		}
	}
...
// TestPaddedFramingAblationCollapsesBothModes is the framing ablation, kept from and still
// load-bearing for a reason that OUTLIVED the defect. It pins that padded manifest framing
// ALSO collapses the modes — which is exactly why it must stay visible next to the real fix.
// Someone reading the gate above could conclude that re-padding the frame is an equivalent
// remedy. It is not: it lifts the oracle only while both modes fit one frame, because
// len(Entry.ManifestChunks) then separates them on-chain with no fetch at all, and it
// reinstates the 3.9x store growth. This test records the weaker remedy as weaker, in the
// same file as the stronger one.
func TestPaddedFramingAblationCollapsesBothModes(t *testing.T) {
	cs := pipeline.DefaultChunkSize
	for _, L := range []int{1, 1024, 262135} {
		...
		if conv != priv {
			t.Fatalf("ABLATION FAILED TO ABLATE — under padded framing (ManifestFrameBytes=%d) a %d-byte object "+
				"still frames convergent=%d private=%d. Re-read 's fix analysis before proceeding.",
				cs, L, conv, priv)
		}
		if conv != cs {
...
// ══════════════════════════════════════════════════════════════════════════════
// The chunk-size salt collapse is TOTAL, not three-point.
//
...
func TestSubFrameRootIsChunkSizeIndependentAcrossTheLegalRange(t *testing.T) {
	const L = 111
	...
	for _, cs := range probes {
		if got := rtRootAt(t, payload, cs); got != predicted {
			t.Fatalf("PIN IS RED — a %d-byte object published at chunkSize=%d now yields root %s, "+
	...
	if len(salted) == 0 {
		t.Fatal("no chunk size in [9,4096] salts at all — the complement arm is vacuous and cannot " +
	...
	if salted[0] != chunk.MinChunkSize || salted[len(salted)-1] != wantBoundary || len(salted) != wantBoundary-chunk.MinChunkSize+1 {
		t.Fatalf("BOUNDARY MOVED — the salting chunk sizes for a %d-byte object are %d values spanning "+
	...
	if len(collapsed) != 4096-wantBoundary {
		t.Fatalf("%d of the chunk sizes in [%d,4096] collapse to the attacker-predicted root; the "+
	...
	// "three values". NOTE FOR THE RECORD — the measurement says 134,217,609 of
	...
	if rtRootAt(t, payload, wantBoundary) == predicted {
		t.Fatalf("arm (d): chunkSize=%d (the largest salting value) produced the attacker-predicted "+
...
// ══════════════════════════════════════════════════════════════════════════════
// ports.Entry.FileSize is the exact byte count.
//
...
// publisher writes the exact count. It does NOT assert anything about what silt CLAIMS. The
// Two published claims measured false — "before, padding blurred it to
// 'somewhere in one chunk'"'s "no confirmation surface" for private mode. Correcting what silt
// claims about privacy is a published-claim change: research-gated. A test asserting doc text
// would be this seat deciding a question that is not its to decide, and would ALSO decay the
// moment the doc was reworded for any other reason (the
// a claim about a gate is itself a claim). The measurement is the durable half. The
// doc is not this file's to make.
//
// This is also the finding that makes "re-pad the sub-frame" a no-op, so it must stay visible
// next to TestPaddedFramingAblationCollapsesBothModes.
// ══════════════════════════════════════════════════════════════════════════════

// rtSFO5Pin returns "" while Entry.FileSize is still the exact count, and the instruction once
// it is blinded. Teeth: TestPinRedensWhenFileSizeIsBlinded.
func rtSFO5Pin(wantExact int64, got int64, mode crypto.Mode) string {
	if got == wantExact {
		return ""
	}
	return fmt.Sprintf(
		"PIN IS RED — ports.Entry.FileSize IS NO LONGER THE EXACT BYTE COUNT.\n"+
			...
			"  object on the chain. Blinding or bucketing it is the ONLY remedy that closes the length\n"+
			"  oracle — re-padding the sub-frame does not (§'Consequence for the fix').\n"+
...
func TestEntryFileSizeIsTheExactByteCount_PINNED_DEFECT(t *testing.T) {
	cs := pipeline.DefaultChunkSize
	...
	if o.entry.FileSize != 262135 {
		t.Fatalf("mechanism arm: Entry.FileSize=%d for a 262,135-byte object", o.entry.FileSize)
	}
}

// TestPinRedensWhenFileSizeIsBlinded is the TEETH.
func TestPinRedensWhenFileSizeIsBlinded(t *testing.T) {
	// Bucketed to a coarse ladder — one of the two named remedies. Must redden.
	if msg := rtSFO5Pin(999, 1024, crypto.Private); msg == "" {
		t.Fatal("rtSFO5Pin stayed silent when a 999-byte object reported FileSize=1024 — the pin cannot detect " +
			"the bucketing remedy it exists to detect")
	}
