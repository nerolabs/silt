...
// These are the M0 PoCs INVERTED as regressions: the prefix-plot Sybil and its enablers, which per-root dedup structurally could not catch,
// are now DENIED by the verified labeling check. See and the researcher memo.
...
func TestPrefixPlotIsNotAValidSmallerPlot(t *testing.T) {
	pkA := pk(11)
	const bigN = 512
...
func TestPlotBoundToClaimedIdentity(t *testing.T) {
	pkA, pkB := pk(21), pk(22)
	c := Seal(pkA, 1<<20)
	const nonce = 42
...
// rejected.
func TestArbitraryBytesFailLabeling(t *testing.T) {
	pkA := pk(31)
	const n = 256
...
func TestPrefixFamilyYieldsNoExtraStandings(t *testing.T) {
	pkA := pk(41)
	const bigN = 512
	big := sealN(pkA, bigN)
	const nonce = 3
...
// at k=0 too.
func TestUnsetKStillDeniesPrefixSybil(t *testing.T) {
	pkA := pk(51)
	const bigN = 512
	big := sealN(pkA, bigN)
	const m = 256
