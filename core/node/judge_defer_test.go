...
// regression (the transient-deny sub-mode): a claim that is unjudgeable RIGHT
// NOW — the judge's survivor fetches come up short, which in the field
...
func TestJudgeDefersTransientlyUnjudgeableClaim(t *testing.T) {
	...
	if judge.Stats.BountiesReleased != 1 {
		t.Fatalf("a transiently-unjudgeable honest claim was not paid after the transient cleared: BountiesReleased=%d (a terminal deny loses the bounty forever — emission is one-shot)", judge.Stats.BountiesReleased)
	}
	if judge.Stats.FalseRepairSlashes != 0 {
		t.Fatal("the deferred claim was slashed")
	}
}
