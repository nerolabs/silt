...
// measurement rig: how long does ONE repair sweep take when holders die, and
// where does the time go? The field attribution (runs the field run / the field
// run) measured ~2.1s healthy → ~3-4 min after killing 2 holders, and named the
// suspect coarsely: probe/lookup timeouts toward the dead. This rig reproduces
// the shape deterministically in sim time (instant wall clock) under
...
// the sweep narration plus request-layer counters. The eventual fix turns the
// measured numbers into a bound assertion here.
...
// captureLog records every narration line so the test can read the phase
...
// phaseReport pulls the phase attribution off the caretaker's narration.
...
// TestMeasure_SweepDurationUnderDeadHolders is the attribution
...
// not yet a bound: the fix adds the assertion.
func TestMeasure_SweepDurationUnderDeadHolders(t *testing.T) {
	r := newSweepRig(t)
	...
	// bound 1 — the first sweep after a kill is BOUNDED: ≤ one full
	// discovery ladder per corpse (~22s each at these settings, phases meet
	// the corpses disjointly). Measured pre-fix: 159.2s (intra-sweep
	// cooldown lapses re-paid ladders five times over); post-fix with the
	...
	if firstDeadMS > 100_000 {
		t.Fatalf("first sweep after kill took %d ms — the sweep-scoped corpse gate is not bounding it (pre-fix behavior was ~159s)", firstDeadMS)
	}

	// bound 2 — the re-discovery tax DECAYS. Pre-fix, any sweep starting
	...
	for i := rounds - 6; i < rounds; i++ {
		if loud[i] {
			t.Fatalf("35s-gap sweep %d of %d still costs %d ms — the corpse cooldown is not decaying "+
	...
	if second >= first {
		t.Fatalf("%d loud sweeps in the first half of the drive and %d in the second — the re-discovery "+
