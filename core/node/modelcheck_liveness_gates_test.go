...
// quiescentDesigneeObserved gives the workless-designee gate teeth. As it stood that gate was
// VACUOUS for its stated purpose — it can only verify the HARNESS's capability and the shared refill
// helper's uniformity, never whether any REAL oracle in the package actually EXERCISED a non-uniform
// arming in a load-bearing way, so it could never catch the fixture silently reverting to uniform
// arming. Any test that confirms — at RUNTIME, on live Node state, not by trusting its own
// construction — that a round's designee is a LIVE, ELIGIBLE proposer holding none of the height's
// pending work (the M4 workless-designee shape this whole arc is about) calls
// recordQuiescentDesigneeObserved to leave that evidence here. The gate below asserts this is
// non-empty AT THE END of the package run — package tests within one file run in declaration order
// and files are compiled in filename order, so any test earlier in file order (or in an earlier
// file) has already run and recorded by the time the gate checks; running it in isolation (a -run
// filter that excludes the recorders) will correctly and informatively fail, per its own message.
var (
	quiescentDesigneeMu       sync.Mutex
	quiescentDesigneeObserved []string
)

func recordQuiescentDesigneeObserved(gate string, designee ports.NodeID) {
	quiescentDesigneeMu.Lock()
	defer quiescentDesigneeMu.Unlock()
	quiescentDesigneeObserved = append(quiescentDesigneeObserved, fmt.Sprintf("%s: designee %s confirmed live+eligible+workless", gate, designee))
}

// The remaining RED-first gates from
// the round-ladder-desync analysis
// (already live in modelcheck_arming_test.go on this branch).

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_DesigneeMustProposeAtCertificateRound (the M2
...
// /§3 M2: a 4-anchor launch network (tier2AnchorNet — the
// strict-anchor-majority regime the field ran under). One anchor (the field's
// val-d) is killed. The other two non-designee anchors (val-a, val-c) each
// independently sweep their OWN round to 1 on their own local timeouts
...
// clock while the quorum-completing envelope was still in flight (§2: val-b's
// r1/r2/r3 timestamps, each on its own local timer, all before the 08:55:26
// catch-up jump) — and only THEN is the second envelope delivered through the
// real MsgRoundChange handler, completing the round-1 quorum while rs.Round
// == 3. That is val-b's exact recorded state.
//
...
//
// RED at HEAD (M2, chainrole.go): recordRoundChange's automatic
// designee trigger (rounds.go) fires proposeAtNewView(rs, 1, raws,
// nil) — forced == nil on every round-change, since none carried a lock
// — which falls through to proposeBlock (chainrole.go), and
// proposeBlock RE-DERIVES the round from rs.Round (chainrole.go)
...
func TestModelCheck_DesigneeMustProposeAtCertificateRound(t *testing.T) {
	...
	// validators[i]/i+1 recursion, chainrole.go) never resolves
	...
	// quorum math itself (RequiredQuorum/the anchor majority are pure
	...
	// strict anchor majority of 3) — recordRoundChange must not fire
	...
	// quorum-completing envelope was still in flight;
	...
	// MsgRoundChange handler — this is the exact moment that matters:
	// recordRoundChange now sees 2 non-proposer anchors + the designee
	// (an anchor) itself = 3 of 4 anchors, meeting both RequiredQuorum(2) and
	// the strict anchor majority (3 of 4), and fires the designee
	...
	// ──────── recordRoundChange (via checkRoundQuorum/fireDesignee)
	// runs entirely on this test's own call stack inside the
	// handleChain call above — no network delivery is needed to trigger
	// it — so the moment designee.handleChain(.) RETURNS, the natural
	// trigger has ALREADY fired (queuing real prepare requests in
	// `net`, not yet delivered) and critically has NOT touched
	// rs.Round: checkRoundQuorum's `rs.Round < round` branch
	// (rounds.go) is false (3 < 1), so it falls straight to
	// fireDesignee without re-entering advanceToRound. rs.Round is
	// therefore STILL 3 and Changes[3] STILL empty at this exact point
	// — this test's manual SECOND call below (reproducing exactly the
	// call proposeAtNewView's forced == nil leg makes, chainrole.go)
	// hits the identical round-check path regardless of the natural
	// trigger's in-flight gather, since that guard is not gated on
	// bondDrainInFlight. This MUST run before draining the network
	// below: once the real gather completes, the height commits and
	// rs.Round == 3 no longer exists (a fresh height's rs.Round starts
	// at 0) — the ablation's premise is specifically about the STALE
	// state M2 left behind, not about anything true after a successful
	// commit.
	...
	if ablationErr == nil || !strings.Contains(ablationErr.Error(), wantSig) {
		t.Fatalf("ablation: expected the HEAD failure signature %q (chainrole.go:1120, M2), got %v — "+
	...
	t.Logf("ablation: HEAD failure signature confirmed — %v", ablationErr)
	...
	if !sawPropose || capturedRound != 1 {
		t.Fatalf("REPRODUCED: designee %s held rs.Round=3 with a quorum-grade round-1 certificate "+
			...
			"(build-immutable #6); fix direction (C) in the rule.", designee.id, sawPropose, capturedRound)
	}
	t.Logf("designee %s proposed at round %d — M2 is fixed on this branch.", designee.id, capturedRound)
}

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_CatchUpTargetMustReachTheSuffixProvenFrontier is
// With a quorum-weight population at r_low and a
...
// quorum already sits."
//
// THE MINIMAL REPRODUCTION of M3's structural defect, "the catch-up target is
// pinned to the LOWEST quorum-bearing round, and can only ever be" —
// rs.Changes[r] is a POINT-IN-TIME record, not a SUFFIX claim: 8 anchors
// (f=2, RoundCatchupMet threshold = f+1 = 3). Four of them (the "D" group)
// all broadcast a round-change for r_low = 1 — a genuine quorum-weight
// population AT that one round (4 ≥ 3). Three OTHERS (the "A/B/ C" group) are
// each, independently, genuinely past round 3 — but their round-change
// broadcasts for rounds 4, 5, and 6 respectively land in THREE DIFFERENT
// Changes buckets on the observer, one sender each. This is exactly the field
// shape: three real honest members (val-a, val-b, val-c) each individually
// ahead of round 3, no single round bucket ever holding all three at once. In
// AGGREGATE — under a SUFFIX reading ("declared round ≥ r") — the three are
// unambiguous proof an honest quorum sits at round ≥ 4 (3 ≥ the threshold);
// under the shipped POINT reading, no single bucket {4}, {5}, or {6} ever
// reaches 3, so no round above 1 can ever indvidually qualify, and the
// observer's catch-up target is structurally pinned to round 1 — the "can
// only ever be the LOWEST quorum-bearing round".
//
...
//
// RED at HEAD (M3, rounds.go's per-round RoundCatchupMet scan):
...
func TestModelCheck_CatchUpTargetMustReachTheSuffixProvenFrontier(t *testing.T) {
	const nAnchors = 8
	...
	// the round-1 cascade below) — that is the property this test exists to
	...
	// legitimately catches itself up to round 1 too — ingredient (b)
	...
	if rs.Round == 0 {
		t.Fatalf("maybeCatchUpRound did not even reach round 1, where a genuine quorum-weight population (4 of 8 anchors) already sits — regression below an existing quorum round")
	}
	if rs.Round < 4 {
		t.Fatalf("REPRODUCED: 3 real, distinct, honest-quorum-weight anchors are genuinely at round >= 4 "+
			...
			"the frontier sat at r4/r5\" (research §2/§3.1). Consensus-adjacent, research-gated (build-immutable "+
			"#6); fix direction (B) — suffix semantics — in the rule.", rs.Round)
	}
	t.Logf("observer caught up to round %d — M3 is fixed on this branch.", rs.Round)
}

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_DroppedDirectRoundChangeMustStillRelay: "A
...
// exposes exactly the per-target drop this test needs —
// (*simnet.Network).DropPending(id), "the model-check's message-loss
// control" (simnet.go) — removes one specific parked message WITHOUT
...
//
// RED at HEAD (rounds.go): a round-change broadcast is one-shot,
// unacked, and never relayed — Z's having assembled the certificate does
// nothing for X, since Z (and every other node) only ever forwards a
// round-change it ITSELF originates via its own advanceToRound, never one it
// received. No wire object carries "I have a quorum for round 1" between
// peers — ports.MsgRoundCert does not exist in the port enum (grepped this
// session: zero occurrences repo-wide). X's Changes[1] stays at 2 forever,
// below the 3-anchor threshold, and X's round never leaves 0. GREEN only
// once fix direction (B)'s transferable round certificate ships (a node that
// assembles a quorum broadcasts it once per (h, r); a receiver validates it
// exactly like a proposal-carried certificate and enters r).
func TestModelCheck_DroppedDirectRoundChangeMustStillRelay(t *testing.T) {
	const nAnchors = 7
	...
	// Topology (NOT full mesh — the field's real gossip graph is not a
	// complete graph either): Y1/Y2/Y3 and every OTHER anchor talk to
	// everyone, including X. But X talks to, and is talked to by, ONLY
	// Y1/Y2/Y3 — no other anchor (Z included) has X in its own sync set.
	// This is what makes X's isolation from the dropped Y1 message genuine:
	// without it, a FULL MESH lets any node that independently catches up
	// (Z, having directly heard all three) accidentally "relay" by
	// re-broadcasting its OWN fresh round-change(1) to X, which reaches the
	// threshold through indirect diffusion — an artifact of the test's
	// connectivity, not the transferable-certificate mechanism this test
	// targets. Restricting X's peer set to exactly the three direct senders
	...
	// EXISTING ingredient (b) — has already caught its own round up to
	...
	// ── The oracle ──────────────────────────────────────────────────────────
	if xrs.Round < 1 {
		t.Fatalf("REPRODUCED: X never directly received Y1's round-change(1) (dropped), and although Z "+
			...
			"X.roundsFor.Round=%d. Round-changes are one-shot, unacked, and never relayed ( "+
			...
			"certificate — in the rule.", xrs.Round)
	}
	t.Logf("X caught up to round %d via a relayed certificate — the transferable round certificate is fixed on this branch.", xrs.Round)
}

// ────────────────────────────────────────────────────────────────────────────
//
// This gate is I1 non-regression, §4.1(3): "the S1/S2 mature oracles and
// TestModelCheck_I4_WedgedHeightMustRecover stay GREEN; plus a new oracle
// — a delayed lower-round prepare-QC arriving at a node that already
// prepared at a higher round never yields a second commit at the height."
// THIS GATE IS GREEN AT HEAD BY DESIGN — it is a non-regression pin, not a
// reproduction of a defect; none of the liveness mechanisms (M1/M2/M3) touch
// the watermark this oracle exercises.
//
// (i) CONFIRMED GREEN on this branch: `go test -count=1 -v
// -run '...'./core/node/`, full output captured):
//
//	--- PASS: TestModelCheck_SilentAuthorLockedValueMustStillCommit (0.57s)
...
//
// (ii) TestModelCheck_DelayedLowerRoundPrepareQCNeverForcesASecondCommit,
// below — the NEW oracle, expressing(3) ("a late lower-round quorum
// cannot complete at a node that has moved up and signed. slotCompare
// orders round before phase. the watermark itself is the enforcement") as
// a real, driven test over the actual node loop, plus an ablation pinning
// the round term's necessity.
func TestModelCheck_DelayedLowerRoundPrepareQCNeverForcesASecondCommit(t *testing.T) {
	nodes, ids, net, g, _ := tier2AnchorNet(t, 4)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
		all[i] = ids[i].NodeID()
	...
	// gather path uses (chainrole.go), so the mark is genuine, not
	// fabricated bytes.
	...
	if n0.signMark.Round != 2 || n0.signMark.Phase != chain.PhasePrepare || n0.signMark.Hash != hashA {
		t.Fatalf("VIOLATION: n0's watermark moved in response to a delayed LOWER-round prepare-QC — "+
			"mark is now (height=%d round=%d phase=%d hash=%x), want the untouched (1, 2, Prepare, %x); "+
			"a delayed lower-round quorum must never force a signature past the watermark (research §4.1(3))",
			n0.signMark.Height, n0.signMark.Round, n0.signMark.Phase, n0.signMark.Hash, hashA)
	}
	if rs := n0.roundsFor(); rs.Lock != nil {
		t.Fatalf("VIOLATION: n0 adopted a lock (round %d, hash %x) from the delayed lower-round prepare-QC", rs.Lock.Round, rs.Lock.Hash)
	}
	if _, h := n0.chain.Head(); h != 1 {
		t.Fatalf("VIOLATION: n0's head advanced (h=%d) — the delayed lower-round QC must never yield a second commit at the height", h)
	}
	t.Log("n0's watermark refused the delayed lower-round prepare-QC — the round term in slotCompare enforced it. GREEN at HEAD (non-regression).")
	...
	if c := slotCompare(1, 0, chain.PhasePrecommit, n0.signMark); c >= 0 {
		t.Fatalf("ablation premise: slotCompare(1, 0, Precommit, mark) should be < 0 (refuse), got %d", c)
	}
	...
	// a parallel predicate rather than a source edit (
	...
	if c := slotCompareNoRoundAblation(1, chain.PhasePrecommit, n0.signMark); c <= 0 {
		t.Fatalf("ablation: the round-blind predicate should ALLOW (c > 0) on n0's exact recorded inputs "+
	...
	t.Log("ablation: with the round case removed, the SAME inputs that slotCompare correctly refuses would " +
		"instead be ALLOWED — the round term is load-bearing, confirming the GREEN result above is attributed to it, not incidental.")
}

// slotCompareNoRoundAblation is chainrole.go's slotCompare (chainrole.go)
// with the ROUND case deleted — the "deliberately reverted round term"
// ablation, expressed as a parallel test-only predicate. Orders purely by (height, phase),
// exactly as the pre-rounds height-only watermark did.
...
// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_EntryLaneWorklessDesigneeMustCommitWithinFPlus1Rounds
// (decisive, the M4 gate),: "The schedule, unchanged, with two additional
// assertions: commitRound <= f, and the committing block's ProposerID equals
// designatedProposer(contested, commitRound)."
//
// ENTRY LANE, not reg lane: the reg-lane oracle stays as it is, and the
// entry-lane arm is added, because that is where the asymmetry is real. A bond reg already reaches every peer directly from
...
// permanent field asymmetry — the field's actual shape (parent:
// "pending_entries=2,3,5" on val-a/b/c's advancing lines).
//
// THE SCHEDULE: identical to the reg-lane oracle (matureWorld12, nodes[0,1,2] armed,
...
// during the timed loop — the own fix, since D1-entry legitimately
// forwards pending entries to the round's designee and a periodic strip
// would delete that forwarded work).
//
// RED (before ANY of A/B/C/D): M1 alone keeps the height from converging
// within the bound at all — evidence below. GREEN: D1-entry (capped at
// livenessForwardEntries) forwards the designee's missing entry, D3 (re-keyed
// to the round's own designee) backstops a lost forward, and D4 restates the
// bound — the own measurement on this SHA: the entry lane commits at 104 s,
// inside the 190 s f=1 bound.
//
// NOT independently RED, evidence-checked, and reported rather
// than papered over (build-immutable #7): a3e4e72 already carries
// forwardPendingWorkToDesignee (labelled WIP in its own commit message,
// uncapped and un-deduped) wired into advanceToRound. In THIS full-mesh,
// short-latency (5-50ms), 12-seat timed schedule, the SAME event that lets
// an armed seat's round-change contribute to a round's quorum (entering the
// round via advanceToRound) is what fires that seat's forward to that
// round's designee, in the same function call — so by the time enough
// round-changes accumulate to complete round r's quorum, the workless
// designee has, in this topology, already-near-certainly received a forward
// for it. Verified directly: running this exact construction (round-0 AND
// round-1 designees confirmed quiescent — logged below) committed cleanly
// at round 1 by its own designee, the GREEN shape, not the M4 RED shape the
// own field probe records. The probe's
// the field probe observation is real and is not in
// question — it is a different topology/timing (WAN-realistic, not this
// fixture's full mesh) than this in-process schedule reproduces, so
// The pre-fix tree, not the post-fix one, is this test's honest RED baseline.
func TestModelCheck_EntryLaneWorklessDesigneeMustCommitWithinFPlus1Rounds(t *testing.T) {
	...
	// arming 3 HEAVY (64 MiB anchor/maturer) seats carries > 1/3 of
	// the 516 MiB frozen epoch weight (192 > 172), so the PRE-EXISTING
	// maybeCatchUpRound (called unconditionally from every message
	// handler, regardless of arming) would move the quiescent seats on
	// its own — masking whether (A)'s replicated-arming fix is what's
	// actually doing the work. Prefer SYBIL seats (nodes[8.11], 1 MiB
	// each) for `armed`, so the armed set's total weight stays FAR under
	// the 1/3 catch-up bar — verified below against
	// chain.RoundCatchupMet, the real function, not hand arithmetic.
	// `killedIdx` prefers a HEAVY seat (keeps "one heavy seat killed" —
	// the field's val-d shape).
	...
	}
	{
		...
		if nodes[0].chain.RoundCatchupMet(armedIDs) {
			t.Fatalf("premise: the armed set %v MEETS RoundCatchupMet on its own weight — the pre-existing "+
				...
				"regardless of arming (A), making this schedule VACUOUS for testing it (a review F-1/F-4)", armed)
		}
	}
	...
	t.Logf("premise: round-0 designee is seat %d (quiescent), round-1 designee is seat %d (quiescent), armed=%v, killed=%d", d0idx, d1idx, armed, killedIdx)
	// Teeth: seat d0idx is confirmed live+eligible (chosen from the
	...
	recordQuiescentDesigneeObserved("", nodes[d0idx].id)
	for i, nd := range nodes {
		if i == killedIdx {
			continue
		}
		...
		if armedSet[i] {
			nd.pendingEntries = []pendingEntry{{E: mkEntry(fmt.Sprintf("-9-entry-%d", i)), At: contested}}
		} else {
	...
	// THE BOUND, at f=1 (unchanged from the reg-lane oracle — the restated D4 bound
	...
	// Keep stepping to the FULL deadline (never past it) even after
	// the first commit:, and a straggler's round-change or relayed
	// certificate may still be in flight — the height committing (some
	// node's head advanced) does not mean every OTHER node has yet
	// heard about it or acted on its own pacemaker. Bounded by the
	// SAME deadline the commit-latency measurement above used, so this
	// changes nothing about what "within the bound" means.
	...
	if !committed() {
		...
		t.Fatalf("height %d did not commit within the f=1 bound %v of the kill (per-seat rounds "+
			"reached: %v) — even on the entry lane, with D1-entry landing", contested, bound, rounds)
	}

	// the arming assertion — quiescent seats RUN THEIR OWN
	// PACEMAKER (reach round >= 1 AND emit their own round-change) —
	// is what makes the bound above non-vacuous for (A). Below the
	// catch-up weight bar (verified above), nothing but (A) can move
	// these seats.
	for i, nd := range nodes {
		...
		rs := nd.roundsFor()
		if rs.Round < 1 {
			t.Fatalf("REPRODUCED (arming): quiescent seat %d never left round 0 (M1: the round clock "+
		...
		if _, ok := rs.Changes[rs.Round][nd.id]; !ok {
			...
			if !selfAuthored {
				t.Fatalf("REPRODUCED (arming): quiescent seat %d reached round %d but never recorded "+
	...
	// ── The oracle (M4, decisive; both halves — neither alone is
	// sufficient,: "the first without the second reads as a stall, the
	// second without the first reads as a healthy takeover") ────
	if commitRound > f {
		t.Fatalf("REPRODUCED: h%d committed at round %d, above the f=%d bound — a live, "+
			...
			"speed before the takeover recovered the height. Consensus-adjacent, research-gated "+
			"(build-immutable #6); closers D1-entry(capped)+D3(re-keyed)+D4(restated bound) in the "+
			"workless-designee research.", contested, commitRound, f)
	}
	if proposerID != wantProposer {
		t.Fatalf("REPRODUCED: h%d committed at round %d by proposer %s, but designatedProposer(%d, %d) "+
			"= %s — the height committed via the rank-staggered takeover, a NON-designee work-holder, "+
			"not the round's own designee (M4, the designee rotation is work-blind: rounds.go "+
			"designatedProposer). Matches -probe-evidence.txt:398 (n2 commits h9 r3) against :4 "+
			"(designee(h9, r3) = n5).", contested, commitRound, proposerID, contested, commitRound, wantProposer)
	}
	t.Logf("h%d committed at round %d by its own designee %s — M4 is fixed on this branch.", contested, commitRound, proposerID)
}

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_ForwardCommitsEntriesNeverRegs (the forward,
// both lanes),: "One work-holder, a workless designee at round 1.
// Assert (a) the designee's round-1 block is non-empty and the height
// commits at round 1; (b) the reg lane is asserted absent — no
// MsgSubmitBondReg is sent to a designee that is not the reg's validator."
//
// THE SCHEDULE: 4 anchors (tier2AnchorNet — the strict-anchor-majority
...
// This test is specifically about what DOES and does NOT cross that wire.
//
// RED on (b) (D1 was WIP, both lanes still forwarded there):
...
// [MsgSubmitBondReg] > 0 AND D's own reply is OK: false. GREEN: the reg
// loop in forwardPendingWorkToDesignee is deleted outright (§3.1,
...
func TestModelCheck_ForwardCommitsEntriesNeverRegs(t *testing.T) {
	nodes, ids, net, g, _ := tier2AnchorNet(t, 4)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
		all[i] = ids[i].NodeID()
	...
	w.pendingEntries = []pendingEntry{{E: mkEntry("-10-entry"), At: height}}
	...
	// ── (b) the reg lane is asserted ABSENT, on the wire — the exact
	// assertion is that no MsgSubmitBondReg is EVER SENT to a designee that
	// is not the reg's validator, not merely that a sent one is refused (a
	// sent-and-refused reg still pays the ~1.5 MB egress and burns the
	// forwarder's own submit budget at the designee, per §3.1) ────────────
	if regsReceived != 0 {
		t.Fatalf("REPRODUCED (part b): D received %d MsgSubmitBondReg for a THIRD-PARTY registration "+
			"(validator %s, neither W's nor D's own) — %d of them were queued. The reg lane must never forward a "+
			"relayed registration in the first place: forwardPendingWorkToDesignee "+
	...
	t.Logf("(part b): D received ZERO MsgSubmitBondReg for the whole schedule — the reg forward loop is gone.")
	...
	if !committed {
		t.Fatalf("REPRODUCED (part a): height %d never committed — the workless designee D could not "+
			"produce a valid (non-empty) block and no forward/takeover rescued it within this schedule "+
			"(M4,). Consensus-adjacent, research-gated (build-immutable #6).", height)
	}
	if commitRound != 1 {
		t.Fatalf("REPRODUCED (part a): height %d committed at round %d, not round 1 — the forward did "+
			"not land in time for D's own round-1 attempt (hash %x)", height, commitRound, committedHash)
	}
	if entriesInBlock == 0 {
		t.Fatalf("REPRODUCED (part a): height %d committed at round 1 but the block carries ZERO entries — "+
			"D proposed EMPTY despite W's forwarded entry existing on the wire", height)
	}
	t.Logf("(part a): h%d committed at round %d with %d entries (D's own round-1 block, forwarded from W) — M4's entry lane is fixed on this branch.", height, commitRound, entriesInBlock)
}

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_ForwardEntriesCappedNotBurstBudget,: the
// entry forward's OWN constant recommended
...
// constant.
//
// RED: the forward loop's cap was `entrySubmitBurst` (32) — with only 6 queued,
// ALL 6 cross the wire (6 < 32), so this test's "exactly 4" assertion fails
// there with a DIFFERENT count. GREEN+: the forward has its own constant
// (livenessForwardEntries, recommended and shipped at 4) — exactly 4 of the 6
// cross, and the remaining 2 keep their FIFO seniority in W's own queue (still
// there, unconsumed) for the next round entry. This test cites the observed
// count, never the constant's name (livenessForwardEntries does not exist — the
// SAME file must compile there too).
func TestModelCheck_ForwardEntriesCappedNotBurstBudget(t *testing.T) {
	...
	for i := 0; i < queued; i++ {
		w.pendingEntries = append(w.pendingEntries, pendingEntry{E: mkEntry(fmt.Sprintf("-10a-entry-%d", i)), At: height})
	}
	...
	if entriesReceived != wantForwarded {
		t.Fatalf("REPRODUCED: D received %d MsgSubmitEntry out of W's %d queued entries on ONE round "+
			"entry, want exactly %d — the forward's cap is not its own constant: "+
	...
	if remaining := len(w.pendingEntries); remaining != queued {
		t.Fatalf("premise: W's own pendingEntries should be untouched by forwarding (still %d), got %d", queued, remaining)
	}
	t.Logf("D received exactly %d of W's %d queued entries on one round entry — the forward has its own, tighter cap.", entriesReceived, queued)
}

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_RoundCertNarrowedToDesigneePlusAbsent,: the assembled round
// certificate must go to the round's DESIGNEE plus a BOUNDED relay to the
// peers absent from it (DiemBFT: "sends the TC to L_{r+1}") — NEVER an
// all-peers broadcast, since a round-change carries the sender's full locked
// block and the certificate can be O(N * block).
//
...
// anchor-majority quorum on Z's OWN view. Z's `broadcastRoundCert`
...
//
// RED: `broadcastRoundCert(height, round, raws)` (3 params, no `present`
// map) loops `n.syncTargets` unconditionally except self — S1, S2, S3
// (already holding their own envelopes) ALSO receive the redundant
// certificate. GREEN+: `broadcastRoundCert` takes a `present` map and skips
// any non-designee peer already in it — S1/S2/S3 are skipped; D (designee)
// and W (absent) still receive it.
func TestModelCheck_RoundCertNarrowedToDesigneePlusAbsent(t *testing.T) {
	const nAnchors = 6
	nodes, ids, net, _, _ := tier2AnchorNet(t, nAnchors)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
	...
	// anchor-majority quorum this test needs (4) — so as soon as S1+S2
	// land, Z's OWN maybeCatchUpRound (the real, existing mechanism,
	...
	// the anchor-majority quorum on Z's OWN view (S1+S2+S3+Z = 4 of 6
	// anchors) and is what triggers the REAL broadcastRoundCert over `net`.
	z.advanceToRound(z.roundsFor(), 1, "test")
	drainHeld(t, net, fifo)

	if !received[d.id] {
		t.Fatalf("D (the round's designee) never received the certificate Z assembled — the designee must always receive it")
	}
	if !received[w.id] {
		t.Fatalf("W (never round-changed — ABSENT from the certificate) never received it — an absent peer must still learn the round within one hop ('s own premise)")
	}
	for _, s := range []*Node{s1, s2, s3} {
		if received[s.id] {
			t.Fatalf("REPRODUCED: %s — already PRESENT in the certificate (its own envelope is one of "+
				"the 3 Raws) — ALSO received a redundant broadcastRoundCert from Z: "+
				...
				"round at N=12, per the rule). Consensus-adjacent, research-gated (build-immutable #6); "+
				"closer: narrow the broadcast to the designee plus peers absent from the certificate.", s.id)
		}
	}
	t.Logf("Z's certificate reached D (designee) and W (absent) but NOT S1/S2/S3 (already present) — the broadcast is narrowed.")
}

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_RoundCertBudgetedBeforeVerify covers the remote-CPU-DoS class —
// the THIRD recurrence of it — in two independently-observed halves.
//
...
//
// RED: no per-sender budget exists on this path at all — every delivery is
// gated only by content validity, never by rate, so a 5th delivery from
// the same sender is not distinguishably refused BY RATE (it is refused,
// if at all, by content — indistinguishable from the outside). This half
// of the gate instead directly targets the OBSERVABLE budget state via
// allowWindowed's own table, checked below.
//
...
// port, not the wire), capturing the Go error text itself:+ the cap fires
// first, naming the governing-set size in its own text; no cap exists, so
// the SAME oversized, garbage-envelope input instead fails inside
// newViewFor's per-envelope CBOR decode — a DIFFERENT error origin, proving
// the old path actually attempted the work the cap is supposed to prevent.
func TestModelCheck_RoundCertBudgetedBeforeVerify(t *testing.T) {
	t.Run("cap_fires_before_verify", func(t *testing.T) {
		nodes, _, _, _, _ := tier2AnchorNet(t, 4)
		d := nodes[0]
		rs := d.roundsFor()
		// GoverningSetCap does not exist/a3e4e72 (a new port this
		// closer adds), so this test hardcodes the value it would
		// return for THIS fixture rather than calling it:
		// tier2AnchorNet(t, 4) has 4 anchors, no bonded identities and
		// no frozen epoch set yet (a fresh launch-window chain) —
		// anchors(4) + bonded(0) + epochSet(0) = 4.
		const capN = 4
		...
		if err == nil {
			t.Fatalf("premise: an oversized (%d > cap %d), garbage-envelope certificate must be refused", len(raws), capN)
		}
		wantSig := fmt.Sprintf("governing set is %d", capN)
		if !strings.Contains(err.Error(), wantSig) {
			t.Fatalf("REPRODUCED (part b): a certificate carrying %d envelopes (> the governing-set cap "+
				"%d) was refused for the WRONG reason — got %q, want it to name the cap (%q) — meaning the cap "+
				"check does not run before the per-envelope verify (the shape: unbounded work on unverified "+
		...
		t.Logf("(part b): an oversized certificate is refused by the cap, named in the error: %v", err)
	})
	...
	t.Run("per_sender_burst", func(t *testing.T) {
		...
		// Build the certificate's Raws from rs.Changes directly (the
		// SAME field both commits populate) rather than reading the
		// cached rs.Certs[round] — that cache does not exist/a3e4e72
		// (a new field this closer adds).
		...
		t.Logf("(part a) results across %d deliveries from ONE sender identity: %v", attempts, results)
		if !results[0] {
		...
		if refused == 0 {
			t.Fatalf("REPRODUCED (part a): all %d MsgRoundCert deliveries from ONE sender identity in "+
				"one window were ACCEPTED (OK=true) — no per-sender rate budget gates this path before "+
				"decode/verify (the shape, third recurrence). Consensus-adjacent, research-gated "+
		...
		t.Logf("(part a): %d of %d deliveries from one sender identity in one window were refused (OK=false) — the per-sender budget gates it.", refused, attempts)
	})
}
...
// event name it observes — used below to count proposal ATTEMPTS
// without needing to reference the new rs.Attempted field this closer adds
// (it does not exist/a3e4e72 — the same test file must compile there too).
...
// TestModelCheck_ProposeSpinDedupedPerRound, two
// independently-observed halves, item
// 5.
//
...
// same call the ablation makes):+ the error is "propose: nothing to carry"
// (chainrole.go, checked immediately after the content folds, before
// MintVersion/root population); the SAME workless attempt instead reaches
// chain.Sign and PopulateEra4Roots FIRST, failing only at
// ValidateProposal's "chain: empty block" afterward ("propose: local
// pre-check: chain: empty block") — a DIFFERENT error origin, proving the
// old path paid the cost this closer moved the check in front of.
//
...
// field) and directly observes the measured ~40 attempts in one round: count occurrences of
// "new-view proposal", the line proposeAtNewView logs on EVERY entry,
// success or failure. A THIRD sender's round-change for the SAME round,
// delivered after the designee's certificate is already assembled and its
// first (doomed, workless) attempt has already run, must not add a second
// occurrence.
//
// RED: recordRoundChange re-verifies and unconditionally re-attempts on
// every arrival that clears newViewFor and finds designatedProposer ==
// self, with no per-(h,r) memory of a prior attempt — the third sender's
// arrival adds a SECOND "new-view proposal" line.
func TestModelCheck_ProposeSpinDedupedPerRound(t *testing.T) {
	t.Run("empty_check_before_era_roots", func(t *testing.T) {
		...
		if oldSig {
			t.Fatalf("REPRODUCED (part a): the workless attempt failed with %q — the OLD signature "+
		...
		t.Logf("(part a): the workless attempt failed fast, before the era roots: %v", attemptErr)
	})
	...
	t.Run("dedup_per_round", func(t *testing.T) {
		...
		cl := &countingLogger{}
		d.SetLogger(cl)
		const attemptEvent = "new-view proposal (view-change)"
		...
		if afterThird > afterFirst {
			t.Fatalf("REPRODUCED (part b): a REDUNDANT round-change arrival for a round D already "+
				...
				"(h, r) (the rule own probe: ~40 attempts in one round). "+
		...
		t.Logf("(part b): the redundant arrival did NOT add another attempt (%d occurrences of %q) — deduped.", afterThird, attemptEvent)
	})
}

// ────────────────────────────────────────────────────────────────────────────
//
// TestModelCheck_RoundZeroCertificateRefusedUnverified, per the composed-diff re-/§5: the merge
// blocker. `newViewFor` short-circuits at round 0 (`if round == 0 { return nil, nil }`) — the SAME
// shape as "verified, distinct senders, at quorum" — without decoding or verifying a single byte.
// Before the fix, `acceptRoundCert` had no round-0 guard, so ANY peer — no signature, no
// eligibility, not even a registered identity — could send a `MsgRoundCert{Round: 0}` carrying
// attacker-chosen envelopes under attacker-chosen sender IDs claiming attacker-chosen target rounds,
...
//
// FOUR ASSERTIONS, all RED: (1) rs.Changes gains no entry at all (not even
// under the fabricated NewRound); (2) rs.Certs[0] stays nil (no unverified
// "certificate" is cached); (3) the victim's own round does not move; (4) a
// LATER, GENUINE round-1 certificate — from real, registered, signing
// anchors — still assembles normally afterward (the refusal must not poison
// or wedge the round state for the real synchronizer that follows it).
func TestModelCheck_RoundZeroCertificateRefusedUnverified(t *testing.T) {
	nodes, ids, _, _, _ := tier2AnchorNet(t, 4)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
		all[i] = ids[i].NodeID()
	...
	if got := len(rs.Changes[5]); got != 0 {
		t.Fatalf("REPRODUCED (1/4): the forged round-0 certificate wrote %d unverified entry(ies) "+
			"into rs.Changes[5] — an attacker-chosen (fabricated sender, forged signature, never verified) "+
			"round-change claim was stored..", got)
	}
	for r, m := range rs.Changes {
		if len(m) != 0 {
			t.Fatalf("REPRODUCED (1/4): the forged round-0 certificate wrote an unverified entry into "+
				"rs.Changes[%d] —.", r)
		}
	}
	// (2) no cached "certificate" at round 0.
	if c := rs.Certs[0]; c != nil {
		t.Fatalf("REPRODUCED (2/4): rs.Certs[0] was populated (%d envelopes cached) from an unverified "+
			"round-0 certificate —.", len(c.Raws))
	}
	// (3) the victim's round did not move.
	if rs.Round != 0 {
		t.Fatalf("REPRODUCED (3/4): the victim's round moved to %d from an unverified round-0 "+
			"certificate —.", rs.Round)
	}
	...
	if got := len(rs.Changes[1]); got < 2 {
		t.Fatalf("REPRODUCED (4/4): after the round-0 attack, a LATER genuine round-1 certificate did "+
			"not assemble (rs.Changes[1] holds %d senders, want >= 2) — the refusal poisoned or wedged the "+
			"round state for the real synchronizer..", got)
	}
	t.Logf("the forged round-0 certificate was refused with zero effect on rs.Changes/rs.Certs/rs.Round, and a later genuine round-1 certificate still assembles (%d senders) — the round-0 hole is closed.", len(rs.Changes[1]))
}
