...
// The peer-submit path for a demand-issuer key registration. Gates held to, the bonded
// clause at arrival and drop-never-defer at fold (S2), the submitter in the sync tick and
// the FOLDABLE drain predicate (S3), the decoder pinned to one reg (S4), a derived burst
// (S5), and the end-to-end property the Rock names: an attest-only validator's key reaches
// the chain without it ever proposing.
...
// TestMsgKindNumbersArePinned (S1): MsgKind is a positional uint8. The pair is APPENDED,
// so every pre-existing kind keeps its number — an old peer's MsgRelayPay must never be
// read by a new node as MsgRelayOpen. The literals are the tree's wire contract.
func TestMsgKindNumbersArePinned(t *testing.T) {
...
// TestArrivalGateRefusesInOrderAndLoudly (F1): every refusal class is exercised against
...
func TestArrivalGateRefusesInOrderAndLoudly(t *testing.T) {
...
// TestUnbondedFloodThenHonestBondedSubmit (§7 gate 2): two hundred fresh unbonded
...
func TestUnbondedFloodThenHonestBondedSubmit(t *testing.T) {
	proposer, attest, ids, _, _ := r211Swarm(t)
	_, next := proposer.chain.Head()
	epoch := proposer.chain.BlockEpoch(next)
	for i := 0; i < 200; i++ {
...
// TestArrivalGateIsInertOffTheObjectivePath (S2a pin): the whole handler sits inside
...
func TestArrivalGateIsInertOffTheObjectivePath(t *testing.T) {
	sched := simclock.New()
...
// TestNonFoldableQueueArmsNoProposal (§7 gate 5, S3b): a peer queue holding ONLY regs
...
func TestNonFoldableQueueArmsNoProposal(t *testing.T) {
...
// TestIdleChainCarriesTheKeyThroughTheRealDrainDriver (§7 gate 4, F2): with NO entries
...
func TestIdleChainCarriesTheKeyThroughTheRealDrainDriver(t *testing.T) {
	proposer, attest, _, net, _ := r211Swarm(t)
	// FORCE THE TAKEOVER PATH FIRST: the designation is height-keyed (height %
	// len(props)), so at some heights the proposer IS the designee and the `dist > 0`
	// branch under test is never entered. Propose fillers — before any reg is staged, so
	// nothing rides them — until the proposer is NOT the designee.
...
// TestPruneIsWiredToTheSweepAndTheEnqueue is a source gate for F3's two wiring lines:
// prunePeerIssuerKeys is called from chainSyncTick (beside SubmitIssuerKeyRegs) and at the
// top of queuePendingPeerIssuerKey. RUNTIME GATE: TestPeerQueueIsPrunedWithoutAProposal
...
func TestPruneIsWiredToTheSweepAndTheEnqueue(t *testing.T) {
...
// TestPeerQueueIsPrunedWithoutAProposal (F3): a receiver that never proposes still drops
...
func TestPeerQueueIsPrunedWithoutAProposal(t *testing.T) {
	proposer, _, _, _, _ := r211Swarm(t)
	_, next := proposer.chain.Head()
	epoch := proposer.chain.BlockEpoch(next)
...
// TestAttestOnlyValidatorsKeyIsCommittedWithoutItEverProposing is the Rock's property,
...
func TestAttestOnlyValidatorsKeyIsCommittedWithoutItEverProposing(t *testing.T) {
	...
	// Before this is where the story ended: the attest-only node holds its reg and
	...
	t.Logf("S6 measurement: %d of %d committed blocks carried IssuerKeys (attest-only key delivered via peer submit in %d carrier proposal(s))", carriers, len(all_), blocks)
}

// TestFoldDropsNeverDefersAnInadmissiblePeerReg (S2b): a queued peer reg whose issuer
...
func TestFoldDropsNeverDefersAnInadmissiblePeerReg(t *testing.T) {
	proposer, attest, _, net, all := r211Swarm(t)
	_, next := proposer.chain.Head()
	epoch := proposer.chain.BlockEpoch(next)
...
// TestQueueIsOneSlotPerIssuerEpochLatestWins: a resubmission of the same (issuer,
// epoch) replaces rather than appends; a different epoch is a different slot.
func TestQueueIsOneSlotPerIssuerEpochLatestWins(t *testing.T) {
