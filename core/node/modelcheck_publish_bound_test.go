...
// The durability-turnover discrimination oracles: run the field run's GAP left
// mature-regime publish reliability honestly unpinned between " discovery"
// and "a mature-quorum residual" — but the run's own captured client error
// (console-the field run.log:28, 'accepted but not committed within 3m0s')
// places the failure in ACCEPT→COMMIT: the token was gathered and the entry
// entered the mempool, so discovery is refuted for that run. What remained
// unpinned is HOW an accepted entry can miss a 180s poll window on a live
// matured network. These two oracles pin the remaining candidates
// deterministically (the tier order: the model discriminates, the field run
// confirms):
//
// A — fold starvation under steady mature contention (the residual
// candidate): with delivery intact, an accepted entry must ride the VERY NEXT
// committed block. RED here = a real mempool-fold residual → research
// before any fix. B — delivery fragility (the candidate the sheet never
// named): SubmitEntry's peer broadcasts are fire-and-forget (entrypool.go —
// empty reply callback, no retry), and the client submits once then only
// polls (httpregistry.go publishPollTimeout loop). If the broadcast burst is
// lost, the entry lives ONLY in the accepting node's mempool and commits when
// the designee rotation reaches that node. B MEASURES that rotation wait. It
// is a bounded-liveness measurement, not a defect repro: GREEN means the wait
// is bounded by ~|eligible| heights — which at the field's per-height escape
// bound (H_ESCAPE 220s, integration/cloudtest scenarios.sh) is tens of
// minutes, far beyond ANY client poll window. The consequence is client-side:
// the design's drop-recovery lever ("a dropped submission is re-sent by the
// client's retry loop") must actually fire INSIDE the poll window — periodic
// re-submit, a mempool-dedup no-op on the happy path — and the poll bound
// itself must cover at least one in-spec escape height.
...
// Oracle A — the residual discriminator. Steady mature contention (renewal
// queues re-armed every sweep), delivery intact: an entry submitted through a
// bonded epoch member (the field's registry validator, val-a's shape) must be
// folded into the VERY NEXT committed block — three times in a row.
// foldPendingEntries is unconditional in every proposal build (chainrole.go),
// so ANY miss is a real fold/scheduling residual of the machinery: RED routes
// to research, never a unilateral fix.
func TestModelCheck_MatureSteadyState_AcceptedEntryRidesNextBlock(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if !ok {
			t.Fatalf("publish %d: entry not committed after the next height resolved — a mempool-held entry missed an unconditional fold (residual: research-gate before touching the round machinery)", i)
		}
		if at != h0 {
			t.Fatalf("publish %d: entry committed at h%d, not the next block h%d — the fold deferred an accepted entry under steady contention (residual)", i, at, h0)
		}
		// Anti-vacuity: the carrying block must ALSO carry drain work —
...
// optional.
//
// THIS TEST IS ALSO (the research's own attribution): it is the pin for the
// MONOTONE `drainWaitSweeps` walk (chainrole.go's `maybeProposeBondDrain` —
// the accumulated wait is never reset on round entry, only when a node's OWN
// rank distance from the current designee reaches 0). A rank-`k` takeover
// needs `3 + k` accumulated sweeps regardless of how many rounds have
// elapsed since the wait began; this test's ROTATION BOUND (≤ heightBudget)
// is only correct under that monotonicity — a PER-ROUND reset would instead
// need the wait to re-accumulate `3 + k` sweeps freshly inside EACH round's
// own (increasing) duration, and rounds 0-3 last only 2, 3, 5, 8 sweeps
// respectively (`sweepsForRound`, rounds.go) — too few to ever clear a rank
// distance anywhere past the low single digits before the round itself
// advances out from under it, wedging exactly the rotation this oracle
// measures as bounded. This test therefore GOES RED under a per-round reset
// — VERIFIED via a controlled revert (n.drainWaitSweeps = 0
// added at advanceToRound's round-entry, rounds.go, in a scratch worktree,
// discarded after the run): "h17 never resolved within 8 sweeps" (this
// test's own STARVED failure, fired by the reset with no other change). No
// second, dedicated gate is needed for this regression — that failure
// already covers it.
func TestModelCheck_DroppedSubmitBroadcast_RotationWaitBounded(t *testing.T) {
	nodes, all, net, refill, honestSlashed := mcPublishWorld(t)
