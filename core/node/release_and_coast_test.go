...
// Release-and-coast, INVERTED as a regression. The red team
// broke the Sybil corner (over our own G2 merge) through the bond-TTL surface: a
// validator registers a genuine bond ONCE, then RELEASES the plot and keeps voting
// forever off that single one-time proof — because standing never expired (the TTL
// shipped OFF by default). This is the third instance of "fixed but off by default"
// (strategy doc §1). The fix is Invariant B: the TTL is safe-by-default on the
// untrusted objective path (cmd/silt effectiveBondTTL), unblocked by the
// non-proposer renewal path so honest validators don't lapse (proven over the wire
// in sim TestObjectiveBondRenewalSustainsAttestOnlyValidator).
//
...
func TestReleaseAndCoast(t *testing.T) {
	...
	// TTL OFF (the shipped-but-inert state exploited): the released validator
	// keeps FULL standing no matter how long it coasts — the live break.
	if got := releasedStandingAfter(t, 0 /*ttl off*/, 20); got != 1<<20 {
		t.Fatalf("setup: with the TTL off a released validator should coast (this is the vuln): got %d, want %d", got, 1<<20)
	}
	...
	if got := releasedStandingAfter(t, 3 /*ttl on*/, 20); got != 0 {
		t.Fatalf("regression: with the TTL on, a released validator kept %d standing — the coast attack survived", got)
	}
}
