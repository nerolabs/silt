...
// TestForgedPeerForkDoesNotQueueAPendingSlash is the gate (I5-cross-height-pruned-slash-forgery-
// FIX-DIRECTION-, reproducing §8.1). §8.1's finding was ANALYTIC, not reproduced, at
// research time: the attack needs NO Byzantine proposer. A Byzantine PEER — no proposer
// role, no quorum, no key material of its own — serves a "fork" whose block at OUR OWN
// honest height H is a forgery: Pruned set to the hash of some OTHER honest block the
// culprit genuinely signed at a DIFFERENT height H', carrying that block's real attestation.
// slashEquivocators runs on DETECTION (core/node/chainrole.go), BEFORE and
// INDEPENDENT of adoption, so the honest node convicts its own honest validator purely from
// a peer-served forgery it never adopts, and queues the forged proof for on-chain inclusion
// (chainrole.go, n.pendingSlashes).
//
// This test FIRST reproduces §8.1 by construction — the fixture below is exactly the
// shape described there — then gates the post-fix property. RED today: the forgery IS
// caught (slashedLocal set, a proof queued in pendingSlashes) exactly as §8.1 predicts.
func TestForgedPeerForkDoesNotQueueAPendingSlash(t *testing.T) {
	...
	if chain.VerifyEquivocation(&chain.Equivocation{Culprit: culpritPub, A: *ownA, B: *realOther}, ports.Hash{}, testEraFloor(0)) {
	...
	if len(nd.pendingSlashes) != 0 {
		t.Fatalf("T-5 RED (expected): a peer-served forged fork queued %d pending on-chain "+
			...
			"no key material weaponised an honest validator's own real signatures",
