...
// spends a credit with NO per-publish debit. See (The network-layer link —
...
func TestCreditDecouplesFeeFromPublish(t *testing.T) {
	const fee = 100
...
func TestCreditAndTokenDomainsAreDistinct(t *testing.T) {
	key, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		t.Fatal(err)
	}
...
func TestForgedCreditRefused(t *testing.T) {
...
func TestNoCreditChargesLegacy(t *testing.T) {
	const fee = 100
	nd, ledger, key := newIssuerNode(t, fee)
	pub := &key.PublicKey
	durable := identity.FromSeed(2).NodeID()
...
// unblind step runs the RFC 9474 §4.4 Finalize verification.
