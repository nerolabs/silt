...
// shared node-tier fixtures for the ANCHORED relay lane.
// Every relay session now opens against verified prepayment anchors under the
// relay's own chain-committed key, so every earlier test that opened a session
// needs three things it did not before: a relay with a committed key_0 and a self
// keyset, real ephemeral keypairs (sha256(Fetcher) == from), and anchors. These
// helpers give the existing tests those three things with a one-line change per
// open, so the properties they pin (the M0 guards, the clamp, the sweep, the
// reaper, the resolver seam, single-settle) stay pinned unchanged.
//
...
func commitSelfDemandKey(t *testing.T, n *Node, ident *identity.Identity, key *rsa.PrivateKey) *chain.Chain {
	...
	g := chain.Block{
		...
		Height:     0,
		Entries:    []ports.Entry{{Root: ports.HashBytes([]byte("relay-fixture-genesis"))}},
