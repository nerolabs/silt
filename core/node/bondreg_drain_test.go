...
// — an IDLE young objective network must drain its pending bond
// registrations without depending on unrelated publish traffic.
//
// The design deferred founding bond registrations off the lean genesis "to
...
// state and quiescing when there is none (B6), under the same byte
...
func TestIdleYoungNetworkDrainsPendingBondRegs(t *testing.T) {
	...
	if got := a1.Chain().BondedSize(sid.NodeID()); got < cfg.MinBond {
		...
		t.Fatalf("the idle young network never drained the non-anchor's submitted bond reg "+
	...
	if got := a1.Chain().BondedSize(a1id.NodeID()); got < cfg.MinBond {
		t.Fatalf("the proposing anchor never banked its OWN bond registration (bonded %d)", got)
	}
	// The non-anchor validator must also have SYNCED the committed chain it now
	// has standing on (issue finding 2: s1/s2 stuck at an empty chain).
	_, ah := a1.Chain().Head()
	_, sh := s.Chain().Head()
	if sh != ah {
		t.Fatalf("the non-anchor validator never caught up (s head %d, a1 head %d)", sh, ah)
	}
}

// finding 2 (the structural leg): the configured persistent-peer tier IS
...
// targets — configure-not-discover.
func TestSyncTargetsIncludeStaticPeers(t *testing.T) {
	sched := simclock.New()
	...
	if !found {
		t.Fatal("a configured static (persistent) consensus peer must be a chain-sync target — " +
			"a validator with no attester seed and no bond gossip otherwise has no path to the committed chain")
	}
}

// TestNonAttesterSyncsViaStaticPeerOnly mirrors the CLOUD sybil topology
...
func TestNonAttesterSyncsViaStaticPeerOnly(t *testing.T) {
	const bondSize = int64(2) << 20
	sched := simclock.New()
	...
	if sh != ah {
		t.Fatalf("field GAP: a non-anchor validator with an empty attester seed reached head %d, "+
...
// TestDivergentQuorumFloorStrandsSyncingNode pins the CLOUD root cause of the
...
// (max(Quorum, bftThreshold)) in every objective regime until direction
...
//
// REGIME (c), and why this test now STAYS this way: this fixture's `anchorCfg`/`highFloorCfg` never set
// `ByzantineQuorum`, so it defaults false — the EXPLICIT trusted opt-out regime,
// where `RequiredQuorum` stays `cfg.Quorum` VERBATIM even after direction (1) ships
// ("no derived rule to defer to. a `-byzantine-quorum =false` operator has declared
// a trusted swarm and owns its own config uniformity"). This test is therefore the
// REGIME-(c) CONTROL: it documents that `max(Quorum, bftThreshold)` — now more
// precisely, the BARE `cfg.Quorum` floor with no `bftThreshold` involvement at all —
// governs ONLY in this opt-out regime, and a divergent local floor there is
// expected, permanent stranding, both before AND after the fix lands. (The product
// question the old version of this comment left open — "should objective mode ignore
// the local floor when validating committed blocks?" — is answered for the DEFAULT
// `ByzantineQuorum: true` regimes by20 (“ (20)); the regime-(a) analogue of THIS
// fixture, where the answer is "must sync", is
// TestDivergentQuorumFloorSyncsUnderDerivedFloor below.)
func TestDivergentQuorumFloorStrandsSyncingNode(t *testing.T) {
	const bondSize = int64(2) << 20
	sched := simclock.New()
	net := simnet.New(sched, 4, simnet.DefaultConfig())
	...
	if sh == ah {
		t.Fatalf("expected the divergent-floor node to be STRANDED (the cloud mechanism, REGIME (c)), "+
...
// TestDivergentQuorumFloorSyncsUnderDerivedFloor is the CORRECTED arm 8a, per the
// predicate research “ §5 arm 8a: TestDivergentQuorumFloorStrandsSyncingNode above
// (the ORIGINAL fixture) never sets `ByzantineQuorum`, so it exercises REGIME (c) —
// the explicit trusted opt-out the predicate change does NOT touch — and inverting
// its assertion alone would be VACUOUS (the fixture would strand under the fixed
// code exactly as it does today, for the unrelated reason that regime (c) has no
// derived rule to defer to). This test corrects that: both configs set
// `ByzantineQuorum: true` — "also the faithful field posture, since it defaults ON
// for the untrusted objective path" (`daemon.go`) — which reaches REGIME
// (a), §1's row: `!handedOff` (`MatureValidators: 99`) makes `N = len(Anchors) = 2`,
// so `bftThreshold(2) = 1`; the anchors' 1-attestation blocks clear the DERIVED bar,
// and `requiredLaunchAnchors = ⌊2/2⌋+1 = 2` is met by proposer-if-anchor plus one
// anchor attester (`chain.go`). Under the fix, `RequiredQuorum`
// in this regime becomes `bftThreshold(validatorSetSize)` UNCONDITIONALLY — the
// `max` with the local `cfg.Quorum` disappears — so the divergent-floor node's local
// `Quorum: 3` no longer raises its own validity bar above what the anchors' blocks
// already clear, and it must sync.
//
// RED-FIRST: `RequiredQuorum` still computed `max(c.cfg.Quorum,
// bftThreshold(N))` in this regime — the OLD predicate, the "Today" column
// — so the high-floor node's own `RequiredQuorum = max(3, 1) = 3` exceeded
// the 1-attestation blocks the anchors (`RequiredQuorum = max(1, 1) = 1`)
// actually produce, `ValidateCommit`/`Reconcile` refused with
// `ErrNoQuorum`, and the node stayed stranded — the SAME mechanism as the
// regime-(c) control above, reached through the regime the fix changes.
// GREEN under direction (1): both sides compute bftThreshold(2) = 1.
// Ablation: restore the max ⇒ RED.
func TestDivergentQuorumFloorSyncsUnderDerivedFloor(t *testing.T) {
	const bondSize = int64(2) << 20
	sched := simclock.New()
	net := simnet.New(sched, 4, simnet.DefaultConfig())
	...
	anchors := map[ports.NodeID]bool{a1id.NodeID(): true, a2id.NodeID(): true}
	g := &chain.Block{Version: 1, Height: 0, Entries: []ports.Entry{mkEntry("genesis--8a")}}
	chain.Sign(g, a1id.Signer())
	...
	_, sh := sHigh.Chain().Head()
	// arm 8a 20: in regime (a), a divergent local
	// -quorum floor must not stop a node from syncing the anchors'
	// derived-bar-clearing blocks. RED-FIRST: RequiredQuorum took the
	// max with the local floor, so the high-floor node's own bar (3)
	// exceeded what the anchors' blocks actually carry (1), and
	// Reconcile refused every one of them with ErrNoQuorum.
	if sh != ah {
		t.Fatalf("arm 8a REGRESSED: the divergent-floor node did NOT sync in regime (a) "+
			...
			"drop the max with the local floor in this regime (predicate research §1 row (a)) before "+
			"this can pass", sh, ah)
	}
}
