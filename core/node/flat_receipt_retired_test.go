...
// The flat receipt is RETIRED, and the retirement itself has teeth.
//
// The rule is "the build refuses a receipt that names no open session". Until this gate
// existed nothing asserted it: a future refactor could restore a banking-and-paying
// body in handleDeliveryReceipt and every named package stayed green. The
// ablation-first discipline says the retirement
// is gated on its RUNTIME effect, not on the presence of the constant.
//
...
// RED.
//
// C1 removed the primitive itself, so there is no longer a type in
...
// "the retired lane ANSWERED OK"; reply OK with an empty body → the same
// line.
...
//
// These three declarations are byte-for-byte what core/demand exported until C1: the
// same CBOR field names (no keyasint tags, so the map keys are the Go field names) and
// the same signed message under the "silt/demand/receipt/v2" domain. They exist so
// this gate can drive what an OLD PEER actually sends, which is the only input the
// retirement makes a claim about. Nothing in production reads them.
// ---------------------------------------------------------------------------
