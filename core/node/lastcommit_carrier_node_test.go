...
// =============================================================================
// The node-tier reply-order reproducer for the carrier
// =============================================================================
//
...
//
// THE THREE MASKS the reproducer must clear:
//
...
func era4CarrierNet(t *testing.T, nAnchors, nExtra int) ([]*Node, []*identity.Identity, *simnet.Network, *chain.Block) {
	...
	// membership. Under the epoch path a mid-epoch-bonded fifth node is unqualified and
	// BOTH reply orderings would be green for the wrong reason.
...
// TestCarrierNodeTierReplyOrder drives all three arms of gate G2.
//
// RED (captured): every arm failed with "propose: commit rejected by own replica: … StateRoot
// does not equal the recomputed post-apply committed state root".
func TestCarrierNodeTierReplyOrder(t *testing.T) {
	...
	//
	// THE DISCRIMINATOR IS THE COUNT, NOT "seen != 0" (of the, §9). Asserting only
	// `seen != 0` is satisfied by the three pre-existing anchors, so it does not
	// witness the new seat at all: a regression that dropped the fifth node from the
	// carrier would still pass. Each arm therefore pins the EXACT carrier membership
	// and the EXACT seated count.
	//
	// THE ARITHMETIC, and why the two arms differ. Quorum stops
	// at the first satisfying reply and finishPC DISCARDS every later one (chainrole.go
	...
	// — the defect rejected it); it does NOT prove the fifth is seated, and no assertion here
	// may claim it does. The fifth is seated only once it makes a first-to-quorum prefix at
	// some height.
	for _, arm := range []struct {
		name string
		// First places the fifth node first in the attester list (held-delivery
		// FIFO).
		first bool
		...
		t.Run(arm.name, func(t *testing.T) {
			...
			// ----: the exact seated COUNT, plus the membership it implies. ----
