...
// — this file carries NO build tag, because the fact it pins is true in both builds
// and the enforcement has to live in the ordinary (untagged) test job.
//
// WHAT WENT WRONG. Four sites justified leaving a bond-path first-seen stamp alone by saying RecordBondChallenge is
// "stamped from the bond auditor's own request counter rather than from a wall clock, and never for a fetcher". A measurement
// measured the opposite on two real bonded validators. The tick is uint64(n.clock.Now)+1, and the daemon's node clock is
// the walltime adapter, so the tick is time.Now.UnixNano+1 — a wall-clock nanosecond, passed for this node's own id and for
// every bonded peer that answers a challenge. That first-seen stamp has since been DELETED because nothing read it. The
// tick's unit still matters: the ledger keeps it as lastBondTick, and DecayStale compares that against a nanosecond
// BondMaxAge, so a counter here would silently disable retention.
//
...
// TestBondAuditStampsAWallClockNanosecondNotACounter drives the real self-record
...
// TestRetentionReadsLastBondTickInNanoseconds), so what this test measures is the
// value retention subtracts from `now`.
func TestBondAuditStampsAWallClockNanosecondNotACounter(t *testing.T) {
	...
	if want := uint64(t0) + 1; rec.ticks[0] != want {
		t.Fatalf("bond-audit tick = %d, want %d (uint64(clock.Now)+1). The stamp is DERIVED FROM THE CLOCK; four texts once called it the auditor's request counter and were wrong (residual, since CLOSED under)", rec.ticks[0], want)
	}
