...
// — the evicted-identity re-registration storm,
// research-outcome/503-bond-renewal-storm-. An F2 slash deletes bonded[id],
// which makes BondRenewalDue's first clause (bonded < MinBond) true forever; the
// daemon re-submits its ~1.5 MB reg every sweep, no layer consults slashed, and
// honest proposers commit the banned identity's registration as a fresh block
// every ~30 s, unbounded — the island OOM's dominant driver (build-immutable
// #8). Certified fix, zero block-validity change: (c) the client never submits
// once it observes its own slash; (a) an honest receiver refuses the reg at
// arrival and an honest proposer never folds it. The structural close (a
// validity-rule rate bound, Q3) is a separate, version-gated issue.
//
...
// TestSlashedSelfNeverSubmitsRenewal — Q1(c): a validator that observes its
...
func TestSlashedSelfNeverSubmitsRenewal(t *testing.T) {
	...
	for _, held := range net.Pending() {
		if held.Kind == ports.MsgSubmitBondReg {
			t.Fatal("Q1(c): a permanently evicted identity submitted a bond renewal — the storm's client half (must back off forever, with a log naming the eviction)")
		}
	}
}
...
func TestSlashedIdentityRegRefusedAtSubmit(t *testing.T) {
	...
	if verifies != 0 {
		t.Fatalf("Q1(a): a slashed identity's reg reached the expensive verify (%d) — must be refused at arrival by a map lookup", verifies)
	}
	if len(r.pendingBondRegs) != 0 {
		t.Fatal("Q1(a): a slashed identity's reg was queued for the next block")
	}
...
// TestSlashedRegNeverFolded — Q1(a), fold: a slashed identity's reg that
...
func TestSlashedRegNeverFolded(t *testing.T) {
	...
	for _, reg := range last.BondRegs {
		if reg.ValidatorID() == evicted.NodeID() {
			t.Fatal("Q1(a): the proposer folded a slashed identity's reg into its block — the storm's commit path")
		}
	}
}
