...
// observed-address keying of the DHT eclipse cap, node tier: the gates that need the walk
// (reply-introduced ids), the simnet NAT model (ponies behind a relay) and the
// announce/resolve path (discoverability). Table-tier halves live in
// core/dht/address_keying_test.go. The
// node never sees an IP: r43bOracle stands in for the transport's ports.PeerClassifier.
...
// TestLabelledKeySurroundInOneObservedGroupStaysDiscoverable: at the announce/resolve tier
// (the H5-B shape, dht_key_discoverability_test.go): ten eclipsers own the NodeIDs closest to a
// key, declare TEN DISTINCT -domain labels (the free evasion decision measured at 0/6) and all
// answered from ONE observed /24. Under
...
func TestLabelledKeySurroundInOneObservedGroupStaysDiscoverable(t *testing.T) {
	...
	if discoverable(t, dht.AddressCapOff) {
		t.Fatal("control: under off a ten-label one-/24 surround should suppress discovery — the fixture is not a real surround ")
	}
	if !discoverable(t, dht.AddressCapOn) {
		t.Fatal("RED: a key surrounded by ten labelled eclipsers from ONE observed /24 is NOT discoverable under -dht-address-cap=on — the declared label still keys the cap, and N free labels are N domains")
	}
}

// TestReplyLearnedIDsAreNeverDirectUntilTheyAnswer: at the walk. Under held
...
func TestReplyLearnedIDsAreNeverDirectUntilTheyAnswer(t *testing.T) {
	sched := simclock.New()
	net := simnet.New(sched, 1, simnet.DefaultConfig())
	...
	for _, x := range xs {
		if r43bTabled(nA, x.NodeID()) {
			...
			if !known || c != ports.ClassUnverified || g != 0xB {
				t.Fatalf("RED: a reply-learned, never-contacted id is tabled as (class %d, group %#x, known %v); want (UNVERIFIED, introducer B's group 0xb, true) — the address book / a reply is being read as DIRECT", c, g, known)
			}
		}
	}
	if kept > 2 {
		t.Fatalf("RED: %d of 8 never-contacted ids from ONE FindNodeReply tabled in one bucket (want ≤ cap 2 per introducer group)", kept)
	}
	if kept == 0 {
		t.Fatal("fixture: no introduced id was tabled at all (the charge must admit up to the cap)")
	}
	...
	if !r43bTabled(nA, queried) || !known || c != ports.ClassDirect || g != 0xC {
		t.Fatalf("RED: an introduced id that ANSWERED A is (tabled %v, class %d, group %#x, known %v); want tabled as (DIRECT, its own /24 0xc, true)", r43bTabled(nA, queried), c, g, known)
	}
}

// TestADistinctSenderFloodEvictsNoTabledEntry: at the node: the Finding-C
// shape (4096+ distinct senders overflowing the peer-keyed soft caches) must remove NO
// tabled entry and change no stored class. The class lives with the table entry, not
// in an evictable cache.
func TestADistinctSenderFloodEvictsNoTabledEntry(t *testing.T) {
	sched := simclock.New()
	net := simnet.New(sched, 1, simnet.DefaultConfig())
	...
	for i, p := range tabled {
		if !r43bTabled(n, p.NodeID()) {
			t.Fatalf("RED (C-4): tabled peer %d was EVICTED by a %d-distinct-sender flood — the Finding-C cache overflow became an eviction weapon", i, maxPeerInfo+1000)
		}
		c, g, known := n.table.EntryClass(p.NodeID())
		if !known || c != ports.ClassDirect || g != 0x600+uint64(i) {
			t.Fatalf("RED (C-4): tabled peer %d's stored class is (class %d, group %#x, known %v) after the flood; want its admission class (DIRECT, %#x, true) — the class does not live with the entry", i, c, g, known, 0x600+uint64(i))
		}
	}
}

// r43bTopology is the swarm: public nodes, one relay, NATed ponies each on its own home
// /24. classFor(observer) mirrors what a real transport at observer would see under the
// simnet NAT model: a public sender is DIRECT at its own /24; a pony is DIRECT at its
// NAT /24 at every public node (it dialled out —) and RELAYED at the relay's /24 at
// another pony (relay-spliced both ways until a punch).
...
// TestThirtyPoniesBehindOneRelayStayDiscoverable, the honest-pony liveness sim: 30
// NATed ponies behind ONE relay + 10 public nodes under on (R=4, cap_relay=2). Every
// pony tabled at a public node is DIRECT there (it dialled out); pony presence in
// public tables is not below the off baseline; at pony observers the relayed entries
// never exceed K − R per bucket; and every pony stays discoverable from a fresh public
// fetcher.
//
// The presence arm is summed over ten simnet seeds: the proxy is
...
func TestThirtyPoniesBehindOneRelayStayDiscoverable(t *testing.T) {
	...
	for _, p := range tp.public {
		...
		for _, e := range nd.table.Closest(p.NodeID(), 1<<20) {
			...
			if !known || c != ports.ClassDirect || g != tp.group[e] {
				t.Fatalf("RED: pony %s at public node %s is (class %d, group %#x, known %v); want (DIRECT, its own NAT /24 %#x, true) — a pony that dialled out is being keyed on the relay", e.String()[:8], p.NodeID().String()[:8], c, g, known, tp.group[e])
			}
		}
	}
	if presence < baseline {
		t.Errorf("RED: pony entries across the 10 public tables, summed over 10 seeds = %d under on, %d under off (the baseline) — honest NATed presence dropped", presence, baseline)
	} else {
		t.Logf("pony presence in public tables (10 seeds) on=%d off=%d", presence, baseline)
	}
	for _, q := range tp.ponies {
		...
		for b, k := range relayedPerBucket {
			if k > 8-4 {
				t.Fatalf("RED (C-2): pony %s holds %d RELAYED entries in bucket %d; want ≤ K−R = 4", q.NodeID().String()[:8], k, b)
			}
		}
	}
	...
	if found != len(tp.ponies) {
		t.Fatalf("RED: %d of %d ponies behind ONE relay discoverable from a fresh public fetcher under on (R=4, cap_relay=2); want all %d", found, len(tp.ponies), len(tp.ponies))
	}
}

// TestSpreadSwarmDiscoverabilityUnderOn — the density/discoverability harness
// re-run with the class oracle, second clause: 30 honest spread nodes in distinct /24s;
// 10 eclipsers per key from ONE /24 surrounding six keys, joining BEFORE the honest swarm
// (incumbent) or AFTER (late); six provider/fetcher pairs. Pass: incumbent ≥ 4/6, late
// 6/6. Control: the same fixture under off must do worse than on, or the fixture does not
// discriminate.
func TestSpreadSwarmDiscoverabilityUnderOn(t *testing.T) {
	const pairs = 6
	...
	t.Logf("spread harness: incumbent on=%d/6 off=%d/6; late on=%d/6 off=%d/6", incOn, incOff, lateOn, lateOff)
	if incOn <= incOff && lateOn <= lateOff {
		t.Fatalf("control: on (incumbent %d, late %d) is no better than off (incumbent %d, late %d) — the fixture does not discriminate ", incOn, lateOn, incOff, lateOff)
	}
	if incOn < 4 {
		t.Errorf("RED: incumbent eclipsers (one /24, ten per key) — %d/6 keys discoverable under on; the cert's pass bar is ≥ 4/6", incOn)
	}
	if lateOn != pairs {
		t.Errorf("RED: late eclipsers — %d/6 keys discoverable under on; the cert's pass bar is 6/6", lateOn)
	}
}
