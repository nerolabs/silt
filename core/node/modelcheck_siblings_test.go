...
// The sibling oracles — the merge gate around the
// entries-are-mempool fix, alongside the (formerly born-RED)
// starvation oracle in modelcheck_publish_starvation_test.go:
//
...
// TestModelCheck_LaunchFace_EntryOnlyWorkArmsTheEscape is §6.1 — the soak
// run the field run face: a height whose r0 prepare slots are consumed, on a
...
func TestModelCheck_LaunchFace_EntryOnlyWorkArmsTheEscape(t *testing.T) {
...
// TestModelCheck_AdversarialDesigneeDropsEntry is §6.2 — the fairness bound:
// a designee that never receives (models: silently drops) the entry delays it
// at most its own height; the next honest designee's block carries it.
// Commits within O(f+1)=2 heights for f=1.
func TestModelCheck_AdversarialDesigneeDropsEntry(t *testing.T) {
	...
	// Drive: the dropper's height commits (its drain block, no entry),
	// then the next designee's height must carry the entry. refill keeps
	// the renewal traffic flowing — the adversarial case is
	// drop-under-load.
	for i := 0; i < 2; i++ {
		refill()
...
// TestModelCheck_EntryFIFONoInternalStarvation is §6.3 — under a budget
...
func TestModelCheck_EntryFIFONoInternalStarvation(t *testing.T) {
	nodes, ids, net, refill := matureWorld(t)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
		all[i] = ids[i].NodeID()
...
// TestModelCheck_EntryFloodDoesNotCrowdRenewals is §6.4 — Addition 1's
...
func TestModelCheck_EntryFloodDoesNotCrowdRenewals(t *testing.T) {
	nodes, ids, net, refill := matureWorld(t)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
		all[i] = ids[i].NodeID()
	...
	if len(blk[0].BondRegs) == 0 {
		t.Fatal("§6.4 CROW: the entry flood displaced the pending bond renewal from the designee's block — the budgets are not independent and consensus standing can be starved by publish traffic")
	}
...
// TestModelCheck_S1WithEntries_LockNeverDisplaced is §6.5 — the
...
func TestModelCheck_S1WithEntries_LockNeverDisplaced(t *testing.T) {
	nodes, ids, net, refill := matureWorld(t)
