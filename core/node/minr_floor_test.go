...
// The node tier of the minimum-requester floor, the census-is-a-superset pin and the
// dead-discriminator pin.
...
// TestNodeSeamAppliesTheFloor is the gate at the node tier. Node.BBootstrap is the
...
func TestNodeSeamAppliesTheFloor(t *testing.T) {
...
//
// It read two hard-coded file paths and claimed a whole-tree property. It was
// ablated past in five lines by adding a second unfloored export to a third file
// Widening it to walk the tree would still not have
...
// core/credit's TestBBootstrapHasOneExportedRoute, a walk of that whole package.
// The published property is run at the wire by cmd/silt's
// TestBelowFloorBlockIsAFunctionOfTheClockAlone.

// --- the census is a SUPERSET -------------------------------------------------

// censusSwarm is three real nodes on one simnet: a serving node with a ledger and two
// peers that will fetch from it over the wire, one on the ordinary reader path and one
// on the repair path.
type censusSwarm struct {
...
func newCensusSwarm(t *testing.T) *censusSwarm {
	t.Helper()
	sched := simclock.New()
	...
	sched.Run()
	return &censusSwarm{sched: sched, srv: srv, viewer: viewer, repair: repair, ledger: ledger}
}

// TestTheCensusIsASupersetOfAnyPopulation is. A peer fetching over the REPAIR
// path lands in the serving node's census indistinguishably from a viewer.
//
...
// DECISION that trades M0 against The pin exists so a later reader cannot mistake the
// census for the population, and so nobody re-proposes separating them at the
// instrument — the research refuted every ledger-visible discriminator.
//
...
func TestTheCensusIsASupersetOfAnyPopulation(t *testing.T) {
	s := newCensusSwarm(t)
	...
	// keypairs and one fetch each, so a fixture can do in eight
	...
	// object — the raw census does not leave core/credit any more, so this gate
	...
	for cell, n := range occupied {
		...
		case n == 2:
			shared++
			t.Logf("the viewer and the repairer share cell %v", cell)
		default:
			t.Fatalf("cell %v carries %d: the pads must occupy distinct bins, one identity each", cell, n)
		}
	}
	if shared != 1 {
		t.Fatalf("found %d cells with two identities in them (%v), want exactly 1: the repairer and the viewer must land in the SAME cell, which the rule says the instrument cannot avoid — if they separated, that is a new published attribute and a privacy question", shared, occupied)
	}
	...
	t.Logf("viewer and repairer land in ONE cell with count 2 among %d occupied cells — the census is a SUPERSET of any population P, and P is unpinned ", len(occupied))
}

// --- the dead discriminator, on the real serve path ---------------------------

// TestNoLedgerVisibleDiscriminatorOnTheServePath is the gate at the node tier. The
// research refuted splitting the census by `servedBytes > 0` on the artifact: the
...
func TestNoLedgerVisibleDiscriminatorOnTheServePath(t *testing.T) {
	s := newCensusSwarm(t)
	...
	// Every REMOTE account: zero. Reading through ServedBytes is safe here only
	// because each id already has an account (the serve created it); on an unseen id
	// that reader goes through acct → Register and MINTS one.
	for _, peer := range []*Node{s.viewer, s.repair} {
		if got := s.ledger.ServedBytes(peer.ID()); got != 0 {
			t.Fatalf("remote account %s carries servedBytes = %d. Research refuted `servedBytes > 0` as a way to separate repair traffic from viewer traffic BECAUSE it selects the empty set; if it is now non-zero for a remote account, the serve path's server argument changed and the refutation needs re-deriving", peer.ID(), got)
		}
	}
	...
	t.Logf("(serve path): servedBytes is 0 for both remote accounts and %d for the node's own, which is not in the census of %d",
