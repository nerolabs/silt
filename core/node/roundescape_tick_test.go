...
// — the field run 10a-stall-drill FAIL, made
// deterministic.
//
...
// therefore ran at dead-peer speed, not tick speed. This is the class — and
// it silently BROKE the Q3 derivation, which bounds cross-node
// round-timeout skew by ChainSyncInterval on the premise that the escape
// runs once per tick.
//
...
// staying in the callback.
func TestRoundEscapeCountsTicksNotCompletedWalks(t *testing.T) {
	...
	if r := nd.roundsFor().Round; r == 0 {
		t.Fatalf("REPRODUCED: after %d chain-sync ticks with pending work and an unresponsive peer walk, the node never left round 0 — maybeAdvanceRound runs only in SyncChain's completion callback, so dead/slow peers stall the round-escape indefinitely (field: first round-change ~8min vs the 430s bound). The escape must count TICKS.", roundAdvanceSweeps+2)
	}
}
