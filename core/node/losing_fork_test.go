...
// TestSeam7_LosingForkEquivocatorIsSlashedOnDetection pins the red-team fix:
...
func TestSeam7_LosingForkEquivocatorIsSlashedOnDetection(t *testing.T) {
	...
	// THREE launch anchors (idA, culprit, h2). makes launch finality a STRICT
	...
	// double-signer is the target. (An EVEN anchor set would force the two
	...
	// lone-culprit property under the rule; detection itself runs pre-Reconcile,
	...
	// yet the equivocator that double-signed onto that losing fork
	// IS slashed.
	if !slashed {
		t.Fatal("losing-fork slash: a validator that double-signed onto a LOSING (never-adopted) fork must be slashed on detection, not left uncosted")
	}
}
