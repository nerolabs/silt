...
// ---------------------------------------------------------------------------
// CLOSED, on the shipped lane. The consensus-attested pin used to
...
// ---------------------------------------------------------------------------
func TestDegenerateCommittedKeyIsRefusedByThePinAndTheLane(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			sched.Run()
			if pinned != 0 {
				t.Fatalf("BREAK REOPENED: the pin HELD a degenerate key "+
...
// ---------------------------------------------------------------------------
// CLOSED, through the real wire handler. The probe measured "the identical
...
// ---------------------------------------------------------------------------
func TestRestartDoesNotRePayTheSameWireReceipt(t *testing.T) {
	sched := simclock.New()
	net := simnet.New(sched, 3, simnet.DefaultConfig())
	...
	boot := func() *Node {
		...
		nd.EnableDeliverySessions(10 * ports.Second) // the token is the session anchor
		return nd
	}
	...
	if sessionPresent(t, srv2, fetcherIdent, tok, obj) {
		t.Fatal("BREAK REOPENED: the identical anchor opened a session AGAIN after a restart. A restart is an eviction of EVERY guarded token, in-window or not, which is the one eviction mode the design forbids.")
	}
...
// ---------------------------------------------------------------------------
// gate (iii), on the shipped wire lane.
//
// The fix makes every guard refusal reverse the serve's eager self-mint. The
...
// never reaches the ledger at all (the session lane). This gate holds it in place,
...
// ---------------------------------------------------------------------------
func TestUnwitnessedReceiptLeavesTheSelfMintAlone(t *testing.T) {
	sched := simclock.New()
	net := simnet.New(sched, 3, simnet.DefaultConfig())
	...
	// expected balance is EXACT: the self-mint is reversed in full and the server holds
	// the settlement's net (gross − skim) and nothing else. Asserting `!= selfMint` alone
	// passes on a lane that reverses nothing, because the settlement always pays
	// something.
	...
	if got := ledger.Balance(srv.ID()) - before; got != wantNet {
		t.Fatalf("after the WITNESSED whole-lane settlement the server holds %+d off pre-serve, "+
			...
			"a partial or absent reversal is the double-pay the negative leg above is the boundary of",
