...
// The catch-up-target oracle — the deterministic RED/GREEN home that must exist before
// the synchronizer rule moves.
//
// THE DEFECT (rounds.go maybeCatchUpRound): the responsive catch-up
...
//
// THE FIX: jump to the HIGHEST round that INDIVIDUALLY meets the
...
func TestModelCheck_CatchUpJumpsToHighestQualifyingRound(t *testing.T) {
	...
	if rs.Round == 2 {
		t.Fatalf("REPRODUCED: catch-up jumped to the SMALLEST round of the union (r2) — a sub-threshold round that cannot form a QC, pinning the duration ladder low (the defect). It must jump to the highest INDIVIDUALLY-qualifying round (r3).")
	}
	if rs.Round != 3 {
		t.Fatalf("catch-up must jump to the highest individually-qualifying round r3; landed at round %d", rs.Round)
	}
	...
	if rs2.Round != 1 {
		t.Fatalf("catch-up must NOT fire on a sub-threshold round alone (weight %d < ⅓) — a Byzantine minority must not drag the round forward; landed at round %d", 2, rs2.Round)
	}
}
