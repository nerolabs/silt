...
// ADVERSARY — the sub-frame size oracle: the column-key half, and the MsgChallenge
// scope question.
//
// Source:
//
// The pipeline-side halves of this pass (2, 4, 5, and the reconstruction arm) are in
// core/pipeline/subframe_manifest_oracle_test.go and subframe_stripe_oracle_test.go, together with the
// full note on the pin idiom used for the live defects.
...
// ══════════════════════════════════════════════════════════════════════════════
// (MED, NEW) — one plaintext guess yields seven DHT column keys.
//
// In convergent mode the ciphertext is a deterministic function of the plaintext alone
// (removed the chunk size as an accidental salt), so an attacker holding a candidate
...
func TestColumnKeysAreDerivableFromThePlaintextAlone(t *testing.T) {
	...
	// THE ATTACKER, holding only a GUESS at the plaintext, re-derives everything offline.
	// It uses a different chunk size on purpose: this gate is what makes the guess
	// sufficient WITHOUT the victim's -chunk-size, and if that ever stops being true this
	// arm reddens here rather than silently narrowing the attack.
	...
	if attackerRoot != victimLayout.Root() {
		t.Fatalf("the attacker's offline root %s does not match the victim's %s. A plaintext guess no "+
			"longer names the object — either the chunk-size salt is back (see "+
			"TestSubFrameRootIsChunkSizeIndependentAcrossTheLegalRange, which pins the collapse) or "+
			"convergent derivation changed. Establish which before assuming is closed; a narrowed attack "+
	...
	// parity leaves at columns k.k+5, per columnAt. Derived with the NODE's own functions
	// so the gate cannot drift from the placement it claims to predict.
	...
	for i := range realKeys {
		if attackerKeys[i] != realKeys[i] {
			t.Fatalf("at leaf %d the attacker-derived DHT key %s differs from the real placement key %s. "+
				...
				"That decision needs re-deriving with it.", i, attackerKeys[i], realKeys[i])
		}
	}
	...
	if len(distinct) != 1+p.ParityShards() {
		t.Fatalf("the %d leaves map to %d distinct column keys, want %d. 's "+
			"'seven holder populations' claim depends on the count; 's duplicate PARITY SHARDS do not "+
			"collapse the KEYS (the keys are derived from the column index, the shards from the matrix), so a "+
			"change here is separate from and needs its own reading.",
			len(realKeys), len(distinct), 1+p.ParityShards())
	}
}

// ══════════════════════════════════════════════════════════════════════════════
// (HIGH, PRE-EXISTING) — the re-specified gate.
//
// WHAT THE ADVERSARY ASKED FOR AND WHY IT IS NOT BUILT. The pass asked for a "challenge does
// not disclose the root to an unrelated peer" test (named verbatim
// atdecision-rt-sfo-6-msgchallenge-scope-2026-09-11.md:167, deliberately NOT spelled as a
// symbol here: it is a test that must never exist, and scripts/check_cited_tests.py is right
// to flag a Test-name in this tree that names nothing). The review ruled that test VACUOUS: it
// would pin a property that is vacuously true today, because no unrelated peer can name a
...
// precisely the decoration a green gate with no demonstrated red forbids. I agree with decision; it is also
// consistent with what TestColumnKeysAreDerivableFromThePlaintextAlone measures above (the
// attacker derives its keys from the ROOT, never from a chunk id).
//
// WHAT IS BUILT INSTEAD, per decision: the ENUMERATION, with a Layout-unseal ABLATION. The §2
// enumeration is an argument that the property holds today; per [[silt-proof-vs-structure]] an
// argument is not a structure that prevents its violation. The ablation is what converts it: the
// SAME search that finds nothing through the seal finds every shard id once the seal is removed, so
// if the outer seal is ever dropped this gate reddens instead of staying quietly green for a new
// reason. ══════════════════════════════════════════════════════════════════════════════
...
func TestShardChunkIDsAreNotDerivableWithoutTheLayoutKey(t *testing.T) {
	...
	if found := rtsfoSearch(public.Bytes(), shardIDs); len(found) != 0 {
		t.Fatalf("BREAK — %d of %d shard chunk IDs are present in the PUBLIC surface (Entry + the sealed "+
			"manifest bytes a stranger can fetch over MsgFetchChunk). First: %s.\n"+
			"  The scope decision turns on exactly this: 'no party can name a coded shard's chunk ID without "+
			...
			"  If this is RED, that decision's premise is false and 's severity must be re-opened with a review "+
			"and the red-team before anything else is decided.", len(found), len(shardIDs), found[0])
	}

	// (b) THE ABLATION — remove ONE condition (the outer seal) and re-run the SAME
	// search. It must find EVERY shard id. Without this arm, (a) is green for an unknown
	// reason: an empty haystack, a broken search, or a layout with no leaves would all
	// pass it. The ablated haystack is the EXACT plaintext the outer seal protects:
	// manifest.Seal encrypts encMode.Marshal(Layout{.}) under the layout key
	// (core/manifest/sealed.go), and encMode is cbor.CanonicalEncOptions. Re-encoding
	// the opened Layout the same way reproduces those bytes, so this is "the seal
	// removed" and not "a different haystack".
	...
	if len(found) != len(shardIDs) {
		t.Fatalf("ABLATION FAILED TO ABLATE — with the outer seal REMOVED the same search found only %d of %d "+
			...
			"the seal, so arm (a)'s green result proves nothing about the seal (an ablation that silently fails to apply reports GREEN: "+
			"a probe that silently fails to probe is indistinguishable from a passing one).",
			len(found), len(shardIDs))
	}

	// (c) THE DISCOVERY KEY IS ROOT-KEYED, NOT CHUNK-KEYED — decision's second premise,
	// asserted rather than inherited (coordination rule 3: verify before you assert). If
	// placementKey ever returns the bare chunk id for a coded shard, a chunk-keyed
	// provider lookup becomes possible and the enumeration in §2 of decision loses a
	// case.
	for i, id := range shardIDs {
		col := columnAt(i, len(layout.Chunks), layout.K, layout.N)
		if col == noColumn {
			t.Fatalf("arm (c): leaf %d of a CODED object mapped to noColumn, so placementKey falls back "+
				"to the chunk's own id and provider discovery becomes chunk-keyed. The a review notes this path "+
				"exists in code (K==0) but is unreachable in the shipped publisher; if it is now reachable, the "+
				"stranger CAN form the question and changes severity.", i)
		}
		if got := placementKey(layout.Root(), id, col); got == ports.Hash(id) {
			t.Fatalf("arm (c): placementKey returned the bare chunk id for leaf %d at column %d. "+
...
// TestManifestChunksCarryNoProofSoChallengeDisclosesNothing is the second half of the gate:
// answerChallenge on the PUBLIC, on-chain manifest chunk ids must return Found=false.
//
// The proof-attachment decision belongs to distributeFrom (a proof is built only when li:=
// grp.members[k] - manifestN is >= 0, so manifest chunks get proof=nil), so this gate drives
// the REAL Distribute path and then asks the REAL handler, rather than declining to store
// proofs itself. A fixture that simply omitted the proofs would construct the safe state whose
// production absence is the whole question.
func TestManifestChunksCarryNoProofSoChallengeDisclosesNothing(t *testing.T) {
	// A real bootstrapped ring, because Distribute is the producer under test: whether a
	// manifest chunk carries a proof is decided by distributeFrom (a proof is built only
	// when li:= grp.members[k] - manifestN is >= 0). A fixture that simply declined to store
	// proofs for manifest chunks would construct the safe state whose production absence is
	// the entire question, so the placement has to be the shipped one.
	const N = 16
	sched := simclock.New()
	...
	for i, cid := range entry.ManifestChunks {
		if found, proof := challengeAll(cid); found || proof != nil {
			t.Fatalf("BREAK — answerChallenge on manifest chunk %d (%s) returned Found=%v proof=%v.\n"+
				"  Manifest chunk ids are PUBLIC. A proof attached to one is a stranger-facing chunk -> root oracle,\n"+
				"  which is exactly the capability a review's scope decision found does NOT exist and on which it based\n"+
				"  'not RC-blocking on the privacy axis'. Re-open the scope call with a review and the red-team before\n"+
	...
	if answered == 0 {
		t.Fatalf("ABLATION FAILED TO ABLATE — answerChallenge returned Found=false for ALL %d SHARD leaves too, so\n"+
			...
			"  deleted (force the fixture into the branch). Fix the fixture\n"+
			"  before trusting the green.", len(full.Leaves()))
	}
	t.Logf("ablation drove the branch: %d of %d shard leaves answered a challenge, while 0 of %d public "+
