...
// / this lane — the PER-TIER work totals at their source.
//
...
// TestPerTierTotalsFollowTheExclusionRuleAndNameTheirAbsences.
//
// FOUR RULES, one fixture, each asserted separately:
//
// 1. REPORTING PEERS ONLY. A peer excluded from the work series by the rule
...
// expected to (a re-run of the first one found the description named the wrong
// line):
...
func TestPerTierTotalsFollowTheExclusionRuleAndNameTheirAbsences(t *testing.T) {
	n := r22Node(t)
	...
	if got, want := es.ReportersByTier[TierPony], 2; got != want {
		t.Fatalf("ReportersByTier[pony] = %d, want %d: the silent pony is excluded from the work series by and must be excluded from its per-tier coverage too", got, want)
	}
	...
	if mapHasInt64(es.RepairsByTier, TierPony) {
		t.Fatalf("RepairsByTier carries a pony entry (%d). A pony's repairs are outside the repair series' denominator (RepairCapable, coupling (b)), so a share published from this key would be a ratio over a population the tier is not in.", es.RepairsByTier[TierPony])
	}
