...
// TestZeroBountyBaseIsNamedNotSilent is the gate: a released repair whose geometry pays a
// ZERO base — k·shardBytes below one credit of fetch — is NAMED, counted in
// Stats.BountyBaseZero and journalled, whatever it ends up paying. The daemon has no
// chunk geometry at start-up to refuse on, so the loud settlement plus the publish-time
// warning (cmd/silt warnBountyChunk) are the built form of the refusal. Ablation: drop
// the base == 0 branch ⇒ RED (counter stays 0, silence).
//
// The signal reads the UNMULTIPLIED base, and this gate is why that matters: since the
// price divides once at the end, a rare stripe's multiplier can lift a zero-base geometry
// to a non-zero payment. Both arms are driven below — a geometry that pays nothing at ANY
...
// matter most.
func TestZeroBountyBaseIsNamedNotSilent(t *testing.T) {
	nd, l := mkJudge(t, 3)
	...
	// its shard is 1,024 + 8 + 16 = 1,048 bytes, far below the one
	...
	// The other arm: a zero base the rarest-shard multiplier LIFTS above zero.
