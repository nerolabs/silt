...
// TestProposerPacksPendingSlashesUnderTheBytesCap is the packing gate the research named as
// OWED: with a backlog of
...
// the overflow queued rather than dropping it. A single proof that alone exceeds the cap can
// never commit on any replica, so it is DROPPED from the queue rather than embedded
...
// TestOverCapProofDoesNotSilenceLaterProofsByTheSameCulprit is the gate (-): the
// once-per-culprit latch that stops the LOCAL penalty repeating must not also stop the
// ON-CHAIN queue. With the over-cap drop, a culprit whose FIRST detected proof is over the
// cap must still have a LATER, small proof queued for on-chain eviction — and the over-cap
// refusal is an announced operator line (S5, the), pinned here.
