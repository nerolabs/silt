...
// RUN rather than described. The auditor fixes the PoR sample space
// itself — want = Blocks(m.ChunkSize + ctOverhead), demanded EXACTLY of every leaf
// (blocksOK) — while an honest prover reports len(tags), which is Blocks(the shard it
// actually holds). Those two must be the same number for every object, including one
// framed at its true length.
//
