...
// The two gossip fields and the two work Ginis.
//
// The design doc named two honesty gaps this closes: per-node repairs-done had no
...
// TestWorkGossipLandsAndRidesTheExistingBound: rows 8-9 must actually populate the
...
func TestWorkGossipLandsAndRidesTheExistingBound(t *testing.T) {
	n := r22Node(t)
...
// TestGossipStampNeedsTheCapacityPledge pins the coupling row 10 depends on: the work
...
func TestGossipStampNeedsTheCapacityPledge(t *testing.T) {
	n := r22Node(t)
	var from ports.NodeID
	from[0] = 0x51
...
// TestTierBandsCutWhereThePublishedTableCuts. The bands are a PUBLISHED number, so
...
func TestTierBandsCutWhereThePublishedTableCuts(t *testing.T) {
	...
	if RepairCapable(TierPony) {
		t.Fatal("the pony is repair-CAPABLE in this build.  coupling (b) is that durability is guaranteed by the persistent tiers, never the transient edge — and if the pony is in the repair sample the repair Gini goes back to ~0.99 by construction")
	}
...
// TestRepairGiniIsScopedToTheCapableSubset is the CORRECTION forced on this track's
// own design doc, encoded.
//
...
func TestRepairGiniIsScopedToTheCapableSubset(t *testing.T) {
	n := r22Node(t)
	...
	// The threshold for the healthy baseline is 0.40 within
	// the capable subset. Perfectly even repair inside the subset must
	// sit far below it.
	if s.RepairGini > 0.40 {
		t.Fatalf("repairGini within the capable subset = %.4f on a PERFECTLY EVEN repair distribution — the healthy baseline must clear the 0.40 threshold or the gate is unusable", s.RepairGini)
	}
	// And the network-wide series must sit ABOVE it on that same
	// healthy network, which is the whole reason the design doc's
	// gate was vacuous.
	if networkWide <= 0.40 {
...
// TestServeGiniCoversTheWholeSampleAndSeparatesFromRepair. Serve-work is NOT scoped:
...
func TestServeGiniCoversTheWholeSampleAndSeparatesFromRepair(t *testing.T) {
	n := r22Node(t)
	for i := 0; i < 60; i++ {
	...
	s := n.EconomySample()
	if s.ServeGini > 0.15 {
		t.Fatalf("serveGini = %.4f over a perfectly even serve distribution, want <= 0.15 (the federated baseline)", s.ServeGini)
	}
	if s.RepairGini <= 0.40 {
...
// TestSelfWorkIsZeroWithoutAWorkReportingLedger: a node whose ledger does not expose
...
func TestSelfWorkIsZeroWithoutAWorkReportingLedger(t *testing.T) {
	n := r22Node(t)
