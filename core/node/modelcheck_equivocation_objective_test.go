...
// The equivocation drivability oracle — OBJECTIVE mode, 3-of-4 (the cloud
// regime). The wire drill's placement can't
// be commit-based (a 2-attestation single-target commit is quorum-short under
// RequiredQuorum=3, and no minority can commit a fork — I1), so it must be
// slash-on-DETECTION (chainrole.go): the crime is SIGNING two conflicting
// blocks at one height, not COMMITTING two forks.
//
...
// fails. Ref: core/node/losing_fork_test.go (the era-1 / A=3
// ancestor); this is its era-2 / A=4 form, the wire drill's merge gate.
func TestModelCheck_ObjectiveEquivocationSlashedOnDetection(t *testing.T) {
...
// TestModelCheck_PlaceConflictingSignedSlashedOverSync exercises the WIRE
...
// drill's core mechanism (the dedicated-net drill) proven in-process.
func TestModelCheck_PlaceConflictingSignedSlashedOverSync(t *testing.T) {
	nodes, ids, net, refill := matureWorld(t)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
		all[i] = ids[i].NodeID()
