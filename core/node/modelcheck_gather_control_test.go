...
// ──, arm 8e — the gather target survives (the control) ─────────────
//
// TestGatherTargetSurvivesTheDerivedFloor is arm 8e, per the predicate arm
// 8e: "In the young window with cfg.Quorum BELOW the derived bar,
// proposeBlockAt still gathers RequiredQuorum (assert the gathered
// attestation count, not the config). And in a uniform swarm with
// cfg.Quorum ABOVE it, committed blocks still carry cfg.Quorum
// attestations. Both halves are GREEN today and must stay GREEN after; this
// is the control 'unobservable under uniform config' argument rests on."
//
// WHY THIS IS A CONTROL, NOT a RED-first gate: `chainrole.go`
// (`proposeBlockAt`'s `if req:= n.chain.RequiredQuorum; req > quorum {
// quorum = req }`) is UNCHANGED by direction (1) — "it should still raise to
// RequiredQuorum. This is 'the proposer-side gather target' the sentence
// preserves." Both halves below must stay GREEN whether or not the predicate
// change has landed, because neither depends on WHICH regime RequiredQuorum
// computes from — only on the fact that `proposeBlockAt` always raises to
// it. Without this control, §4.4's claim that "the change is a no-op on the
// graded topology's accept path" (because every correctly-configured swarm
// already gathers to `max(cfg.Quorum, RequiredQuorum)`) has no test carrying
// it, and `chainrole.go` would read as dead code once the floor is
// derived.
//
// Half 1 (BELOW the derived bar): `tier2AnchorNet(t, 4)`'s own config —
// `Quorum: 1, ByzantineQuorum: true`, 4 anchors — has `RequiredQuorum =
// bftThreshold(4) = 2` (regime (a)), above `cfg.Quorum`. A proposal
// offered 3 non-proposer anchors must commit with >= 2 attestations (the
// DERIVED bar), not the bare `cfg.Quorum = 1` the caller passed as its own
// gather-target argument.
//
...
// all three must gather the same target: the client-publish path passes the
// caller's -quorum (chainhost.go → ProposeEntry, the sub-case
// "above_derived_bar_still_gathers_cfg.Quorum"), while the round/new-view
// re-proposal (chainrole.go proposeAtNewView) and the bond-reg drain
// (chainrole.go maybeProposeBondDrain) pass 0. With the local floor removed
// from RequiredQuorum, a raise to RequiredQuorum ALONE made those two paths
// gather bftThreshold (2) while publish gathered 3 — a path-dependent gather
// target, and an un-upgraded -quorum 3 peer (old rule: max(3, 2) = 3) refuses
// a 2-attestation block: the strand via version skew. proposeBlockAt
// therefore raises to cfg.Quorum as well (the operator's floor), so every
// path gathers max(cfg.Quorum, RequiredQuorum). The two path sub-cases were
// RED before that raise (committed block carried 2, cfg.Quorum
// 3) and GREEN after.
func TestGatherTargetSurvivesTheDerivedFloor(t *testing.T) {
	t.Run("below_derived_bar_gathers_RequiredQuorum_not_cfg.Quorum", func(t *testing.T) {
		...
		b := &chain.Block{Version: 1, Height: 1, Prev: g.Hash(), Entries: []ports.Entry{mkEntry("g--8e-below")}}
		var done bool
		var commitErr error
		// Deliberately pass the BARE cfg.Quorum (1) as the caller's own gather
		// target, exactly as the substrate test (TestModelCheck_RoundCommitsOverHeldDelivery)
		...
		// `seen` by every quorum check, RequiredQuorum included, chain.go),
		// so a raw len(blk.Atts) over-counts by exactly one.
		got := nonProposerAttCount(&blk)
		if got < nodes[0].chain.RequiredQuorum() {
			t.Fatalf("arm 8e CONTROL VIOLATION: committed block carries %d non-proposer attestations, "+
		...
		t.Logf("arm 8e half 1 confirmed: committed block carries %d non-proposer attestations (>= RequiredQuorum=%d, > cfg.Quorum=%d)",
			got, nodes[0].chain.RequiredQuorum(), cfg.Quorum)
	})

	t.Run("above_derived_bar_still_gathers_cfg.Quorum", func(t *testing.T) {
		nodes, ids, net, g, cfg := uniformQuorum3Net(t, 8700, "g--8e-above")
		// Under direction (1) RequiredQuorum is the derived
		// bftThreshold(4)=2 and IGNORES cfg.Quorum (the helper checks
		// it); the caller's own gather-target argument (cfg.Quorum=3) is
		// the binding term here.
		ids0 := ids[0].NodeID()
		attesters := []ports.NodeID{ids[1].NodeID(), ids[2].NodeID(), ids[3].NodeID()}
		all := []ports.NodeID{ids0, ids[1].NodeID(), ids[2].NodeID(), ids[3].NodeID()}

		b := &chain.Block{Version: 1, Height: 1, Prev: g.Hash(), Entries: []ports.Entry{mkEntry("g--8e-above-blk")}}
		var done bool
		var commitErr error
		nodes[0].proposeBlock(b, attesters, all, cfg.Quorum, func(err error) { done, commitErr = true, err })
		drainHeld(t, net, fifo)
		...
		t.Logf("arm 8e half 2 confirmed: committed block carries exactly cfg.Quorum=%d non-proposer attestations", got)
	})

	// The round / new-view re-proposal path (chainrole.go proposeAtNewView →
	...
	t.Run("new_view_path_gathers_cfg.Quorum", func(t *testing.T) {
		nodes, _, net, _, cfg := uniformQuorum3Net(t, 8710, "g--8e-newview")
		...
		designee.pendingEntries = []pendingEntry{{E: mkEntry("g--8e-newview-work"), At: height}}
		...
		if got := nonProposerAttCount(&blk); got != cfg.Quorum {
			t.Fatalf("arm 8e CONTROL VIOLATION (new-view path): the round-1 re-proposal committed with %d "+
				...
				"EVERY path, or an un-upgraded -quorum %d peer refuses this block (the strand via version skew)",
				got, cfg.Quorum, designee.chain.RequiredQuorum(), cfg.Quorum)
		}
		t.Logf("arm 8e new-view path confirmed: committed block carries exactly cfg.Quorum=%d non-proposer attestations", cfg.Quorum)
	})

	// The new-view FORCED leg (chainrole.go proposeAtNewView →
	// gatherTwoPhase DIRECTLY with quorum 0, never through proposeBlockAt
	// A): two peers LOCKED on a block X by a third node declare round 1
	// carrying that lock, so the round-1 designee's certificate forces X and
	// it RE-PROPOSES the locked value. The lock is a synthetic prepare-QC
	// (author + two peers, verified by VerifyPrepareQC as a premise) adopted
	// through the node's own adoptLock — the same object a real prepare phase
	// would have persisted.
	t.Run("new_view_forced_leg_gathers_cfg.Quorum", func(t *testing.T) {
		nodes, ids, net, g, cfg := uniformQuorum3Net(t, 8730, "g--8e-forced")
		...
		x := &chain.Block{Version: chain.BlockVersion, Height: height, Prev: g.Hash(), Entries: []ports.Entry{mkEntry("g--8e-forced-X")}}
		...
		if got := nonProposerAttCount(&blk); got != cfg.Quorum {
			t.Fatalf("arm 8e CONTROL VIOLATION (new-view FORCED leg): the locked re-proposal committed with %d "+
		...
		t.Logf("arm 8e new-view FORCED leg confirmed: the locked value X was re-proposed and committed with exactly cfg.Quorum=%d non-proposer attestations", cfg.Quorum)
	})
	...
	t.Run("bond_drain_path_gathers_cfg.Quorum", func(t *testing.T) {
		nodes, _, net, _, cfg := uniformQuorum3Net(t, 8720, "g--8e-drain")
		_, height := nodes[0].chain.Head()
		...
		designee.pendingEntries = []pendingEntry{{E: mkEntry("g--8e-drain-work"), At: height}}
		...
		blk := designee.Chain().Blocks(1)[0]
		if got := nonProposerAttCount(&blk); got != cfg.Quorum {
			t.Fatalf("arm 8e CONTROL VIOLATION (bond-drain path): the drain proposal committed with %d "+
				"non-proposer attestations, want cfg.Quorum = %d — maybeProposeBondDrain passes quorum 0, so the gather "+
				"target is whatever proposeBlockAt raises it to; it must be max(cfg.Quorum, RequiredQuorum()=%d) on "+
				"EVERY path, or an un-upgraded -quorum %d peer refuses this block (the strand via version skew)",
				got, cfg.Quorum, designee.chain.RequiredQuorum(), cfg.Quorum)
		}
		t.Logf("arm 8e bond-drain path confirmed: committed block carries exactly cfg.Quorum=%d non-proposer attestations", cfg.Quorum)
	})
}
...
// (RequiredQuorum/SupportMeetsQuorum, chain.go) actually counts.
