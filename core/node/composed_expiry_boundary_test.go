...
func (f *composedFixture) advanceEpochs(t *testing.T, n int) {
	...
	for i := 0; f.a.chainEpoch() < want; i++ {
		...
		b := &chain.Block{Version: 1, Height: next, Prev: prev,
			Entries: []ports.Entry{mkEntry("composed-advance-" + string(rune('a'+i)))}}
...
// mintToken performs a real blind withdrawal against key_0 and returns the token.
// The fetcher pays the fee, which is the credit the redeem later conserves.
func (f *composedFixture) mintToken(t *testing.T) demand.Token {
	...
	pub := &f.issuerPriv.PublicKey
	blinded, secret, err := demand.Withdraw(rand.Reader, pub, serial)
	if err != nil {
	...
	return demand.Unblind(pub, serial, demand.SignWithdrawal(f.issuerPriv, blinded), secret)
}
