...
// The repro — the clean re-run the field run's 10a stall, made
...
// slashed. If this test is RED, it is the deterministic face of and the
// research evidence; the machinery under test is the
...
func TestModelCheck_SilentAuthorLockedValueMustStillCommit(t *testing.T) {
...
// TestModelCheck_StaggeredSweepsMustStillConverge is THE oracle — the merge
// gate and the model-check method fix made concrete: per-node round-advance
// SKEW as a first-class adversarial schedule dimension. The lockstep
// freeze-frame above is GREEN because everyone sweeps together; the field's
// 14-round/370s stall came from independent 30s timers with arbitrary phase
// smearing the live members across rounds so that no round ever held a quorum
// of round-changes SIMULTANEOUSLY. This schedule drives exactly that smear:
// after the author cohort goes silent, the live members sweep one at a time in
// rotation — each advancing its own round on its own "timer" while the others
// stand still — the deterministic worst case of timer skew.
//
...
// GREEN with the synchronizer (increasing round duration makes rounds
// eventually outlast the smear; f+1/weight catch-up collapses it at
// message speed).
func TestModelCheck_StaggeredSweepsMustStillConverge(t *testing.T) {
	...
	// GST: the network stabilizes. A synchronized escape converges within
	// the bound; a synchronizer-less one pays the whole smear climb.
	...
	if !committed {
		...
		t.Fatalf("REPRODUCED: after GST the contested height %d did not commit within %d rotations — the pre-GST smear left the members at rounds/marks up to %d and a synchronizer-less escape must climb the whole ladder one fixed timeout at a time (research 451-view-synchronization: locking without a SYNCHRONIZER)", contested, gstBudget, maxRound)
	}
}

// TestModelCheck_DeadPeersMustNotTaxTheGather is the merge gate — the
// model-check's first COARSELY-TIMED oracle: the untimed sim models order and
// Byzantine behavior but charges NOTHING for an unreachable peer, so it was
// structurally blind to bounded liveness — the 5th recurrence of the blind
// spot. Here the sim CLOCK is the cost model: the sybil cohort is made
// UNRESPONSIVE (receiver-side drop — a request is delivered nowhere and only
// the requester's RequestTimeout timer, fired by the driver via simclock.Step,
// resolves it), so every ask to a dead member costs real simulated time. The
// oracle drives the 10a shape (a live designee proposing with a third of the
// epoch silent) and asserts the commit lands within a bounded SIMULATED
// elapsed time.
//
...
// concurrent gather — all asks in flight at once, collection completes on the
// quorum predicate at the speed of the slowest NEEDED (live) replier, and the
// dead peers' timers fire harmlessly off the critical path.
func TestModelCheck_DeadPeersMustNotTaxTheGather(t *testing.T) {
	nodes, ids, net, sched, refill := matureWorld12(t)
	all := make([]ports.NodeID, len(ids))
	for i := range ids {
		all[i] = ids[i].NodeID()
	...
	bound := 3 * timeout
	if elapsed > bound {
		t.Fatalf("REPRODUCED (coarsely-timed): the live designee's commit took %v of simulated time with 4 dead members — the SEQUENTIAL gather put every dead peer's timeout on the critical path (bound: %v = 3× the per-ask timeout; the concurrent gather pays ~one)", elapsed, bound)
	}
