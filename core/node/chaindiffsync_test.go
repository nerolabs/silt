...
// — chain-sync must not re-fetch + re-validate every peer's WHOLE chain
...
func TestChainSyncHeadProbeElidesFullFetchOnAgreement(t *testing.T) {
	...
	if got := n2.Stats.ChainSyncFullFetches - before; got != 0 {
		t.Fatalf("5 sweeps against an AGREEING peer must elide every full fetch, got %d full fetches "+
			"(the head probe should have matched and skipped)", got)
	}
	if n2.Stats.ChainSyncHeadMatches < 5 {
		t.Fatalf("expected ≥5 head-match elisions, got %d", n2.Stats.ChainSyncHeadMatches)
	}
	_ = a2
}

func TestChainSyncHeadProbeStillCatchesUp(t *testing.T) {
	...
	if added != 1 {
		t.Fatalf("n2 must catch up the 1 block it lacks via the full-fetch fallback, added=%d", added)
	}
	if n2.Stats.ChainSyncFullFetches-fetchBefore != 1 {
		t.Fatalf("a head difference must trigger exactly one full fetch, got %d", n2.Stats.ChainSyncFullFetches-fetchBefore)
	}
	_, h1 := n1.Chain().Head()
	_, h2 := n2.Chain().Head()
	if h1 != h2 {
		t.Fatalf("heads must converge after catch-up (n1=%d n2=%d)", h1, h2)
	}
	...
	if n2.Stats.ChainSyncHeadMatches-matchBefore != 1 {
		t.Fatal("once heads re-agree, the sweep must elide the full fetch again")
	}
}
