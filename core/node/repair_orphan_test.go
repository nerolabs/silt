...
// regression: a daemon restart between a repair's survivor fetches and
...
func buildOrphanWorld(t *testing.T) (store *memstore.Store, proofs *memproofs.Store, data []byte, orphans []ports.ChunkID) {
	...
	for i, nd := range nodes {
		...
		// were framed at true length and landed on the kill candidate.
		holdsManifest := false
	...
	// The confirmation gate defers a repair until the SECOND consecutive
...
func TestCareBootReconcilesOrphanedWorkingSet(t *testing.T) {
	...
	// reach quiescence (the adversary rig's careJudge comment) — drive
	// bounded Steps until the reconcile narrates, then assert on the store
	// state.
	...
	for _, id := range orphans {
		...
		if id == legit {
			if !ok {
				t.Fatalf("reconciliation dropped a PROOF-BACKED leaf %s — the discriminator is broken", id)
			}
			continue
		}
		if ok {
			t.Fatalf("orphaned working-set chunk %s survived the boot reconciliation", id)
		}
	}
	// The warm-start manifest redundancy is untouched.
	for id := range manifestHeldBefore {
		if ok, _ := care.Store().Has(bg(), id); !ok {
			t.Fatalf("reconciliation dropped a warm-start manifest chunk %s", id)
		}
	}
	// And the cleanup narrated itself for the journal.
	rec := lg.last("repair working set reconciled")
	if rec == nil {
		t.Fatal("no 'repair working set reconciled' narration")
	}
	if d, _ := rec["dropped"].(int); d != len(orphans)-1 {
		t.Fatalf("narration reports dropped=%v, want %d", rec["dropped"], len(orphans)-1)
	}
}
