...
// TestFlixzStorageProofMemoryIsOHot is the DAEMON-realistic measurement
...
// O(total held) — the bug fixed. It populates a real diskproofs store with N
...
func TestFlixzStorageProofMemoryIsOHot(t *testing.T) {
	if os.Getenv("SILT_FLIXZ_DIAG") != "1" {
		t.Skip("diagnostic; set SILT_FLIXZ_DIAG=1 to run (measures storage proof RAM)")
	}
	N := 100000
	...
	// independent of N (the O(hot) property delivers).
	if cacheUsed > budget {
