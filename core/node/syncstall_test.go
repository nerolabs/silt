...
// — the field run val-c catch-up stall, asked
// deterministically.
//
...
// chain-append of fully-era-2 blocks (prepare-QC + precommit from every
// majority seat — well over the frozen-weight threshold), crossing the h16
// epoch rotation. The behind seat's chain is exactly the majority's prefix —
// the field's severed-then-healed shape, with no live gather needed.
func world12BehindSeat(t *testing.T, ahead uint64) (behind *Node, majority []*Node, majorityIDs []ports.NodeID, net *simnet.Network, sched *simclock.Scheduler) {
	...
	for h := next; h < next+ahead; h++ {
		...
		b := &chain.Block{Version: chain.BlockVersion, Height: h, Prev: prev, Entries: []ports.Entry{mkEntry(fmt.Sprintf("stall-%d", h))}}
...
func assertCaughtUp(t *testing.T, behind *Node, ref *Node, tag string) {
	...
	if gotNext != wantNext || gotHash != wantHash {
		t.Fatalf("REPRODUCED (%s): the healed behind seat did not catch up — head next=%d hash=%x, majority next=%d hash=%x. The field ran this exact shape for 100+ min of sweeps without adopting a block.",
			tag, gotNext, gotHash[:6], wantNext, wantHash[:6])
	}
}

// Control: no mark manipulation — a healed behind seat must catch up in one sweep.
func TestSyncStall_Control_BehindSeatCatchesUp(t *testing.T) {
...
// across (chain h8, mark h17 r1 — the field run 24-vs-33 shape). The I2
// mark gates signing only; adoption of committed blocks must be unaffected.
func TestSyncStall_BehindChainAheadMark(t *testing.T) {
	behind, majority, majorityIDs, net, sched := world12BehindSeat(t, 12)
	if !behind.recordSign(17, 1, chain.PhasePrecommit, ports.HashBytes([]byte("stall-mark"))) {
...
// behind must emit the warn naming what every branch did — the exact
// line whose absence made the field stall unattributable. Shape: all peers
// dead, every probe fails, nothing adopted.
func TestSyncStall_NoProgressSweepWarns(t *testing.T) {
	...
	for _, ev := range lg.events {
		if ev == "chain sync sweep made NO progress while behind" {
			return
		}
	}
	t.Fatalf("a no-progress sweep while behind completed SILENTLY — the diagnostic warn is missing (events: %v)", lg.events)
}
