// Rounds: the node-side state and wire envelopes for the two-phase
// (prepare→precommit) gather with a lock-carrying view-change — PBFT/Tendermint
// locking instantiated over silt's launch anchor majority and mature weight quorum. The
// chain layer (core/chain, era 2) owns WHAT a valid certificate is; this file
// owns WHEN this node signs, locks, advances a round, and what a view-change
// carries.
package node
...
// roundAdvanceSweeps is the BASE round duration + the view-synchronization
// research: after sweepsForRound(r) chain-sync sweeps at one height with
// pending work and no commit, a validator broadcasts a round-change. A pure
// function of local deliveries (sweeps are message-driven under the sim clock
// — B2), never wall-clock.
//
// WHY 2, DERIVED: the base round duration must OUTRUN the cross-region skew in
...
// round); = 3+ is larger than necessary — slower recovery + more churn on the 2
// GB box, against the "smallest base, not larger" M1 guidance. Even
...
// must be re-derived.
//
// ADDENDUM: the deep-drive crawl asked whether the two-phase GATHER latency G,
// not skew, is the binding lower bound (the research held this base pending a
// measured G). Measured: intrinsic G ≈ 10 s at 12-seat WAN (the field run h74)
// — the crawl's 90–150 s "gather" was event-loop saturation from redundant
// Block.Hash work on the chain-sync path (fixed: hash memo,
// chain.TestReconcileHashWorkIsLinear). Skew remains binding; base stays 2.
const roundAdvanceSweeps = 2

// sweepsForRound is the synchronizer's load-bearing ingredient (a):
...
// co-round — the field's 14-round/370s stall, run the field run, and the
...
// nodeLock is the validator's lock: the highest-round prepare-QC it has
// witnessed for the height it is working on. Monotone in Round; carried in
// every round-change; persisted with the precommit sign-mark
// (ports.SignMark.LockQC) so a restart re-presents it.
...
type heightRounds struct {
	...
	// Armed: this node has VERIFIED at least one consensus
	// message for this height — a proposal, a prepare-QC, a round-change or
	// a round certificate — so the round clock runs here whether or not
	// this node holds pending work of its own. The arming condition is
	// REPLICATED (one member's first round-change arms every recipient
	// within one hop), which is the precondition every published liveness
	// bound silently assumes: all correct members are in the pacemaker
	// (PBFT §4.4 request-arms-the-timer, restored to network uniformity;
	// Tendermint L21). Recreated with the height, so a commit disarms every
	// seat — B6 quiescence is exactly today's when nothing is in flight
	// anywhere.
	Armed bool
	// CertSent: the rounds whose transferable certificate this node
	// has already broadcast or received — a certificate travels one hop from
	// its assembler, never floods.
	CertSent map[uint64]bool
	// Certs: the quorum-grade certificate this node holds per round,
	// VERIFIED ONCE when first assembled or received. Later arrivals for
	// a round already held cost no signature work — the verification
	// budget is O(N) per round per node, not O(N³) (the as-built first
	// cut re-ran newViewFor on every arrival and on every envelope of
	// every received certificate; the research priced that at
	// ~1,000–1,700 verifies per round per node at N = 12).
	Certs map[uint64]*roundCert
	// Attempted: the mempool signature at this node's last designee
	// proposal attempt per round. A designee proposes at most once per (h,
	// r) unless its mempool changed — the as-built first cut re-fired on
	// Every certificate envelope (~40 attempts in one round, each paying
	// the fold + era roots + Sign before the empty-block check).
...
func (n *Node) roundsFor() *heightRounds {
	...
	if n.rounds == nil || n.rounds.Height != next {
		...
		// Restart continuity: if the persisted mark is for
...
// roundCertEnv is the TRANSFERABLE round certificate (the
// DiemBFT/Jolteon TC schema, generalised to all peers as Tendermint's gossip
// does): the quorum of signed round-change envelopes for exactly (Height,
// Round). Carries no signature of its own — every envelope inside is
...
// envelope from a sender OTHER than the round's designee. With the
// mature-epoch count floor at 0, SupportMeetsQuorum — which skips `id ==
// proposer` — over a set with no non-designee sender reduces to the weight
// rule over the designee alone, so a > 2/3-weight designee could otherwise
// validate a zero-envelope certificate, or one carrying only its own envelope,
// and propose fresh, dropping any carried lock. A genuine view change always
// has >= 1 other sender; the same short-circuit class as the acceptRoundCert
// Round == 0 guard. Node-side (liveness), not a validity rule.
var errEmptyNewView = errors.New("new-view certificate carries no round-change envelope from a sender other than the designee")
...
func (n *Node) newViewFor(height, round uint64, raws [][]byte) (forced *nodeLock, err error) {
	...
	others := 0 // senders other than the designee — the population SupportMeetsQuorum counts
	var best *nodeLock
	...
	if others == 0 {
		return nil, errEmptyNewView // floor 0 must never validate a certificate with no other sender
	}
...
// maybeAdvanceRound is the deterministic round-advance rule, called once per
// chain-sync sweep: with pending consensus work and no commit at the working
// height for roundAdvanceSweeps sweeps, broadcast a signed round-change
// carrying the current lock and enter the next round. Quorum-observability:
// nobody PROPOSES at the new round until a quorum of round-changes exists
// (newViewFor), so one griefing validator can neither force nor stall a
// view-change alone.
func (n *Node) maybeAdvanceRound() {
	if n.chain == nil || !n.chain.Objective() || n.signer == nil {
		return
	}
	// The arming rule is REPLICATED: the clock runs while this node
	// holds pending work (registrations, entries, or a drain in
	// flight) OR it has verified any consensus message for the
	// working height (rs.Armed). Before this, the guard read only
	// LOCAL mempool content, so the round number was a function of
	// unreplicated private state: on the field run a 13-seat
	// network ran its pacemaker on the 3 seats holding entries
	// while 10 sat at r0 for ten minutes, and the bound was proved
	// over a population that did not exist on the wire (M1). B6
	// quiescence is preserved when TRULY idle — no work here,
	// nothing seen for this height — which is exactly today's idle
	// behaviour; the price is ≤ N round timers per CONTESTED
...
// (maybeAdvanceRound) and the catch-up paths (maybeCatchUpRound and the
...
func (n *Node) advanceToRound(rs *heightRounds, next uint64, via string) {
	...
	n.logf(ports.LogInfo, "round-change: advancing (view-change)",
	...
	// operator visibility (S5): if this height is a mature-epoch boundary
	...
	if n.chain.BoundaryLivenessFloorLost(rs.Height) {
		n.logf(ports.LogWarn, "stalled-at-boundary: live-qualified weight is at or below the frozen 2/3 bar — no live coalition can commit this epoch boundary; if the weight loss is a CONFIRMED real outage (not a partition or attack), a coordinated -liveness-recovery-height at this height is the recovery",
	...
	// necessity). Then broadcast to every sync target.
	rs.Round = next
	rs.Sweeps = 0
	// The takeover walk is keyed on (height + round) in the drain path,
	// so its rank distance tracks THIS round's designee. The
	// accumulated wait is deliberately NOT reset here: rounds 0–3 last
	// 2/3/5/8 sweeps while a rank-k walk needs 3+k, so a per-round
	// reset could never reach a far rank until the ladder outgrew it
	// (the rotation-wait oracle went RED on exactly that) — monotone
	// accumulation keeps the ≤ (N+2)·ChainSyncInterval backstop for a
	// designee whose forwarded work was lost, at the price of the
	// pre-existing Q2b-1 residue (a near-rank taker may race the
	// designee at one (h, r); the watermark bounds it to one value per
	// attester per slot).
...
// livenessForwardEntries caps the ENTRIES one work-holder forwards to a round's
// designee on entering the round. Its OWN
// constant, never entrySubmitBurst: that budget was derived for one CLIENT's
// honest cadence, whereas here up to N−1 forwarders fire at ONE seat on every
// round entry — at N = 12 inheriting 32 would land 352 ValidateEntry calls (an
// RSA verify each under -require-tokens) on the designee inside one window. The
// designee needs ONE entry to make a non-empty block; 4 is headroom over one
// and far under the client burst, so a genuine client submit in the same window
// is never starved. FIFO, so a longer queue drains over rounds with seniority
// intact.
const livenessForwardEntries = 4

// forwardPendingWorkToDesignee (on the ENTRY lane —
// M4): a round whose designee is LIVE but holds none
// of the height's pending work is wasted like a round on a DOWN designee — silt
// refuses an empty block (a validity rule, chain.go "chain: empty block") and
// the designee has priority at its round — so the ≤ f+1 bound is only as good
// as the designee's mempool. The literature never has this problem because the
// request reaches every replica (PBFT §4.4: the client multicasts to all;
// Tendermint gossips the mempool), whereas silt's entry lane reaches only the
// peers the CLIENT knew and a receiver never re-gossips. So, on entering round
// r, a work-holder forwards its queued ENTRIES to the designee of r over the
// same submit lane a client uses — an entry is self-validating (root, token,
// serial), re-validated on arrival and again at fold, so a forwarded entry is
// indistinguishable from a client submit. Leader-directed mempool gossip: ≤
// livenessForwardEntries messages per round entry, to ONE peer.
//
// REGISTRATIONS ARE NOT FORWARDED (REFUTED by the research): the reg lane is
// sender-bound — a receiver refuses any reg not submitted by its own
// validator BEFORE the ~ms space-time verify ("submit REFUSED (relay)", the
// CPU-DoS closer) — so a forwarded reg is dropped 100 % of the time after
// ~1.5 MB of egress and after burning the forwarder's own per-window submit
// budget at the designee. And it is redundant: an owner's SubmitBondRenewal
// already broadcasts its due reg to every peer it syncs with, so in the
// field every eligible proposer already holds it.
func (n *Node) forwardPendingWorkToDesignee(height, round uint64) {
	...
	for i := range n.pendingEntries {
		if entries >= livenessForwardEntries {
			break
		}
	...
	if entries > 0 {
		n.logf(ports.LogInfo, "round-change: pending entries forwarded to the round's designee", "height", height, "round", round, "designee", d, "entries", entries)
	}
}

// maybeProposeAtRound: work that reaches the designee AFTER its
...
// recordRoundChange stores a verified round-change (arming this node
// (A)) and, once the round-EXACT quorum for newRound is met at THIS node: (i)
// broadcasts the certificate once as a transferable object (the
...
//
// Cost (re-derived after the first estimate proved understated): every envelope
// stored here was verified ONCE by its wire
...
// the envelope count exceeds the governing set.
...
// storeRoundChange records one verified envelope and arms this node.
...
func (n *Node) checkRoundQuorum(rs *heightRounds, round uint64) {
	if round == 0 {
		return // round 0 needs no certificate and newViewFor verifies nothing there
	}
	if c := rs.Certs[round]; c != nil {
		if round < rs.Round {
			return // a stale arrival for a round we have left never re-fires a proposal
		}
...
// per (h, r) unless the mempool changed since the last attempt. The
// designee has PRIORITY, never exclusivity: the takeover (re-keyed to the
// round, D3) lets another work-holder propose at the same round after its
// window, and every attester admits either — I1 rests on the watermark and
// the forced value, never on who proposed.
...
//
// Packing: the queues are each capped at maxMempool (entries and regs;
// slashes are bounded by the culprit set), far below the 2^20 field width,
// so the fields cannot collide; if a queue cap ever exceeds 2^20 the shifts
// must widen with it.
...
// that formed it (a node whose only copy of a peer's round-change was dropped
// still learns the round within one hop of whoever assembled the quorum).
// NEVER to every peer: a round-change carries the sender's full locked block
// (sigBytes commits HashBytes(LockBlock)), so a certificate is O(N · block)
// once locks are carried, and up to N nodes can assemble one independently
// before any hears another's — an all-peers broadcast was priced at ~1.2 GB
// per contested round at N = 12.
// Fire-and-forget like a round-change: the certificate is idempotent
// evidence, and every jump it causes gossips a fresh round-change of its own.
func (n *Node) broadcastRoundCert(height, round uint64, raws [][]byte, present map[ports.NodeID][]byte) {
	...
	sent := 0
	for _, p := range n.syncTargets() {
		// The GOVERNING set only: syncTargets is seed ∪ static ∪ every
		// peer that ever advertised a bond root — up to maxPeerInfo
		// storage-tier peers that can never use a certificate.
	...
	n.logf(ports.LogInfo, "round-cert: sent (transferable certificate)", "height", height, "round", round, "envelopes", len(raws), "to", sent, "designee", designee)
}

// acceptRoundCert is the receive side of the transferable certificate (B):
// validate it with exactly the rule an attester applies to a
...
// that is us. The entry is made HERE, not left to the handler's following
// maybeCatchUpRound. Returns an error if the certificate is for another
// height, for round 0, empty, over the governing-set cap, or invalid.
...
// maybeCatchUpRound is the synchronizer's responsive ingredient (b),
...
// it may sign: the locking is untouched.
//
// JUMP TO THE HIGHEST INDIVIDUALLY-QUALIFYING ROUND. The threshold is evaluated
// PER ROUND — the target is the highest round whose OWN round-change senders
// meet RoundCatchupMet — not the smallest round of the senders UNIONED across
// all above-rounds. The prior union+smallest rule was a mis-application of
// PBFT's "smallest view ≥ v" (a SUFFIX quorum, where smallest-in-suffix is
// backed): silt's cross-round union is not a suffix, so its smallest member can
// carry only a fraction of the union's weight — a round structurally incapable
// of forming a QC. Targeting it pinned the effective round LOW, so the
// increasing round duration (sweepsForRound) never grew past 3-region WAN + 30s
// timer skew, so the after-GST convergence guarantee never engaged — the field's
// h68 26-minute r1-congestion thrash (deterministic RED home
...
// Still safe: a round carrying > ⅓ weight of round-changes has ≥ 1 HONEST member
// genuinely there (Byzantine < ⅓ cannot fabricate it), so the jump never
...
//
// SUFFIX SEMANTICS. A round-change for r is the claim "I am at round ≥ r" (PBFT §4.5.2),
// so the per-round membership is {senders whose DECLARED round ≥ r} — each sender's highest recorded round
// (declaredRounds) — not the point-in-time Changes[r]. The target rule is unchanged — still the highest
// individually-qualifying round — but evaluated over a suffix, which is monotone-decreasing in r and therefore
// well-defined. Under point semantics a node at r4 was invisible at r2 and r3, so the high rounds an armed
// minority climbed could never self-prove and the catch-up target was structurally the LOWEST quorum-bearing
// round (the field's ten seats jumping to r1 while the frontier sat at r4/r5 — M3).
...
// has a recorded round-change for — from the recorded envelopes (suffix
// semantics). Derived, never a second store: whatever populates Changes (the
// wire handler, a received certificate, our own advance) populates this.
...
// `above` — the candidate catch-up targets.
