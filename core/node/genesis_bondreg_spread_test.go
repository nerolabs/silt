...
// Layer 2b: a fresh multi-validator objective genesis must NOT pile every founding
// validator's ~1.5 MB space-time bond registration into one block. The GCP field run stalled
...
func TestGenesisBondRegsSpreadAcrossBlocks(t *testing.T) {
	...
	// is exactly the 308 SubmitBondReg retries the research showed). The proposer embeds at most the
	...
	for h := 1; h <= 8 && !bondedAll(); h++ {
		...
		// is what wedged the WAN field run). Exactly one oversized reg is allowed through so the
		// queue never stalls, so the byte check only bites when >1 reg is present.
		if nRegs > 1 && regBytes > a.cfg.MaxBondRegBytesPerBlock {
			t.Fatalf("block %d carries %d bond regs = %d bytes > budget %d — the ~8 MB genesis pile-up (2b) is back",
