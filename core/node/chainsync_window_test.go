...
// — chain-serve pagination. The serve side must never marshal the whole
...
// (RequestSizeFloorBytesPerSec, requestSizeExtensionCap) so they cannot
// drift.
...
// requester). Modeled on twoAgreeingValidators.
...
// the catch-up test uses), each carrying a distinct entry so blocks have
...
// TestMaxChainReplyBytesDerivation: the window is DERIVED from the two
...
func TestMaxChainReplyBytesDerivation(t *testing.T) {
	n, _ := aloneNode(t, 0)
...
// TestChainReplyDeadlineCoversWindow: the load-bearing finding — the size
// extension keys off the OUTBOUND payload, and MsgGetChain{Height} has empty
// Data, so before the fix a windowed reply had to arrive within the BASE
...
// is base + cap/2.
func TestChainReplyDeadlineCoversWindow(t *testing.T) {
	n, _ := aloneNode(t, 0)
...
// TestChainServeReplyIsWindowBounded is the defect made RED/GREEN: a
...
func TestChainServeReplyIsWindowBounded(t *testing.T) {
	...
	if got > window+oneBlock {
		t.Fatalf("the serve reply must be window-bounded (≤ %d B + one-block slack %d B), got %d B — "+
			"the server marshaled the whole suffix into one buffer (the 144 MB OOM driver)", window, oneBlock, got)
	}
}

// TestWindowedCatchUpConverges: correctness across the window loop — a
...
func TestWindowedCatchUpConverges(t *testing.T) {
	const floor = 64
	n1, n2, a1, a2, sched := windowedPair(t, floor, floor)
	const grown = 24
...
// TestNewRequesterAcceptsWholeSuffixFromLegacyServer: mixed-fleet, the
...
func TestNewRequesterAcceptsWholeSuffixFromLegacyServer(t *testing.T) {
...
// TestLegacyRequesterConvergesAgainstWindowedServer is the mixed-fleet drill:
// an un-upgraded requester does ONE MsgGetChain fetch per sweep and reconciles
// whatever it got (this test replays that exact legacy loop body against a
// windowed server). Termination is head-match, which a truncated window cannot
// satisfy, so the legacy requester converges across SWEEPS — more round-trips,
// never a silent under-sync. Each window's adoption advances its
// FinalizedHeight (adopted blocks carry the quorum attestations), so windows
// are additive sweep over sweep.
func TestLegacyRequesterConvergesAgainstWindowedServer(t *testing.T) {
	const floor = 64
	n1, n2, a1, a2, sched := windowedPair(t, floor, floor)
	const grown = 24
	growChain(t, n1, a1, a2, grown)
...
// TestSplicedWindowsFailClosed pins the safety hinge the whole scheme rests
...
func TestSplicedWindowsFailClosed(t *testing.T) {
	const floor = 64
	n1, n2, a1, a2, sched := windowedPair(t, floor, floor)
	...
	// DIFFERENT history — re-signed with a different entry at height 4, so
	// its Prev chain does not link onto window 1's last block.
	...
	if full, ferr := n2.reconstructFork(spliced); ferr == nil {
		if ok, _ := n2.Chain().Reconcile(full); ok {
			t.Fatal("a suffix spliced from inconsistent windows was ADOPTED — linkage validation must fail closed")
		}
	}
	if n2.Chain().Len() != before {
		t.Fatal("a rejected splice must leave the local chain untouched")
	}
}
