...
// Q4(i): detection of the SAME equivocation on every reconcile sweep must
// apply the local ledger slash (and fire the callback / warn log) exactly ONCE
// per culprit — not once per sweep. On the wire (run the field run) the
// missing latch re-slashed and re-logged both culprits every ~2s indefinitely.
func TestSlashEquivocatorsIsIdempotentAcrossSweeps(t *testing.T) {
	sched := simclock.New()
	...
	// THE NODE MUST HOLD A CHAIN, and that is a property of the path under test, not a
	// convenience. slashEquivocators derives its ERA FLOOR from n.chain (M2), and a chainless
	// node supplies none, which chain.CheckEquivocation refuses — the same direction n.chainID
	// already takes with its zero hash: a node that cannot know which network it is on convicts
	// nobody. The only production caller (syncOnce) dereferences n.chain several lines above the
	// call, so a chainless detection sweep is unreachable there; a fixture without one would be
	// driving a state production cannot produce.
	...
	if got := slashes[v.NodeID()]; got != 1 {
		t.Fatalf("culprit slashed %d times across 3 sweeps of the same fork — the local slash must be idempotent-once (4-i)", got)
	}
}

// Q4(ii): a pending on-chain slash record that does NOT land in a
...
func TestPendingSlashRequeuedUntilCommitted(t *testing.T) {
	sched := simclock.New()
	net := simnet.New(sched, 1, simnet.DefaultConfig())
	ledger := credit.New(50_000, 0)
	...
	if len(tn.pendingSlashes) != 1 {
		t.Fatalf("pending on-chain slash record dropped after one failed proposal (len=%d) — it must requeue until a commit confirms it (4-ii)", len(tn.pendingSlashes))
	}
}
