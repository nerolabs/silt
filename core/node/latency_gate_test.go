...
// network jitter/loss as a cheat and starved durability. Network delay is
...
func TestC1TimingIsSoftNotAHardGate(t *testing.T) {
	...
	standingUnderLatency := func(t *testing.T, hop, deadline ports.Duration, rounds int) (int64, ports.Duration, bool) {
		...
		for i := 0; i < rounds; i++ {
			...
			// prover's per-challenger rate-limit refuses the excess — an
	...
	// hard gate is gone. This is the assertion the old BREAK-1 test had
