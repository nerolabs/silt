...
// TestJudgePaysTheUndividedRepairPrice is the gate at the seam where the runtime value is
...
// earlier build paid 4. (The chunk size moved 52412 → 524264 with F1: the price is one
// SHARD of witnessed fetch, not k of them, so the shard carrying an exact 1.99996 credits
// is 10× larger. Every credit figure below is unchanged — this gate is a property of the
// division ORDER, not of the price basis.)
//
...
func TestJudgePaysTheUndividedRepairPrice(t *testing.T) {
	...
	if floorFirst != 4 {
		t.Fatalf("fixture: the pre-fix price was %d, want 4", floorFirst)
	}
