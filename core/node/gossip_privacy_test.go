...
// The gossip half — the form and its two behavioural conditions.
//
// The two work fields are gossiped ONLY under -privacy=off. The route clause is in cmd/silt
// (TestAPublishedGiniNeverReconstructsAPrivacyWithheldCounter) and its second leg is the
// self-term exclusion asserted below.
//
// WHY, in one line: the two gossiped integers are BIT-IDENTICAL to two of the three
// quantities withholds from an HTTP reader, published to a wider audience over a more
// public port at a rate the OBSERVER picks — which is the exact "bounded by the poll rate"
// non-bound was settled to close.
...
func r22PledgingNode(t *testing.T, publish bool, served int64) (*Node, *capturingTransport) {
	...
	if got, _ := n.selfWork(); got != served {
		t.Fatalf("precondition: selfWork served %d, want %d — the fixture must hold real work or 's gate is vacuous", got, served)
	}
	return n, tr
}
...
// TestWorkCountersAreGossipedOnlyWhenTheOperatorPublishesThem is the BEHAVIOURAL half
// of merge condition; adapters/tcpnet's TestWorkGossipIsAdditiveForAnOldPeer is the
...
// There: adapters/tcpnet's TestWorkGossipIsAdditiveForAnOldPeer proves by an EXACT SCAN
...
func TestWorkCountersAreGossipedOnlyWhenTheOperatorPublishesThem(t *testing.T) {
	...
	// The pledge itself still rides: gates the WORK fields, not the capacity gossip
...
// TestSelfsWorkTermsStayOutOfTheSampleOnTheWithholdingPosture is the gate's second leg. The
// route clause in cmd/silt is what the research requires; this makes the aggregate itself
// carry nothing to republish, so a future open route cannot reintroduce the break.
func TestSelfsWorkTermsStayOutOfTheSampleOnTheWithholdingPosture(t *testing.T) {
	const secret = int64(987_654_321)
...
// TestRepairsOnlySybilsSteerTheServeGiniUpwards encodes the re-decision measurement
// (register row) as a KNOWN, DISCLOSED behaviour rather than leaving it to be rediscovered
// as a surprise.
//
...
//
// IT IS NOT A REGRESSION and the fix is not to change the discriminator. Before the same
...
// either direction and may never become an input to anything.
...
func TestRepairsOnlySybilsSteerTheServeGiniUpwards(t *testing.T) {
	honest := func() *Node {
		n := r22Node(t)
		for i := 0; i < 3; i++ {
	...
	t.Logf("%d repairs-only sybils moved serveGini %.4f -> %.4f over a perfectly even network. Disclosed on the wire, never an input to anything.",
		sybils, before.ServeGini, after.ServeGini)
}

// TestGossipSampleIsExactlyGiniOverSampledValues is what lets cmd/silt's reconstruction
...
func TestGossipSampleIsExactlyGiniOverSampledValues(t *testing.T) {
	n := r22Node(t)
...
// TestANonReportingPeerIsExcludedNotCountedAsZero.
//
...
// them apart. Under the withholding posture is the default, so counting the absence as a
...
//
// THE ABLATION: flipping a peer from reporting to withholding must NOT move the Gini toward 1.
func TestANonReportingPeerIsExcludedNotCountedAsZero(t *testing.T) {
	...
	if got.ServeGini > base.ServeGini {
		t.Fatalf("five of six peers switching to the WITHHOLDING DEFAULT moved serveGini from %.4f to %.4f. Nothing about the work distribution changed — they went quiet — and the panel now reads as total capture on the shipped posture (research research)",
	...
	if cs.RepairGini <= 0.40 {
		t.Fatalf("all repair on ONE of six reporting capable nodes gives repairGini %.4f — 's exclusion has eaten the concentration signal it was supposed to leave alone", cs.RepairGini)
	}
...
// TestTheWorkStampHasExactlyOneProductionWriter closes register
// row.
//
// The gate is a COMPOSITION across two packages: adapters/tcpnet proves a Message with
// both work fields at zero emits neither CBOR key, and TestWorkCountersAreGossipedOnlyWhenTheOperatorPublishesThem above proves send stamps
...
//
// RUNTIME GATE: TestWorkCountersAreGossipedOnlyWhenTheOperatorPublishesThem observes the
...
func TestTheWorkStampHasExactlyOneProductionWriter(t *testing.T) {
	root := filepath.Join("..", "..")
	...
	err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			...
			// Never descend into ANOTHER CHECKOUT of this repo.
			// `.claude/worktrees/` holds full copies on other seats'
			// branches, and this gate reported one of their files as the
			// production writer's new home, PR — the same class, in a Go
			// test the Python fix did not reach. A checkout is identified
			// by its `.git` MARKER, a directory OR a file, not by a
			// directory name: a worktree's marker is a FILE and can live
			// anywhere.
			if path != root {
