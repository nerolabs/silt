...
// I5's honest-never-slashed / the delivery-race face).
//
...
// that signed at a height, after a crash+restart, refuses a competitor there (the
// watermark survives restart), failing-first BY CONSTRUCTION via a non-persisted-mark
// control; (3) the honest-never-slashed oracle — two proposers cross-attest-race a
// height in a WEAK config where both forks can commit (the earlier baseline: objective,
// no anchors, ByzantineQuorum off, Quorum 1, N=4 → no finality gate, no anchor gate), and
// no honest validator is slashed. the I1 shields this fork in the normal objective tree,
// so the weak config is what makes it reachable — the resolution of the deferral noted.
...
// TestModelCheck_RoundCommitsOverHeldDelivery is the substrate proof: a real
...
func TestModelCheck_RoundCommitsOverHeldDelivery(t *testing.T) {
	...
	for i, nd := range nodes {
		// Head returns the NEXT height, so a committed height-1 block ⇒
		// Head==2.
...
// gets a fresh store (the earlier in-memory-only behavior). A competitor B at height 1,
...
func runI2Restart(t *testing.T, persist bool) bool {
	...
	// the durable mark store; !persist ⇒ a fresh one (the never- crash-wipes case).
	restartMark := marks[1]
	if !persist {
...
// TestModelCheck_I2_RestartRefusesContradiction is the I2 oracle over the real loop: a
// validator that signed a block at a height, after a crash+restart, must refuse a
// competitor at that height — the never-sign-twice watermark must survive restart.
// Failing-first BY CONSTRUCTION: the same scenario with a NON-persisted mark (the
// earlier crash-wipe) attests the competitor, so the control proves the persistence is
// what does the work — this test is not vacuously green.
func TestModelCheck_I2_RestartRefusesContradiction(t *testing.T) {
	if refused := runI2Restart(t, true); !refused {
		t.Fatal("I2 VIOLATION — a restarted validator attested a block contradicting its pre-crash signature; the persisted sign-mark must survive restart (1b)")
	}
...
// runI5HonestSlash drives the honest-slash race over the real loop + held-delivery
...
// environment the fixes deliberately removed; recreating it here isolates the
// propose-time watermark as the SOLE protection against an honest double-sign.
// The two proposers are each other's ONLY attester, so the cross-attest wedge is
...
// commit message — the watermark is not test-toggleable, so this mirrors the
// controlled-revert failing-first proof used by the I1/I3/ oracles.)
func runI5HonestSlash(t *testing.T) (honestSlashed bool) {
	...
	// Detect slashes via the OnSlash callback — slashEquivocators fires it on
	// DETECTION (fetched chain vs local), before any on-chain slash record. (Checking
	// Chain.IsSlashed would be wrong: that reflects only a COMMITTED slash block,
	// which this scenario never proposes — so the honest slash would go undetected.
	// All four validators are honest, so ANY slash here is an honest one.)
...
// TestModelCheck_I5_HonestNeverSlashed is the honest-slash oracle: under a
// two-proposer cross-attest race, in a config where BOTH forks can commit, no honest
// validator is ever slashed — the propose-time never-sign-twice watermark makes each
...
func TestModelCheck_I5_HonestNeverSlashed(t *testing.T) {
	if runI5HonestSlash(t) {
		t.Fatal("I5/ VIOLATION — an honest validator was slashed under the two-proposer cross-attest race; the propose-time never-sign-twice watermark must stop any honest double-sign")
	}
}
