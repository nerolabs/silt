...
// The node-tier twin: fast-path / slow-path equivalence through the
// REAL functions, through the same coupling. The chain-tier statement of the same property is
// TestFastSlowPathSameHead in core/chain; this one drives appendExtension on one node and
// reconstructFork -> Reconcile on its twin, with the served suffix carrying a DIFFERENT valid
// certificate in each serving. Both must land on the same head.
//
// Today (59509b1) the doc comment at chainrole.go justifies the fast path with "strictly
// positive weight" — a property that has been false on every era-2 certificate since. Under T
// the justification is "strictly greater height", a theorem. GREEN now (the dead term is 0),
// GREEN after T; RED under a non-extension-monotone certificate term (see the memory).
...
func TestNodeFastSlowPathSameHead(t *testing.T) {
