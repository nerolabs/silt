...
func newRepairAdv(t *testing.T, seed int64) *repairAdv {
	...
	// 512 KiB frames: the shard clears one credit of fetch, so the bounty base
	// is NON-ZERO (/; 4 KiB chunks paid 0).
	//
	...
	// is what it prices at AFTER the F1 re-pricing. No
	...
	//
	// ⚠ RECORD CORRECTION, MEASURED 2026-09-12. This comment used to read "exactly
	// one k=10 stripe", and the research that routed the short-final-stripe fix
	// repeated it. BOTH ARE FALSE. splitFile reserves chunk.HeaderSize bytes of
	// every frame for its header, so 10·(512<<10) BYTES yield ELEVEN chunks, and
	// the object is TWO stripes: stripe 0 full (realData = 10, 16 stored refs) and
	// STRIPE 1 SHORT (realData = 1, 7 stored refs). The conclusion that rested on
	// the wrong premise still holds — every pre-existing test in this package
	// claims STRIPE 0, so none of them ever measured a short stripe — but the
	// fixture did not need a geometry change to grow one. finalStripeParityTarget
	// names it.
...
// state directly (registry + care-link) rather than calling Care, which would also
// start the perpetual repair-tick loop and stop sched.Run from ever reaching
...
// TestRepair_GarbageClaimIsSlashed (§11 a): a stripe position is really lost,
...
func TestRepair_GarbageClaimIsSlashed(t *testing.T) {
	s := newRepairAdv(t, 42)
...
// TestRepair_ComputeButDontStoreIsDenied (§11 c): a stripe position is really
...
// under test.
func TestRepair_ComputeButDontStoreIsDenied(t *testing.T) {
	s := newRepairAdv(t, 43)
	s.fundEscrow(5_000_000)
	judge := s.careJudge()
	attacker := s.nodes[2]
...
// TestClaimWithNoLossIsPaid_PINNED_DEFECT exists to pin, and it is why the
...
//
// F8 / Until 2026-09-13 all three controls stood on stageShardOn's NO-LOSS
// arrangement: fetch a shard that was never missing and re-place a live copy of it
// on a second holder. Under a durability bounty pays for a REPAIR having happened,
// so that arrangement must pay NOTHING — which made the positive control an
// assertion that the defect was correct behaviour, and the two negative controls
// rest on that control's non-vacuity.
//
...
//
// ⚠ THIS ASSERTS ONE HALF OF THE INTENT, AND ONLY ONE. The rule has two halves: a
// real repair IS paid, and a no-loss claim is NOT. The judge delivers the first
// today; the second is still broken, still pinned by
// TestClaimWithNoLossIsPaid_PINNED_DEFECT, and its closer — the loss witness
// — is GATED behind. A repair erases its own evidence (T-LOSS-IS-A-TRANSIENT), so
// the JUDGE still cannot see the loss these helpers stage; they stage it for the
// FIXTURE, not for the judge. Nothing in this file may be cited as "the bounty now
// pays for repair" or as grounds to retire the pin.
// ─────────────────────────────────────────────────────────────────────────────
...
// That distinction is the one this gate says a resolveProviders check misses, and it
// is exactly what a loss premise has to get right.
...
//
// ⚠ THE FOUR t.Fatalf LINES BELOW ARE THE ENTIRE ENFORCEMENT OF THE LOSS. Reduce this
// body to t.Helper and all three §11 controls still PASS — measured. That is not a
// fixture weakness, it is the residual showing through: nothing on
// judgeRepairClaim's path reads whether the claimed position was ever lost, so every
// judge-observable outcome (BountiesReleased, FalseRepairSlashes, EscrowPaid,
...
// TestRepair_ControlsStageTheLossAtTheirCallSites, which holds the other half — that
// the controls still CALL this helper.
...
func (s *repairAdv) rebuildLostShard(t *testing.T, repairer, holder *Node, stripe, pos int, id ports.ChunkID, leafIdx int) {
	...
	for _, r := range s.stripeRefsOf(stripe) {
		...
		if r.pos != pos { // by POSITION, never by id
...
// TestRepair_HonestClaimIsPaid is the positive control: a stripe position is
...
//
// ⚠ READ THE LOSS ARRANGEMENT BANNER ABOVE BEFORE CITING THIS TEST. It asserts one half
// of — that a real repair is paid. The other half, that a no-loss claim pays NOTHING,
// is still broken and is what TestClaimWithNoLossIsPaid_PINNED_DEFECT pins. This
// test does not move that pin.
func TestRepair_HonestClaimIsPaid(t *testing.T) {
	s := newRepairAdv(t, 44)
	s.fundEscrow(5_000_000)
	judge := s.careJudge()
...
// TestRepair_QuorumIsDiscoverableAcrossNodes (§11 d, availability side): the quorum
// a repairer broadcasts to is discovered through the careKey rendezvous, and EVERY
// caretaker that announces there is found — so honest caretakers can't be silently
// excluded from the vote by a repairer that only knows nodes near itself. (Full
// domain-DIVERSE quorum SELECTION — refusing to let one failure domain pack the
// vote — is future hardening; today the guarantee is that all announcers are
// reachable.) Uses real Care, so it advances the clock a bounded amount rather than
// running to quiescence (the repair loop reschedules forever).
func TestRepair_QuorumIsDiscoverableAcrossNodes(t *testing.T) {
	s := newRepairAdv(t, 45)
...
// stays GREEN while the F8 row silently becomes false. Nothing else in the repo
// notices: the judge has no loss check, so a control cannot observe its own premise
// being removed. That is the repo's proof-vs-structure rule with a known substitution
// attached, and this is the structure.
//
// ⚠ WHAT IT PROVES, AND WHAT IT DOES NOT. A source gate proves the call is WRITTEN. It
// cannot prove the call RUNS, and it cannot prove the helper still asserts anything —
// reduce loseStripePosition's body to t.Helper and this gate stays GREEN, as do all
// three controls. That half is held by the four in-line t.Fatalf guards inside the
...
//
// RUNTIME GATE: TestRepair_HonestClaimIsPaid (and the two negative controls). They are
// the tests that EXECUTE loseStripePosition's four t.Fatalf guards, which is the half
// this gate cannot see. This annotation follows scripts/check_source_gates.py's rule — a
// source-text gate says it is one and names its runtime cover — even though that lint's
// trigger is an os.ReadFile of a literal.go path and does not reach a parser.ParseFile
// gate like this one.
//
// IT IS FILE-SCOPED ON PURPOSE. The no-loss arrangement is the entire point of
// TestClaimWithNoLossIsPaid_PINNED_DEFECT and of the judge fixtures in
// repair_claim_gates_test.go, repairclaim_judge_fixes_test.go and
// judge_defer_test.go. They must keep staging it, and a repo-wide form of this
// rule would demand they stop.
// ─────────────────────────────────────────────────────────────────────────────

func TestRepair_ControlsStageTheLossAtTheirCallSites(t *testing.T) {
	const file = "repair_claim_adversary_test.go"
	...
	if len(offenders) > 0 {
		...
		t.Fatalf("SOURCE GATE: a §11 REPAIR-CLAIM CONTROL NO LONGER CALLS THE LOSS HELPER:\n%s\n\n"+
			...
			"  claim for a position that was never missing. Under that\n"+
			"  arrangement must pay NOTHING, and the shipped judge pays it — which is the defect\n"+
			"  TestClaimWithNoLossIsPaid_PINNED_DEFECT pins, not a control.\n"+
			"  NOTHING ELSE CATCHES THIS. The judge has no loss check (is still\n"+
			"  GATED), so the control itself stays GREEN either way and the roadmap F8 silently becomes false.\n"+
			"  If a no-loss stage is genuinely the point of a new test, put it in repair_claim_gates_test.go\n"+
			"  beside, where that arrangement is the subject rather than the premise.",
