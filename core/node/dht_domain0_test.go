...
// The DHT domain-0 question, as RULED: the "unknown ⇒ capped" table change was built and
// then STRIPPED after the red-team showed it is a regression in the default (domainless)
// swarm: with everyone undeclared, two early Sybils lock a K=8 bucket. No declared-label
// design prices an eclipse (N free labels ⇒ N domains). The close is, observed-address
// keying. These gates encode the findings so the regression cannot return and the open
// residual cannot close silently. Source:
...
// TestTwoUndeclaredSybilsDoNotLockADomainlessBucket is the Attack A, inverted into a
// regression gate: in the default swarm (nobody sets -domain), two undeclared Sybils
// observed first must NOT exclude the honest undeclared peers that follow. Under the
// stripped "unknown ⇒ capped" rule honest_kept was 0 (RED); with the exemption kept it
// is 6 (the bucket fills to K). Exclusion cost stays 8 identities.
func TestTwoUndeclaredSybilsDoNotLockADomainlessBucket(t *testing.T) {
...
// TestLabelledSybilsDefeatTheDomainCap_OPEN_BREAK — FLIPPED 2026-09-04.
// Until this gate asserted the LIVE break: N Sybils declaring N distinct free labels
// were N domains, the per-domain cap never fired, and they filled the bucket. the gate keys
...
// address rule alone that defends. Name kept: the change log cites it.
func TestLabelledSybilsDefeatTheDomainCap_OPEN_BREAK(t *testing.T) {
	self := identity.FromSeed(1).NodeID()
	...
	if s > 2 {
		t.Fatalf("RED: %d of 8 labelled Sybils from ONE observed /24 admitted into a K=8 bucket under -dht-address-cap=on (cap_direct=2) — a declared label still keys the cap; N free labels are N domains", s)
	}
	if h != 2 {
		t.Fatalf("RED: %d of 2 honest peers (distinct observed /24s) admitted past the labelled surround under on; want 2", h)
	}
}

// TestHelloWritesOnlyTheSendersOwnDomain is the boundary that HELD: the only write to
// peerDomains is keyed by the authenticated sender, so a peer can set its own domain
// and nobody else's. A third-party poisoning path would be a new exemption/eviction
// lever; this pins its absence.
func TestHelloWritesOnlyTheSendersOwnDomain(t *testing.T) {
	sched := simclock.New()
