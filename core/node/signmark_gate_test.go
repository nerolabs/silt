...
// =============================================================================
// THE DURABLE SIGN-MARK MUST NOT BE RE-INTERPRETED BY THE ERA-4 UPGRADE.
// =============================================================================
//
// THE HAZARD (the seam the project-call-A proposal hid).
//
// ports.SignMark is FSYNCED TO DISK before any consensus signature is released — it is silt's
// copy of Tendermint's priv_validator_state, i.e. the artifact itself. slotCompare orders it
...
//
// THE FIX, verifies, and it is one line of discipline rather than a migration:
//
...
// encoding is then BYTE-IDENTICAL across the upgrade — no migration, no replay.
//
...
func TestUpgradeMustNotReinterpretTheDurableSignMark(t *testing.T) {
	const H, R = 7, 2
	...
	for _, step := range []uint8{chain.PhaseLegacy, chain.PhasePrepare, chain.PhasePrecommit} {
		if restarted.signAllowedAt(H, R, step, blockY) {
			t.Fatalf("VIOLATED: after an upgrade, a node holding a durable (H=%d, r=%d, precommit) "+
				"mark signed a DIFFERENT block at step %d. That is a self-manufactured double-sign produced "+
				"by the upgrade itself — the crash variant, through the front door", H, R, step)
		}
	}
	...
	if !restarted.signAllowedAt(H, R, chain.PhasePrecommit, blockX) {
		t.Fatal("an idempotent re-sign of the SAME block at the marked slot must stay allowed, " +
	...
	for _, form := range []uint8{chain.PhasePrepareV5, chain.PhasePrecommitV5} {
		if !restarted.signAllowedAt(H, R, form, blockY) {
			t.Fatalf("GATE IS DECORATION: probing the durable mark with the era-4 constant %d must "+
	...
	t.Logf("ablation RED as required: the durable mark (H=%d, r=%d, phase=%d) does NOT block a probe "+
...
//
// The fix's whole claim is "no migration": ports.SignMark's persisted encoding is
// byte-identical across the era-4 upgrade. Driven against the REAL disk store, not the mem one.
func TestDurableSignMarkEncodingIsUnchangedAcrossTheUpgrade(t *testing.T) {
	...
	// previously persisted mark is being re-interpreted and the replay window is open.
	if !strings.Contains(string(raw), `"phase":2`) {
		t.Fatalf("VIOLATED: the persisted mark does not carry the canonical precommit step (2). "+
			"On-disk: %s", raw)
	}
	for _, form := range []uint8{chain.PhasePrepareV5, chain.PhasePrecommitV5} {
		if strings.Contains(string(raw), `"phase":`+string(rune('0'+form))) {
			t.Fatalf("VIOLATED: an era-4 wire constant (%d) reached the DURABLE watermark. The "+
	...
	if err != nil || !ok || got.Height != want.Height || got.Round != want.Round ||
		...
		t.Fatalf("VIOLATED: the mark did not round-trip unchanged: %+v ok=%v err=%v", got, ok, err)
	}
}
...
// exactly how the drain-slot gate (chainrole.go, the field wedge) came to be missing.
//
...
//	recordSignLock the write with the lock QC
//	slotCompare the comparison both rest on, called directly by the drain-slot gate
func TestNoEra4ConstantReachesTheWatermark(t *testing.T) {
	...
	if len(missing) > 0 {
		t.Fatalf("SOURCE GATE IS VACUOUS: no non-test call site found for %v. The watermark "+
			"functions were renamed or deleted; re-derive this gate rather than leaving it green over "+
			"nothing", missing)
	}
	if len(problems) > 0 {
		sort.Strings(problems)
		t.Fatalf("SOURCE GATE — an era-4 wire constant can reach the DURABLE watermark:\n%s\n\n"+
			"  THE WATERMARK RECORDS THE STEP; THE ATTESTATION RECORDS THE ERA-FORM (T-STEP-VS-FORM,\n"+
			"  research §5.4). ports.SignMark is fsynced to disk and slotCompare orders it\n"+
	...
	t.Logf("source gate: %d watermark call sites, all on the canonical {PhaseLegacy, PhasePrepare, "+
...
// would lose its era discriminator (the wedge) or the watermark would gain one (the replay above).
func TestWireCarriesTheFormWhileTheWatermarkCarriesTheStep(t *testing.T) {
