...
// round 3 — the FIELD-SHAPE latch-replay probe.
//
// TestLatchSurvivesReplay is GREEN on matureWorld12, whose drive stops at the
// latch (h8) with no TTL config, no renewals, and no post-latch rotations. The
// field chain that failed to re-latch (the field run val-d, restored 32
...
func fieldWorld12(t *testing.T) (nodes []*Node, ids []*identity.Identity, net *simnet.Network, sched *simclock.Scheduler) {
	...
	g := &chain.Block{Version: chain.BlockVersion, Height: 0, Entries: []ports.Entry{mkEntry("latch-genesis")}}
...
func TestLatchReplayFieldShape(t *testing.T) {
	...
	drive(0, 1, "latch-a", all[1:])
	nodes[1].queuePendingBondReg(mkReg(0, 64<<20, 0))
	drive(1, 2, "latch-b", append(append([]ports.NodeID{}, all[4:]...), all[0], all[2], all[3]))

	// h3.h26: the renewal treadmill at the field's cadence —
	// anchors+maturers renew at TTL/2 (h11, h22 — clear of the R-rule), the
	// sybils skip their renewals so their h1 bonds LAPSE at h24 (TTL 22)
	// and re-register at h25 (the lapse+re-entry churn the field window
	// carried). Crosses the latch (~h2), the HANDOFF rotation at h8, and
	// rotations h16/h24.
	for h := uint64(3); h <= 26; h++ {
		...
		drive(0, h, fmt.Sprintf("latch-%d", h), nonAnchorsFirst)
	}
	...
	if err != nil || n != len(blocks) {
		t.Fatalf("REPRODUCED (replay refuses the field-shape history): Reload restored %d of %d: %v", n, len(blocks), err)
	}
	if !fresh.EverMature() {
		t.Fatalf("REPRODUCED (field shape): the live drive latched everMature but replaying the same %d committed blocks did NOT — the 474718e-deep val-d under-latch, deterministic", len(blocks))
	}
	if src.EverMature() != fresh.EverMature() {
		t.Fatal("latch divergence")
	}

	// THE FIELD MECHANISM (named by the field run save/restore regime
	// pair: saved seen=12 everMature=true → restored seen=0
	// everMature=false over the same 40 blocks): the daemon replayed
	// BEFORE EnableObjectiveChain wired the verifier, and objective =
	// MinBond>0 && verifyBond!=nil — so the replay ran the LEGACY
	// rep-gated qualification with an empty boot ledger and validatorsSeen
	// rebuilt empty. The Reload guard must REFUSE that replay loudly
	// instead of silently under-latching. RED-provable by removing the
	// guard: this exact call then "succeeds" with EverMature==false.
	// MinAttesterRep>0 is the field's
	// -min-rep: with the verifier unwired the legacy branch gates on
	// rep>=MinAttesterRep and the boot ledger is 0 — nobody qualifies, seen
	// rebuilds empty (a 0 threshold would mask this: 0>=0 passes everyone,
	// which is why earlier oracles stayed green).
	...
	if n, err := unwired.Reload(blocks); err == nil {
		t.Fatalf("REPRODUCED: an objective-config replica replayed %d blocks with NO bond verifier and did not refuse (EverMature=%v) — the daemon-boot ordering bug silently demotes qualification to the empty-ledger legacy path and loses the latch", n, unwired.EverMature())
	}
}
