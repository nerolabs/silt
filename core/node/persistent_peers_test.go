...
// Layer 2 Q4 /§8: a CONFIGURED static consensus peer (-persistent-peers →
// AddStaticPeer) must NEVER be evicted from the routing table on a reachability timeout
// — its address is configured, not discovered, so a transient WAN miss is a retry
// situation, not an eviction. A normal DISCOVERED peer is still evicted once retries
// exhaust. Evicting a validator mid-genesis is what let a proposer lose an attester's
// address and stall quorum on the cross-region cold start.
func TestStaticPeerSurvivesReachabilityEviction(t *testing.T) {
	sched := simclock.New()
	...
	if !inTable(staticID) {
		t.Fatal("L2 Q4: a configured static peer was EVICTED on a reachability timeout — it must be retained")
	}
	if inTable(normalID) {
