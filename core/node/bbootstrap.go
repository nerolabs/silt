...
// The node-tier read of the B_bootstrap histogram. core/credit/bbootstrap.go is the
// source and carries the reasoning; this file only reaches the ledger.
//
...
//
// THIS FILE COMPILES ONLY UNDER THE `bbootstrap` BUILD TAG. There is no untagged
// stub, because nothing untagged calls it: the only caller is cmd/silt's status
// renderer, which is tagged the same way. A default silt binary has no
// Node.BBootstrap method at all.
//
...
// census reaches a consumer" a property of one line in one file. It was ablated past
...
// even if it wanted to, and neither can any future publisher, in any file, under any method
// name. The rule, its derivation and the exact scope of the compiler's guarantee live in
// core/credit; (cmd/silt/census_floor_equivalence_test.go) runs the resulting property at
// the wire.
