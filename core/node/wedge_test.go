...
// Repro for: on the GCP field run (4 objective validators, -quorum 2, Byzantine
...
func TestWedge313_ObjectiveByzantineMultiBlock(t *testing.T) {
	...
	//
	// The bond registration carries the (potentially ~1.5MB) space-time proof.
	...
	if regBlocks != 1 {
		t.Fatalf("expected exactly ONE bond-registration block (the first), got %d — a re-registering proposer bloats every block and wedges cross-region ", regBlocks)
	}
...
// The fix must NOT break the release-and-coast defense (bond-ttl): a
...
func TestWedge313_RenewalStillHappensUnderTTL(t *testing.T) {
	...
	if len(regHeights) >= 10 {
		t.Fatalf("still re-registering every block (%d) — the bloat is not fixed", len(regHeights))
	}
}
