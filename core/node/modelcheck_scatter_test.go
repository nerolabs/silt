...
// repro — the DEEP field the field run h68 stall, made deterministic.
//
// FIELD OBSERVATION (attributed, GitHub): after the B2 capture drill + WS
...
// ever formed. Ruled out: boundary (h68 non-boundary), memory (RSS 1.22G),
...
//
// THE DISTINGUISHING DIMENSION from the existing GREEN staggered-sweep
// oracle (TestModelCheck_StaggeredSweepsMustStillConverge): that test
...
// shape — itself decisive for the research (the trigger is a real-WAN
// scale/timing dimension the untimed model does not capture; the harness
...
func TestModelCheck_MixedWeightScatterMustConverge(t *testing.T) {
	...
	// kept the sybils in the set (unlike the staggered test's partition).
	for i, nd := range nodes {
	...
	// NOTE (corrected by the research): this untimed GREEN does NOT mean the
	// synchronizer logic is sound — it means this oracle cannot ISOLATE the catch-up-target
	// defect. Advances here are driven by timeouts (maybeAdvanceRound), which reach a
	// committing round regardless of whether catch-up jumps to the smallest or the highest
	// round; and the untimed sim cannot model the wall-clock timer skew that produced the
	// field smear. The research found a REAL synchronizer-logic defect (catch-up jumped to the
	// smallest round of a cross-round union — a sub-threshold round that cannot form a QC —
	// pinning the duration ladder low). That defect and its fix are proven deterministically
	// by TestModelCheck_CatchUpJumpsToHighestQualifyingRound. This test remains a useful
	// convergence regression guard (weight-aware catch-up converges a mixed-weight scatter),
	// but it is NOT evidence the logic was correct.
	t.Logf("mixed-weight scatter converged (h%d at round %d) — a convergence regression guard; the catch-up-target defect is isolated by TestModelCheck_CatchUpJumpsToHighestQualifyingRound", contested, blk[0].CommitRound)
}
