...
// ADVERSARY: classes the task named that I could NOT break. Each RUNS the attack and
...
// (REFUTED). Two epoch clocks run in this system:
...
func TestChainPruneBandNeverUndercutsTheKeysetWindow(t *testing.T) {
	sched := simclock.New()
	...
	t.Logf("REFUTED over 10 epoch boundaries: max(node clock - prune clock) = %d, "+
...
// (REFUTED). Shipped-lane Height != E. The withdrawal names E in Message.Height and
// refuses resp.Height != E. The attack is an issuer that ECHOES the requested E
// (passing that check) while signing under a DIFFERENT epoch's key that it ALSO holds
// legitimately — the shape a shared/persisted RSA key across epochs allows.
func TestEchoedEpochWithAnotherEpochsKeyIsStillADenial(t *testing.T) {
	sched := simclock.New()
	net := simnet.New(sched, 3, simnet.DefaultConfig())
	...
	t.Logf("REFUTED: under ONE key committed for epochs 0 and 1, an epoch-0 token " +
