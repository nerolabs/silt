...
// ---------------------------------------------------------------------------
// THE PROPOSER DRAIN RE-CHECKS ADMISSIBILITY.
// ---------------------------------------------------------------------------
//
...
//
// VACUOUS ON THE SHIPPED ROUTE, and that is said rather than discovered: with
...
func TestProposerDrainRefusesEvidenceBelowTheEraFloor(t *testing.T) {
	sched := simclock.New()
	...
	if len(blk.Slashes) != 0 {
		t.Fatalf("RED: the proposal embedded %d proof(s) the node's own v5ValidateSlashes refuses. "+
