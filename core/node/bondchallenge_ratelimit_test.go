...
// answering a bond challenge forces a fresh sequential VDF-eval on the node's
// single goroutine, so an unbounded challenger is a remote CPU-DoS.
// allowBondChallenge is the cheap gate in front of the eval: it admits an honest
// cadence with headroom but caps a flood, PER CHALLENGER so a flooder can't starve
// honest challengers of their own budget.
func TestBondChallengeRateLimitPerChallenger(t *testing.T) {
	...
	// The burst budget of challenges from X is admitted (honest
	// cadence + retries have wide headroom under it).
	...
	// The next one, in the same window, is refused — BEFORE any
	// VDF-eval runs.
	if nd.allowBondChallenge(x) {
