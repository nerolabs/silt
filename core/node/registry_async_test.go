...
// — the remaining face of the concurrent-publish 502 class: on a CHAINLESS
...
// defers completion (the contract: done never runs on the caller's stack).
...
// TestChainlessSweepUsesAsyncRegistryAndKeepsLoopLive: an Audit on a
...
func TestChainlessSweepUsesAsyncRegistryAndKeepsLoopLive(t *testing.T) {
	...
	if reg.syncCalls != 0 {
		t.Fatalf("a chainless sweep made %d BLOCKING Lookup call(s) against an async-capable "+
			"registry — on httpregistry that is an HTTP round-trip holding the event loop", reg.syncCalls)
	}
	if reg.asyncCalls != 1 {
		t.Fatalf("expected exactly one async lookup, got %d", reg.asyncCalls)
	}
	if !ticked {
		t.Fatal("the loop did not stay live while the registry round-trip was in flight")
	}
	if done {
		t.Fatal("the sweep completed before the registry answered")
	}

	reg.release()
	sched.Run()
	if !done {
		t.Fatal("the sweep did not complete after the registry answered")
	}
}
...
// TestSyncRegistryFallbackDefersCompletion: with a sync-only registry the
// fallback still must not run the continuation on the caller's stack (the
// contract) — completion arrives only when the loop drains.
func TestSyncRegistryFallbackDefersCompletion(t *testing.T) {
	n, sched := aloneNode(t, 0)
	...
	if done {
		t.Fatal("the sync-fallback lookup completed INLINE on the caller's stack")
	}
	sched.Run()
	if !done {
