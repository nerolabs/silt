...
func (n *Node) bondAuditOnce(now uint64) {
	...
	//
	// ADVERSARY-SHAPE: capability=PeerAcceptedSelfAssertedBond UNCOVERED: no capability-holding fixture exists anywhere in the tree granting an adversary a self-asserted bond that a PEER's ledger accepts. The block's defence is the second sentence -- a self-assertion buys nothing with the quorum -- and nothing witnesses it from the peer's side. ROADMAP row F1.
