...
// TestModelCheck_RoundLadderDesyncMustStillConverge — the deterministic
// home for the field run's height-43 stall (evidence:
// the field stall evidence,
// logs). Register row; freeze-manifest item 15.
//
// FIELD OBSERVATION: with one validator STOPPED, the live members' round
...
// and the height fell back to the staggered-takeover rank walk. / (A) repairs
// it with the REPLICATED arming term rs.Armed (set by storeRoundChange, so one
// member's first round-change arms every recipient within one hop) plus M1b
// (hold the counter when disarmed, never zero it).
//
// THE DEFECT IS A BOUND, NOT A DEADLOCK — this is what the previous version of
// this test got wrong twice over. The field's stalled height committed; it took 17m20s
// against a 650s baseline. In-process the pre-fix tree also commits, via the
// rank walk, whose own backstop is (N+2)·ChainSyncInterval — the D3 comment in
// rounds.go says so in as many words: "bounded at (N+2)·ChainSyncInterval, the
// backstop, but not at the published ≤ f+1 rounds". So an oracle that only
// asks "does it ever commit" is GREEN on both trees. Measured: the previous
// version was GREEN (the whole fix absent), GREEN, and GREEN under a direct
// ablation of the arming rule at the IDENTICAL virtual time — the rule never
// bound on its schedule, because it drove the fixture's refill, which installs
// pending work on EVERY node. Uniform work satisfies the pre-fix local-only
// guard at every seat at once, so the arming term is never consulted and the
// ladders cannot desynchronize.
//
...
// 3. FREEZE-FRAME (the shape): that designee proposes at (contested, r0)
...
// For — "≤ f′+1 rounds after GST", 190s at f = 1, N = 12 with
// ChainSyncInterval = 30s this lane, item A1. One seat is down, so f′ = 1
// and the ladder must reach round f′+1 within sweepsForRound(0) +
...
// and (build-immutable #6).
func TestModelCheck_RoundLadderDesyncMustStillConverge(t *testing.T) {
	...
	t.Logf("LIVENESS: contested h%d, r0 designee %s down for the whole budget, %d live members locked at r0, pending work on %d of %d live seats (below chain.RoundCatchupMet)",
	...
	// sweepsForRound(1) is the ladder to round f′+1 at f′ = 1; the
	...
	if honestSlashed {
		t.Fatal("I5 VIOLATION: an honest validator was slashed under the round-ladder-desync schedule")
	}
	if !committed() {
		t.Fatalf("LIVENESS REPRODUCED IN-PROCESS — BOUND ARM: with the r0 designee down, round 0 burnt at every seat, and pending work on %d of %d live members, height %d did NOT commit within the published budget of %d sweeps (≤ f′+1 rounds, 190s at f = 1, N = 12). Round histogram per sweep: %v; largest round-change set seen per round: %v. The ladders desynchronized exactly as run c450985-deep recorded — the work-holders climbed while the workless majority sat at r0, so no round ever held a round-change quorum. Consensus-adjacent and research-gated (build-immutable #6): do NOT patch from here",
			len(holders), len(live), contested, budget, ladderAt, maxChanges)
	}
	if !quorumSeen {
		t.Fatalf("LIVENESS REPRODUCED IN-PROCESS — MECHANISM ARM: height %d committed within the budget, but NO live member ever held a round-change set meeting chain.SupportMeetsQuorum at any round ≥ 1 (largest set seen per round: %v). The round ladder did not carry this commit; the staggered-takeover rank walk did, and its bound is (N+2)·ChainSyncInterval, not the published ≤ f′+1 rounds. Consensus-adjacent and research-gated (build-immutable #6): do NOT patch from here",
			contested, maxChanges)
	}
	blk := nodes[0].Chain().Blocks(contested)
	if len(blk) == 0 {
		t.Fatalf("LIVENESS: head moved past h%d but node 0 holds no block there", contested)
	}
	...
	t.Logf("LIVENESS: h%d committed at round %d in %d of %d budgeted sweeps; the round-change quorum formed at round %d (largest set per round: %v) — one member's round-change armed the rest and the ladder carried the commit",
