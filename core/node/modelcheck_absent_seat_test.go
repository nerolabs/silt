...
// — the field run h91 stall, made a TIMED oracle.
//
// FIELD OBSERVATION: after val-d's OOM→genesis-fallback removed one
...
// Confounded in the field by (walk-delayed sweeps ran the ladder ~2×
// slow) and by the capture window ending mid-ladder.
//
// THE ORACLE (later, tick-cadence escapes): a 12-member mature epoch with
// ONE heavy seat ABSENT (killed — dials fail, nothing is ever heard from it
// again), pending work armed, per-node sweep phases staggered across the
// interval (the field's independent 30 s timers), REAL timed delivery on the
// sim clock. The claim: the increasing round durations + weight catch-up
// converge the live weight onto a common round and the height COMMITS within
// a bounded ladder. Bound: the ladder through round 5 (Σ sweepsForRound(0.4)
// = 30 sweeps = 900 s) with 2× margin — generous against sweep phase +
// request-timeout noise, tight against the field's 25-minute non-commit.
//
...
// wasted, …) → the deterministic home for a fix (consensus-adjacent →
// research-gated, build-immutable #6). GREEN = the machinery converges once
// escapes run at tick cadence — decisive evidence the field stall's
// driver was the walk-starved ladder plus the truncated observation window,
// recorded on (the "GREEN is evidence too" pattern, as with the
// scatter oracle).
func TestModelCheck_AbsentHeavySeatMustStillCommitInBound(t *testing.T) {
	...
	net.Kill(absent.id)
	t.Logf("contested h%d, absent heavy seat %s (r0 designee absent: %v)",
	...
	// The bound: the ladder through round 5 (30 sweeps = 900 s) with 2×
	...
	for sched.Now() < deadline && !committed() {
		if !sched.Step() {
			t.Fatalf("scheduler drained at %v with no commit — the machinery went quiescent with pending work armed", sched.Now())
		}
		steps++
		if steps%64 == 0 {
	...
	t.Logf("h%d committed at round %d in %v virtual with the heavy seat absent — the machinery converges at tick cadence; the field's 25-minute non-commit is attributed to the walk-starved ladder (~2× slow climbs observed) + the truncated capture window, not a convergence defect. Recorded on.", contested, r, sched.Now())
}
