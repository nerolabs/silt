...
// The paid DELIVERY SESSION's node-tier RED-first gates. the (C1–C10), §6 (settle-monotone, the idle reaper, receipt v3),
// §8 — and the 2026-09-04 build-questions.λ-8-2 live in cmd/silt (they need core/relaypay)
// and in core/credit; this gate is OPEN (named residual): its lift (b), returning the face at close, IS the; its lift (a),
// recording the face only at the first verified increment, is REFUTED by (durable-before-admission closes the restart double-spend) — so
// it is not buildable now on either ground.
//
// ABLATIONS that must redden: drop the `count·p ≤ budget`
// ceiling or make the counter non-monotone; delete the handle on the first
// settle; reap on the admit stamp instead of the last settlement;
// add a map[ports.Hash]… field to DeliverySession; admit before the durable
// append; verify the anchor with VerifyAnchorInWindow; drop a field from
// receiptMsgV3.
...
// TestSessionSettlesMonotonicallyAndNeverAboveBudget. Many settlements over the wire: Σ
// settled ≤ Σ face; a lower or re-presented count pays exactly 0; a count the budget
// cannot fund is refused whole (top up first); the ledger's own conservation oracle moves
// by settled − Σ face over the cycle.
...
// TestSessionSpansObjectsAndIsNotClosedByASettlement — (C7). Two objects settle on one
// session; the session stays live between them and its budget carries.
func TestSessionSpansObjectsAndIsNotClosedByASettlement(t *testing.T) {
	...
	// cumulative settled value: a = ⌊10/8⌋ − ⌊0/8⌋ = 1, b = ⌊15/8⌋ − ⌊10/8⌋ = 0 — the
	// CUMULATIVE rule; the aggregate ⌊15/8⌋ = 1 is exact. (A per-settlement floor gives
...
// TestSkimAggregatesExactlyAcrossObjectsInOneSession. Three objects at deltas 10, 5, 5
// (cumulative 10, 15, 20): the per-object split is (1, 0, 1) and the aggregate is ⌊20/8⌋ =
// 2. Ablation: a per-settlement floor ⇒ (1, 0, 0), aggregate 1; routing the boundary
// credit to the FIRST object moves the split.
...
// TestSessionSkimIsPartitionIndependent, the attack named. Two sessions at two servers settle
// the same 40 increments: one in 5 deltas of 8, one in 40 deltas of 1. The two escrow totals
// are EQUAL and both ⌊40/8⌋ = 5. Ablation: a per-settlement floor ⇒ 5 vs 0.
...
// TestSessionReaperKeysOnIdleNotOnAdmit — (C9). A session settling every tick survives
// well past the idle window measured from its open; a silent one is reaped at the
// window, its remainder accounted ONCE (forfeits ≤ its unsettled budget), by the
// node's own clock.
func TestSessionReaperKeysOnIdleNotOnAdmit(t *testing.T) {
	...
	// The stamp is COARSE: never finer than idle/4.
...
// TestSessionCarriesNoPerObjectMap, the Don't #3 refutation. The session struct
// exposes no object-keyed collection, and a settlement for object O leaves no
// O-keyed residue on the node once the lane is superseded.
func TestSessionCarriesNoPerObjectMap(t *testing.T) {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		case reflect.Map:
			if f.Type.Key() == hashT {
				t.Fatalf("DeliverySession.%s is keyed by object — a per-session per-object map is INSIDE Don't #3", f.Name)
			}
...
// TestOpenIsAllOrNothingAndDurableBeforeAdmission — (C2; T-3/T-10 per open and per
// top-up). A refused open records nothing and admits nothing; a store that cannot
...
// TestDeliveryAnchorIsNotARelayAnchor — B-7 (the twin). A relay-domain credential
...
// TestReceiptBindsSessionAnchorsAndCount — B-8 (receipt v3). Tampering the count, the
// commitment, the object, the server, the fetcher, or the handle breaks the binding; a
// receipt for a session opened by another identity is refused; a receipt naming no
// live session is refused (B-9's anchored half).
...
// ---- the witnessed-demand observable under sessions (the
// §7.

// shortSettleLedger settles FEWER increments than asked — the seam under test in
// without the node's own ceiling check being the thing measured.
...
// TestWitnessedDemandReadsTheLedgersSettledNotTheReceiptCount. Ablation: bump by the
// delta (or r.Count − s.count) instead of the ledger's settled.
...
// TestWitnessedDemandNeverExceedsCreditsSettled — (P-SESSION as a property over a
...
// TestSettlementIsNotGatedOnTheDemandObservable. P3b ON, the same bonded fetcher settles
// twice on one object: the second settlement still PAYS. Ablation: carry the v2 `if
// credited { … }` shape onto the v3 path.
...
// ---- the deposit's release epoch is the anchor's REAL epoch:
...
// on a server with NO other guarded ledger activity, the periodic sweep alone returns a due
// deposit. Ablation: delete the ReleaseDueRefunds call from the sweep.
...
// node's chain epoch is a separate clock in these fixtures — wires them to one
