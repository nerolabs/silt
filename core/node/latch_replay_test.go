...
// round 2 — the field mechanism named by the shipped diagnostic (the field
// run: 507 warns; 391 on val-d, all `suffix stop: immature network requires
// anchor attestations: 2 of required 3` at h32 with suffix-appends=0).
//
...
// state ⇒ the everMature latch is NOT replay-deterministic (the class: live
// validation ≠ replay validation, this time for the maturity regime rather
// than era signatures).
//
...
func TestLatchSurvivesReplay(t *testing.T) {
	...
	if err != nil || n != len(blocks) {
		t.Fatalf("REPRODUCED (replay refuses live history): Reload restored %d of %d: %v", n, len(blocks), err)
	}
	if src.EverMature() && !fresh.EverMature() {
		t.Fatalf("REPRODUCED (the latch is not replay-deterministic): the live chain latched everMature but replaying its own %d committed blocks did NOT — a restarted/behind validator wakes PRE-handoff in a POST-handoff world and refuses every mature commit with ErrAnchorRequired (474718e-deep val-d, 391 warns at h32)", len(blocks))
	}
}
