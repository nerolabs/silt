...
// regressions: the repair trigger is minimum-filtered (network-durability
...
// missing; the captured run "repaired" 3 never-lost shards and placed the
...
// unconverged record view — the captured false repair); the second
...
func TestRepairNeedsTwoConsecutiveObservations(t *testing.T) {
	...
	if r.care.Stats.Repairs != 0 {
		t.Fatalf("the FIRST over-slack observation fired a repair — one probe sample must never trigger (network-durability §3)")
	}
	if r.count("stripe repair pending confirmation — one more sweep must agree") == 0 {
		t.Fatal("the deferred repair did not narrate 'pending confirmation'")
	}

	r.sweep(t)
	if r.care.Stats.Repairs == 0 {
		t.Fatal("the SECOND consecutive observation must fire the repair — the gate is stuck, a real loss goes unrepaired")
	}
}
...
func TestRepairConfirmResetsOnCleanSweep(t *testing.T) {
	...
	if r.care.Stats.Repairs != 0 {
		t.Fatal("a clean sweep after the blip still repaired")
	}
	...
	r.sweep(t)
	if r.care.Stats.Repairs != 0 {
		t.Fatal("the counter survived a clean sweep — a single fresh observation fired")
	}
	r.sweep(t)
	if r.care.Stats.Repairs == 0 {
		t.Fatal("the second fresh observation must fire")
	}
}
