...
// (the rate-gate half) — the MsgSubmitEntry CPU gate, the same
// wBondSubmit shape one message kind over. The entry-submit path had
// NO per-sender bound: under -require-tokens, ValidateEntry runs an RSA verify
// per token signature, so one authenticated peer floods entry submits and
// rides per-message crypto onto the single consensus loop. The gate: a
