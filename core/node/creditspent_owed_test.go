...
// The two gates.4, "the restart-replay gate": the unloaded-guard refusal
// and the append-before-sign crash-order pin.
...
// TestUnloadedCreditStoreRefusesCreditBearingRequests: a store attached but not
...
func TestUnloadedCreditStoreRefusesCreditBearingRequests(t *testing.T) {
...
// TestAppendLandsBeforeSignBlinded pins the crash order the fix relies on, as SOURCE
// order at the two sites that compose it: (1) blindtoken.Issue calls charge before
// SignBlinded, (2) tokenChargeFor's settlement closure Appends to the store before
// it marks creditSpent. Together: the durable record exists before any signature
// does, so a crash between them is an under-issue (a lost fee), never a token a
// restart cannot see.
//
...
func TestAppendLandsBeforeSignBlinded(t *testing.T) {
