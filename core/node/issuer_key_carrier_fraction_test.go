...
// / input — the IssuerKeys-CARRIER block fraction, measured.11, F6: the unit fixture's "1
// of 5" was a demonstration, not a rate. The floor box marks any block carrying a
// demand-issuer key registration Indeterminate (stateRootScopeGate), so the accept-flip's
// witness-path coverage falls with this fraction. Before a validator's registrations
// rode ONLY its own proposals; after whoever proposes next carries every peer's pending
// registration. This test measures both regimes on the same harness at three validator counts
// and LOGS the table. It asserts only that the measurement ran; the number is evidence for,
// not a gate on it.
...
// TestIssuerKeyCarrierFractionMeasurement logs the carrier fraction before and after
...
func TestIssuerKeyCarrierFractionMeasurement(t *testing.T) {
	if testing.Short() {
	...
	for _, V := range []int{2, 4, 8} {
		...
		t.Logf("carrier fraction | V=%d | %d round-robin blocks from a cold key schedule | BEFORE  (own proposals only): %d/%d carriers (%.0f%%) | AFTER  (peer submit): %d/%d carriers (%.0f%%)",
