...
// creditSpent durability. RED-first the gate ….
//
//.1 (the reproduction), §3 (the fix shape). Deliberation:
//
...
// demand token with a distinct serial per credit per restart. Measured:
// one 50,000 credit → two tokens → payouts 43,750 + 43,750 against one burn.
//
// THE FIX SHAPE these gates pin: a SECOND guardstore.Disk at creditspent.log behind the
// unchanged ports.PaidSerialStore (Serial = credit serial, Epoch = 0), attached and
// loaded the way the paid-serial guard is (SetPaidSerialStore / LoadPaidSerials),
...
// own live set and would evict every credit record (pins the two files apart).
//
// Fixture: the overlay probe, verbatim in shape — boot the issuer the way the
...
// ---------------------------------------------------------------------------
// The reproduction as a gate. One credit, one token, ACROSS a restart.
// ---------------------------------------------------------------------------
func TestCreditSpentSurvivesIssuerRestart(t *testing.T) {
	...
	if n, has := csStoreHas(t, store, cr.Serial); !has {
		t.Errorf("after the credit was spent the credit-spent store holds %d entries and NOT the spent serial — "+
	...
	if !iss2.creditSpent[string(cr.Serial)] {
		t.Errorf("after LoadCreditSpent the restarted issuer's creditSpent does not hold the spent serial "+
	...
	if ok {
		t.Fatalf("RED (a review §2.1): the restarted issuer honoured the SAME credit again — a second demand "+
...
// ---------------------------------------------------------------------------
// A store whose Append fails refuses the withdrawal with the named reason and does NOT
// mark the credit spent in memory, so a later successful append spends it once.
// ---------------------------------------------------------------------------
func TestCreditSpentStoreFailureRefusesTheWithdrawal(t *testing.T) {
	...
	cerr := charge()
	if cerr == nil {
		t.Fatalf("RED: the settlement closure returned nil while the credit-spent store's Append fails — " +
	...
	if !errors.Is(cerr, errCreditStore) && !errors.Is(cerr, injected) {
		t.Fatalf("the refusal is not the named reason: got %v, want errors.Is errCreditStore (or the injected error wrapped)", cerr)
	}
	if iss.creditSpent[key] {
		t.Fatalf("the credit was marked spent in memory although the durable append failed — " +
	...
	if _, ok := csWithdraw(t, iss, eph, &committed.PublicKey, &cr); ok {
		t.Fatal("RED: the wire handler issued a demand token although the credit-spent store's Append fails")
	}
...
// ---------------------------------------------------------------------------
// At the cap a NEW credit is refused; no recorded credit is evicted; after a restart
// every recorded credit is still refused.
//
...
func TestCreditSpentCapRefusesNeverEvicts(t *testing.T) {
	...
	if got := len(iss.creditSpent); got != maxCreditSpent-1 {
		t.Errorf("after load creditSpent holds %d, want %d (the store's contents)", got, maxCreditSpent-1)
	}
	...
	if n, has := csStoreHas(t, store, crA.Serial); !has || n != maxCreditSpent {
		t.Errorf("after the last-slot spend the store holds %d records (crA present: %v), want exactly the cap %d", n, has, maxCreditSpent)
	}
	...
	if err == nil {
		t.Fatalf("RED: at the cap (%d recorded credits) a NEW credit was honoured — the guard has no cap, "+
			"or it evicted a record to make room (the eviction the design refuses)", maxCreditSpent)
	}
	if !errors.Is(err, errCreditGuardFull) {
		t.Errorf("the cap refusal is not the named reason: got %v, want errors.Is errCreditGuardFull", err)
	}
	if _, ok := csWithdraw(t, iss, eph, &committed.PublicKey, &crB); ok {
		t.Fatal("RED: the wire handler issued a token for a new credit at the cap")
	}
	if iss.creditSpent[string(crB.Serial)] {
		t.Error("the refused credit was marked spent")
	}
	...
	if n, has := csStoreHas(t, store, crA.Serial); n != maxCreditSpent || !has {
		t.Fatalf("store holds %d records (crA present: %v) after the refusal, want %d and present — a record was EVICTED", n, has, maxCreditSpent)
	}
	for i, s := range []int{0, len(pad) / 2, len(pad) - 1} {
		if !iss.creditSpent[string(pad[s])] {
			t.Errorf("padded record %d (sample %d) is no longer held in memory — evicted", s, i)
		}
	}
	if _, ok := csWithdraw(t, iss, eph, &committed.PublicKey, &crA); ok {
		t.Fatal("RED: a RECORDED credit was honoured at the cap — the record was evicted")
	}
	...
	if got := len(iss2.creditSpent); got != maxCreditSpent {
		t.Errorf("after a restart at the cap creditSpent holds %d, want %d", got, maxCreditSpent)
	}
	if _, ok := csWithdraw(t, iss2, eph, &committed.PublicKey, &crA); ok {
		t.Fatal("RED: after a restart the recorded credit crA was honoured")
	}
	if _, ok := csWithdraw(t, iss2, eph, &committed.PublicKey, &crB); ok {
		t.Fatal("RED: after a restart the cap no longer refuses the new credit crB")
	}
	if n, _ := csStoreHas(t, store, crA.Serial); n != maxCreditSpent {
		t.Fatalf("store holds %d after the restart, want %d unchanged", n, maxCreditSpent)
	}
}

// ---------------------------------------------------------------------------
// The second file. The node's credit-spent store is a guardstore.Disk at
...
//
// The handle clause on this file is the ADAPTER's gate, path-agnostic on the same
// Disk type reused unchanged: adapters/guardstore
// TestPostRenameOpenFailureOrphansTheAppendHandle (plus the R213_ backstop
...
func TestCreditSpentDiskStoreIsASecondFileBesidePaidSerials(t *testing.T) {
	...
	if n, has := csStoreHas(t, cs, cr.Serial); !has || n != 1 {
		t.Errorf("creditspent.log holds %d records (spent serial present: %v), want exactly 1 — the spend was not written to the second file", n, has)
	}
	if n, has := csStoreHas(t, ps, cr.Serial); has || n != 0 {
		t.Errorf("paidserials.log holds %d records (credit serial present: %v), want 0 — the credit record went into the PAID-SERIAL file, which the sweep compacts with the ledger's live set", n, has)
	}
	...
	if n, has := csStoreHas(t, cs, cr.Serial); hadBefore && !has {
		t.Errorf("compacting paidserials.log removed the credit record from creditspent.log (%d records left)", n)
	}
	...
	if fi, err := os.Stat(csPath); err != nil || fi.Size() == 0 {
		t.Errorf("creditspent.log is absent or empty on disk after a spend (err=%v)", err)
	}
	...
	if _, ok := csWithdraw(t, iss2, eph, &committed.PublicKey, &cr); ok {
		t.Fatal("RED: after a re-open of creditspent.log the restarted issuer honoured the spent credit")
	}
}
