...
// signer subset then stops being a per-publisher quasi-identifier.
func TestFetchCanonicalIssuers_ReturnsLedgerRankedSet(t *testing.T) {
	...
	// FetchCanonicalIssuersFromAny must SKIP a validator that serves no chain
	// (un-synced / just restarted) and get the deterministic set from the next one
	// — so a single down validator can't drop the publisher into the
	// anonymity-narrowing
	...
	sched.Run()
	if !done || anyErr != nil {
		t.Fatalf("FromAny must fall through the un-synced validator to a chain-holder: done=%v err=%v", done, anyErr)
	}
	if len(any) != 3 || any[0] != heavy.NodeID() {
		t.Fatalf("FromAny must return the chain-holder's ledger-ranked set (heaviest first): got %d, want 3", len(any))
	}
}
