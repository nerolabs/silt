...
// red-team coverage caveat: I2's fsync-before-the-wire durability was
...
// hash — the guarantee, proven against disk, not memory.
func TestI2DiskDurability_RefuseAcrossDiskRestart(t *testing.T) {
	...
	if !restarted.signAllowedAt(1, 1, chain.PhasePrepare, otherHash) {
		t.Fatal("I2 durability VIOLATION: a disk-restarted validator refused a strictly-HIGHER round — a crash must not re-wedge the height ")
	}
}
