...
// drain proposer at the current height (the node whose behavior the Q2b
// closures constrain).
...
// Q2b-2 — SUBMIT-DON'T-PROPOSE: a NON-designated proposer whose only pending
// work is its OWN due registration must never propose for it; the registration
// reaches the chain through the designated proposer's queue (SubmitBondRenewal
// broadcasts it every sweep). This removes the field wedge's driver —
// genesis-aligned renewal clocks made two anchors both propose height 6 and
// cross-attest into a double slash (the field run).
func TestOwnRenewalNeverMakesNonDesignatedProposerPropose(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if other.bondDrainInFlight {
			t.Fatalf("sweep %d: a non-designated proposer proposed for its OWN renewal — must submit, never propose (2b-2)", i)
		}
	}
...
// Q2b-1 — STAGGERED TAKEOVER: when the designated
...
func TestTakeoverIsStaggeredByProposerRank(t *testing.T) {
	...
	for sweep := 0; sweep < 3+dist; sweep++ {
		other.maybeProposeBondDrain()
		if other.bondDrainInFlight {
			t.Fatalf("sweep %d: takeover fired inside the stagger window (want wait of %d sweeps for rank distance %d — one proposer readmitted per sweep,  Q2b-1)", sweep, 3+dist, dist)
		}
	}
