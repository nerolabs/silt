...
// The relay-lane prepayment anchor: the NODE-tier RED-first gates. (a refused open records nothing), §2.4 (doors
// (i) self keyset, (ii) the guard, (iii) domain, (v) never acct(ephID), (vi) budget = Σ face never S), §5 (guard window == keyset
// window; verify cost), §8 (v1 open ⇒ errRelayNoAnchor; dark until era-4: no self keyset ⇒ named refusal), §9 (open half
// wire, T-3 two-relay). Build shape: (RelayOpen v2 + the
// commitment M), §1.4 (the verify ORDER: free guards → k bounds → sha256(Fetcher)==from → ed25519 over M → RSA under the SELF keyset
// newest-first, stop at first failure → SpendRelayAnchors all-or-nothing → seen-maps → budget = Σ face → admit), §1.5 (ceiling =
// min(count, budget) × B), §1.6 (settle min(count, budget) to acct(relay); log reason=anchored), §5 steps 5-6 (OpenRelaySession(ephID,
// root, S, funding, anchors, fetcherPub, sig); AcquireRelayAnchors; the named S5 errors).
//
// ABLATIONS that must redden: verify under peerDemandKeys instead of self (T-3
// two-relay); remove the all-or-nothing check (core/credit
// TestRelayCredentialIsSpentOncePerLedger — at k_max = 1 the node tier has nothing to
// be partial about); restore budget:= S × inc, or drop the
// min(count, budget) ceiling (TestRelayBudgetIsTheLedgerFaceNotTheChainLength — the
// only node-tier gate that reaches the RUNTIME budget l.fee); touch
// acct(ephID).
//
// Every gate here is RED on main. Most fail at anchor mint (" not built"),
...
const (
	shippedFee   = int64(50_000)
	shippedGrant = int64(500_000)
	// relayAnchorKMax is MaxAnchorsPerSession at the shipped fee: 1 since the 2026-09-06
	// re-price (⌈50,000 / 50,000⌉; it was ⌈262,144 / 50,000⌉ = 6 at the 4 KiB increment).
	relayAnchorKMax = relaypay.MaxAnchorsPerSession
	// relayOpenCommitmentDomain is the domain of the ed25519 commitment M
	// M = sha256(domain ‖ relayID ‖ Root ‖ uint32BE(S) ‖
	// uint32BE(k) ‖ serial_1 ‖ … ‖ serial_k). The reading of the widths
	// (uint32BE) — sentence does not fix them. If another encoding is chosen,
	// encoding, align relayOpenCommitment below to it; the BINDING property
	// (every field is under the signature) does not change.
...
func newAnchorCluster(t *testing.T, seed int64, n int, epochBlocks uint64, grant int64, store ports.PaidSerialStore) *anchorCluster {
	...
	g := chain.Block{
		...
		Height:     0,
		Entries:    []ports.Entry{{Root: ports.HashBytes([]byte("relay-genesis"))}},
		IssuerKeys: regs,
	}
	...
	for i, r := range relays {
		...
		r.ledger = credit.New(shippedFee, grant)
		// The relay's OWN account exists on its own ledger before any session (
		...
		r.ledger.SetEpochSource(f8EpochFunc(r.node.chainEpoch)) // the daemon's wiring, in-process
...
func (cl *anchorCluster) advanceEpochs(n int) {
	...
	for i := 0; r.node.chainEpoch() < want; i++ {
		...
		b := &chain.Block{Version: 1, Height: next, Prev: prev,
			Entries: []ports.Entry{mkEntry(fmt.Sprintf("relay-advance-%d", cl.advanceSeq))}} // mkEntry: equivocation_test.go (manifest-bearing)
		cl.advanceSeq++
...
// relayOpenCommitmentIndependent is M, recomputed independently here
...
// TestRelayOpenRefusesUnanchoredSession is the node half of: "v1 open at a v2
// relay: len(Anchors) == 0 ⇒ errRelayNoAnchor": an open with zero anchors and an
// otherwise valid commitment is refused with the named reason, no session is
...
func TestRelayOpenRefusesUnanchoredSession(t *testing.T) {
	cl := newAnchorCluster(t, 8100, 1, 0, shippedGrant, nil)
	r := cl.relay()
...
// TestRelayAnchorsAreBoughtOnTheRelaysOwnLedger is the gate through the WIRE: the
...
// the paying ledger is the settling ledger, (iii)); a fresh EPHEMERAL E opens over
// the wire, pays c, and the relay settles. Then: D's balance fell by exactly k·fee;
// settled == min(c, k·fee); Δ Σ_L == settled − k·fee ≤ 0; E is never registered;
// and the S5 line carries reason=anchored (the recorded goalpost move from the
// interim's no-anchor).
func TestRelayAnchorsAreBoughtOnTheRelaysOwnLedger(t *testing.T) {
	const k = relayAnchorKMax // 1 since the 2026-09-06 re-price (was 2 of a k_max of 6)
	const S = 6               // c = S × inc = 6 ≪ k·fee: partial consumption, Δ < 0
	log := &capturingLogger{}
	cl := newAnchorCluster(t, 8200, 1, 0, shippedGrant, nil)
	r := cl.relay()
	...
	if got := r.ledger.Balance(d.id); got != dBefore-k*shippedFee {
		t.Fatalf("D's balance on the RELAY's ledger is %d after buying %d anchors, want %d − %d·fee = %d — the burn did not land on the paying ledger", got, k, dBefore, k, dBefore-k*shippedFee)
	}
	...
	want := minI64(int64(S)*relaypay.RelayIncrementCredit, k*shippedFee)
	if paid != want {
		t.Fatalf("settled %d, want min(c = %d, k·fee = %d) = %d", paid, S*relaypay.RelayIncrementCredit, k*shippedFee, want)
	}
	after := ledgerTotal(r.ledger)
	if after-before != paid-k*shippedFee || after > before {
		t.Fatalf("Δ Σ_L = %d, want settled − k·fee = %d (≤ 0)", after-before, paid-k*shippedFee)
	}
	...
	// The ephemeral must never be registered: Balances lists every
	// registered account; the count must equal what was registered before
	// the session.
...
// TestRelayCredentialIsSpentOncePerLedger is the gate at the node tier: a second open
// on the same anchor (fresh eph + fresh root) is refused as SPENT; and the
// two-relay variant: relay B, which holds A's key_0 PINNED AS A PEER key,
...
func TestRelayCredentialIsSpentOncePerLedger(t *testing.T) {
	cl := newAnchorCluster(t, 8300, 2, 0, shippedGrant, nil)
	...
	if !errors.Is(err, errRelayAnchorInvalid) {
		t.Fatalf("relay B ADMITTED an anchor issued by relay A (sess=%v, err=%v), want errRelayAnchorInvalid — the burn landed on A's ledger, the settle would land on B's (door (i))", sess != nil, err)
	}
...
// TestRelaySettlementIgnoresForwardedBytesIsBoundedByAnchor is the gate at the node tier:
// Count = S (the WHOLE chain revealed), forwarded = 0 (nothing was ever pumped), one
// anchor ⇒ settled ≤ face; the ledger total moves at settle by exactly settled. Before
// the 2026-09-06 re-price S was chosen ABOVE face so the S × inc budget the fix deleted
// would over-pay and the ablation reddened here; since the re-price S_max × credit ==
// face BY DERIVATION, so no chain can exceed one face and the over-pay
// is closed STRUCTURALLY — pinned first — and this gate keeps the node-tier settlement
// identity at the ceiling: a full chain, zero bytes forwarded, settles exactly
// min(count, face) = face.
func TestRelaySettlementIgnoresForwardedBytesIsBoundedByAnchor(t *testing.T) {
	if relaypay.MaxChainLength*relaypay.RelayIncrementCredit != int(shippedFee) {
		t.Fatalf("S_max·credit = %d != face %d — the structural close of the S × inc over-pay is gone", relaypay.MaxChainLength*relaypay.RelayIncrementCredit, shippedFee)
	}
	const S = relaypay.MaxChainLength // the longest chain a relay admits: exactly one face
	cl := newAnchorCluster(t, 8400, 1, 0, shippedGrant, nil)
	r := cl.relay()
	...
	// Reveal the whole chain at once (bounded to S hashes by the clamp). The
	// fix may refuse a claim past the budget ("either form"); the
	...
	if paid > shippedFee {
		t.Fatalf("settled %d > face %d with count %d and ZERO bytes forwarded — settlement is bounded by S, not by the anchor", paid, shippedFee, sess.Count())
	}
	if paid != minI64(int64(sess.Count())*relaypay.RelayIncrementCredit, shippedFee) {
		t.Fatalf("settled %d, want min(count = %d, face = %d)", paid, sess.Count(), shippedFee)
	}
...
// TestRelaySettlementNeverLeavesAnAccountNegative is the gate at the node tier: after
...
func TestRelaySettlementNeverLeavesAnAccountNegative(t *testing.T) {
	t.Run("full_chain_against_k_max", func(t *testing.T) {
		const k = relayAnchorKMax
		const S = relaypay.MaxChainLength // the longest chain a relay admits (≤ S_max)
		cl := newAnchorCluster(t, 8500, 1, 0, shippedGrant, nil)
		r := cl.relay()
		...
		_ = sess.PayTo(c.Preimage(S), S)
		paid := r.node.SettleRelaySession(handle)
		if paid <= 0 || paid > k*shippedFee {
			t.Fatalf("settled %d, want 0 < settled ≤ k·fee = %d", paid, k*shippedFee)
		}
		for _, bal := range r.ledger.Balances() {
			if bal < 0 {
...
// TestRelayAnchorDomainIsNotADemandToken is the gate: under the SAME
...
func TestRelayAnchorDomainIsNotADemandToken(t *testing.T) {
	cl := newAnchorCluster(t, 8600, 1, 0, shippedGrant, nil)
	r := cl.relay()
	...
	//
	// C1: asserted through the keyset the delivery open reads
...
// TestRelayOpenRefusesCheaplyBeforeRSA is the gate, §5 verify cost; order and
// §4.9: the RSA verify counter (the ValidatePubHardnessRuns idiom) shows a
// guard-(ii)-refused open runs 0 modexps; a k > k_max open runs 0; a k = k_max GARBAGE
// open runs ≥ 1 and ≤ W+1 (newest-first, stop at the first failure — the counter must
// move, or it is not wired); an honest k = k_max open runs ≥ k_max and ≤ 6·(W+1).
func TestRelayOpenRefusesCheaplyBeforeRSA(t *testing.T) {
	const wPlus1 = int(demand.DefaultWindow) + 1
	cl := newAnchorCluster(t, 8700, 1, 0, shippedGrant, nil)
	r := cl.relay()
	...
	// Honest k = k_max.
	honest := r.mintAnchors(t, 0, relayAnchorKMax)
	c1 := freshChain(t, "t7-honest", 8)
	e1 := newEphemeral(8790)
	var err error
	n := runs(func() { _, err = r.open(e1, c1.Root(), 8, honest) })
	if err != nil {
		t.Fatalf("honest k=%d open: %v", relayAnchorKMax, err)
	}
	if n < relayAnchorKMax || n > relayAnchorKMax*wPlus1 {
		t.Fatalf("honest k=k_max open ran %d RSA verifies, want %d ≤ n ≤ %d (one per anchor, newest-first)", n, relayAnchorKMax, relayAnchorKMax*wPlus1)
	}
	...
	if n != 0 {
		t.Fatalf("a guard-(ii)-refused open ran %d RSA verifies, want 0 — RSA before the free guards is the open-path CPU DoS ", n)
	}

	// k > k_max: refused before any modexp.
	c3 := freshChain(t, "t7-kmax", 8)
	n = runs(func() { _, err = r.open(newEphemeral(8791), c3.Root(), 8, garbageAnchors(relayAnchorKMax+1)) })
	if err == nil {
		t.Fatalf("k = %d > MaxAnchorsPerSession was admitted", relayAnchorKMax+1)
	}
	if n != 0 {
	...
	n = runs(func() { _, err = r.open(newEphemeral(8792), c4.Root(), 8, garbageAnchors(relayAnchorKMax)) })
	if !errors.Is(err, errRelayAnchorInvalid) {
...
// TestRelayAnchorGuardSurvivesRestart is the gate at the node tier: spend an anchor on
// relay R (ledger attached to a durable store); build R again from the same store
// (a restart: same identity, fresh Node, fresh Ledger, loaded); re-presenting the
// anchor is refused as SPENT. An attached-but-unloaded store refuses opens
// outright.
func TestRelayAnchorGuardSurvivesRestart(t *testing.T) {
	store := &memPaidSerialStore{}
	cl := newAnchorCluster(t, 8800, 1, 0, shippedGrant, store)
	r := cl.relay()
	anchor := r.mintAnchor(t, 0)
	...
	restarted := credit.New(shippedFee, shippedGrant)
	...
	unloaded := credit.New(shippedFee, shippedGrant)
...
// TestRelayCeilingNeverExceedsBudget is the gate; with k = 1 (budget =
// face = 50,000 increments) AuthorizedBytes never exceeds budget ×
// RelayIncrementBytes — the pump never forwards past the funded budget — and below
// the budget the ceiling tracks the count (liveness). Since the 2026-09-06 re-price
// S_max == budget by derivation, so a chain LONGER than the budget
// cannot be opened at all (the clamp refuses it, pinned below); the over-budget arm
// is closed structurally and the gate holds the ceiling at the exact boundary: a
// chain of S_max fully paid authorizes exactly budget × B.
func TestRelayCeilingNeverExceedsBudget(t *testing.T) {
	const budget = shippedFee // one anchor, in increments (RelayIncrementCredit = 1)
	if relaypay.MaxChainLength != int(budget) {
		t.Fatalf("S_max %d != one-anchor budget %d — a chain longer than its funding is openable again", relaypay.MaxChainLength, budget)
	}
	const S = int(budget)
	cl := newAnchorCluster(t, 8900, 1, 0, shippedGrant, nil)
	r := cl.relay()
	anchor := r.mintAnchor(t, 0)
...
func TestRelayBudgetIsTheLedgerFaceNotTheChainLength(t *testing.T) {
	...
	cl := newAnchorCluster(t, 9300, 1, 0, shippedGrant, nil)
	r := cl.relay()
	r.ledger = credit.New(lowFee, shippedGrant) // the relay's OWN ledger charges a lower fee
	r.ledger.Register(r.id())
	r.node.SetLedger(r.ledger)
...
// TestRelayOpenRefusalRecordsNoAnchor is the gate: an open presenting an already-spent
// anchor is refused AND an unrelated anchor 1 is still spendable in a later open (at
// the pre-re-price k_max of 6 both rode in ONE open and the all-or-nothing reserve
// was the property; at k_max = 1 the open carries the spent anchor alone); the
// refused open left no guard entry (anchor 1 opens), no durable append (the store
// holds exactly the anchors of ADMITTED sessions), and no seen-map entry (the
// refused ephemeral and root are admissible later). The ablation "remove the
// all-or-nothing check" reddens at the CREDIT tier
...
func TestRelayOpenRefusalRecordsNoAnchor(t *testing.T) {
	store := &memPaidSerialStore{}
	cl := newAnchorCluster(t, 9000, 1, 0, shippedGrant, store)
	r := cl.relay()
	...
	// table (a "want 1" here would assume an insert).
...
// TestRelayOpenCommitmentBindsRelayRootAndSerials is the gate; M binds
// relayID, Root, S, k and every serial. Tampering any of them (the relayID case is
// a replay of A's commitment at relay B) is refused on the ed25519 check BEFORE any
// RSA verify (counter delta 0); sha256(Fetcher) != from is refused likewise. The
// anchors are garbage on purpose: the property under test is the commitment, and it
// must be decided before the anchors are touched.
func TestRelayOpenCommitmentBindsRelayRootAndSerials(t *testing.T) {
	cl := newAnchorCluster(t, 9100, 2, 0, shippedGrant, nil)
	a, b := cl.relays[0], cl.relays[1]
	const S = 8
	anchors := garbageAnchors(relayAnchorKMax) // within the k bound, so the commitment check is reached
...
// TestRelayAnchorGuardWindowMatchesKeysetWindow is the gate at the node tier (the ledger
// twin is in core/credit): with the epoch clock on, an anchor spent at epoch 0 is
// still refused as SPENT at epoch W (the guard remembers exactly as long as the
// keyset verifies), a fresh epoch-0 anchor still opens at epoch W (key_0 is in the
// window), and at epoch W+1 a fresh epoch-0 anchor is refused as INVALID (the self
// keyset pruned key_0 — evicted ⇒ expired). SpendRelayAnchors is driven with the
// same n.chainEpoch the keyset is pruned with; the boundary is observed on both
// sides at once.
func TestRelayAnchorGuardWindowMatchesKeysetWindow(t *testing.T) {
	const W = int(demand.DefaultWindow)
	cl := newAnchorCluster(t, 9200, 1, 2, shippedGrant, nil) // EpochBlocks = 2
	r := cl.relay()
...
// TestRelayOpenRefusesWithoutSelfKeyset is the gate: no chain
// / a chain with no committed key_E for this relay / a committed key but keys
// never scheduled (relay-accept on without the demand-key schedule) ⇒ every open
// is refused with the NAMED reason (the dark-lane direction), no session, no
...
func TestRelayOpenRefusesWithoutSelfKeyset(t *testing.T) {
	...
	t.Run("no chain", func(t *testing.T) {
		...
		l := credit.New(shippedFee, shippedGrant)
		n.SetLedger(l)
		n.EnableRelayAccept()
		present(t, n, n.id, l)
	})
