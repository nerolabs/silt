...
// TestColumnHoldersByteConfirmsDropsPhantomRecord is the deterministic
// reproducer for the e2e flake, at the tier where the selector logic lives.
//
...
// record but NOT the bytes (a lost-ack / stale record); node REAL
...
// decoration.
func TestColumnHoldersByteConfirmsDropsPhantomRecord(t *testing.T) {
	cfg := DefaultConfig()
	cfg.DHTDomainCap = 0
	sched := simclock.New()
	...
	if contains(confirmed, phantomID) {
		t.Fatalf("byte-confirmed holders still lists the phantom record-holder %v — "+
			"the selector would under-kill (kill the record, leave the bytes)", confirmed)
	}
	if !contains(confirmed, realID) {
		t.Fatalf("byte-confirmed holders dropped the REAL byte-holder: got %v", confirmed)
	}
	_ = real
	_ = phantom
}

// TestConfirmColumnHoldersCorpseGatesDeadRecord is the liveness RED for the fix:
// PR the confirmColumnHolders dialed a stale/dead provider record ONCE PER SHARD
// of the column, at a full HolderDialTimeout each — the dead-holder dial-storm
// re-entering through the holders read. On a multi-stripe object (one shard per
// stripe) that is stripes × HolderDialTimeout serial per dead record. The fix
// corpse-gates confirmColumnHolders like probeShard: a provider proven dead this
// walk is dialed AT MOST ONCE, then skipped.
//
...
// the dial-storm decision flagged.
func TestConfirmColumnHoldersCorpseGatesDeadRecord(t *testing.T) {
	cfg := DefaultConfig()
	cfg.DHTDomainCap = 0
	sched := simclock.New()
	ln := &linkNet{sched: sched, ends: map[ports.NodeID]*linkEnd{}}
	...
	if deadDials != 1 {
		t.Fatalf("liveness: DEAD record-holder was dialed %d times (want exactly 1); "+
			">1 is the once-per-shard dial-storm the decision flagged", deadDials)
	}
	if me.Stats.HolderDialsSkipped < 1 {
		t.Fatalf("liveness: the corpse-gate never skipped DEAD's remaining shards (HolderDialsSkipped=%d) — "+
