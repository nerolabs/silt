...
// PoD relay lane — the relay-accept role. A relay/gateway forwards content-blind
// bytes toward a fetcher and is paid as-it-goes by a sender-funded PayWord hash
// chain (core/relaypay). It cannot sign a completed-delivery receipt because it
// never holds a verifiable object, so there is no delivery-receipt lane here — the
// fetcher commits a chain root once, reveals one preimage per forwarded increment,
// and the relay redeems the highest preimage it holds at session close
// (credit.RedeemRelayCredit; balance-only, never standing).
//
// THE CHAIN ROOT IS ANCHORED. A session opens only against k prepayment anchors — blind signatures under THIS relay's own
// chain-committed per-epoch demand key (blindtoken relayAnchorDomain) that the fetcher's durable identity bought from this relay through
// the ordinary withdrawal (a refusable ChargePublish on this relay's ledger) — spent once into the ledger's bounded, durable guard
// BEFORE admission, and the session's budget is the ledger's own Σ face of what it spent. Rivest–Shamir's authorization half, bilateral
// form (issuer == relay). interim (pays 0) is retired by it. BUILT ≠ LIVE: an anchor verifies only under a
// CHAIN-COMMITTED IssuerKeyReg, so the lane is dark until this node commits one and every open is refused with a named reason until
// then. Era-4 is NOT that gate: -era4-activation-height defaults to 1, so v5 is live from height 1 on every fresh network. The posture
// that is actually missing is objective
...
// durable identity that paid — guard (i) is satisfied cryptographically.
// FundingSource is kept as the guard's TEST OBJECT (a durable-funded
// declaration is still refused); it enforces nothing on its own. The
// NETWORK residual is the delivery lane's D3 residual, unchanged: the
// relay saw the buyer's IP at purchase and the ephemeral's IP at open, so
...
// failing-first guards are in relayrole_test.go and relay_anchor_test.go.
...
const (
	// FundingEphemeralBlind is the ONLY accepted funding source. the funding
	// that actually settles is the k blind-signed anchors bought under the
	// fetcher's DURABLE identity (AcquireRelayAnchors → ChargePublish on the
	// relay's ledger); the D3 path (a publish credit converted via
	// WithdrawDemandTokenPrivately) is NOT anchor-eligible: F-4 (creditSpent
	// durability) is closed by, but eligibility is the research, not a
...
const (
	...
	errRelayChainTooLong   = relayError("relay: committed chain length S exceeds S_max = MaxChainLength (DoS clamp: bound the AdvanceTo walk)")
	errRelayBadRoot        = relayError("relay: chain root must be exactly one hash wide")

	// The anchor refusals — named S5 reasons, each surfaced in the open ack.
	errRelayNoAnchor        = relayError("relay: session open carries no prepayment anchor (an unanchored session funds nothing)")
...
// relayOpenDomain is the domain of the session-open commitment M (the
// M = sha256(relayOpenDomain ‖ relayID ‖ Root ‖ uint32BE(S)
...
// relayRetentionEpochs is the seen-map retention window in epochs. Keeping
...
// sessions (Batch-2 leak fix). The session table is written from the wire on every
// guard-passing MsgRelayOpen and drained only at settlement (never called on the
// live path yet), so without a cap a flood of cheap fresh- identity opens grows it
// without bound between epoch sweeps. This ceiling bounds growth BETWEEN sweeps:
// past it, a new open is refused (OK=false) rather than admitted. The specific
// value is a TUNING call, not a correctness one — 4096 is a conservative default
// (~4096 × ~200 B ≈ 800 KB worst-case table), well above any honest
// concurrent-session count on a single relay and far below an OOM. Raise it if a
// legitimate deployment ever saturates it.
...
// floor. The floor never lowers: a reorg that moves the epoch backward
...
// a session admitted in an epoch below the floor is an unsettled leak — the
// settlement path is not wired on the live wire path yet — so it is reaped here.
// This is the epoch/TTL half of the fix; relayMaxLiveSessions is the hard-cap half
// that bounds growth BETWEEN sweeps.
func (n *Node) sweepRelaySeen(epoch uint64) {
	...
	// goroutine. closeSession is idempotent (wake is a non-blocking send), so a
	// later SettleRelaySession close on the same handle is harmless; but the
	// delete here removes the handle, so the normal-close settle finds it absent
	// and cannot double-settle (design §3b). A reaped-but-unsettled session
	// forwarded only up to its authorized ceiling, so dropping it without a settle
	// forfeits no owed credit beyond what an unclaimed session already forfeits.
...
type RelaySession struct {
	...
	budget int64        // Σ face of the anchors spent at open — the conservation cap (never S × inc)
	...
	// retention window, mirroring the seen-map eviction. A wire open that is
...
// over-delivers.
//
// The ceiling is min(count, budget) × RelayIncrementBytes: the relay
// never forwards past the increments the spent anchors fund, so the funding cap
// is visible on the wire rather than discovered at settlement.
...
// OpenRelaySession opens a relay-payment session for a fetcher's PayWord chain, in
// the verify ORDER; so a refused open costs the least it can and
// leaves no trace:
//
...
// peer's (an anchor is a claim on the ledger that burned its fee, and only
...
func (n *Node) OpenRelaySession(ephID ports.NodeID, root []byte, S int, funding FundingSource,
	...
	// sweep stale-epoch seen entries lazily on the open path (this is exactly
	...
	// Hard per-node concurrent-session cap (Batch-2 leak fix). The sweep above
	// frees any stale-epoch sessions first; if the table is STILL at the ceiling,
	// refuse the open rather than admit an unbounded entry.
	...
	// open-side clamp: reject a chain longer than the relay will ever forward.
	...
	// step 4: k bounds and field widths, before any crypto.
	if len(anchors) == 0 {
	...
	// Step 6: RSA under the SELF keyset only. With no chain, no committed
	// key_E for this relay, or keys never scheduled, there is no keyset and
	// the lane is dark with a named reason — the direction until era-4,
	if n.ledger == nil {
	...
	cur := n.chainEpoch() // the epoch the keyset was just pruned with; the ledger reads the SAME clock through its EpochSource
	...
	// refusal records nothing and leaves no seen-map entry either — the
	...
	// Step 8: admit. Record the admit epoch so eviction can age the entry out.
...
// hashes by the clamp. On success it raises the paid pump's ceiling to the new
