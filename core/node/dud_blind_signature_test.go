...
// The dud-blind-signature composition, at the NODE tier.
//
// The unit-tier gates are core/blindtoken's unblind/withdrawal refusals. This one drives the composition
// that makes this more than conformance:
//
...
//	 unwitnessed self-mint is never reversed (the session lane; the flat
...
// TestAnIssuerThatReturnsADudIsRefusedAtWithdrawal drives the real withdrawal path
...
func TestAnIssuerThatReturnsADudIsRefusedAtWithdrawal(t *testing.T) {
	sched := simclock.New()
