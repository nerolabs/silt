...
// regressions: NetGet's pulled chunks are a WORKING SET (dropped after
...
// counting against the pledge while invisible to every fetcher (the
...
func TestNetGetDropsWorkingSet(t *testing.T) {
	...
	for _, id := range r.candidates(t) {
		...
		if id == preHeld {
			if !ok {
				t.Fatalf("NetGet dropped a chunk the node held BEFORE the call (%s)", id)
			}
			continue
		}
		if ok {
			t.Fatalf("NetGet retained pulled chunk %s — the working set must drop after assembly", id)
		}
	}
	if len(consumer.proofMeta) != 0 {
		t.Fatalf("NetGet left %d orphan proof entries", len(consumer.proofMeta))
	}
}
...
func TestNetGetRetainHostsAnnouncesAndServes(t *testing.T) {
	...
	if held := r.heldOf(consumer, dataLeaves); held != len(dataLeaves) {
		t.Fatalf("retainer holds %d/%d data leaves", held, len(dataLeaves))
	}
	for _, id := range r.m.Leaves() {
		...
		if _, proved := consumer.proofMeta[id]; !proved {
			t.Fatalf("retained leaf %s has no registered proof — not audit-answerable", id)
		}
	}
	...
	r.sched.Run()
	if !done2 || err2 != nil {
		t.Fatalf("third node could not retrieve from the retainer after all original holders died: done=%v err=%v", done2, err2)
	}
	if !bytes.Equal(out2.Bytes(), r.data) {
		t.Fatal("third node got wrong bytes from the retainer")
	}
}
...
func TestNetGetRetainKeepsNothingOnFailure(t *testing.T) {
	...
	if held := r.heldOf(consumer, r.candidates(t)); held != 0 {
		t.Fatalf("a failed NetGetRetain kept %d chunks — a failure must retain nothing", held)
	}
}
