...
// — the h≈56 liveness knee. A catch-up reconcile re-validated the ENTIRE
...
// round durations, starved the sweeps, and h57 never committed (RC run the
// field run, 198 ChainReply watchdog HANGs; 2/2 deterministic with the field
// run). The fix: a served window that provably EXTENDS our exact committed
// head (finality active; first new block's Prev chains from our head hash)
// is adopted through the normal Append commit path — validating only the new
// suffix — and never touches reconstructFork/Reconcile. Every other shape
// (divergence, equal-height fork, legacy no-finality config, peer behind)
// keeps the unchanged slow path.

// TestSuffixAppend_CatchUpSkipsGenesisReplay is the defect made RED/GREEN:
...
func TestSuffixAppend_CatchUpSkipsGenesisReplay(t *testing.T) {
	const floor = 64 // window = 960 bytes — a few blocks per window (the rig)
	...
	if got := n2.Stats.ChainSyncFullReconciles; got != 0 {
		t.Fatalf("a pure-extension catch-up ran %d full genesis replay(s) — the O(height) reconcile cost is the liveness knee; it must be 0", got)
	}
	if got := n2.Stats.ChainSyncSuffixAppends; got != grown {
		t.Fatalf("all %d blocks must arrive via the suffix-append fast path, got %d", grown, got)
	}
}

// TestSuffixAppend_DivergentHeadStillFullReconciles pins the boundary: a
// peer whose block at our head height DIFFERS from ours (a same-height fork —
// the shape the comment warns is invisible to "give me blocks above my
...
// is untouched.
func TestSuffixAppend_DivergentHeadStillFullReconciles(t *testing.T) {
	const floor = 64
	n1, n2, a1, a2, sched := windowedPair(t, floor, floor)
	...
	// entry ⇒ distinct hash), so n2's committed head conflicts with n1's
	// history.
...
// TestSuffixAppend_InvalidTailKeepsValidPrefix: a lying peer's window that
...
func TestSuffixAppend_InvalidTailKeepsValidPrefix(t *testing.T) {
...
// nodeForChain wraps an existing chain in a minimal Node so appendExtension can
// be exercised directly (no network traffic is involved in the fast path).
func nodeForChain(t *testing.T, id *identity.Identity, ch *chain.Chain) *Node {
	t.Helper()
	sched := simclock.New()
...
// TestSuffixAppend_NearHeadCatchUpCostIsDeltaNotHeight is the knee made
...
// validation-count assert in TestSuffixAppend_CatchUpSkipsGenesisReplay;
...
func TestSuffixAppend_NearHeadCatchUpCostIsDeltaNotHeight(t *testing.T) {
	const height = 60
	...
	// Slow path: the earlier shape — reconstruct the genesis-rooted fork and
	...
	nd := nodeForChain(t, a2, behindFast)
	t1 := time.Now()
	...
	t.Logf("near-head catch-up on a %d-block heavy-reg chain (%d KiB/reg): slow full replay %v, suffix append %v (%.0f×)",
