...
// C3 close — the node-layer gates for the confirmed breaks. Each was
// a PASSING red-team probe against the pre-fix tree; each is inverted here
// into a permanent assertion, with its ablation named.
//
...
//
// HARNESS NOTE: a v5 block's committed root covers validatorsSeen, which apply
// derives from the block's Atts. A minter that introduces an attester not yet in
// validatorsSeen produces roots the proposer signed before gathering, so the commit
// mismatches. Every chain here therefore seeds its fixed attester into GENESIS Atts
// and re-uses it. (The general hazard is filed as an open residual and routed for its
// own research; it is not this file's to fix.)
...
func c3Chain(t *testing.T, era4At uint64, genesisSigner ed25519.PrivateKey, regs ...chain.IssuerKeyReg) *chain.Chain {
	...
	// A v5 genesis's Atts write NOTHING under the LastCommit carrier rule (the carrier re-point:
...
// TestCohortKeyIsADenialOnEveryShippedLane (break 5, probe C). the gate made the bank
...
// verified you" became a tag on an ACCEPTED token. Earlier the bank held ONE key,
...
func TestCohortKeyIsADenialOnEveryShippedLane(t *testing.T) {
	...
	// epoch 1. Nothing here is off-commitment — that is the point. Earlier the bank
	// held ONE key, so a cohort key was a denial; the gate made W+1 keys acceptable at
	...
	// refuses it at the client — one door earlier than the keyset.
...
func TestDemandLaneOutlivesTheWindowAndARestart(t *testing.T) {
	...
	boot := func() *Node {
		...
		nd.SetLedger(l)
		l.SetEpochSource(f8EpochFunc(nd.chainEpoch)) // the daemon's wiring (cmd/silt/ledger_epoch.go) — one clock for the keyset and the guard
		nd.EnableChain(c, signer)
	...
	//
	// C1: the cycle was withdraw → ack → Bank.Redeem → RedeemDeliveryCredit
...
// honest signer now REFUSES an out-of-range representative (RFC 8017 §5.2.2,
// A blind produced under key_A lands above key_B's modulus about half the
