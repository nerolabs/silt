...
// (a-domain-fresh) repair self-hold — the new hosting primitive. When the
// economy is on and the paramedic's own failure domain is unused by a stripe,
// it KEEPS the shard it rebuilt (funding the node that bore the
// reconstruction cost/RAM) instead of pushing it to a stranger — but only
// when self-holding cannot reduce failure-domain diversity. hostShardLocally
// is the store+provider+proof path that makes the paramedic a real,
// audit-answerable holder.
...
// dispersal-safe (vs unconstrained (a), which trades S2 away).
