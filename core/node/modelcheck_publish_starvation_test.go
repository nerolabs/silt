...
// The oracle — BORN RED on this branch (the wedge-oracle pattern): the
// deterministic repro of the mature-regime PUBLISH starvation observed on run
// the field run (zero entry-blocks committed post-latch across 33+ heights;
// the live-captured client error 'propose height 45 round 1: insufficient
// valid attestations: 2 prepares of 2 gathered').
//
// THE MECHANISM UNDER TEST (code-cited): the view-change machinery is
...
// The fix is research-gated; this oracle is the consult's evidence and the
// fix's merge gate, parked on its branch until then.
const publishHeightBudget = 3

func TestModelCheck_PublishStarvedAcrossRounds(t *testing.T) {
	...
	for h := 0; h < publishHeightBudget; h++ {
		...
		// The client's action — the submit path. Pre-fix
		// this was pub.ProposeEntry racing the designee for the same
		// (h, r0) slots — recorded RED at commit 8f55cf0: the entry
		// could win no round of any height. The oracle property is
		// unchanged: the continuously-resubmitted entry commits within
		// the height budget.
		var pub *Node
		...
		if after == height {
			t.Fatalf("h%d: the height never resolved — this schedule wants starvation-with-progress, not a wedge (the escape must still commit the DRAIN)", height)
		}
	}
	...
	if !entryCommitted() {
		...
		t.Fatalf("PUBLISH STARVATION: the chain is LIVE (head next=%d, drain blocks committing every height) but the client's entry NEVER committed within %d heights of continuous re-submission — the round machinery's new-view seat is drain-only, so the publish can win no round once its r0 slot is consumed", head, publishHeightBudget)
	}
}
