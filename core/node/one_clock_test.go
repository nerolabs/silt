...
// ONE CLOCK. (the source is the node's chainEpoch and nothing else; finalized-head REFUTED
// twice), §3.4 (the anchors on the same source), §6.
//
// The property: the ledger's epoch and the keyset's prune epoch are the SAME value
// at every height, including boundary blocks. Two arms on the cluster shape
// EpochBlocks = 2, one relay, the relay's key_e committed at genesis for e =
// 0.issuerKeyPrePublish so an anchor can be minted AT the keyset's epoch on every
// boundary:
//
// 1. TestLedgerEpochIsTheNodesChainEpochAtEveryBlock — with the ledger's source
...
// reads no source.
// 2. TestLedgerFollowsItsSourceNotTheCaller — the DUAL, and the gate that refutes
...
// and not the caller's epoch, and (b) exactly why finalized-head is inadmissible
// in production (an honest-anchor denial once per epoch). RED on main:
// SpendRelayAnchors still reads the caller's `cur`, so the anchor is admitted.
...
// production wiring is the gate in cmd/silt).
...
func newF8Cluster(t *testing.T, seed int64, epochBlocks uint64) *f8Cluster {
	...
	r.ledger = credit.New(shippedFee, shippedGrant)
...
// TestLedgerEpochIsTheNodesChainEpochAtEveryBlock is arm 1.
func TestLedgerEpochIsTheNodesChainEpochAtEveryBlock(t *testing.T) {
	const EB = 2
	...
	boundaries := 0
	// 2·f8PrePublish blocks walk epochs 1.f8PrePublish; every odd block is a
	// boundary (chainEpoch = (last.Height + 1) / EB).
	for i := 0; i < EB*f8PrePublish; i++ {
		...
		if got := r.ledger.Epoch(); got != e {
			t.Fatalf("after block %d (head %d): ledger.Epoch() = %d, chainEpoch() = %d — two clocks; "+
				"the keyset prunes at %d and the guard expires at %d (ONE clock)",
		...
		if err != nil {
			if strings.Contains(err.Error(), credit.ReasonAnchorFuture) {
				t.Fatalf("boundary block %d (epoch %d): an anchor minted AT the keyset's epoch was refused %q — "+
					"the ledger's clock is behind the keyset's (the finalized-head break): %v",
...
// TestLedgerFollowsItsSourceNotTheCaller is arm 2, the dual (see the file note).
func TestLedgerFollowsItsSourceNotTheCaller(t *testing.T) {
	const EB = 2
	cl := newF8Cluster(t, 9450, EB)
	r := cl.relay
	// The REFUTED source: the finalized-head epoch. Read exactly as the
	...
	if src := finalizedHead.Epoch(); src == 1 {
		t.Fatalf("precondition: the finalized-head epoch equals the keyset's at the first boundary (%d) — "+
			"the refutation's premise does not hold on this fixture; this subtest cannot discriminate", src)
	}
	...
	if err == nil || sess != nil {
		t.Fatalf("with the ledger's source at %d and the keyset at 1, an epoch-1 anchor was ADMITTED (sess=%v, err=%v). "+
			...
			". Want the refusal %q — which is also the honest-anchor denial that makes the "+
			"finalized-head source inadmissible", finalizedHead.Epoch(), sess != nil, err, credit.ReasonAnchorFuture)
	}
