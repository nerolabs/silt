...
// TestModelCheck_HeterogeneousArmingMustCommitWithinFPlus1Rounds is, decisive, per §4.2: "matureWorld12 variant with heterogeneous arming:
// exactly 3 of 12 seats hold pending work, one heavy seat killed, staggered sweep phases, timed delivery on the sim clock."
//
// THIS TEST ALSO pins the takeover re-key `(height + rs.Round) % N`, which is the
// same pin as the reg-lane re-shape; one gate is enough if it names both. The "── The
// pin (D3) ──" block below reads chainrole.go's `maybeProposeBondDrain`'s `d:=
// int((height + rs.Round) % uint64(len(props)))` live, against the ACTUAL
// committing proposer — that IS the re-key pin; no second gate duplicates it.
//
// THE ROOT (M1): core/node/rounds.go arms the round clock
// on LOCAL mempool content — `maybeAdvanceRound` quiesces (`rs.Sweeps = 0`)
// whenever a node's OWN pendingBondRegs/pendingEntries/ bondDrainInFlight
// are empty, so a seat that never holds work never runs the pacemaker at
// all. Every EXISTING oracle in this family (`TestModelCheck_SilentAuthorLockedValueMustStillCommit`,
// `TestModelCheck_AbsentHeavySeatMustStillCommitInBound`) calls the shared `refill` from `matureWorld12`,
// which sets `pendingBondRegs` on EVERY node
// (`modelcheck_locked_stall_test.go`) — so none of them can ever
// exercise this branch. This test is the first in the family that does not.
//
// THIS IS THE REG LANE, PLAIN STATEMENT: a bond registration is NEVER forwarded to a
// round's designee — the receiver decisively refuses any reg it does not own ("bond-reg
// submit REFUSED (relay)", chainrole.go) — so THIS lane gets no
// D1-entry rescue and its bound is the D4 BACKSTOP `(N+2)·ChainSyncInterval + G`, never the
// tighter `f+1` ladder bound (that bound is the ENTRY lane's). What THIS gate pins, once M1
// no longer masks it, is D3: the rank-staggered takeover's walk is re-keyed from the
// ROUND's own designee `props[(height+round) % N]` (chainrole.go's
// `maybeProposeBondDrain`), not from the height's `props[height % N]` (the pre-D3 walk,
// still what `462478d` computes).
//
...
// then back mid-ladder" shape, layered onto the "one heavy seat
...
// lanes) — the field's silent rotation seats.
//
// THE ARMING ASSERTION IS WHAT MAKES THE BOUND NON-VACUOUS.F-4: the
// ORIGINAL armed set, three real 64 MiB anchors (192 MiB), MEETS
// RoundCatchupMet's
...
// armed=[2, 4, 9] (129 MiB) is BELOW the bar — verified against
// chain.RoundCatchupMet itself, not hand arithmetic, in the premise check
// below — so ONLY (A) can arm the quiescent 8. The BOUND alone (commits
// within the D4 backstop) is therefore not sufficient evidence (A) did the
// work; the SEPARATE per-seat assertion below it (every quiescent seat
// reaches round >= 1 and records its OWN round-change envelope — never
// merely observed via someone else's) is what makes this gate decisive for
// (A) specifically, and the two are read TOGETHER.
//
// THE SEARCH FOR A WINNER-NAME DIVERGENCE, AND WHY THIS FIXTURE CANNOT SHOW
// ONE — reported with the arithmetic, per instruction, rather than shipping
// a seat assignment that only coincidentally passes. props:=
...
// arming it makes round 1 a DIRECT proposal (verified — armed
...
// round 2's designee workless and (b) make the height-keyed and round-keyed
// walks name different winners — the shift is order-preserving over exactly
// the candidates that remain eligible to compare. Verified directly:
// armed=[2, 4, 9] names the SAME winner (node 9) under both the
...
// worktree), so the pin cannot even be reached pre-fix. A
...
// different liveness fixture at a height further from its own round-0
...
func TestModelCheck_HeterogeneousArmingMustCommitWithinFPlus1Rounds(t *testing.T) {
	...
	// ARM nodes[2, 4, 9] (one anchor, one maturer, one sybil — the
	// docstring's arithmetic explains why THIS fixture cannot separate the
	// height-keyed and round-keyed walks by winner NAME; convergence
	// carries RED instead); KILL nodes[3] (val-d); force the rest of
	// nodes[4.11] QUIESCENT — the heterogeneous-arming premise this fixture
	// checks the harness CAN express.
	armed := []int{2, 4, 9}
	killedIdx := 3

	// props is the SAME EligibleProposers ordering both the takeover
	// (chainrole.go's maybeProposeBondDrain) and this test's pin read
	// — a pure function of committed state, fixed for the whole run.
	...
	t.Logf("premise: props fixed (N=%d); height %d's OWN designee sits at position %d; the OLD "+
	...
	// The earlier setup-only-entries relaxation bought nothing on
	// THIS (reg) lane — refill seeds pendingBondRegs only, regs are
	// never forwarded, and the entry-lane arm this relaxation was
	// written for (D1-entry, forwardPendingWorkToDesignee) lives in a
	// DIFFERENT file (modelcheck_liveness_gates_test.go), not this
	// one — so restore the FULL periodic strip on BOTH lanes every
	...
	// verify the armed set's WEIGHT is below the >1/3
	// RoundCatchupMet bar against the REAL function, not hand
	// arithmetic — otherwise the pre-existing, unconditional
	// maybeCatchUpRound would move the quiescent seats regardless of
	// arming (A), and this test would be VACUOUS for what it claims to
	// prove (measured: with the ORIGINAL 3-anchor armed set [0,1,2] =
	// 192 MiB > the 172 MiB bar, RoundCatchupMet returns true; this
	// armed set, verified below, does not).
	{
	...
	// 30s timers, re-randomized by restart — the convention).
	...
	// The field's val-d: down then back mid-ladder (coordinator's
	// requested shape, layered onto "one heavy seat killed").
	// 430s < 540s, so the restated D4 backstop below is decided before
	// this fires in the RED run; it exists in the schedule so a future
	// GREEN run also covers the return dynamic, not just the
	...
	// THE RESTATED BOUND (D4/§3.4): the reg lane gets no forward-based
	// rescue (D1-entry does not apply to it, by design),
	// so its worst case is the takeover walk, never the f+1 ladder
	// escape (that bound belongs to the entry lane): `(N + 2) ·
	// ChainSyncInterval + G`.
	const gatherG = 10 * ports.Second // the measured intrinsic gather latency at 12-seat WAN
	const nGoverning = 12
	...
	for sched.Now() < deadline && !committed() {
		...
		steps++
		if steps%64 == 0 {
			// Top up the 3 armed seats' pendingBondRegs (refill reseeds
			// ALL 12 uniformly), then re-strip BOTH lanes on the quiescent
			// seats: this test never arms entries and no entry-lane
			// forward test lives in this file, so there is nothing to
			// preserve.
			arm()
		}
	}
	...
	if !committed() {
		...
		t.Fatalf("REPRODUCED: with armed=%v, 8 quiescent, one heavy seat killed (node 3, restart "+
			...
			"reg lane has no forward-based rescue by design (deleted at "+
			"8368196), so its only rescue is the takeover — which does not converge within the backstop "+
			...
			"research-gated (build-immutable #6); fix direction (A) (M1) + D3 (the re-key) in the rule.",
	...
	// — is what makes the backstop above non-vacuous for (A). Below
	// the catch-up weight bar (verified above), nothing but (A) can
	// move these seats.
	for i, nd := range nodes {
		...
		if !selfAuthored {
			t.Fatalf("REPRODUCED (arming): quiescent seat %d reached round %d but never recorded its "+
	...
	// Teeth: this IS the runtime-confirmed, load-bearing observation of a
	// live+eligible+workless designee that the workless-designee gate needs
	// evidence of.
	recordQuiescentDesigneeObserved("reg-lane oracle", roundDesigneeID)
	...
	if proposerID != nodes[roundKeyedWinner].id {
		t.Fatalf("REPRODUCED (D3 not applied, or applied incorrectly): height %d committed at round %d "+
	...
	if heightKeyedWinner == roundKeyedWinner {
		t.Logf("note: the height-keyed and round-keyed walks name the SAME winner (node %d) for this "+
		...
		t.Logf("the walks diverge exactly as designed — height-keyed (from position %d) favors node "+
	...
	t.Logf("h%d committed at round %d, %v (virtual), by node %d (%s) — the nearest work-holder from "+
...
// TestModelCheck_AtLeastOneRoundLivenessOracleArmsNonUniformly is the
// fixture-blindness pin: the sixth recurrence of one blind spot.
//
...
// through the shared `refill` (`modelcheck_locked_stall_test.go`)
...
// trap: a test that reached for `refill`
// alone could never have produced heterogeneous arming, so a REGRESSION
// that silently reverted the fixture to plain `refill` and
...
// the runtime level) prove that the reg-lane oracle itself does not regress; that
// is the own premise check
// (TestModelCheck_HeterogeneousArmingMustCommitWithinFPlus1Rounds's
// "heterogeneous arming not established" fatal, immediately above it).
func TestModelCheck_AtLeastOneRoundLivenessOracleArmsNonUniformly(t *testing.T) {
	...
	// (modelcheck_locked_stall_test.go) — so every node ALREADY holds
	// uniform pending work the instant the fixture returns, before any
	// oracle gets a chance to choose otherwise. This is a THIRD layer of
	// the same blind spot at the caller level
	// (every oracle's OWN refill loop) — it is baked into the constructor
	// too. Confirm that fact first (evidence, not an assumption), then
	// confirm the harness still PERMITS reaching a non-uniform state by
	// clearing.
	...
	if !uniformAtReturn {
		t.Fatal("finding check: matureWorld12 no longer arms every node uniformly at construction — " +
			"re-verify the THIRD-layer uniform-arming claim (constructor-level) before relying on it")
	}
	t.Log("finding: matureWorld12's OWN constructor arms all 12 nodes uniformly before returning " +
		"(its internal refill call, modelcheck_locked_stall_test.go:147) — a third layer of the same " +
		"blind spot, beneath the caller-level refill loop the rule names.")
	...
	for i, nd := range nodes {
		hasWork := len(nd.pendingBondRegs) != 0 || len(nd.pendingEntries) != 0
		if i == 0 && !hasWork {
			t.Fatal("capability check: setting pendingBondRegs on node 0 directly did not take")
		}
		if i != 0 && hasWork {
			t.Fatalf("capability check: arming node 0 alone leaked pending work onto node %d — "+
				"the fixture does not permit non-uniform arming", i)
		}
	}
	nodes[0].pendingBondRegs = nil // reset before the teeth half

	// HALF 2 — teeth: the shared `refill` every PRE-EXISTING oracle in
	// this family calls arms EVERY node — confirmed by reading the live
	// state after calling it, not by trusting the docstring.
	refill()
	uniform := true
	for i, nd := range nodes {
		if len(nd.pendingBondRegs) == 0 && len(nd.pendingEntries) == 0 {
			...
			t.Logf("teeth check: node %d was NOT armed by refill() — the shared helper is no longer uniform; "+
				"re-verify which pre-existing oracle this changes before treating as satisfied by them", i)
		}
	}
	if !uniform {
		t.Fatal("teeth check FAILED: the shared refill helper no longer arms every node uniformly — " +
	...
	t.Logf("capability confirmed (matureWorld12 permits a strict-subset arm), teeth confirmed "+
		"(the shared refill helper arms all %d nodes uniformly — the exact trap). "+
		"TestModelCheck_HeterogeneousArmingMustCommitWithinFPlus1Rounds is, as of this commit, the first "+
		"oracle in the family that does not rely on refill() alone.", len(nodes))

	// HALF 3 — LOAD-BEARING EXERCISE, because halves 1 and 2 above prove only that
	// the HARNESS permits non-uniform arming and that refill is uniform —
	// neither can catch a fixture silently reverting to
	// plain refill, because neither reads whether any REAL oracle actually
	// exercised the capability. quiescentDesigneeObserved is written, at
	// runtime, by any test that confirms a LIVE, ELIGIBLE designee held
	// none of the height's pending work — the M4 shape this whole arc
	// targets. Package tests within one file run in declaration order, and
	// `go test` compiles a package's files in filename order
	// ("modelcheck_arming_test.go" precedes
	// "modelcheck_liveness_gates_test.go"), so the reg-lane oracle (declared earlier in
	// THIS file) has already run and recorded by the time this check runs.
	quiescentDesigneeMu.Lock()
	observed := append([]string(nil), quiescentDesigneeObserved...)
	quiescentDesigneeMu.Unlock()
	if len(observed) == 0 {
		t.Fatal("REPRODUCED (load-bearing-exercise check): quiescentDesigneeObserved is EMPTY — no " +
			"test in this package run has confirmed, at runtime, a live+eligible+workless designee. Either " +
			"this test ran in isolation (a -run filter that excludes the recorders —  must run " +
			"first; this failure is then expected and uninformative, not a regression) or 's fixture has " +
			"silently reverted to a uniform refill() (the exact regression capability+teeth above cannot catch).")
	}
	t.Logf("(load-bearing-exercise): %d real observation(s) recorded this package run: %v", len(observed), observed)
}
