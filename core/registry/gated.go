...
func (g *Gated) Publish(ctx context.Context, e ports.Entry) error {
	...
	if !g.ledger.CanPublish(e.Publisher) {
		// THE REFUSAL DECISION for the CanPublish gate. It is counted
		// here and not inside CanPublish, which is a predicate the sim also calls
		// for display — a counter that moved when a dashboard read it would be
		// worse than a missing one. Reached through an optional interface so this
		// package keeps importing ports alone; a ledger without the method simply
		// records nothing.
