...
// The THIRD spend gate's refusal decision is counted; the research as-built §4(d).
//
...
// C6 makes ANY rise in spendRefusersDistinct a HARD canary abort, and the adversarial
// workload runs in the sim tier, where sim/economy.go grades FreeloadersRejected off
// exactly these refusals. A floor reading zero beside a non-zero rejection count is an
// abort input that silently under-reads.
//
// TWO ABLATIONS run RED before this shipped: dropping the
...
func TestGatedPublishRefusalMovesTheAffordabilityFloor(t *testing.T) {
	...
	if fs.SpendRefusedInsufficientCredit != 3 || fs.SpendRefusersDistinct != 2 {
		t.Fatalf("after three Gated.Publish credit refusals by two identities: refused %d distinct %d, want 3 and 2 — the CanPublish gate's refusal decision is miscounted (0 = uncounted, so the floor under-reads beneath a HARD canary abort; more than 3 = counted twice, which trips that abort on honest traffic) (the roadmap C6)",
