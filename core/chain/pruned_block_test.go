...
// TestBlockPrunePreservesHashAndDropsAnswer pins the Opt 1 pruned-block
// representation: a payload-selective prune drops the heavy BondReg.Answer (~1.5 MB)
// while KEEPING the block's hash (so it still links) and the light BondReg fields
// (Validator/Root/Size/Sig/Domain — needed by STATE + slashing). Because Hash commits
// BondRegs, the pruned block must carry its pre-prune hash; Hash returns that stored
// value.
...
func TestFullBlockHashIgnoresUnsetPrunedField(t *testing.T) {
	...
	stripped.hashMemoSet = false // the copy keeps full's memo; a stripped wire block decodes without one
	if stripped.Hash() == h1 {
...
//
// PRE-FREEZE / PRE-STAMP-RAISE CHECKLIST ITEM. Red-team ("test the invariant before the stamp
// raise; it is not a flip gate"). Do NOT change Hash or Prune to make this test go the other
// way: covering pruned bodies with the hash defeats pruning, which exists for build-immutable
// #8.
//
...
//
// THE INVARIANT THIS DOCUMENTS (stated at chain.go's Hash): a pruned block's integrity rests on the
// recompute chain to the first NON-pruned descendant plus trustFloor. NO CONSENSUS DECISION MAY
...
func TestPrunedBlockHashDoesNotCoverCarrierOrStateRoot(t *testing.T) {
	...
	if len(pruned.LastCommit) != len(b.LastCommit) {
		t.Fatalf("Prune() dropped LastCommit (%d -> %d) — the documented property has changed; re-read "+
			"before adjusting this test", len(b.LastCommit), len(pruned.LastCommit))
	}
	...
	if got := forged.Hash(); got != fullHash {
		t.Fatalf("PROPERTY CHANGED: a mutated pruned block's Hash() moved (%x != %x). If Hash() now "+
			"covers pruned bodies, is closed and this test should be replaced by the "+
	...
	// (4) The carrier's own validity rule still runs on a pruned block and still binds to
	// b.Prev, so a ZERO-SIGNATURE entry is refused. That is ALL it refuses: an entry harvested
...
// TestPrunedCarrierRewriteIsCaughtOnlyByTheDescendant is the TWO-SIDED pruned-carrier gate
// (P-table /; floor-box structure round 1A step 12).
//
...
// harvested from the parent's Atts, recomputes its (uncovered) StateRoot with the real apply, and
// every signature still verifies because Hash returns b.Pruned unchanged. No key material.
//
...
// Ablation: remove validateEra3Roots from appendStructural ⇒ the forgery survives ⇒ RED.
// The pruned-carrier hole stays OPEN; this gate bounds it, it does not close it.
func TestPrunedCarrierRewriteIsCaughtOnlyByTheDescendant(t *testing.T) {
	...
	// THE ATTACK. Prune h3, harvest the parent's real precommits into its carrier, recompute
	// its (now uncovered) StateRoot with the real apply over the honest h2 state.
	...
	if rewritten.Hash() != honest[3].Hash() {
		t.Fatal("PROPERTY CHANGED: a mutated pruned block's Hash moved —  is closed; rewrite this gate")
	}
	...
	if n != 4 || !errors.Is(err, ErrEra3StateRootMismatch) {
		t.Fatalf("(ii): the rewritten pruned ancestor must be ACCEPTED and the first non-pruned descendant "+
			"REFUSED on its committed StateRoot (want n=4, ErrEra3StateRootMismatch); got n=%d err=%v", n, err)
	}
	if _, next := replay.Head(); next != 4 {
		t.Fatalf("(ii): the head must be SILENTLY TRUNCATED at the rewritten block (next height 4); got %d", next)
	}
	if !replay.validatorsSeen[victim] {
		t.Fatal("(ii): the forged seat must be LIVE in the replayed state — the descendant catch does not undo it")
	}
