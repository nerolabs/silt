...
// (the reorder half): ValidateEntry must reject a REPLAYED (already-spent)
// token on the cheap serial map-lookup BEFORE it runs the RSA signature verify.
// Committed tokens are public on the append-only chain, so an attacker pairs a
// harvested valid token with a novel Root and floods; if the spent-check sits
// AFTER publishtoken.Verify, every genuine signature is verified to completion
// (N RSA modexps on the single consensus loop) before the replay is caught —
// the CPU amplifier F-1 names.
//
...
func TestValidateEntry_SpentTokenRejectedBeforeVerify(t *testing.T) {
	prop := key(1)
	...
	if !errors.Is(err, ErrTokenSpent) {
		t.Fatalf("F-1: a spent token with tampered sigs must be rejected by the cheap spent-check BEFORE the RSA verify (want ErrTokenSpent, proving verify was skipped); got %v", err)
	}
	...
	if err := c.ValidateEntry(fresh); err == nil || errors.Is(err, ErrTokenSpent) {
		t.Fatalf("F-1: a FRESH tampered token must still fail at verify (not ErrTokenSpent, not nil); got %v", err)
	}
}
