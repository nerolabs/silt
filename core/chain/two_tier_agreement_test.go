...
// C3 re-break — BREAK 1 (F1), THE TWO-TIER AGREEMENT GATE. Inversion of the red-team
// probe rt_c3b_split_test.go, which measured a consensus SPLIT in BOTH
...
// tiers to AGREE. It is the entry-criteria gate for the accept-flip: the split was
...
// tierVerdicts is one block's verdict at each validation tier the flip spans.
type tierVerdicts struct {
	box      error // the O(payload) floor-box recompute — the accept surface
...
// ambiguous recovery boundary. A project decision deleted the directive, which collapsed those two into
// one; the simplicitydecision then deleted the scaffold itself, which removed the last.
//
...
// no Accept among them; that property is now held where a flip could actually happen, at the
...
// Accept at all until the gate flips it.
...
// TestEpochTurnHonestBlockDoesNotSplitTheTiers is the gate closed: the honest
// zero-registration block at the epoch turn, over a pre-state holding a committed issuer key.
func TestEpochTurnHonestBlockDoesNotSplitTheTiers(t *testing.T) {
...
// TestEpochTurnForgedRootDoesNotSplitTheTiers is the gate closed: the forged root — the
...
func TestEpochTurnForgedRootDoesNotSplitTheTiers(t *testing.T) {
	...
	if v.box == nil {
		t.Fatalf("BREAK REOPENED: the box AGREES with a forged root the full node "+
...
// TestRegistrationCarryingBlockStallsRatherThanSplits keeps the other half honest. When
...
func TestRegistrationCarryingBlockStallsRatherThanSplits(t *testing.T) {
	f := buildPruneFixture(t, true)
	b := f.boundaryBlock(nil)
