...
// directions (two_tier_agreement_test.go): the box AGREED with a forged root the full node
...
// "len(b.IssuerKeys) == 0 ⇒ no issuerKeyCommit write" is now a property of apply rather than
// an assumption of the box. The keyspace stays bounded because every ADD is in-band by
// validity, so pruning at each add re-establishes the 2W+2-bucket bound on every block that
// can grow it. See core/chain/issuerkey.go ("re-break round").
...
// TestZeroRegistrationBlockWritesNoIssuerKeyCommitLeaf is the direct inversion of
...
func TestZeroRegistrationBlockWritesNoIssuerKeyCommitLeaf(t *testing.T) {
...
// TestBoundaryRecomputeAgreesWithAndWithoutAPriorRegistration is the inversion of
...
func TestBoundaryRecomputeAgreesWithAndWithoutAPriorRegistration(t *testing.T) {
...
// TestLeafDiffMinusFoldIsEmptyAtThePruneBoundary is the inversion of BREAK-C: the
...
// reproduces.
func TestLeafDiffMinusFoldIsEmptyAtThePruneBoundary(t *testing.T) {
	f := buildPruneFixture(t, true)
	b := f.boundaryBlock(nil)
	post := f.c.cloneForDryRun()
	post.apply(b)
...
// TestPruneStillBoundsTheKeyspace earns the other half of the close. Making the prune
...
func TestPruneStillBoundsTheKeyspace(t *testing.T) {
	cfg := Config{Quorum: 1, MinBond: era4MinBond, ByzantineQuorum: true,
