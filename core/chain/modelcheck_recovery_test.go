...
// Consensus model-check — fix (3): the operator-directed weak-subjectivity
// liveness-floor escape (design:; research:
// 535-epoch-boundary-liveness-cliff-).
//
// The recovery stack for the boundary wedge is (4) + (3). Fix (4) (R-gate
// restore exemption, shipped) heals a RETURNING member. Fix (3) is the recovery
// for a genuine > ⅓-of-frozen-weight loss that does NOT return:
...
// boundary only, off by default, never automatic. The trust anchor is the HUMAN
// who confirms the loss is a real outage (the WSCheckpoint trust class);
// automatic re-basing was REFUTED (fix (2), modelcheck_rebasing_test.go —
// excluding possibly-honest lapsed weight raises the Byzantine fraction and
// reopens I1).
//
...
// attestation counts; the size-set == membership-set law) — and rotates,
// so the chain resumes.
...
func setup535Fix3(t *testing.T, recoveryHeight uint64) (c *Chain, ak, mk, yk []ed25519.PrivateKey, fresh ed25519.PrivateKey) {
	...
	// THE LAPSE (the mechanism, the fix (4) test's surgery): maturers 1-3
...
// naming a DIFFERENT boundary) the bled boundary stalls, exactly the
// no-re-basing rule. The escape is NEVER automatic.
func TestModelCheck_Fix3_SafeDefaultStalls(t *testing.T) {
	// LivenessRecoveryHeight = 0 (off, the default).
	c, ak, mk, yk, fresh := setup535Fix3(t, 0)
	b := boundary535Fix3(c, ak, mk, yk, fresh, true, 0)
	if err := c.Append(*b); err == nil {
		t.Fatal("fix (3) SAFE DEFAULT BROKEN: the bled boundary committed with LivenessRecoveryHeight unset — the re-base must NEVER be automatic")
	...
	if err := c2.Append(*b2); err == nil {
		t.Fatal("fix (3): a recovery directive for a DIFFERENT boundary must not re-base this one")
	} else if !errors.Is(err, ErrNoQuorumWeight) {
		t.Fatalf("the stall must be the frozen-weight rule, got: %v", err)
	}
	...
	if len(c2.effectiveEpochSet(3)) != len(c2.epochSet) {
		t.Fatal("fix (3): a non-boundary LivenessRecoveryHeight must never re-base (I3)")
	}
}
...
func TestModelCheck_Fix3_RecoveryAtDirectedBoundary(t *testing.T) {
	...
	if err := c.Append(*bNoFresh); err == nil {
		t.Fatal("fix (3): the re-based boundary must still demand >⅔ of the LIVE set — 324 of 580 must not commit")
	} else if !errors.Is(err, ErrNoQuorumWeight) {
	...
	if !c.SupportMeetsQuorum(idOf(ak[0]), coalition535Fix3(ak, mk, yk, fresh, true), 2) {
		t.Fatal("fix (3): SupportMeetsQuorum must accept the live coalition at the recovery boundary — the gather would never assemble what ValidateCommit accepts")
	}
	if err := c.Append(*b); err != nil {
		t.Fatalf("fix (3) RECOVERY FAILED: the live coalition must commit the directed boundary, got: %v", err)
	}
	...
	if _, ok := c.epochSet[idOf(mk[1])]; ok {
		t.Fatal("fix (3): a lapsed maturer must be out of the post-recovery epoch set")
	}
	if _, ok := c.epochSet[idOf(fresh)]; !ok {
		t.Fatal("fix (3): the fresh joiner must be seated in the post-recovery epoch set")
	}
	if n := c.validatorSetSize(); n != 10 {
		t.Fatalf("fix (3): post-recovery epoch must seat the 10 live members, got %d", n)
	}
	...
	if err := c.Append(*b3); err != nil {
		t.Fatalf("fix (3): the chain must resume after recovery, h3 got: %v", err)
	}
	...
	rh, rhh := r.Head()
	if ch != rh || hh != rhh {
		t.Fatalf("fix (3) DETERMINISM BROKEN: replicas diverge (head %s@%d vs %s@%d)", ch, hh, rh, rhh)
	}
}
