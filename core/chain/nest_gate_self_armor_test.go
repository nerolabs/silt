...
// "evidence self-armor".
//
// CLAIM DRIVEN HERE: a lone equivocating proposer can make its own double-sign unslashable,
// with no bond, no coalition, and no misconfiguration, by packing its own blocks with
// junk-but-VALID equivocation proofs so a legitimate proof about those blocks exceeds
// chain.SlashesBytesCap. Routed by pair closes it" by
// Both name the standing theorem T-NEST:
// evidence that embeds the object it accuses, committed inside an object of the same kind
// under a shared size bound, has NO satisfying bound — the recursion is cut only by making
// the carried quantity independent of the accused body's size, routed to D1.
//
// MECHANISM (verified at these sites): CheckEquivocation (equivocation.go) checks only
// culprit key SIZE, not-pruned, equal Height, differing bodyHash, matching era, and a
// shared (round, phase) scope — never that the culprit is bonded, that the evidence
// blocks belong to this chain, or that the proof is a duplicate. Equivocation carries two
// FULL Blocks (equivocation.go); a Block carries its own Slashes field
// (chain.go), bounded only by SlashesBytesCap, and Prune never recurses into
// Slashes (chain.go) so a committed proof stays resident forever.
//
...
// proof exceeds cap by 1,125 B). **This CORRECTS the research's arithmetic figure of
// ~8.39 MiB per side** ("the armor threshold
// is |b| > (κ − c)/2... at κ = 16 MiB that is |b| > 8.39 MiB per side") — that seat
// had no shell and flagged the number as not-yet-driven. The driven threshold is
...
// Q3/Q4 over-cap checks to t.Fatalf — a legitimate proof about a maximally-packed block
// must then always fit under SlashesBytesCap, at any Slashes fill level, because
// evidence size no longer scales with the accused body. Until then this file's job is
// to keep the four numbers true and catch the day the construction itself stops working
// (Q1/Q2 go RED — see their own doc comments for what that would mean).
...
// need not be a validator at all (equivocation.go, "verified at source" per T1). This
// is what makes the self-armor construction FREE to build (no bond, no coalition). It is
// cheap and always runs (not -short-gated).
//
...
// good news — but would NOT close the residual itself: T0 (armoring with two GENUINE
// committed proofs from an honest chain's own pendingSlashes backlog) needs no throwaway
// key at all. Re-derive this file's measurements either way; do not read a RED here as
// the residual closed.
func TestRNestGate_JunkKeyProofIsAcceptedEvidence(t *testing.T) {
	...
	if err := CheckEquivocation(&e, ports.Hash{}, eraFloorOf(0)); err != nil {
		t.Fatalf("ROOT CAUSE GONE: CheckEquivocation now REFUSES a throwaway-key proof about "+
			...
			"is closed; 's T0 variant (two genuine committed proofs) is not — see this "+
			"test's doc comment. Update SLASHCAP-NESTE"+
...
func TestRNestGate_SelfArmorMeasurement(t *testing.T) {
	if testing.Short() {
		t.Skip("measurement harness (OPEN break, closer: route (C) " +
	...
	if outcome, err := v5ValidateSlashes(liveView{w.c}, armored); outcome != Accept || err != nil {
		t.Fatalf("Q2 REGRESSED: the armored block (Slashes at %d B, %d proofs, shipped DefaultConfig, "+
			...
			"route (C) fixed-size-evidence close (good) or an ad hoc body bound (the research "+
			"research's §3 REFUTED-in-the-unsafe-direction finding: a body bound HALVES the "+
	...
	t.Logf("Q4 ARMOR THRESHOLD: %d junk proofs/side (%d B = %.4f MiB per side) keeps the legitimate "+
		...
		"pushes it over cap (%d B, %d B over). Compare to the research research's arithmetic "+
		"figure of ~8.39 MiB per side (SLASHCAP-NESTE"+
