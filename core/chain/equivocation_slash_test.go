...
// Blind (consensus, equivocation slash inert) INVERTED as a regression: in the
// M0-default objective mode, a proven double-sign now costs the actor its
// standing. The slash is an ON-CHAIN record (Block.Slashes) the objective set
// honors — a self-verifying equivocation proof that, on commit, evicts the
// culprit from c.bonded and bars it from re-earning standing. The old inert
// path (slashing only the reputation ledger the objective set never reads) is
// replaced.
...
func TestObjectiveEquivocationSlashEvicts(t *testing.T) {
	...
	if !VerifyEquivocation(&proof, ports.Hash{}, eraFloorOf(0)) {
