...
// C3 final round — the trace behind the e2e re-scope, PINNED AS A TEST.
//
// THE CLAIM (convergence §1, 2026-09-03): on a `-objective=false` topology the era-4
// readiness tally can NEVER latch, at ANY readiness stamp, on ANY binary. The
...
// gateFStampPin) is the release edit that turns the rollout rule live. Whoever makes
// that edit re-reads the gate-F family, and this is the test that tells them raising
// the stamp does NOT green e2e
// TestPaidDeliveryLaneRefusesWithoutACommittedKeyBinding: that fixture needs an
// OBJECTIVE, bonded, epoch-enabled topology first (residual), or the paid lane stays
// dark no matter what the constant says.
//
...
// TestNonObjectiveTopologyCanNeverLatchEra4 injects the readiness stamp the
...
func TestNonObjectiveTopologyCanNeverLatchEra4(t *testing.T) {
	whale := key(94501)
	...
	build := func(t *testing.T, objective bool) *Chain {
		...
		if objective {
			...
			// `MinBond > 0 && verifyBond != nil`; both halves are set here
			// and neither is set below.
			cfg.MinBond = 1 << 20
		...
		// The era-4 tally runs at every epoch boundary INCLUDING genesis
		// (boundary 0), so an all-ready set locks in there and H_era4 = 0 +
		// EpochBlocks = 4. The v1 commit helper cannot build a v4/v5 block, so
		// stop below H_era4 — which is enough: the latch has already happened or
		// it never will.
		for c.Len() < 4 {
	...
	for h := uint64(0); h <= 64; h++ {
		if v := dark.MintVersion(h); v >= BlockVersionWitnessable {
			t.Fatalf("MintVersion(%d) = v%d on a -objective=false chain with every "+
				"validator signalling readiness %d. The convergence's trace is wrong and "+
