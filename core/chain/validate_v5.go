...
// era-4 (v5) — THE ONE ACCEPT COMPOSITION, with TWO entry points.
//
// ValidateProposalV5 (P1…P13) is the attester's pre-sign rule; ValidateCommitV5 is ValidateProposalV5
// followed by C1…C5 (the two quorum stacks). The full node runs both over liveView (chain.go
// dispatches for Version >= BlockVersionWitnessable); the trustless floor box runs ValidateCommitV5
// over a witness-backed provenView. There is ONE body per rule, so a box can no longer reproduce a
// predicate's TAIL while missing the precondition a different validation stage established — the
// defect shape defeated six times in the 2026-09 cycle (N1 the author screen
// the parent binding, N2/N3/N6/N7 the driver-asserted box state, N5 the malformed-leg divergence).
// And there is ONE implementation for both node entry points, so the attester signs under the rule
// the committer accepts under — the one-function-two-callers law applied to the surface that drifted
// (P-table research).
//
// Certified (binding):
//
...
// only; AppendGenesis is height 0, which has no parent to bind. Both are OUTSIDE this guarantee.
//
// THIS EXTRACTION IS v5-ONLY, AND IT IS ON THE LIVE PATH. The node dispatches here only for
// b.Version >= BlockVersionWitnessable; the era-1 and era-2 legs are byte-untouched. On a
...
//
// Identity of a replica before this round and after is provable by construction, WITH… (§6
// of the research): without the legacy leg the composition refuses a v5 block a legacy node
// accepts; without P8b/the P5 clause it accepts where the node rejects.
//
// HELD-IN-TENSION, AND IT GROWS HERE. Once the node's v5 path IS this composition, a box-vs-node
// gate is structurally BLIND to a bug inside the shared body: both sides move together. grows
// it further: the attester and committer legs share one implementation, so a bug inside it is
// invisible to every box-vs-node AND attester-vs-committer comparison. The debt lives in the
// composition's own unit gates and in the STAGE-COVER gate (composition_stage_cover_v5_test.go),
// which derives the node's stage list from ValidateProposal/ValidateCommit and asserts this file
// covers every one.
//
...
var (
	// ErrViewNoWitness marks an IndeterminateTrustlessly whose cause is a read the view could
	// not see. It NAMES the read, because a stall whose reason is "something was missing" is
	// how the next round talks itself into a default. It is never returned by liveView (pins
	// that as a diagnostic); if it ever is, the cost is a refusal, never an acceptance.
	ErrViewNoWitness = errors.New("chain: floor-box state view has no witness for a required committed read — stall")

	// ErrWitnessBudgetUnset marks the STALL: the view's Budget is the zero value. Zero is
	...
	// ErrWitnessBudgetExceeded marks an IndeterminateTrustlessly from the box-owned byte budget:
	// the block's frame is larger than this view will pay for. It is checked BEFORE any
	// per-entry crypto and before any witness is looked at, so an out-of-budget block costs one
	// encode. Stall-adding only — it can never turn a node-Reject into a box-Accept. See Budget
	// for the measured 2.67 GiB exposure it bounds.
	ErrWitnessBudgetExceeded = errors.New("chain: floor-box witness budget exceeded — stall (the validity-rule ceiling is not built yet)")
...
// stall builds the named-read stall reason. Naming the read is the attribution lesson: a
...
type nodeStage struct {
	// ID is the label (P1…P13b, C1…C5, Q1…Q4).
	ID string
...
// nodeStages is the node's validity stage list for a v5 block, in the node's order — P-table (24
// rows). The STAGE-COVER gate DERIVES the call-shaped rows from ValidateProposal / ValidateCommit
// / requireQuorumStack by AST walk and asserts this table equals the derivation, element for
// element, including order; then asserts every row is carried by the composition. Do not hand-edit
// this table without the gate: it drifted once in four days.
//
// TWO ACCELERATOR SUBSTITUTIONS THIS TABLE DOES NOT SHOW. The one SUBSTITUTED row is
// validateEra3Roots. Two node helpers that return no error — and so are not rows — are also
// not mirrored but replaced by the era-4 committed accelerator: qualifiedCount (chain.go, a
// live filter over bonded/slashed/MinBond) is read as len(Qualified) in v5ValidatorSetSize,
// and liveQualifiedSet is read as Qualified in v5EffectiveEpochSet's recovery arm. Equal iff
// the era-4 maintenance invariant qualified == filter(bonded, slashed, MinBond) holds at the
// five apply sites; the guard is TestQualifiedMaintenanceDriftGuard
// (modelcheck_era4_maintenance_test.go), which must not be weakened — for a v5 block
// len(qualified) is now an input to RequiredQuorum, a safety quantity. The v4/v5 parity oracle
// drives both reads against the node's live filters (the recovery-boundary and de-mature
// regimes).
var nodeStages = []nodeStage{
	...
	{ID: "P4", Node: "", Mirror: "v5RequireProposerQualified"}, // proposerQualifiedAt (+ the branches, P4a)
	{ID: "P5", Node: "", Mirror: ""},                           // non-empty block, SIX clauses
	...
	{ID: "P8b", Node: "validateIssuerKeys", Mirror: "v5ValidateIssuerKeys"}, // v5-ONLY
...
// the two box-owned screens (0: the L1 version partition; 0b: the frame budget).
//
// It takes NO *Chain receiver. The order is the NODE's path (ValidateProposal, chain.go), not a
// function list. P4 (proposer qualification) runs BEFORE the quorum stacks in ValidateCommitV5: the
// author screen the box missed is not inside requireEpochWeightQuorum — that function's tail is
// `support:= set[proposer]` with no screen, and that is CORRECT, because P4 already refused the
...
// 0b. frame byte budget v.WitnessBudget; node: unlimited one encode
...
//
// There is NO S2 mode fence here: the node's liveView takes the legacy branch through
...
func ValidateProposalV5(v StateView, b *Block) (FloorBoxOutcome, error) {
	...
	// ---- 0b. FRAME BYTE BUDGET — before any per-entry crypto, before any witness. ----
	...
	// This is the missing precondition behind: validateCarrier verifies
	...
	// ---- 1b. DIGEST CONSISTENCY. Block-local, no state read, no witness. ----
	// The MIRROR of validateBlockDigests on the node path (a node body the composition
	// mirrors changed, so the mirror is re-derived here in the SAME commit — updating the
	// pin alone would split the v5 accept path from the v2/v4 path, which is).
	//
	...
	// The mirror of validateParamsPlacement on the node path. Only the genesis block may
	...
	// ---- 3. P4 PROPOSER QUALIFICATION (+ P4a, the attribution branches, verbatim). ----
	if out, err := v5RequireProposerQualified(v, b); out != Accept {
		return out, err
	}

	// ---- 4. P5 non-empty block — SIX clauses (IssuerKeys is the sixth). ----
...
// POL threshold IS the commit threshold.
//
...
// load-bearing (F1, N5), so the composition calls the shared constructor and never takes `seen`
// as a parameter.
//
// It is the ONLY entry the floor box calls (the box validates COMMITTED blocks —
// build-plan).
...
// v5CheckBudget is the gate at step 0b. The zero Budget STALLS; an unlimited budget costs
