...
// Consensus model-check — tier 1, launch replay (fork-choice / no-reorg-of-final).
//
// The "reorged onto a heavier fork, new head height 0" was a zero-weight tiebreak
// degeneracy — during the drain window anchor attesters have bonded=0, so a committed
// chain weighed ≈0 and `heavier` fell to a height-blind hash tiebreak that a bare-genesis
// fork could win, dropping committed blocks. The fix that stands: a quorum-finality gate
// makes a super-quorum-committed block irreversible — fork-choice chooses only among
// descendants of the finalized head — and the §1b height preference, now the PRIMARY
// fork-choice term (height → head-hash; the anchor bootstrap weight that
// shipped alongside them is retired).
//
// This oracle asserts the invariant that closes: over adversarial competing forks
...
// (the earlier no-finality-gate state), the taller conflicting fork is adopted by
// heavier and the finalized chain is reorged — RED. With the shipped gate — GREEN.

// rampLaunch builds a committed anchor-attested launch chain [g, b1@1, b2@2] (4 anchors,
...
func rampLaunch(t *testing.T) (*Chain, []ed25519.PrivateKey, *Block) {
	t.Helper()
...
// TestModelCheck_NoReorgOfFinalizedLaunchBlock is the oracle: a super-quorum-
// finalized launch chain is never reorged by any competing fork, and the result is
// order-independent.
func TestModelCheck_NoReorgOfFinalizedLaunchBlock(t *testing.T) {
	c, ak, g := rampLaunch(t)
	finalHead, finalHeight := c.Head()
	if finalHeight != 3 {
		t.Fatalf("setup: committed head must be at height 3 (Head()==3), got %d", finalHeight)
	}
	// (The " weight face" positive control — `Weight > 0` — that stood here was
	// DELETED with the weight term (the twin site). It
	// asserted a property production does not have: green only because this fixture
	// hand-builds Version: 1 blocks with era-1 attestations. The oracle below is the
	// part that guards and it stands.)
	...
	for name, fork := range forks {
		...
		if adopted {
			t.Fatalf("VIOLATION — the %q fork reorged a super-quorum-finalized launch block (D-1: finalized history is irreversible)", name)
		}
		if !errors.Is(err, ErrPreFinalityReorg) && err != nil && !errors.Is(err, ErrForeignGenesis) {
			// Refusal is expected; only a SILENT adopt (adopted==true) is the failure.
			// A non-finality error (e.g. re-validation) is also a refusal — fine.
		}
		if hh, h := c.Head(); h != finalHeight || hh != finalHead {
			t.Fatalf("VIOLATION — head moved after reconciling the %q fork: height %d (want %d) — finalized block was dropped", name, h, finalHeight)
		}
	}
}

// TestModelCheck_ForkChoiceIsOrderIndependent asserts I5 determinism: reconciling
// the same set of competing forks in different orders yields the same head — fork-choice
// is a pure function, never a hash-luck tiebreak that depends on arrival order.
func TestModelCheck_ForkChoiceIsOrderIndependent(t *testing.T) {
	...
	if h1 != h2 || h1 != h3 || ht1 != ht2 || ht1 != ht3 {
		t.Fatalf("I5 VIOLATION — fork-choice is order-dependent: heads %x@%d / %x@%d / %x@%d", h1, ht1, h2, ht2, h3, ht3)
	}
}
