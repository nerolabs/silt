...
// The two-level v5 block hash: digest-consistency validity.
//
...
// unbound on v5, which is strictly worse than before. It is the price of the purchase and it is
// not optional.
//
...
// re-interpreted (the pointers are nil there and omitempty omits them — chain.go).
//
// Gates:.(blockdigests_test.go), ablation red first.

var (
	// ErrBlockDigestMismatch is a v5 digest that does not equal sha256 of its own content.
	ErrBlockDigestMismatch = errors.New("chain: v5 payload digest does not match its content")
	// ErrBlockDigestMissing is a v5 payload present with no digest committing it.
	ErrBlockDigestMissing = errors.New("chain: v5 payload carries no digest")
	// ErrBlockDigestPreV5 is a pre-v5 block carrying an era-4 digest field.
	ErrBlockDigestPreV5 = errors.New("chain: pre-v5 block carries a v5-only digest field")
)
...
// validateBlockDigests is the digest-consistency rule. It runs on every disk-write path, beside
// validateEra3Version / validateEra4Version, so the commit path and the own-disk Reload path enforce
// the identical rule — the symmetry discipline.
func validateBlockDigests(b *Block) error {
	if b.Version < BlockVersionWitnessable {
		// PRE-v5: neither digest may appear. Refusing here (rather than ignoring)
		// keeps the frozen-format story honest — a v2/v4 block whose bytes carry a
		// v5-only field would hash differently from the same block written by a pre-
		// binary.
		for i := range b.BondRegs {
			if b.BondRegs[i].AnswerDigest != nil {
				return fmt.Errorf("%w: v%d block, bondreg %d carries AnswerDigest", ErrBlockDigestPreV5, b.Version, i)
			}
		}
		if b.SlashesDigest != nil {
			return fmt.Errorf("%w: v%d block carries SlashesDigest", ErrBlockDigestPreV5, b.Version)
		}
		return nil
	}
	...
	for i := range b.BondRegs {
		...
		if r.AnswerDigest == nil {
			return fmt.Errorf("%w: bondreg %d (validator %s) has no AnswerDigest; a v5 registration commits its space-time proof by digest whether the proof is carried or pruned",
				ErrBlockDigestMissing, i, r.ValidatorID())
		}
		if r.Answer == nil {
		...
		if want := answerDigestOf(r.Answer); *r.AnswerDigest != want {
			return fmt.Errorf("%w: bondreg %d (validator %s) AnswerDigest %x != sha256(Answer) %x",
				ErrBlockDigestMismatch, i, r.ValidatorID(), (*r.AnswerDigest)[:8], want[:8])
		}
	}
	...
	case len(b.Slashes) == 0 && b.SlashesDigest != nil:
		return fmt.Errorf("%w: block carries SlashesDigest with no Slashes — the digest must be absent, not a digest of nothing", ErrBlockDigestMismatch)
	case len(b.Slashes) > 0 && b.SlashesDigest == nil:
		return fmt.Errorf("%w: block carries %d slash proof(s) and no SlashesDigest; on v5 the preimage folds the DIGEST, so an absent digest leaves the payload unbound",
			ErrBlockDigestMissing, len(b.Slashes))
	case len(b.Slashes) > 0:
		if want := slashesDigestOf(b.Slashes); *b.SlashesDigest != want {
			return fmt.Errorf("%w: SlashesDigest %x != sha256(canonical(Slashes)) %x",
				ErrBlockDigestMismatch, (*b.SlashesDigest)[:8], want[:8])
		}
	}
	return nil
}

// setBlockDigests populates the v5 digest fields from the block's own payloads. Proposer-side only —
// it is never a substitute for validateBlockDigests, which every replica runs.
func setBlockDigests(b *Block) {
	if b.Version < BlockVersionWitnessable {
		return
	}
	for i := range b.BondRegs {
