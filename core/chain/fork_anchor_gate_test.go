...
// — the launch anchor gate is the LAUNCH FACE of the intersecting-quorum
...
//
// The field run (the field run) ran with the gate OFF (`-anchor-quorum` unset →
// default 0 → inert; see issue attribution correction), so a two-sybil- signature
// quorum committed a sybil-side fork with NO anchor at all. Once the gate is
// configured, the consult's proposed `AnchorQuorum=⌈A/2⌉` (=2 for A=4) is STILL
// insufficient: two sybil-proposed competing blocks with the four anchors split
// 2-2 as attesters each satisfy AnchorQuorum=2, so BOTH pass ValidateCommit and
// the finality gate then CEMENTS a permanent conflicting-finalization partition.
//
// Certified fix (this file is failing-first for it):
// - Encoding (B): launch proposing is ANCHOR-ONLY (sybils drain via
// MsgSubmitBondReg submit-don't-propose — removes the sybil-proposed
// fork at the source.
...
// launchObjective builds a fresh objective launch network: A anchors + S bonded sybils
...
func launchObjective(t *testing.T, nAnchors, nSybils, aq int) (*Chain, []ed25519.PrivateKey, []ed25519.PrivateKey) {
	t.Helper()
...
// TestBothSybilProposed22SplitCannotBothFinalize is the failing-first
...
// ValidateCommit → permanent partition. The rule refuses both (sybils can't
// propose AND neither reaches a 3-anchor majority), while the honest
// 3-of-4-anchor block still commits.
func TestBothSybilProposed22SplitCannotBothFinalize(t *testing.T) {
	c, ak, sk := launchObjective(t, 4, 8, 2) // AnchorQuorum=2 — the consult's proposed (insufficient) rule
	prev, _ := c.Head()
	...
	if passA || passB {
		t.Fatalf("(I1): a both-sybil-proposed 2-2 anchor split must NOT both-finalize; "+
			"forkA passed=%v forkB passed=%v — AnchorQuorum=2 is insufficient (the rule is ⌊A/2⌋+1)", passA, passB)
	}
}

// TestSybilCannotProposeAtLaunch asserts encoding (B): during the launch window
...
func TestSybilCannotProposeAtLaunch(t *testing.T) {
	c, ak, sk := launchObjective(t, 4, 8, 2)
	prev, _ := c.Head()
	...
	if err := c.ValidateProposal(b); !errors.Is(err, ErrLowReputation) {
		t.Fatalf("encoding-B: a sybil-proposed launch block must be refused at ValidateProposal (anchor-only proposing); got %v", err)
	}
	if err := c.ValidateCommit(b); err == nil {
		t.Fatal("encoding-B: a sybil-proposed launch block must not commit")
	}
}

// TestDerivedAnchorMajorityIgnoresConfig is the STRUCTURAL guarantee that closes
...
func TestDerivedAnchorMajorityIgnoresConfig(t *testing.T) {
	c, ak, sk := launchObjective(t, 4, 8, 0) // AnchorQuorum=0 — the field footgun
	prev, _ := c.Head()
	...
	if err := c.ValidateCommit(fork); !errors.Is(err, ErrAnchorRequired) {
		t.Fatalf("structural: with AnchorQuorum=0 the DERIVED ⌊A/2⌋+1 majority must still refuse a 1-anchor fork; got %v", err)
	}
}

// TestHonestThreeOfFourCommitsAndOneDownTolerated pins the liveness cost the
// research accepts: 3-of-4 anchors up commits (1-fault-tolerant); 2-of-4 does not.
// Same before and after the fix for the passing leg — the failing leg is RED
// pre-fix only if the old knob was < 3 (documented in-line).
func TestHonestThreeOfFourCommitsAndOneDownTolerated(t *testing.T) {
	c, ak, sk := launchObjective(t, 4, 8, 0)
	prev, _ := c.Head()
...
// TestSupportMeetsQuorumRequiresAnchorMajority pins that the proposer's gather
...
func TestSupportMeetsQuorumRequiresAnchorMajority(t *testing.T) {
	c, ak, sk := launchObjective(t, 4, 8, 0)
