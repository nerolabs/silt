...
// Consensus model-check — the EPOCH-BOUNDARY LIVENESS CLIFF (field DEEP run
// the field run, 2026-08-23). This is the deterministic face of the field
...
// membership — chain.go; the design comment there names the exact
...
//
// This oracle is a CHARACTERIZATION, not a born-RED-fix-me: > ⅓ offline denying a commit IS
// the BFT liveness bound and is correct for a LIVE set. The finding is that the denominator is
// the FROZEN set, so the effective bound is measured against stale membership — a live-1/3 dip
// can wedge when the frozen set over-counts departed members, and the rotation that would fix
// it is gated behind the very quorum the departure denies. Whether to change that (exclude
// provably-lapsed members from the frozen denominator; a boundary-local quorum re-basing; an
// R-gate exemption for a returning frozen member) is a consensus-rule call — research-gated
// (build-process rule 5), consult filed at This test pins the exact arithmetic so decision has
// a deterministic reference.
func TestModelCheck_FrozenWeightBoundaryWedge(t *testing.T) {
	...
	err := c.Append(*b)
	if err == nil {
		t.Fatal("NOT REPRODUCED: a 9-of-12 live coalition committed — the frozen-weight cliff did not bite (check the weight arithmetic)")
	}
	if !errors.Is(err, ErrNoQuorumWeight) {
		t.Fatalf("the refusal must be the frozen-weight rule, got: %v", err)
	}
	...
	// predicate must not disagree with ValidateCommit — the lesson).
	if c.SupportMeetsQuorum(anchorID(0), []ports.NodeID{
		...
		t.Fatal("SupportMeetsQuorum says the 9-of-12 live coalition is sufficient, but ValidateCommit refused it — the two must agree")
	}
	...
	if err := c.Append(*b2); err != nil {
		t.Fatalf("control: with >⅔ frozen weight live (388 of 516 MiB) the height MUST commit, got: %v", err)
	}
}
