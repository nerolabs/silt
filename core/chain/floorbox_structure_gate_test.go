...
// PERMANENT GATES for the FLOOR-BOX STRUCTURE round 1A (main-only16).
//
// Governing documents:
// - build brief: §7
// - P-table delta:
...
// precondition a DIFFERENT validation stage had established (N1 the author screen
...
//
// THE FIXTURE IS v5-DRIVEN, AND THAT IS ASSERTED FIRST (arm D /). BlockVersionRounds (2) is what
// the proposer mints today and Era4ActivationHeight has no cmd/silt flag, so a fixture that does
// not BUILD its v5 chain never enters the composition and every equivalence gate over it is
// vacuously green — a gate that does not hold the seam. TestFixtureCommitsAWitnessableBlock is
...
// mkBlock builds a fully v5 block on the fixture's head: proposer signature, committed roots,
// prepare QC and precommit certificate — a block the NODE accepts. mutate shapes the payload
// before the roots are computed.
...
// assertHonestTwinAccepts is the NON-VACUITY control every structure gate carries: the
...
// =============================================================================
// D — the fixture reaches the v5 path (NON-VACUITY, runs first)
// =============================================================================

// TestFixtureCommitsAWitnessableBlock. Ablation: force b.Version = BlockVersionRounds in
...
func TestFixtureCommitsAWitnessableBlock(t *testing.T) {
...
// =============================================================================
// BOTH node entry points dispatch to the composition
// =============================================================================

// TestBothNodeEntryPointsDispatchToTheComposition pins the structural claim of: the
...
// under — the one-function-two-callers trap inside the node.
// Ablation: revert the ValidateProposal dispatch hunk ⇒ RED.
// SOURCE GATE: reads chain.go and asserts the FIRST statement of each root is the guarded dispatch.
// RUNTIME GATE: TestLegacyRepLegOracle drives ValidateCommit (the dispatch) on one block.
func TestBothNodeEntryPointsDispatchToTheComposition(t *testing.T) {
	f := buildStructFixture(t)
	...
	for root, entry := range map[string]string{"ValidateProposal": "ValidateProposalV5", "ValidateCommit": "ValidateCommitV5"} {
		...
		if !ok {
			t.Fatalf("SOURCE GATE: — %s's FIRST statement must be the v5 dispatch (anything before it runs "+
		...
		for _, needle := range []string{"BlockVersionWitnessable", entry, "liveView"} {
			if !strings.Contains(src.String(), needle) {
				t.Fatalf("SOURCE GATE: — %s's v5 dispatch must be guarded by BlockVersionWitnessable and call %s over "+
...
// =============================================================================
// NoWitness is the zero of Availability; liveView never answers it; provenView with no
// source answers nothing else
// =============================================================================

// TestLiveViewNeverAnswersNoWitness. IndeterminateTrustlessly is unreachable under liveView
...
// tells us the adapter is not silently stalling. Ablation: return NoWitness from one liveView
// accessor ⇒ RED.
func TestLiveViewNeverAnswersNoWitness(t *testing.T) {
	if Availability(0) != NoWitness {
		t.Fatal("NoWitness must be the ZERO of Availability — a forgotten field must read as a stall, never as absent")
	}
	f := buildStructFixture(t)
	b := f.mkBlock(t, nil)
	assertHonestTwinAccepts(t, f.c, b)
	...
	if !v.WitnessBudget().Unlimited() {
		t.Fatal("liveView must return UnlimitedBudget — a ceiling on the node is a new validity rule")
	}
	if h := v.Head(); h.Empty || h.StateRoot == nil || h.LogRoot == nil || h.NextHeight != b.Height || h.Hash != b.Prev {
		t.Fatalf("liveView.Head must reproduce Chain.Head with the parent's roots present: %+v", h)
	}
	...
	for _, ck := range checks {
		if ck.av == NoWitness {
			t.Errorf("liveView.%s answered NO WITNESS. A full node holds the whole state; this is an adapter bug. "+
	...
	_, av = pv.Rep(id)
	if av != NoWitness {
		t.Fatalf("provenView.Rep must answer NoWitness (legacy rep is not a committed leaf); got %s", av)
	}
	for name, get := range map[string]func() Availability{
		...
		if av := get(); av != NoWitness {
			t.Errorf("provenView.%s with no source answered %s, want NO_WITNESS — a sourceless box must stall, never see absence", name, av)
		}
	}
}
...
// =============================================================================
// the zero Budget STALLS; only UnlimitedBudget is unlimited
// =============================================================================

// TestZeroBudgetStallsAndOnlyLiveViewIsUnlimited. Ablation: make v5CheckBudget treat
// the zero Budget as unlimited ⇒ the zero-budget view proceeds past step 0b ⇒ RED.
func TestZeroBudgetStallsAndOnlyLiveViewIsUnlimited(t *testing.T) {
	if !(Budget{}).IsZero() || (Budget{}).Unlimited() {
		t.Fatal("the zero Budget must be IsZero and NOT Unlimited")
	}
	if _, err := ByteBudget(0); !errors.Is(err, ErrBudgetNotPositive) {
		t.Fatalf("ByteBudget(0) must REFUSE; got %v", err)
	}
	if _, err := ByteBudget(-1); !errors.Is(err, ErrBudgetNotPositive) {
		t.Fatalf("ByteBudget(-1) must REFUSE; got %v", err)
	}
	f := buildStructFixture(t)
	b := f.mkBlock(t, nil)
	assertHonestTwinAccepts(t, f.c, b)
	...
	if out != IndeterminateTrustlessly || !errors.Is(err, ErrWitnessBudgetUnset) {
		t.Fatalf("VIOLATED: the zero Budget must STALL at step 0b with ErrWitnessBudgetUnset; got %s / %v", out, err)
	}
	...
	if out != IndeterminateTrustlessly || !errors.Is(err, ErrWitnessBudgetExceeded) {
		t.Fatalf("an over-budget frame must stall on the BUDGET before any crypto; got %s / %v", out, err)
	}
	...
	if !errors.Is(err, ErrBadSignature) {
		t.Fatalf("ablation arm: within budget the garbage signature must fail P3; got %s / %v", out, err)
	}
	if !(liveView{f.c}).WitnessBudget().Unlimited() {
		t.Fatal("liveView must be the one view that returns UnlimitedBudget")
	}
}

// =============================================================================
// an IssuerKeys-only v5 block: accepted; a bad issuer key: refused by name
// =============================================================================

// TestIssuerKeysOnlyBlockParity. The two halves of the finding (a), driven:
...
func TestIssuerKeysOnlyBlockParity(t *testing.T) {
	f := buildStructFixture(t)
	...
	if err := f.c.ValidateProposal(&good); err != nil {
		t.Fatalf("VIOLATED (P5): ValidateProposal refused an IssuerKeys-only v5 block: %v", err)
	}
	if err := f.c.ValidateCommit(&good); err != nil {
		t.Fatalf("VIOLATED (P5): ValidateCommit refused an IssuerKeys-only v5 block: %v", err)
	}
	if out, err := ValidateCommitV5(liveView{f.c}, &good); out != Accept {
		t.Fatalf("VIOLATED (P5): the composition refused an IssuerKeys-only v5 block: %s / %v", out, err)
	}
	...
	for name, err := range map[string]error{
		...
		if !errors.Is(err, ErrIssuerKeyUnbonded) {
			t.Fatalf("VIOLATED (P8b): %s must refuse an UNBONDED issuer's registration by name (ErrIssuerKeyUnbonded); got %v", name, err)
		}
	}
	if out, err := ValidateProposalV5(liveView{f.c}, &bad); out != Reject || !errors.Is(err, ErrIssuerKeyUnbonded) {
		t.Fatalf("VIOLATED (P8b): the composition must refuse an unbonded issuer by name; got %s / %v", out, err)
	}
}
...
// useObjective:= *objective && *minRep > 0). The composition dispatches on VERSION, not on mode, so
// it must take this branch faithfully.
...
// mkBlock builds a v5 block on the legacy chain's head.
...
// TestLegacyRepLegOracle pins: the composition's LEGACY leg reads the local reputation view
// through StateView.Rep, and liveView answers it PRESENT — so a MinBond == 0 node accepts a v5
// block, refuses a low-reputation proposer by name (ErrLowReputation) with the node's own
// rendering ("has 10, needs 100"), and drops a low-reputation attester from the quorum.
//
// This is an ORACLE with absolute assertions, not a differential. Under the node's
// ValidateCommit IS ValidateCommitV5(liveView{c}, b) for a v5 block, so comparing the two would
// compare one function with itself (the own spec was self-defeating; the gate struck the
// clause). The v4/v5 differential for the legacy leg lives in the parity oracle
// (parity_oracle_v4v5_test.go, the legacy regime).
//
// Ablation: make liveView.Rep answer NoWitness ⇒ the accept case stalls ⇒ RED (never a
// skip: the fixture asserts it is legacy).
func TestLegacyRepLegOracle(t *testing.T) {
	lf := buildLegacyFixture(t)
	b := lf.mkBlock(t)
	assertHonestTwinAccepts(t, lf.c, b)

	if err := lf.c.ValidateCommit(&b); err != nil {
		t.Fatalf("VIOLATED (accept): a legacy node must accept a v5 block — the legacy leg's Rep read must be PRESENT on liveView; got %v", err)
	}
	if err := lf.c.Append(b); err != nil {
		t.Fatalf("the legacy chain must COMMIT the v5 block: %v", err)
	}
	...
	if !errors.Is(err, ErrLowReputation) {
		t.Fatalf("VIOLATED (refusal): the legacy leg must refuse on MinProposerRep by name (ErrLowReputation); got %v", err)
	}
	want := "proposer " + idOf(lf.prop).String() + " has 10, needs 100"
	if !strings.Contains(err.Error(), want) {
		t.Fatalf("VIOLATED (attribution): the legacy refusal must render the node's way (%q); got %v", want, err)
	}
	...
	if err := lf.c.ValidateCommit(&b3); err != nil {
		t.Fatalf("(attester leg): one demoted attester of three must still meet Quorum: 2; got %v", err)
	}
	lf.reps[idOf(lf.vals[1])] = 10
	if err := lf.c.ValidateCommit(&b3); !errors.Is(err, ErrNoQuorum) {
		t.Fatalf("(attester leg): two demoted attesters of three must miss Quorum: 2 (ErrNoQuorum); got %v", err)
	}
}

// =============================================================================
// the P13b LogRoot conjunct on BOTH views
// =============================================================================

// TestForgedLogRootIsRefusedOnBothViews. A revocation-free v5 block with a mutated b.LogRoot is
// REFUSED by the composition on liveView (the node's recompute) AND by the proven view's k = 0
// equality against the head's LogRoot. Ablation: delete the k == 0 equality in
// provenView.CommittedRoots ⇒ the mutant passes P13b ⇒ RED.
func TestForgedLogRootIsRefusedOnBothViews(t *testing.T) {
	f := buildStructFixture(t)
	...
	if err := f.c.ValidateCommit(&forged); !errors.Is(err, ErrEra3LogRootMismatch) {
		t.Fatalf("VIOLATED (liveView): a forged LogRoot must be refused by name; got %v", err)
	}
	...
	if out, err := pv.CommittedRoots(&forged); out != Reject || !errors.Is(err, ErrEra3LogRootMismatch) {
		t.Fatalf("VIOLATED (provenView): the k = 0 LogRoot equality must REJECT a forged LogRoot; got %s / %v", out, err)
	}
	...
	if out, err := pv.CommittedRoots(&honest); out != IndeterminateTrustlessly || !errors.Is(err, ErrRecomputeGated) {
		t.Fatalf("twin: the honest block must pass P13b and stall at the unwired P13a; got %s / %v", out, err)
	}
}

// TestRevocationBearingBlockStallsWithoutAWitness. A revocation-bearing v5 block with NO
...
//
// Ablation: delete the k > 0 branch in CommittedRoots => the block falls to the k = 0
// equality and is REJECTED (its LogRoot moved) => RED. The refusal must never render as a
// disproof, and never as an accept.
func TestRevocationBearingBlockStallsWithoutAWitness(t *testing.T) {
	f := buildStructFixture(t)
	...
	if out != IndeterminateTrustlessly || !errors.Is(err, ErrRevLogSizeUnauthenticated) {
		t.Fatalf("VIOLATED: a revocation-bearing block with no witness source must STALL on the proven view with ErrRevLogSizeUnauthenticated; got %s / %v", out, err)
	}
}
