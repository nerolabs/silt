...
// TestMaturityLatchDoesNotRearmAnchorsOnDemature is the regression, INVERTED from the
// attack that found it. That attack drove a matured network back to immature by CONCENTRATION (one
...
func TestMaturityLatchDoesNotRearmAnchorsOnDemature(t *testing.T) {
	...
	rounds := []struct {
		...
		// Anchor a1 proposes the LAUNCH block (anchor-only proposing, encoding
		...
		// (Earlier both were bonded-proposed, which the launch rule now refuses.)
	...
	// The LIVE metric de-matured, but the LATCH held — the whole point of.
	if c.Mature() {
...
// TestDeMatureSuperQuorumReplacesTheAnchorNet is the slice-2 regression: once a
...
func TestDeMatureSuperQuorumReplacesTheAnchorNet(t *testing.T) {
	...
	rounds := []struct {
		...
		// Anchor a1 proposes the launch block (anchor-only, encoding B); the
...
// TestWeakSubjectivityCheckpointRefusesLongRangeReorg is the slice-4 regression:
