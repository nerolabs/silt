...
// — the reg-inclusion rate bound (the Q3 R-rule) behind its version gate, per
// the research: readiness is rule-aware bonded WEIGHT over the
// frozen epoch (never heads), lock-in needs the same >⅔ super-quorum finality
// uses, enforcement starts at the NEXT epoch boundary (H_act, chain-derived),
// activation is monotonic, and the pre-latch trusted fleet declares the
// boundary as genesis config. The signal rides the BOND REG (hash-covered,
// conditionally signed like Domain, prune-surviving) — an attestation-borne
// signal is strippable by any re-serving peer because Block.Hash does not
// commit Atts.

// bondRegV is bondReg with the readiness signal stamped and signed.
...
// TestRegGatePreLatchEnforcesRRule: with the genesis-declared boundary, the
...
// of the rule's three clauses refuses a storm shape the old rules accept: a
// rapid re-registration inside R, a slashed identity's reg (R∞, the Defect-A
// commit path), and two regs for one identity in a single block (the one-block
// storm that validation-against-parent-state alone cannot see). First
// registrations stay exempt, and a legitimate TTL/2 renewal passes.
func TestRegGatePreLatchEnforcesRRule(t *testing.T) {
	...
	if err := c.Append(*storm); !errors.Is(err, ErrRegGate) {
		t.Fatalf("a re-reg %d blocks after the last must be refused past the gate (R=%d), got %v",
	...
	if err := c.Append(*dup); !errors.Is(err, ErrRegGate) {
		t.Fatalf("two regs for one identity in one block must be refused, got %v", err)
	}
	...
	if err := c.Append(*reg); !errors.Is(err, ErrRegGate) {
		t.Fatalf("a slashed identity's reg must be refused past the gate (R∞), got %v", err)
	}
}
...
// TestRegGateLockInIsWeightCountedAndBoundaryExact is the post-latch
// activation path end to end, pinning the three properties at once:
//
...
func TestRegGateLockInIsWeightCountedAndBoundaryExact(t *testing.T) {
	whale := key(50611)
	...
	if c.gateLockedIn {
		t.Fatal("a ¾ HEADS majority at 43% of the weight locked the gate in — readiness must be weight-counted")
	}
	...
	if !c.gateLockedIn {
		t.Fatal("full ready weight at an epoch boundary must lock the gate in")
	}
	if c.gateHeight != 16 {
		t.Fatalf("H_act must be the NEXT boundary after lock-in (want 16), got %d", c.gateHeight)
	}
	...
	if err := c.Append(*storm); !errors.Is(err, ErrRegGate) {
		t.Fatalf("the storm shape at H_act+1 must be refused, got %v", err)
	}
	...
	if !c.gateLockedIn || c.gateHeight != 16 {
		t.Fatalf("activation must be monotonic across a ready-weight collapse (lockedIn=%v H_act=%d)",
	...
	if err := c.Append(*storm2); !errors.Is(err, ErrRegGate) {
		t.Fatalf("enforcement must survive a later ready-weight collapse, got %v", err)
	}
}

// TestRegGateSignalIsSignedAndPruneSurvives: the readiness signal must be
...
func TestRegGateSignalIsSignedAndPruneSurvives(t *testing.T) {
	...
	if err := c.Append(*b); !errors.Is(err, ErrBadBondReg) {
		t.Fatalf("a version-stripped signalling reg must fail its signature check, got %v", err)
	}
	...
	if len(pruned.BondRegs) == 0 || pruned.BondRegs[0].Version != BlockVersionRegGate {
		t.Fatal("Prune must keep the readiness signal (a light field, like Domain)")
	}
}

// TestRegGateReplayDerivesIdenticalActivation: H_act is derived state — a
// fresh replica replaying the identical committed history computes the
// identical activation: every honest node computes H_act identically from
// the committed chain, regardless of when it upgraded.
func TestRegGateReplayDerivesIdenticalActivation(t *testing.T) {
	whale := key(50631)
	...
	if replica.gateLockedIn != c.gateLockedIn || replica.gateHeight != c.gateHeight {
		t.Fatalf("replayed activation diverged (live H_act=%d, replica H_act=%d) — the gate must be a pure function of committed history",
