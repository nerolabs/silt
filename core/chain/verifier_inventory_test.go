...
// =============================================================================
// THE VERIFIER-INVENTORY GATE
// =============================================================================
//
// The retired weight term hid at a THIRD
// verify site: `blockWeight` verified attestations against the BARE block hash while every
// production attestation has signed consensusSigBytes(phase, round, hash) since; the term was
// identically 0 for 18 days and nobody saw it, because the only fixtures that exercised it were
// era-1.
//
// THE RULE the pin encodes (the own words): a domain-separation change is a property of the
...
// MUST-BE-DELETE tombstone and the walk asserts the tombstoned site is GONE. Reason: with the
// alternative form (site simply absent from the allowlist), the RED on main reads as "unclassified
// site" — the same message a NEW bare-hash site would produce, so it could be greened by
// adding an allowlist row: the widening reflex. With the
...
// behaviour. RUNTIME GATE: TestEra2CertificateAcceptedByEveryEra2Verifier (below) drives one
// genuine era-2 certificate through every attestation verifier this allowlist marks
// era-2-reachable.
...
const (
	...
	o3tClassRelayOpenSig       o3tVerifyClass = "relay-open-sig"        // core/node: the relay-open commitment
	o3tClassDeliverySessionSig o3tVerifyClass = "delivery-session-sig"  // the durable fetcher's signature over the delivery session open/fund commitment
	o3tClassAttestationBare    o3tVerifyClass = "attestation-bare-hash" // the defect class
	o3tMustBeDeletedByTMarker                 = "MUST-BE-DELETE"
)

// o3tVerifyPackages is the set of packages the walk covers, keyed by the package clause the walk
// reads from each file, with the directory relative to THIS test's cwd (core/chain). The pin
// covered core/chain only; core/node is the other package that verifies consensus-adjacent
// signatures, and a bare-hash attestation pre-check added there would be the fourth site with no
// pin.
...
// o3tVerifyAllowlist is the CLASSIFIED inventory table, re-enumerated from the code — the research
// was read; PR added carrier.go since, plus the two core/node sites classified 2026-09-04.
var o3tVerifyAllowlist = map[o3tVerifySite]o3tVerifyRow{
	{"chain", "chain.go", "verifyAtt"}: {Count: 3, Classes: []o3tVerifyClass{o3tClassAttestation, o3tClassAttestation, o3tClassAttestation},
		Why: "THE era-aware attestation verifier, now THREE arms: era-1 (a.Round == 0 guard, bare hash), era-2 (consensusSigBytes(phase, round, h)) and era-4 (consensusSigBytesV5(chainID, phase, height, round, h)). The third arm is the dispatcher EXTENDED, which is the shape the pin demands: a fourth bare-hash verifier elsewhere would still be a new site. Every attestation verify in the package routes here except signedBlock's era-1-only arm."},
	{"chain", "equivocation.go", "signedBlock"}: {Count: 2, Classes: []o3tVerifyClass{o3tClassProposerSig, o3tClassAttestation},
		Why: "era-1 evidence ONLY: reachable from CheckEquivocation only when both evidence blocks are < BlockVersionRounds; era-2 evidence goes through consensusSigScopes -> verifyAtt. Folding the attestation arm into verifyAtt touches the evidence path and is out of scope here."},
	...
	{"chain", "chain.go", "validateStructural"}: {Count: 1, Classes: []o3tVerifyClass{o3tClassProposerSig},
		Why: "bare hash is the proposer scheme (the Reload path; its attester loop uses verifyAtt — the first-site fix)"},
	...
	// The era-4 (v5) accept composition MIRRORS the node's proposer-signature and bond-reg-signature
	// checks over StateView (floor-box structure round 1A, validate_v5.go). Same schemes, same
	// classes as the node sites they mirror; every ATTESTATION verify in the composition routes
	// through verifyAtt (v5CollectQuorumSigs, v5RequireProposerPrepare, validateCarrier).
	{"chain", "validate_v5.go", "ValidateProposalV5"}: {Count: 1, Classes: []o3tVerifyClass{o3tClassProposerSig},
	...
	{"node", "deliverysession.go", "OpenDeliverySession"}: {Count: 1, Classes: []o3tVerifyClass{o3tClassDeliverySessionSig},
		Why: "the DURABLE fetcher's signature over demand.SessionOpenCommitment(serverID, anchors) — the delivery session's open binding ; a payment-lane message with its own domain (silt/delivery/open/v1), not consensus."},
	{"node", "deliverysession.go", "FundDeliverySession"}: {Count: 1, Classes: []o3tVerifyClass{o3tClassDeliverySessionSig},
		Why: "the same fetcher's signature over demand.SessionFundCommitment(serverID, handle, anchors) — a top-up bound to its session (C8); same lane, own domain (silt/delivery/fund/v1), not consensus."},
	{"node", "relayrole.go", "OpenRelaySession"}: {Count: 1, Classes: []o3tVerifyClass{o3tClassRelayOpenSig},
		Why: "the fetcher ephemeral's commitment over relayOpenCommitment(relayID, root, S, anchors) — the relay prepayment anchor's session-open binding (cert step 5); a payment-lane message with its own domain, not consensus."},
	// THE TOMBSTONE. Present on main (chain.go). The walk FAILS while it exists.
	{"chain", "chain.go", "blockWeight"}: {Count: 1, Classes: []o3tVerifyClass{o3tClassAttestationBare},
		Why:    "THIRD SITE: verifies attestations against the bare block hash; 0 on every AttestAt certificate since. Retired, not repaired: a repair would make fork-choice a function of the evaluating replica's live state (the oscillation mechanism).",
...
// "crypto/ed25519"; e.Verify(.)` is a hit and `import ed25519 "elsewhere"; ed25519.Verify(.)` is
// not. A dot import makes the bare `Verify(.)` a hit. Known limit, by design: a verify wrapped
// in ANOTHER package (a ports helper) is out of reach of any single-package AST walk; that
// wrapper is a new verifier and belongs in the allowlist of the package that owns it.
	...
	for site, n := range counts {
		...
		case !ok:
			violations = append(violations, "UNCLASSIFIED ed25519.Verify site: "+site.String()+":"+itoa(firstLine[site])+
				...
				"verifyAtt or the era-1-gated signedBlock — a third bare-hash attestation verifier is again.")
...
// TestVerifierInventoryAllowlistIsWellFormed asserts the allowlist's own invariants before the
// walk trusts it: every row classed "attestation" is chain's verifyAtt or signedBlock pass
// condition; no non-tombstone row carries the defect class; the tombstone is exactly
// chain/blockWeight; every row's package is one the walk covers.
func TestVerifierInventoryAllowlistIsWellFormed(t *testing.T) {
	tombstones := 0
	for site, row := range o3tVerifyAllowlist {
		...
		for _, cl := range row.Classes {
			if cl == o3tClassAttestation && !(site.Pkg == "chain" && (site.Func == "verifyAtt" || site.Func == "signedBlock")) {
				t.Fatalf("SOURCE GATE: allowlist row %v is classed attestation but is neither chain's verifyAtt nor signedBlock — "+
					"no attestation is verified outside those two", site)
			}
			if cl == o3tClassAttestationBare && row.Marker != o3tMustBeDeletedByTMarker {
				t.Fatalf("SOURCE GATE: allowlist row %v carries the defect class WITHOUT a tombstone marker — "+
...
// TestVerifierInventoryPin is gate (a1): the walked set of crypto/ed25519 Verify sites in every
// non-test core/chain AND core/node file equals the classified allowlist, and the tombstoned
// blockWeight site is GONE. RED on main (chain.go blockWeight); GREEN after T.
//
// Scope: EVERY non-test *.go in each covered package (not a name glob —
// Resolved by import path, not the identifier.
func TestVerifierInventoryPin(t *testing.T) {
	...
	if v := o3tClassifyVerifySites(hits); len(v) > 0 {
		t.Fatalf("SOURCE GATE: R-558-VERIFIER-INVENTORY — %d violation(s):\n  %s\n\n"+
			...
			"  tombstoned blockWeight site is retired (delete Weight/blockWeight/anchorWeight/\n"+
			"  Config.AnchorWeight; heavier becomes height -> head-hash).", len(v), strings.Join(v, "\n  "))
	}
}

// TestVerifierInventoryPinHasTeeth proves the walk bites, in both directions, over synthetic
...
//	aliased.go G: `import e "crypto/ed25519"; e.Verify` plus e.VerifyWithOptions
//	 -> UNCLASSIFIED (2 calls). Passed the lexical pin.
...
func TestVerifierInventoryPinHasTeeth(t *testing.T) {
...
func (c *Chain) reInjectedWeight(b *Block) int64 {
	...
	for _, a := range b.Atts {
		if !ed25519.Verify(ed25519.PublicKey(a.PubKey), h[:], a.Sig) { // the shape, re-injected
			continue
		}
		n++
	}
...
// TestEra2CertificateAcceptedByEveryEra2Verifier is gate (a2) and the RUNTIME cover for the
...
//	verifyAtt via validateStructural — Reload (the first site)
//	verifyAtt via consensusSigScopes — CheckEquivocation's era-2 branch
//	verifyAtt via HeadCarrier — the carrier the NEXT proposer attaches
//	verifyAtt via validateCarrier — the carrier a v5 child carries over b.Prev
//
// signedBlock is NOT in this set (era-1 only): asserting it accepts an era-2 certificate would be
// asserting a falsehood scope note. This single fixture would have caught all three sites. GREEN now;
// must stay green. RED-first artifact: point any one verifier at the bare hash and the fixture fails
// there.
func TestEra2CertificateAcceptedByEveryEra2Verifier(t *testing.T) {
