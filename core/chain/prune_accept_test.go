...
// C3 re-break — BREAK 1 (F1), the WRONG-ACCEPT direction. Inversion of the red-team
...
// a latent wrong-Accept the flip would turn live.
//
...
// TestForgedPrunedRootIsRefusedByTheBox: a committed root whose only difference from the
...
func TestForgedPrunedRootIsRefusedByTheBox(t *testing.T) {
	...
	if err := recomputeViaHead(f.c, f.prevRoot, forged, b, w); err == nil {
		t.Fatalf("BREAK-D REOPENED (WRON): the floor-box recompute AGREED with a "+
			"committed StateRoot %x that a full node rejects", forged[:8])
	}
}

// TestForgedRootDiffersOnlyInIssuerKeyCommit keeps the measurement of the attack's surface:
// the forged post-state differs from the honest one in exactly one leaf, so the gate above is
// testing the issuerKeyCommit blind spot and nothing else.
func TestForgedRootDiffersOnlyInIssuerKeyCommit(t *testing.T) {
	f := buildPruneFixture(t, true)
	b := f.boundaryBlock(nil)
