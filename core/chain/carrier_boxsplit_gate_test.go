...
// =============================================================================
// the box-vs-node split gates
// =============================================================================
//
// The adversarial probes against the LastCommit carrier: the fix shape and the merge
// conditions.
//
// THE DEFECT,. The trustless floor box reproduced applyCarrier's TRANSITION — class A derives its
// write-set straight off b.LastCommit[i].AttesterID — but never validateCarrier's VALIDITY rule,
// which was wired only onto the full node's two disk-write paths. So an attacker minted a v5 block
// whose carrier named the PUBLIC keys of real qualified validators with zero-byte signatures,
// computed StateRoot with the real apply (applyCarrier does not verify either, by design, because
// validateCarrier already did), and published. Every full node REJECTED. The box AGREED with the
// attacker's root. Cost: no key material; bound: the transport frame. this gate is the escalation —
// the same forged carrier flips the one-way everMature latch, i.e. it forges the MEASURED
// decentralisation quantity the maturity shed gates on.
//
...
//
// THE ORACLE RULES THESE GATES OBEY:
//
...
//	 safe-direction screen; see decision §4 O-2.)
...
// COLD box (New(cfg)+SetBondVerifier, never applied a block). A gate that passed only warm would be
// passing on live state.
...
func rtGateWorldWith(t *testing.T, matureValidators, nVictims int, seed int64) *rtGateWorld {
	...
	// (a same-domain set collapses to one and would be unable to cross the bar).
...
func (w *rtGateWorld) witnessFor(t *testing.T, b Block) StateRootWitness {
	...
	if !w.c.everMature {
		// Unlatched: the box must reconstruct matureNow over the POST-apply committed state to
		// decide the crossing, so the honest prover serves the post-state SeenSet (the arm).
...
// (never applied a block). a gate that passes only warm was passing on live state, which is
// exactly the class of defect the fold-file pin exists for.
...
// -----------------------------------------------------------------------------
// The gate — a forged-signature carrier
// -----------------------------------------------------------------------------

func TestBoxNeverAgreesWithACarrierTheNodeRejects(t *testing.T) {
...
// The gate — the bound is the frame, not one seat: one block steals every qualified id.
func TestBoxNeverAgreesAtScale(t *testing.T) {
...
// The gate — the other validity clauses are equally unreproduced: wrong phase, a precommit over a
// FOREIGN block hash, duplicate ids. All three carry GENUINE signatures, so this arm cannot pass
// by a signature check alone.
func TestBoxNeverAgreesOnPhaseForeignHashOrDuplicateID(t *testing.T) {
...
// The gate — the height-1 rule. Height 1's carrier is EMPTY BY RULE (the genesis is DECLARED, not
// agreed, so its certificate must never pre-seat validatorsSeen). The entry carries a GENUINE
// signature; only the height makes it invalid.
func TestBoxNeverAgreesOnAHeightOneCarrier(t *testing.T) {
...
// -----------------------------------------------------------------------------
// The gate — the escalation: the forged carrier flips the maturity latch
// -----------------------------------------------------------------------------
//
...
// hole does not merely seat ids: it forges the MEASURED decentralisation quantity TENETS Part
// 0's maturity shed and C1's arrival count both read.
//
...
func TestForgedCarrierCannotFlipTheMaturityLatchPastTheBox(t *testing.T) {
