...
// C3 re-break — F9 regression gates. Inversion of the red-team probe
// rt_c3b_gatef_test.go.
//
...
// via gateFStampPin, and TestConfigActivationRouteCarriesTheSameCoverage drives the
...
// TestStampRaiseIsAHardFailure is the teeth-proof. Clause (c) cannot be exercised by
...
func TestStampRaiseIsAHardFailure(t *testing.T) {
	...
	for _, raised := range []uint8{BlockVersionStateRoot, BlockVersionWitnessable} {
		if gateFStampPin(raised) == "" {
			t.Fatalf("clause (c) does not fail on a raised stamp (%d). A one-line constant "+
				"change would turn the rollout rule live with a fully GREEN tree — the "+
				"exact silence red-team F9 measured.", raised)
		}
	}
}

// TestWireCarriesIssuerKeysAndTheHashCoversThem is the wire-level fact behind the rule,
// pinned against real CBOR bytes rather than by inspection: an proposer really does put
...
func TestWireCarriesIssuerKeysAndTheHashCoversThem(t *testing.T) {
	k := key(94100)
