...
// The covering probe for the same-root intra-block bond-registration contention
// fix (Research research same-root-intrablock-bondreg-contention-2026-08-28,
// resolution (a); closes its residual R2). This is the case the disjoint-root
// fixture never built.
//
...
// identities on the SAME root was ADMITTED. apply (chain.go) then
...
// seenReg, run UNCONDITIONALLY (not behind the gate). It rejects the block
// with ErrSharedRootInBlock, so the divergent input can never commit. There is
// then nothing order-dependent left to hash.
//
// The two binding caveats of the research are exercised here:
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil {
				...
				t.Fatalf("block with two distinct-ID proven regs on ONE root was ADMITTED — "+
					"the same-root dedup did not reject it. Head next-height now %d", next)
			}
...
// research: the dedup is on (root × DISTINCT-ID), NOT on root alone. A validator
// re-registering (renewing / resizing) its OWN root within one block — same
// ValidatorID, same root — is legitimate (F1) and MUST still be ADMITTED. If
// this block were rejected, the guard would be over-broad and would break a
// legal renewal.
func TestSameRootSameIDRenewAdmitted(t *testing.T) {
	...
	if err := c.Append(*b); err != nil {
		t.Fatalf("same-ID same-root renew/resize was REJECTED — the seenRoot dedup is "+
			"OVE; it must fire only on DISTINCT-ID collisions (F1 renew is legal): %v", err)
	}
