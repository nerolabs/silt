...
// The pruned-tolerance gate. A payload-pruned
...
// TestPrunedAtOrAboveFloorRejected is the C1 catch: a pruned block presented AT or
...
func TestPrunedAtOrAboveFloorRejected(t *testing.T) {
	c := q2Chain(10)
...
// TestPrunedBelowFloorAccepted: strictly below the floor the reg is finalized-trusted,
...
func TestPrunedBelowFloorAccepted(t *testing.T) {
	c := q2Chain(100)
...
// TestFreshNodeTrustsNoPrunedBlock: a node with no finality and no checkpoint has
...
func TestFreshNodeTrustsNoPrunedBlock(t *testing.T) {
	c := q2Chain(0)
...
// TestMalformedPrunedWithAnswerRejected is the decode-invariant belt: a block marked
...
func TestMalformedPrunedWithAnswerRejected(t *testing.T) {
	c := q2Chain(100)
...
// TestFullBlockUnaffectedAtAnyHeight: a full (unpruned) block is verified in full at
...
func TestFullBlockUnaffectedAtAnyHeight(t *testing.T) {
	c := q2Chain(10)
	v := key(4242)
...
// TestReconcileGatesPrunedInReplay proves the gate is wired into the Reconcile replay
...
func TestReconcileGatesPrunedInReplay(t *testing.T) {
	prop := key(1)
...
// TestPrunedEvidenceIsRefused is the gate (the cross-height forgery fix).
// It SUPERSEDES the gate above (renamed in place) — the I5 /
// accountable-safety corner Opt 1 believed it was protecting was itself the break:
// "Hash returns the stored pre-prune value" is EXACTLY the decoupling that let
// a cross-height forgery through. The fix (F2-EVIDENCE-RECOMPUTE) computes both blocks'
// hashes by re-marshalling the body and never trusts Block.Pruned, so a pruned block is no
// longer admissible evidence at all — not even for a GENUINE, non-forged late-reveal
// double-sign ("held in tension, not closed", bounded to below the prune floor, where
// ErrPreFinalityReorg already forbids adoption, so the safety consequence is nil and only the
// penalty is lost). All three of the original test's behavioral assertions INVERT: RED today
// (the shipped rule still trusts b.Pruned, so all three of the ORIGINAL assertions passed;
// they must now fail).
func TestPrunedEvidenceIsRefused(t *testing.T) {
	...
	// Drive the mixed pair through the real commit path (Append is the oracle
...
// TestReconcileHonorsPositiveReceiverFloor exercises the gate AND the threading on the
...
func TestReconcileHonorsPositiveReceiverFloor(t *testing.T) {
...
// TestReconcileFloorIsReceiversNotForks is the merge-gate invariant (slice5decision):
// the pruned-tolerance floor Reconcile uses is ALWAYS the receiver's OWN trustFloor,
// NEVER derived from the (peer-supplied) fork. A TALLER peer fork — whose own replayed
// history would imply a HIGHER floor — must not get a pruned block above the RECEIVER's
// floor accepted. Here receiver floor = 5, a height-12 peer fork's own floor would be 11;
// a pruned block at height 8 (5 ≤ 8 < 11) must be REJECTED (receiver's 5 governs), not
// accepted (fork's 11). If the override were ever fork-derived this is the Q1 C1 break —
// a peer inflates the floor and slips pruned forgeries under it.
func TestReconcileFloorIsReceiversNotForks(t *testing.T) {
...
// pruned block) — so a pruned own-disk block replays with NO gate change there. This is a
// regression guard (expected GREEN pre- and post-gate); it fails only if someone adds a
...
func TestReloadPrunedBlockRoundTrips(t *testing.T) {
	...
	// Head returns the NEXT expected height; genesis(0) + the height-1 block ⇒
	// 2.
