...
// fix (4): a frozen-epoch member re-proving a Root it ALREADY OWNS is exempt
// from the R interval — a returning participant restoring standing it already
// held, not the reg-flood of fresh identities R defends against. This is the
// layer that removes the field wedge's non-recovery (a returning member was
// R-refused, `re-registering 1 block after its last reg, R=10`, and so could
// not re-bond to heal the stalled boundary). Safe by construction: it can only
// restore weight the honest set already trusted for this epoch, never admit new
// weight (unlike shrinking the quorum denominator — the rejected fix (1), which
// cheapened capture 344→216 MiB).
func TestRegGate_RestoreExemptionForReturningFrozenMember(t *testing.T) {
	...
	// and the exact state the R would refuse without fix (4).
	...
	// genesis reg — inside R=10. Without fix (4) this is ErrRegGate; the research
	// exempts it because it merely RESTORES held (now lapsed) standing.
	h1hash, _ := c.Head()
	...
	if err := c.Append(*b2); err != nil {
		t.Fatalf("fix (4): a returning frozen member re-proving its owned root within R must be EXEMPT, got: %v", err)
	}
	...
	// is the storm, and R must still bite it (the narrowness that keeps
	// the flood protection intact for bonded members).
	c.bonded[member] = bond // re-bond it (live standing)
	if c.restoresHeldStanding(member, reReg.Root) {
		t.Fatal("a bonded member re-proving its root is the storm, NOT a restore — must not be exempt")
	}
