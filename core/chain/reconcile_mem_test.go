...
// memory oracle — the deep OOM's Reconcile spike, measured deterministically.
//
// FIELD OBSERVATION (the field run): val-d kernel-OOM ×2 on the 2 GB box during
...
func TestReconcileMemoryBounded(t *testing.T) {
	if testing.Short() {
	...
	t.Logf("decomposition: peak extra %d MiB; retained after GC %d MiB (transient garbage = %d MiB)",
		extra>>20, retained>>20, (int64(extra)-retained)>>20)

	// Budget: the throwaway replica must cost headers + maps, never a
	// second copy of the payload. 1/4 of the fork payload is ~6× the
	// measured green extra (6 MiB at this shape) while the pre-fix
	// per-block Hash marshal churn blew through it by 2× (69 MiB — the
	// born-RED run) — the defect is an order of magnitude, not a unit, so
	// GC/sampling jitter can't flake it.
	budget := forkPayload/4 + 16<<20
	if extra > budget {
		t.Fatalf("REPRODUCED: Reconcile of a %d MiB fork held %d MiB EXTRA live heap at peak (budget %d MiB) — a second resident copy of the fork payload on the cold-sync path (throwaway replica deep copy, or a held encode/decode buffer). The replica must share payload backing.",
			forkPayload>>20, extra>>20, budget>>20)
	}
	t.Logf("fork payload %d MiB (decode-resident %d MiB, %.2fx wire); Reconcile peak extra %d MiB = %.2fx payload (budget %d MiB)",
...
// block hash (a consensus catastrophe, class). Asserted over a block carrying
// every hashed field populated, including a multi-MB Answer.
func TestHashPooledBufferIdentity(t *testing.T) {
	prop := key(1)
