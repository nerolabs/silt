...
// verifying witnesses against those roots (the posture 2026-08-28). This file
// is the ADDITIVE validation MODE — a SEPARATE path a root-only client calls INSTEAD of
...
// cold-auditor recovery-boundary POLICY UNIT — isAmbiguousRecoveryBoundary and
// recoveryBoundaryDecision. The policy is an UNCONDITIONAL loud stall at an ambiguous boundary
// of, direction (a'): the box-local directive, the live-follower opt-in and the un-gated
// fall-through that used to sit between are all deleted.
//
...
// witnessable recompute that decides Accept/Reject. The", premise 1, and building it soundly
// is blocked on two verified obstructions (PACE doc, Option 3): (1) apply iterates WHOLE
// committed maps (the bondRegHeight TTL sweep, chain.go) that the BOUNDED read-set does
...
// statehash.go), not the typed data apply iterates, so the box cannot enumerate the
...
// consensus/published-claim surface. The box REFUSES to guess it: (*Box).Validate never
...
// - era4-witness-floor-box-readset-v5-AMENDED- (R2: the
...
// seam here honors it: absent a recompute, the box does not accept.
//
// CLOSURE GATE (NOT claimed closed here): the residual's closure is
// gated on the bonded/epochSet keystone probes 2026-08-30 item 3). This
...
const (
	...
	// could not reach a trustless accept/reject verdict — either the recovery directive
...
var (
	...
	// fall-through: the correct qualification set at that height depends on a non-committed
	// operator decision the box cannot witness, so the box declines to have a view of it
	// rather than take one from the proposer.
	//
	// The stall is TERMINAL, not per-block. The box needs a VERIFIED parent
	...
	// re-anchor contract at the foot of this file E2a.
	ErrRecoveryBoundaryStall = errors.New("chain: floor-box v5 height is an ambiguous liveness-recovery boundary (cold auditor: indeterminate-trustlessly, terminal until the operator re-anchors, will not trust the proposer)")
...
// recovery branch, which the floor box cannot witness (amended rule R2). It mirrors that
// branch's gate EXACTLY (chain.go): recovery fires iff cfg.LivenessRecoveryHeight is
...
//
// FOUR conditions, not five. `EpochBlocks != 0` used to sit between epochsEnabled and the modulo as
// a division-by-zero guard; epochsEnabled is `EpochBlocks > 0 && objective` (chain.go) and &&
// short-circuits, so it could never be reached with EpochBlocks == 0 and was dead. Measured:
...
// recoveryBoundaryDecision is the policy unit: whether the box may proceed to trustless
...
// research could preserve I1/I3/I4 in three sentences.
...
// THE RE-ANCHOR CONTRACT — its four clauses, which the box's operator (the S7 driver) is held
// to. It is the Ethereum weak-subjectivity checkpoint schema, which silt already ships as
// -ws-checkpoint HEIGHT:HASH (cmd/silt/daemon.go), so this is not a new trust class:
//
...
//
// The pin binds StateRoot only for a NON-PRUNED block: Hash short-circuits on a pruned block and
// returns a stored token bound to no struct field at all, StateRoot included. That is why
// (*Box).Validate's pruned refusal, and NewBox's refusal of a pruned parent, are part of the same
// decision and not a separate hardening.
//
...
// IndeterminateTrustlessly unconditionally — is DELETED (D0, on the simplicitydecision). It held
// no head, so it could not key its recovery posture on anything it owned; keeping it meant
// shipping two exported box entries with two different recovery semantics, one fixable and one
// not, on the last floor-box item before the freeze. It had zero non-test callers.
