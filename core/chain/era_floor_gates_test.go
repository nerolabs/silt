...
// THE ERA-FLOOR EVIDENCE REFUSAL. The gates for the rule that CheckEquivocation refuses
// evidence whose FORM is below the chain's committed era floor at the evidence's own height.
//
// (Layer 1, GATED: the four-site
// landing these RED-first probes). The face being closed is DRIVEN on main today by
// TestCrossChainHonestSignaturesAreNotEvidence's pre-era-4 arm: an honest validator
// that precommitted ONCE on network X is convicted on network X using a leg harvested from
// network Y, because the era-2 attestation form binds no chain id and era 2 is frozen
// forever.
...
// of the era map, which is the drift shape this whole arc is paying back.
...
// ---------------------------------------------------------------------------
// THE LIVE-ACCEPT / RELOAD SPLIT. The probe whose absence let the omission through.
// ---------------------------------------------------------------------------
//
...
func TestEraFloorRefusesOnBothTheLiveAcceptAndReloadPaths(t *testing.T) {
	...
	case live == nil && reload == nil:
		t.Fatalf("RED (the I5 violation, both paths): an era-4 block carrying a proof whose " +
	...
	case live == nil:
		t.Fatalf("RED (the SPLIT, and it is worse than the defect): the block is ACCEPTED live "+
	...
	case reload == nil:
		t.Fatalf("RED (the SPLIT, mirrored): the block is REFUSED live (%v) and ACCEPTED from "+
...
// ---------------------------------------------------------------------------
// THE TWO FLOOR DERIVATIONS AGREE. The drift shape, bounded.
// ---------------------------------------------------------------------------
//
...
func TestTheTwoEraFloorDerivationsAgree(t *testing.T) {
		...
		for _, h := range heights {
			...
			if got != want {
				t.Fatalf("FLOOR DRIFT (config route, H_era3=%d H_era4=%d) at height %d: the node says v%d "+
					...
					"disagreeing is the shape, and here it decides a SLASH: the node would accept a "+
...
// ---------------------------------------------------------------------------
// WHAT THE ERA FLOOR DELIBERATELY DOES NOT REFUSE, driven.
// ---------------------------------------------------------------------------
//
// The conjunct is SCOPED to heights where the chain requires the chain-bound era-4 form, because
// that is where the closure table says it closes anything. Below that boundary the required form
// is itself chain-blind, so refusing a leg buys no closure — while costing
// TestModelCheck_I5_CrossHeightPrunedExtension_Era1 (the era-1 exhaustive enumeration) and the
// T1 variant of, two artifacts this research does not price and a builder must not amend.
// TestModelCheck_I5_AccountableSafety_Exhaustive is NOT one of them: it enumerates era-2 blocks
// only (Version: BlockVersionRounds), so a v2 floor leaves it untouched.
//
// THE DEFERRAL IS DRIVEN, NOT ASSUMED. If a laterdecision removes the scope, this gate goes RED
// and names what moved — which is the point. It is the complement of: together they say
// exactly which heights the rule binds at.
func TestTheDeferredSubEra4SurfaceIsStillAdmissible(t *testing.T) {
	...
	if err := CheckEquivocation(&era1, ports.Hash{}, eraFloorOf(0)); err != nil {
		t.Fatalf("DEFERRED SURFACE MOVED: an era-1-form pair at a v%d floor is no longer evidence (%v). "+
			...
			"'s T1 variant. Neither is priced "+
			"by NETWORK-IDENTITY-BINDIN. Route the decision, "+
...
func TestAnAbsentEraFloorRefuses(t *testing.T) {
	...
	// And detection DOES select when a floor is supplied, so the line above is
	// about the floor.
