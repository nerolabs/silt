...
//
// This is the harness `` calls the "small-N exhaustive core": it drives the REAL Chain
// finality predicate (ValidateCommit under finalityQuorumActive) over an EXHAUSTIVE
// enumeration of adversarial signer coalitions and asserts the I1–I5 oracle (``) holds
// for EVERY schedule — the complement of `integration/consensus` (which checks that
// specific scenarios converge). Where the field discovered one region
// at a time, this enumerates the same perimeter on a laptop in milliseconds.
//
...
// the invariant all four defects violated at their core. The rest of the I1–I5 perimeter
// named in `` is now BUILT in sibling oracles (paths relative to the repo root):
...
// - I5 fork-choice determinism + honest-never-slashed (the catches) —
// core/chain/modelcheck_i5_accountable_test.go, core/chain/modelcheck_i5_launch_replay_test.go,
// core/node/modelcheck_node_loop_test.go (through the real gather);
...
//
// FAILING-FIRST (DoD #2): temporarily reverting requiredLaunchAnchors to the earlier
// rule (objective launch needs 1 anchor) makes TestModelCheck_I1_LaunchNoDisjointFinality
// go RED — "disjoint anchor coalitions [0] and [1] BOTH finalize" — the exact defect,
// caught on a laptop in milliseconds. At/after the fix (derived ⌊A/2⌋+1) it is GREEN. That
// controlled revert is the recorded proof it would have caught the defect without a field run.
...
// broken. This single property subsumes the launch face of.
func TestModelCheck_I1_LaunchNoDisjointFinality(t *testing.T) {
	for _, A := range []int{4, 5, 3} { // even (the ⌈A/2⌉ off-by-one trap) and odd
		c, ak, sk := launchObjective(t, A, 8, 0) // AnchorQuorum=0: the derived rule must carry it alone
		prev, _ := c.Head()
