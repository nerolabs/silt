...
// =============================================================================
//
// Pins the I5 replacement text, the closure-table rows, the fork-choice sentence and the
// pass conditions. These drift apart silently: I5's Statement once named a term (weight)
// that its own Assert forbids, and the term had been 0 on every real certificate.
//
// What is pinned (key sentences, not the whole block — the §3 text is the source; a
...
//	3. The cap line re-worded; the verbatim cap bullet UNCHANGED.
//	4. Governs cites `heavier` (height -> head-hash).
//	5. The two closure-table rows.
//	6. `weight → height → hash` appears nowhere outside the frozen-history set.
//	7. The objective fork-choice row equals §7 exactly.
//
// RED (I5 still says weight → height → hash; the ledger row still names the pre-T
// legacy-fixture witness). GREEN after the T commit's doc edits.
//
// SOURCE GATE: this test reads Markdown in the tree. It sees strings, not behaviour.
// RUNTIME GATE: TestCertificateVariantNeverRanksHeavier and TestHeavierReadsOnlyHeightAndHeadHash
// cover the property the canon text describes.

// o3tI5StatementSentences are sentences the Statement must carry.
var o3tI5StatementSentences = []string{
	...
	"**A weight term is deliberately absent**.",
...
var o3tI5AssertSentences = []string{
	...
	"**no attestation is verified in non-test `core/chain` outside `verifyAtt` and the era-1-gated `signedBlock`** (the verifier-inventory pin, with a teeth test)",
}

// o3tI5AccountableSafetyVerbatim is the accountable-safety half of the Assert, which the research
// says stays verbatim (§3.3: "from 'and **no honest schedule**…' to the end").
const o3tI5AccountableSafetyVerbatim = "**no honest schedule ever produces a slash** (the accountable-safety oracle — this is the direct catch for). **The accountable-safety oracle must vary the DECLARED height away from the SIGNED height, set `Pruned` to {unset, the real hash, another block's hash} on each side, and cover era 1 as well as era 2** (`TestModelCheck_I5_CrossHeightPrunedExtension_{Era1,Era2}`); the write-path gates drive the real `Append` (`core/chain/i5_evidence_recompute_test.go`, `core/node/i5_evidence_recompute_test.go`)."

// slashesCapLineVerbatim is the cap bullet, verbatim. Nothing here touches a byte of it.
const slashesCapLineVerbatim = "- ** (2026-09-03) — the cross-height `Pruned` slash forgery.** The accountability predicate **quantified over a fact (`Height`) outside the signed message**: `VerifyEquivocation` read the height from a struct field but the signed digest from `Hash`, which returns the accuser-supplied `Pruned` for the two evidence blocks. Two genuine signatures at two different heights, re-labelled with one height, convicted an honest validator through `Append` — in era 1 and era 2 — and a Byzantine *peer* sufficed, because an honest node queued the forgery on detection. The exhaustive I5 oracle never saw it: it fuzzed one height, never set `Pruned`, and was era-2-only. Fix: evidence hashes are always recomputed from the body and a pruned evidence block is refused (``)."

// forkChoiceRewordKey is the load-bearing clause of re-worded line.
const forkChoiceRewordKey = "went inert with the era-2 signature change (2026-08-16) and is retired"

// o3tClosureRows are the two closure-table rows.
var o3tClosureRows = []string{
	"|  uncovered-certificate transition | I5 (transition read a non-hash-covered field) + I4 (operation-liveness: intermittent stall) |",
	"| dead fork-choice weight (third site) | I5 (Statement named a term the Assert forbids; the term was inert on every real certificate) |",
}

// o3tLedgerRow47 is replacement row for:47.
const o3tLedgerRow47 = "| **Objective fork-choice converges** a partition onto one history: a sub-quorum minority cannot commit (intersecting quorum, I1), stalls, and catches up to the majority's head — selected by height → head-hash among descendants of the finalized head | `TestObjectiveConsensusCommitsOverTCP`, `TestObjectiveForkChoiceConvergesByCatchUp`, `TestModelCheck_NoReorgOfFinalizedLaunchBlock` |"

// o3tLedgerRow47Old is the row that must be GONE (it names a mechanism false since 2026-08-16
// and a legacy-config unit fixture as an "objective" witness).
...
// RETIRED. A helper here pinned the I5 statement text against a document that no
// longer exists, so the gate has no
// subject left to read. It is retired here rather than repointed, because no surviving
// file carries the text.
//
// WHAT IS LOST, stated rather than dropped silently: the I5 wording is no longer pinned
// anywhere, so a future restatement of fork-choice cannot be checked against the research. WHAT
// SURVIVES: the RUNTIME gates named in this file's header —
// TestCertificateVariantNeverRanksHeavier, TestHeavierReadsOnlyHeightAndHeadHash,
// TestVerifierInventoryPin and TestModelCheck_I5_CrossHeightPrunedExtension_{Era1,Era2} —
// which assert the BEHAVIOUR the retired text described. The sentence constants above are
// kept as the record of what the claim said, and the banned-literal walks below still refuse
...
//
// WHY THE WHOLE TREE, not docs/. The earlier walk covered `root/docs` only. The M0
...
//	generated pages). A hit in one of these IS a duplicate
...
//	 a dated record of what the claim USED to say, and it is 9k lines of it.
...
func o3tShippedText(t *testing.T, root string) (lines []o3tLine, flat []o3tFlatFile, files int) {
	...
	err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		name := d.Name()
		if name == "canon_text_test.go" || strings.HasPrefix(name, "report-") {
			return nil
		}
		...
		//
		// `.yml`/`.yaml` was added after measuring the
	...
	//
	// LEG 1, THE COUNT, IS A TOTAL-COLLAPSE TRIPWIRE AND NOTHING FINER. A floor that
	// cannot fire is the defect this whole audit kept finding, so its reach is stated
	// rather than implied. Re-driven over this exact filter: 173 files, composed
	// integration/ 101, docs/ 43, repo root 6, cmd/ 6, examples/ 6,.github/ 5, website/ 3,
	// deploy/ 2, scripts/ 1. At 100 it fires only if ~42 % of the walk vanishes. Drop
	// docs/ ENTIRELY and the count is 130 — GREEN, with every gated site
	// outside the gated sites gone from the walk. RAISING the floor does not repair that:
	// any value tight enough to notice one directory goes false-RED on ordinary file
	// churn, and a lint that cries wolf gets disabled — this gate's own admission bar. So
	// the count is kept only for what it genuinely catches (a root that does not resolve,
	// a WalkDir that returns early, an extension filter that admits nothing) and the
	// binding work moves to LEG 2.
	//
	// LEG 2, THE ANCHORS, binds, and it carries no number at all: a named file either survives
	// the walk or it does not, so there is no margin to drift and nothing to re-drive. One
	// anchor per walked AREA that holds the population this gate polices, and one per admitted
	// EXTENSION — the extension filter is exactly what was measured broken
	// (integration/consensus/ docker-compose.yml carried a banned literal that no.md/.sh walk
	// could ever reach), so it gets a witness instead of a comment.
	if files < 100 {
...
//	drop docs/ from the skip switch 130 GREEN RED
...
// the exact 2026-09-12 regression that started this work, a.yml file carrying a banned literal the
// walk could not reach. The last row is honest about its own limit: losing all of integration/ is
// a collapse big enough that the tripwire fires first and returns before the anchor loop runs, so
// that run does not demonstrate its anchor; row 6 does, in isolation.
...
//
// WHY: the per-line scan is defeated by a SOFT LINE WRAP, and not hypothetically. A review
// executed: the retired claim restored verbatim with a newline between `heavier-standing` and
// `chain` renders identically in Markdown and left every gate in this file GREEN. README and the
// harness docs are soft-wrapped prose throughout, and the change that retired this vocabulary re-wrapped
...
// the flattened scan finds the same 4 and 8 hits as the per-line scan and ZERO extra, so the closure
...
// by TestHeavierReadsOnlyHeightAndHeadHash); with the finality gate on, Reconcile admits only
// forks containing the committed head, so a sub-quorum partition commits nothing and CATCHES UP
// rather than reorging. Every phrase below asserts the opposite.
//
// EACH ENTRY IS MEASURED over the tracked tree before it is admitted, and that is the bar: a lint
// that cries wolf gets disabled, so the false-positive rate is a correctness property of this gate.
// A census is a CLAIM — re-drive it against the tree you are standing in, never carry one forward.
// A near-miss phrase stays OUT when its hits are mostly true statements about a genuinely taller
// chain; "heavier chain" and "heavier fork" are both excluded for that reason.
//
// WHAT THIS GATE CANNOT DO.
//
//	SOFT LINE WRAP is closed. The retired claim restored verbatim with a newline inside it renders
//	identically in Markdown and defeats a per-line match. The flattened pass (o3tFlatten) collapses
//	every whitespace run, newlines included, to one space before matching, so no wrap depth evades
//	it. Driven RED in TestSoftWrapDoesNotDisarmTheCanonTextGates.
//
//	PARAPHRASE is not closed and is not closable by a lexical rule. "the chain carrying the most
//	bonded standing" states the retired claim and this gate is silent, because the gate matches
//	LITERALS. Widening the literal set is what the near-miss measurements above reject. So this gate
//	stops a REGRESSION to the exact retired wording and a re-wrap of it. It does NOT stop a rewrite
//	in new words. The cover for that is review plus the runtime gates named on each test. Describe
//	this gate as covering a vocabulary, never as covering "the claim".
//
//	FILES OUTSIDE THE EXTENSION FILTER are not walked at all: .go, .py, .tf, .json, Makefile,
//	Dockerfile, extensionless files, and every directory in the skip list above.
//
//	THE .go POPULATION IS NOT GATED. Two live sites remain by design. cmd/silt/daemon.go's operator
//	narration "chain: reorged onto a heavier fork" is grepped for by integration/consensus/run.sh,
//	and it is still REACHABLE: the finality gate makes dropped > 0 impossible only while
//	finalityQuorumActive, and a legacy-posture chain has no gate. The -equivocate flag help scopes
//	itself to "LEGACY mode" and is accurate as written. A .go regression of this vocabulary goes
//	unnoticed.
//
// ============================================================================================
...
// TestNoRetiredForkChoiceClaimInShippedText bans the retired bond/weight fork-choice vocabulary
...
// ONE file and the banned string is a full table row — it could never match README.md's prose form
// of the same claim. The claim therefore sat in the repo's front door for months with a gate in the
// tree that looked like it covered it. This is that gate.
//
// The replacement wording is a PUBLISHED CLAIM. Do not reword it to make this test pass.
//
// SOURCE GATE: this reads text. RUNTIME GATE: TestHeavierReadsOnlyHeightAndHeadHash pins that
...
func TestNoRetiredForkChoiceClaimInShippedText(t *testing.T) {
	...
	if len(hits) > 0 {
		t.Fatalf("SOURCE GATE: %d line(s) across %d shipped file(s) assert a bond or weight term in fork choice, want 0:\n  %s\n\n"+
			...
			"  REMEDY: use the replacement sentence (o3tCertifiedForkChoiceSentence) VERBATIM where the site\n"+
			"  states the composition claim; where the site states a narrower property, make that property\n"+
			"  true in its own terms. It is a PUBLISHED CLAIM: do not reword it.",
			len(hits), files, strings.Join(hits, "\n  "))
	}
}

// o3tBannedHits reports every banned literal in the FLATTENED text of each file, as `path:line
// (literal)` where line is the source line the match STARTS on. Flattened, so a soft line wrap
// cannot hide a phrase — see o3tFlatFile.
...
// TestNoWeightHeightHashOrderInDocs is gate (d) part 6: the retired ranking order
...
func TestNoWeightHeightHashOrderInDocs(t *testing.T) {
	_, flat, files := o3tShippedText(t, o3tRepoRoot(t))
...
// TestSoftWrapDoesNotDisarmTheCanonTextGates drives the CLOSED evasion, because a coverage
...
// TestNoWeightHeightHashOrderInDocs — is one, and so is `heavier-standing chain`, the
// published-claim literal. `heavier-bonded` is a single token and cannot be
...
func TestSoftWrapDoesNotDisarmTheCanonTextGates(t *testing.T) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got != tc.want {
				t.Fatalf("SOURCE GATE: banned literal %q found=%v, want %v, in:\n%q\nflattened to:\n%q\n\n"+
					"  A per-line match is disarmed by a soft wrap and a review EXECUTED that evasion "+
...
// o3tCertifiedForkChoiceSentence is the M0 composition claim's fork-choice clause, VERBATIM.
//
// DO NOT RE-WORD IT to make a test pass, to fix a typo, or to fit a line. It is a PUBLISHED CLAIM;
// changing the words changes a published claim, and this constant exists precisely so that a
...
// TestForkChoiceSentenceIsPresent is the half the ban set does not have.
//
...
func TestForkChoiceSentenceIsPresent(t *testing.T) {
	root := o3tRepoRoot(t)
	...
	if !strings.Contains(f.Text, want) {
		t.Fatalf("SOURCE GATE: README.md does not carry the fork-choice sentence verbatim:\n\n  %s\n\n"+
			"  It is a PUBLISHED CLAIM. Restore it word for word. Do NOT edit\n"+
			"  o3tCertifiedForkChoiceSentence to match the README — that inverts the gate and changes the\n"+
			"  published claim silently, which is the exact failure this test exists to make impossible.\n"+
			"  Line breaks are free: the match runs over the whitespace-flattened file.", want)
	}
}

// TestTheTextGatesActuallyUseTheFlattenedPass closes the gap the arm above cannot.
//
// WHY BOTH ARMS EXIST. TestSoftWrapDoesNotDisarmTheCanonTextGates drives o3tFlatten and
...
// CALLING the helper. Measured: reverting TestNoWeightHeightHashOrderInDocs to the per-line
...
func TestTheTextGatesActuallyUseTheFlattenedPass(t *testing.T) {
	root := o3tRepoRoot(t)
	raw, err := os.ReadFile(filepath.Join(root, "core", "chain", "canon_text_test.go"))
	if err != nil {
		t.Fatal(err)
	}
	src := string(raw)
	for _, fn := range []string{
		"TestNoRetiredForkChoiceClaimInShippedText",
		"TestNoWeightHeightHashOrderInDocs",
	} {
		...
		if !strings.Contains(body, "o3tBannedHits(") {
			t.Errorf("SOURCE GATE: %s does not obtain its hits through o3tBannedHits — the FLATTENED "+
				"pass. A per-line match is disarmed by an ordinary soft line wrap (a reviewer "+
...
// TestForkChoiceLedgerWitnessesExist is what survives of gate (d) part 7.
//
...
func TestForkChoiceLedgerWitnessesExist(t *testing.T) {
	root := o3tRepoRoot(t)
	for _, name := range []string{"TestObjectiveConsensusCommitsOverTCP", "TestObjectiveForkChoiceConvergesByCatchUp", "TestModelCheck_NoReorgOfFinalizedLaunchBlock"} {
