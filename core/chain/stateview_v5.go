...
//
// Certified: build-plan (the read set, classified) and the P-table research (Rep HeadRef Budget + the two-conjunct substituted step):
//
//
// THE CLASSES (build-plan). Every state read reachable from ValidateCommit lands in one:
//
...
//	LEGACY rep(id) — not a committed leaf.: the composition reads it through Rep; the node's
...
// driver. Certified shape: P-table.
//
...
type HeadRef struct {
	...
	Hash ports.Hash
	// ChainID is the view's own NETWORK IDENTITY: the height-0 block's Hash
	// (Chain.ChainID). It is what the era-4 consensus-signature preimage binds
	// (consensusSigBytesV5), so that a signature made on one silt network can never be read
	// as a vote on another.
	//
	// CLASS 3 AND STRICTLY SO. A DRIVER-SUPPLIED chain id lets the caller choose which
	...
	// second return. The +1 is derived in ONE place so the two views cannot differ by one,
	// and the name carries the semantics so P1 is never written two ways.
	NextHeight uint64
	// ProposerID is the parent's ProposerID (the carrier fold's one excluded id is
	// box-owned, never witnessed).
	...
	// LogRoot is the parent's committed LogRoot; nil = the parent committed none. It is what
	// the P13b k = 0 leg compares against (closed for k = 0 with zero format change). Inside
	// the signed preimage (bodyHash), so a box holding a validated parent block derives it
	// exactly as it derives StateRoot.
	LogRoot *ports.Hash
	// Empty is true iff the chain holds no block — Chain.Head returns (zero, 0). liveView
	// reproduces that; a box refuses it at its entry (a box with no parent cannot bind
	// one).
	Empty bool
}

// Budget is the view's OWN witness/frame BYTE budget: ONE ceiling over the block's
...
//
// THE ZERO VALUE STALLS. Mirrors NoWitness being the zero of Availability: a forgotten
...
//
// WHY BYTES AND NOT A COUNT defect 1. validateCarrier applies no qualification screen, so distinct
// ids are free (carrier.go: "distinctness does NOT bound the carrier … there is no size rule here");
// a count of payload slices never reads the quantity the gate bounds. Measured exposure: 2.67 GiB of
// AttScreens / ~1.79 GiB of readSetAtts for ONE no-op block at registry N = 1,048,576, each over a 2
// GB pony (build-immutable #8). The value rule is a v5 VALIDITY change — research-gated, NOT this
// round. Until it ships the box STALLS above its own budget.
//
...
// Three properties make the budget sound without its own research:
...
// 3. It restores O(1) time-to-stall for an out-of-scope block.
//
// The number a box should use is NOT fixed here — the pony measurement is owed (build-plan) and
// this round does not invent one.
type Budget struct {
	maxBytes  int
	unlimited bool
}
...
// not "unlimited" and not "nothing" — it is the zero Budget, which stalls.
...
// node-side validity rule. It is the ONLY unlimited Budget, and only liveView returns it.
...
// Check is THE ONE budget comparison: the zero Budget STALLS by name, the unlimited budget
...
// Params is the view's OWN consensus configuration (class 3). It is NEVER witnessed and
...
// MinProposerRep / MinAttesterRep for the legacy leg.
//
...
//
// Two rules are deliberately NOT methods, because two implementations of either is the trap:
...
// of the substitution rule — the exact seam the gate was filed on.
...
type StateView interface {
	...
	// PrunedTolerated answers the Q2 pruned-tolerance QUESTION for height h — "may a
	// payload-pruned block at h be trusted without re-verifying its space-time proofs?" — and
	// never the floor it is derived from. That is deliberate, and it is D0's fourth
	// deliverable; the H-4, which REFUTED its own first draft to get here.
	//
	...
	//
	// The node's liveView answers (h < c.trustFloor, Present) — its own rule, unchanged. The
	// box's provenView answers (false, NoWitness) and the composition STALLS: a box cannot audit
	// history below any node's prune floor, which is a held liveness residual, not a defect.
	...
	// the bounded header chain recentBondRegNonces walks (K = BondRegHeadWindow). Each hash
	// is authenticated by the Prev linkage of the one before it, so this is
	// self-authenticating and NOT a class-4 fact. NoWitness ⇒ the composition stalls on any
	// block carrying BondRegs.
	Ancestors(k int) ([]ports.Hash, Availability)

	// ---- LEGACY: the local reputation view. Not a committed leaf. ----
	// The node's liveView answers (c.rep(id), Present); the box's provenView answers (0,
	// NoWitness) and the composition stalls — which is what the box-entry S2 fence achieves,
	// belt and braces. Read ONLY on the !Objective branch of P4 and P9 (the node's own
	// branch).
	...
	//
	// The node substitutes validateEra3Roots → postApplyRoots → cloneForDryRun (a real apply on
	// a deep clone, BOTH roots); the box substitutes the witness recompute for P13a and, for
	// P13b, the k = 0 LogRoot equality plus — since tagRevLogSize landed freeze-manifest item
	// 1) — the k ≥ 1 RFC-6962 extension check against the parent log size that leaf pins. This
	// seam stays an EQUIVALENCE WITH GATES, never a shared function — it is the half that has
	// held under four blind rounds, and collapsing it would be a new soundness claim, not a
	// refactor. It is held in tension over TWO equalities. THIS ROUND IS NOT "one implementation everywhere"
	// and must never be published as such.
	CommittedRoots(b *Block) (FloorBoxOutcome, error)

	// sealedStateView SEALS the interface: an unexported method no type outside this package
	// can declare, so the only views that can drive the exported composition are liveView (the
	// node) and provenView (the box). Without it, an out-of-package caller could implement
	// StateView, answer Accept from CommittedRoots, and take Accept out of ValidateCommitV5
	// with no (*Box).Validate downgrade in front of it — a second door opened by the same
	// round that closed nine. The inventory gate (TestExportedPackageSurfaceInventory) lists
	// every exported entry of the box files with the reason each is permitted.
	sealedStateView()
}
