...
//
// WHY THIS CLASS EXISTS (the write-obligation ledger gap). apply latches everMature false→true
// at the TOP of EVERY block where !everMature && Mature (chain.go-3305), BEFORE the
// boundary gate (chain.go). The write can land on ANY height. reproduced it ONLY inside
// class P's rotateOps (boundary-gated), so the GENERIC off-boundary maturity crossing (h %
// EpochBlocks != 0) had no reproducer → the recompute folded no tagEverMature op → the
// recomputed post-root committed everMature=false while the committed root committed
// everMature=true → mismatch → STALL. Safe (never wrong-accept — the box still never Accepts)
// but a liveness gap fatal to the accept-flip (every objective chain crosses maturity exactly
// once, off-boundary is the generic case).
//
// THE SINGLE OWNER. Class M is the ONLY reproducer of the tagEverMature leaf write. Class P's
// emission was REMOVED; P now only READS the post-latch everMature (threaded
...
// NOT prevStateRoot. The box reuses recomputeMatureNow
...
// since, the single source of the handoff pre-state the class-A screen branches on. The entry
// REQUIRES it on every block, so both readers are served boundary-independently.
type StateRootMaturityWitness struct {
	...
	// (the value apply's attestation loop reads, chain.go, which runs BEFORE
	// rotateEpoch flips it — rotate-LAST); Proof is its inclusion proof against
	// prevStateRoot.
	//
	// WHY IT IS HOMED HERE and not on the class-P rotate witness. The class-A attestation screen
	// needs this value on EVERY block to pick its qualification branch, and the class-P
	// StateRootRotateWitness is nil off-boundary (floorbox_recompute_stateroot_v5.go, isBoundary).
	// The class-M carrier is already REQUIRED on every block (so the latch is never silently
	// skipped) and already carries the sibling EverMature scalar, so the handoff pair travels
	// together. Class P keeps its OWN anchor + emit of the tagMatureEpoch WRITE at a boundary;
	// both Resolve the same leaf against the same prevStateRoot, so they cannot verify to
	// different values and there is no double-emit.
	//
	// The box NEVER reads c.matureEpoch for this. A box that replays no apply never has it set, so
	// the live read screened every mature-epoch block under the PRE-maturity rule — reachable as
	// both a wrong-accept (a mid-epoch joiner folded in) and a false stall, with every witness
	// proof passing.
...
func (c *Chain) maturityLatchOps(
	...
	// The class-M witness and BOTH handoff scalars were already anchored against prevStateRoot
	// by handoffPreState, which the entry runs UNCONDITIONALLY before any class dispatches.
	// One Resolve per scalar per block, not two: the pre-values arrive here already proven
	// present.
	mw := w.Maturity
	...
	// The crossing: everMature false→true. Fold the committed scalar (monotonic one-way latch).
...
type stateRootHandoffPre struct {
	...
	// loop reads (chain.go, BEFORE rotateEpoch:3306, rotate-LAST). It is the class-A
	// screen's branch selector.
	matureEpoch bool
	// handedOff is handedOff (chain.go) evaluated over the anchored pre-values:
	// matureEpoch with epochs enabled, everMature without (covered rather than argued
	// unreachable). It is the launch-anchor eligibility input.
	handedOff bool
}
...
//
// WHY. The class-A screen picked its qualification branch from c.matureEpoch and its anchor
// eligibility from c.launchAnchor → c.handedOff. Those fields are written ONLY by apply→rotateEpoch
// (chain.go) and adopt (chain.go). The deployment target "holds NO registry and replays NO
// apply", so on a COLD box they are never set and the mature-epoch branch is UNREACHABLE: every
// mature-epoch block is screened under the pre-maturity rule. That diverges from what a full node
// accepts in BOTH directions, with every witness proof passing — a mid-epoch joiner (bonded ≥
// MinBond, ∉ the frozen epochSet) is folded in against an attacker's root (wrong-accept, and the
// class-M poisoning entry), and the same attestation inside an HONEST block stalls the box (false
// stall). The verdict must be a function of (prevStateRoot, committedStateRoot, b, w, own-cfg) ONLY.
//
...
func (c *Chain) handoffPreState(prevStateRoot ports.Hash, w StateRootWitness) (stateRootHandoffPre, error) {
	...
	// PRE-STATE ANCHOR. preEverMature is read as a BRANCH predicate (a forged OldValue=true returns
	...
	if err := anchorRotateScalar(prevStateRoot, tagEverMature, mw.EverMature); err != nil {
		return stateRootHandoffPre{}, fmt.Errorf("%w: everMature.OldValue not proven present against prevStateRoot (pre-state anchor): %v",
			ErrRecomputeStateRootMaturity, err)
	}
	if err := anchorRotateScalar(prevStateRoot, tagMatureEpoch, mw.MatureEpoch); err != nil {
		return stateRootHandoffPre{}, fmt.Errorf("%w: matureEpoch.OldValue not proven present against prevStateRoot (pre-state anchor): %v",
			ErrRecomputeStateRootMaturity, err)
	}
	pre := stateRootHandoffPre{
	...
	// Defensive cross-check. matureEpoch ⇒ everMature is emergent from rotateEpoch
	// (chain.go early-returns unless everMature) and pinned by
	// TestMatureEpochImpliesEverMature_InvariantPin. A pre-state that violates it is not one any
	// honest apply can commit, so the box refuses to screen against it. Stall-adding only.
	...
	// handedOff (chain.go) over the ANCHORED pre-values. epochsEnabled is own-cfg +
	// the injected verifier (asserted wired at the entry), never live state.
	if c.epochsEnabled() {
