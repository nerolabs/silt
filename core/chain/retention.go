...
// Rolling retention horizon (the read-only half of the H2 payload-selective-prune OOM fix). A
// validator's chain grows O(all history) in the ~1.5 MB space-time bond proof (BondReg.Answer)
// carried by every registration block, which OOMs a small box (the flixz-corroborated MATURING
// crash). The fix bounds the RESIDENT heavy payload to a recent finalized window: blocks
// strictly below the retention horizon are prune-eligible — their heavy Answer may be dropped
// while the header + consensus signatures (the light chain, ~KB/block) are kept to genesis.
// This file computes the horizon; the prune/serve/Reconcile-tolerance land in the
// consensus-adjacent slice behind the Q2 gate (a pruned block is trusted ONLY strictly below
// the node's OWN finalized anchor).
//
// safetyDepth = 2·BondTTLBlocks, research-outcome/safetyDepth-retention-horizon-: one
// BondTTL for the bond-standing lifecycle (already ~4× the true ~8-head
...
// headers+sigs, never the heavy Answer"): under (F2-EVIDENCE-RECOMPUTE) a pruned block
// is never equivocation evidence, so slashability is BOUNDED BY THE PRUNE FLOOR. The
// derivation survives because honest cross-fork detection only pairs heights at or
...
// where ErrPreFinalityReorg already forbids adoption (held in tension —).

// RetentionHorizon reports the lowest height whose heavy BondReg.Answer this node must
// still RETAIN in full: blocks with height < RetentionHorizon are prune-eligible. It
// is anchored at the node's own finalized head, so it only advances as finality
...
// stranding the re-verify window.
...
// epoch-aligns (validator-set snapshot, Cond A). Returns 0 when bondTTL is degenerate (0 ⇒
// retention-decay disabled, safetyDepth would be 0 — never prune) or there isn't yet a full
// retain window of history. The retain window is >= 2·bondTTL, so the prune floor is always
// <= RetentionHorizon <= trustFloor — a pruned block is therefore always below the Q2
// gate's trust floor (we prune more conservatively than we trust). Err toward retaining:
// the floor only ever LOWERS on epoch-align.
...
// and epoch. Distinct from (and always <=) RetentionHorizon/trustFloor: the prune is the
// more conservative of the two so it never sheds a block the Q2 gate would then reject.
...
// the header + consensus sigs (Block.Prune). Because the durable store and the
...
// already-pruned and entry-only blocks). ENABLED: the node calls this after each
...
// a validator-set snapshot and retains AT LEAST safetyDepth (the
// floor only ever LOWERS the horizon → retains more; err long, per the research). Returns
