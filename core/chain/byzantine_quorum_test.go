...
// Consensus safety must come from quorum ARITHMETIC at the
// Byzantine threshold, not from a fixed quorum or a head-count of validators.
...
func TestMaturityNakamotoResistsOneOperator(t *testing.T) {
	...
	// measures its floor λ_H by
	// -2026-08-27, §2.1). If the two definitions drifted, the λ_H floor would parameterize
	// A different quantity than the theorem binds (T_mature ≤ M_req/λ_H). Guard
	// the tie: the coefficient equals min(NakamotoOperators, NakamotoDomains), and
	// Mature is exactly coefficient ≥ MatureValidators.
