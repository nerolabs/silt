...
func newCarrierWorld(t *testing.T, mature bool) *carrierWorld {
	...
	signCert(h1, w.cert, 0, w.c.ChainID())
...
func signCert(b *Block, keys []ed25519.PrivateKey, proposer int, chainID ports.Hash) {
...
func mintSubject(t *testing.T, w *carrierWorld, carrier []Attestation, proposer int, mut func(*Block)) *Block {
	...
	signCert(b, w.cert, proposer, w.c.ChainID())
	return b
}
...
func newCarrierWorldTTL(t *testing.T, ttl uint64) *carrierWorld {
	...
	signCert(h1, w2.cert, 0, w2.c.ChainID())
...
func TestModelCheck_CarrierSeating_EraSeamV4ParentV5Child(t *testing.T) {
	...
	for _, k := range cert {
		h1.PrepareQC = append(h1.PrepareQC, AttestAt(h1, k, 0, PhasePrepare, c.ChainID()))
	}
	for _, k := range seaters {
		h1.Atts = append(h1.Atts, AttestAt(h1, k, 0, PhasePrecommit, c.ChainID()))
	}
	...
	h2.Atts = append(h2.Atts, AttestAt(h2, viaChildAtts, 0, PhasePrecommit, c.ChainID()))
