...
// =============================================================================
// THE RETIRED-WEIGHT TWIN GATE
// =============================================================================
//
// The `Weight > 0` assertion at forkchoice_ramp_test.go was deleted — but its TWIN at
// modelcheck_i5_launch_replay_test.go (`if c.Weight <= 0`) is still live, in the model-check tier
// that gates every graded field run. The pass condition: with Weight deleted, NO file in the
// tree references the retired term, or the deletion leaves a dangling site (a compile break in test
// code that may not run, or a call kept alive by a fixture nobody reads).
//
...
//
// Comments and string literals are NOT failures (forkchoice_ramp_test.go's deletion comment
// legitimately names the deleted assertion); they are LOGGED as an inventory so a later change can
// true them up. RED (chain.go definitions + 9 test call sites); GREEN after T.
//
// SOURCE GATE: this test parses Go source under the repo's core/, cmd/, sim/, e2e/ roots. It sees
// identifiers, not behaviour.
// RUNTIME GATE: TestCertificateVariantNeverRanksHeavier (fork-choice ranks by height/hash only).
...
// TestNoWeightTermReferenceSurvives is gate (c).
func TestNoWeightTermReferenceSurvives(t *testing.T) {
	...
	self, _ := filepath.Abs("ramp_guard_test.go")
	...
	if len(code) > 0 {
		t.Fatalf("SOURCE GATE: — %d CODE reference(s) to the retired fork-choice weight surface "+
			"(Weight() / blockWeight / anchorWeight / Config.AnchorWeight) across %d files:\n%s\n\n"+
			"  The term is deleted. Every reference goes with it:\n"+
...
// TestRampGuardHasTeeth proves the walk bites on each of the four shapes and does NOT bite on
// the statehash member field `Weight` (a field, not the chain method).
func TestRampGuardHasTeeth(t *testing.T) {
