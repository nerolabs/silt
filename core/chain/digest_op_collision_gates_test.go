...
// Two classes emit a fold op for the SAME digest key, and the last one wins.
//
// DERIVED FROM SOURCE, then reproduced. The probes are preserved verbatim (sha256
// 68acce8d47deb80ecd525691c112154a4eee34a0b01b30534e38ce456f37191c).
//
// ────────────────────────────────────────────────────────────────────────────────────────
// THE BREAK
// ────────────────────────────────────────────────────────────────────────────────────────
...
//
// All three land at the identical key statehash.Key(tag, nil), and all three read their OldValue from
// the SAME digest witness (digestFoldOp does `w:= byTag[tag]; preDigest:= nodeSetMTH(w.PreIDs)`), so
// every duplicate op carries a byte-identical OldValue and a byte-identical Proof. Step (1) of
// statehash.FoldChangedPaths therefore verifies ALL of them against prevStateRoot — none is rejected.
// Step (5) then replays them with trie.Update in slice order, with no dedup and no duplicate-key guard
// anywhere in the function. THE LAST OP ON A KEY WINS.
//
...
//
// ONE LOOSE READING, NAMED SO IT IS NOT INHERITED. "The block folds bondedRoot to B's value,
// silently dropping the slash's eviction" is exact about the winner. What it invites is the
// reading that B's value IS the honest value minus the eviction, and
...
//
// THE DIRECTION IS UNCHANGED, and the measurement below agrees with it. The honest post-set is
// (pre ∪ regs) \ culprits, and the fix is a single cross-class computation — one postBonded
// threaded through S, B and T in apply order — not a dedup. reconstructPostQualifiedWithWrites
// is the algorithm the tree already has. This file sharpens the wording; the remedy is not
// encoded here.
//
// ────────────────────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────────────────────
//
// Measured against the HONEST committed root the box MISMATCHES and stalls — safe, and that stall is what every
// shipped compound-adjacent ablation would observe (D5 below pins that stall, and pins the error CLASS it must
// arrive in). But a real proposer does not commit the honest root. It commits whatever root ITS OWN FOLD
// produces from its own block, and the box's terminal equality (`if postRoot != committedStateRoot`) then
// passes by construction. MEASURED: recomputeViaHead returns nil against the box's own fold root, on both an
// S+B block (D1) and an S+T block (D2). That is box.Accept ∧ node.Reject — the same soundness failure as, by a
// different lever, and the anchoring remedy does not close it: every pre-set here IS anchored and every
// duplicate op DOES verify. The defect is downstream of anchoring entirely.
//
// The ablation oracle is the honest artifact. An
...
// whole-set digest scalar is overwritten. MEASURED: the box accepts a post-state whose
...
// stalled here." Nothing forbids a compound block and apply handles all of them — the controls below
// confirm the single-class blocks agree, so the fixtures are not degenerate.
//
// ────────────────────────────────────────────────────────────────────────────────────────
...
// surface). This file encodes a confirmed break; it does not fix one. So these land as
// PINNED_DEFECT under: each asserts CURRENT BROKEN BEHAVIOUR, passes today, and REDDENS the moment
// the behaviour moves, which forces the record to be updated rather than letting the defect close
// silently. t.Skip is refused by that same decision.
//
// Each pin carries PIN + MECHANISM + TEETH, the idiom documented at the head of
// core/pipeline/subframe_manifest_oracle_test.go:
...
// 3. TEETH — TestPinsFireOnTheirRemediations, which feeds every predicate its
// POST-FIX input and asserts it speaks up.
//
// CONTAINMENT, so nobody reads this as "main is exploitable today": (*Box).Validate still applies the
// downgrade — Accept ⇒ IndeterminateTrustlessly / ErrRecomputeGated. The box never accepts on this
// path yet. That downgrade is the only barrier between this finding and damage, and it is
// deliberately not touched here. The finding is what happens on the day the downgrade flips.
...
// defect is still live (the box the proposer's root). Non-empty ⇒ the behaviour moved.
func rtDigestCollisionPin(defect string, stall error, boxRoot, honest ports.Hash, consequence string) string {
	...
	return fmt.Sprintf(
		"%s PIN IS RED — the box now REJECTS the compound block measured against ITS OWN fold root: %v\n"+
			...
			"  ). A RED here is the EXPECTED outcome of the\n"+
...
// folds and accepts, the per-member leaf bonded||culprit is a DELETE while the bondedRoot digest
...
// ══════════════════════════════════════════════════════════════════════════════
// The emission census, at the unit level
// ══════════════════════════════════════════════════════════════════════════════

// TestTwoClassesEmitTheSameDigestKey_PINNED_DEFECT pins the CAUSE on its own:
...
func TestTwoClassesEmitTheSameDigestKey_PINNED_DEFECT(t *testing.T) {
	...
	for _, tag := range []string{tagBondedRoot, tagQualifiedRoot} {
		if msg := rtDigestCollisionDuplicateOpPin("", tag, ops); msg != "" {
			t.Fatalf("%s", msg)
		}
		// The duplicates must share OldValue AND the pre-state witness, else they would not
		// BOTH verify and the break would be a stall instead of a wrong-accept. digestFoldOp
		// reads both off one witness (w:= byTag[tag]), so the duplicates carry the SAME
		// statehash.Witness value — one wrapped proof, which is byte-identity a fortiori.
		want := statehash.Key(tag, nil)
		...
		for i := range ops {
			...
			if !bytes.Equal(first.OldValue, ops[i].OldValue) {
				t.Fatalf("PIN IS RED — the duplicate ops for %q no longer share an OldValue\n"+
			...
			if first.Proof != ops[i].Proof {
				t.Fatalf("PIN IS RED — the duplicate ops for %q no longer carry the SAME\n"+
...
// ══════════════════════════════════════════════════════════════════════════════
// S+B: the box accepts its own fold root and the slash eviction is dropped
// ══════════════════════════════════════════════════════════════════════════════

func TestCompoundSlashPlusBondRegDropsTheEviction_PINNED_DEFECT(t *testing.T) {
	f := buildSlashFixture(t)
	b, w := rtDigestSBBlock(t, f)
	...
	if boxRoot == honest {
		t.Fatalf("IS VACUOUS — the box's own fold agrees with real apply (%x), so there is\n"+
	...
	if msg := rtDigestCollisionDuplicateOpPin("", tagBondedRoot, ops); msg != "" {
		t.Fatalf("%s", msg)
	}

	// PIN: the box accepts the proposer's root.
	stall := recomputeViaHead(f.c, f.prevRoot, boxRoot, b, w)
	if msg := rtDigestCollisionPin("", stall, boxRoot, honest,
...
// ══════════════════════════════════════════════════════════════════════════════
// S+T: the SAME defect with a different winner, so it is an ORDERING defect
// ══════════════════════════════════════════════════════════════════════════════
...
func TestCompoundSlashPlusTTLSweepDropsTheEviction_PINNED_DEFECT(t *testing.T) {
	f := buildTTLFixture(t)
	b := f.sweepBlock()
	prev, _ := f.c.Head()
	...
	boxRoot := rtDigestFoldOrStall(t, f.prevRoot, ops)
	if boxRoot == honest {
		t.Fatalf("IS VACUOUS — the box's own fold agrees with real apply (%x)", boxRoot[:])
	}
	if msg := rtDigestCollisionDuplicateOpPin("", tagBondedRoot, ops); msg != "" {
		t.Fatalf("%s", msg)
	}
...
// ══════════════════════════════════════════════════════════════════════════════
// The accepted state contradicts itself
// ══════════════════════════════════════════════════════════════════════════════

func TestFoldedDigestContradictsItsOwnPerMemberLeaf_PINNED_DEFECT(t *testing.T) {
	f := buildSlashFixture(t)
	b, w := rtDigestSBBlock(t, f)
	honest := f.applyAndCommittedRoot(t, b)
	ops, err := assembleOpsViaHead(f.c, f.prevRoot, honest, b, w)
	...
	if !found {
		t.Fatalf("PIN IS RED — no folded op for the per-member leaf bonded||culprit at all.\n" +
	...
	if msg := rtDigestCollisionInconsistentStatePin("", perMemberIsDelete, digestCountsCulprit); msg != "" {
		t.Fatalf("%s", msg)
	}
}
...
// ══════════════════════════════════════════════════════════════════════════════
// The fold primitive resolves the duplicate by LAST WRITE
// ══════════════════════════════════════════════════════════════════════════════

func TestFoldChangedPathsResolvesDuplicatesByLastWrite_PINNED_DEFECT(t *testing.T) {
	f := buildSlashFixture(t)
	b, w := rtDigestSBBlock(t, f)
	honest := f.applyAndCommittedRoot(t, b)
	ops, err := assembleOpsViaHead(f.c, f.prevRoot, honest, b, w)
	...
	if msg := rtDigestCollisionLastWriteWinsPin("", f.prevRoot, full, keepFirst, keepLast); msg != "" {
		t.Fatalf("%s", msg)
	}
}
...
// slash-ONLY block and a bondreg-ONLY block, agrees with real apply. The divergence in D1 is
// produced by the compounding, not by a broken fixture or a malformed witness.
...
// ══════════════════════════════════════════════════════════════════════════════
// The honest root is REJECTED, and on the terminal comparison specifically
// ══════════════════════════════════════════════════════════════════════════════
//
...
// of those ablations. What it asserts is therefore held to ONE error class. An earlier draft
...
func TestTheHonestRootIsRejectedOnTheTerminalComparison_PINNED_DEFECT(t *testing.T) {
	f := buildSlashFixture(t)
	b, w := rtDigestSBBlock(t, f)
	honest := f.applyAndCommittedRoot(t, b)
	err := recomputeViaHead(f.c, f.prevRoot, honest, b, w)
	if err == nil {
		t.Fatalf("PIN IS RED — the compound block now AGREES with the honest root. If that is\n" +
	...
	if !errors.Is(err, ErrRecomputeStateRootMismatch) {
		t.Fatalf("PIN IS RED — the compound block is still rejected against the honest root, but\n"+
...
func TestPinsFireOnTheirRemediations(t *testing.T) {
	var a, bh ports.Hash
	a[0], bh[0] = 1, 2
