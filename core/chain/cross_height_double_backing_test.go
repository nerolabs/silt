...
// F7 — cross-height double-backing: a validator signs blocks on two
...
func TestSameHeightDoubleSignIsCaught(t *testing.T) {
	...
	eqs := FindEquivocations([]Block{g, a1}, []Block{g, b1}, ports.Hash{}, eraFloorOf(0))
	found := false
	for _, e := range eqs {
...
func TestHonestReorgFollowerNotSlashed(t *testing.T) {
	propA, propB := key(1), key(5)
	v := key(2)
	g := f7Block(propA, 0, ports.Hash{}, entry(0))
	...
	eqs := FindEquivocations([]Block{g, a1}, []Block{g, b1, b2}, ports.Hash{}, eraFloorOf(0))
	for _, e := range eqs {
		if e.CulpritID() == idOf(v) {
...
func TestObjectiveForkChoiceNeutralizesDoubleBacker(t *testing.T) {
	prop := key(1)
	...
	// Under the BFT model: forkA is committed and therefore super-quorum-FINAL, so a
	// heavier CONFLICTING forkB — which exists only because V double-backed across heights
	// — CANNOT displace it. The finality gate refuses the reorg (ErrPreFinalityReorg);
	// fork-choice weight never even gets to prefer the heavier fork. So the F7 property
	// holds under B, by a stronger mechanism than the old heaviest-chain heal: a
	// double-backer cannot make a second, conflicting history stand by piling weight on it
	// — the FIRST finalized history stands, and V's cross-height double-signing is a
	// slashable equivocation, not a route to reorg finalized state.
