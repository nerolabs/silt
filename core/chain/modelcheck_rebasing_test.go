...
// PROOF-FIRST model-check of the re-basing fix: it may not ship until the
// handoff-intersection proof is model-checked. The fix
// proposes: at the epoch boundary, re-base the rotation quorum against `old ∩
// next` (the continuing members) so a boundary can rotate after `old` has bled
// offline weight. The precise old∩next sizing and the handoff proof are OPEN; the
// safety they must preserve is that the boundary block is final under BOTH the old
// and the new set, so two competing boundaries cannot each finalize.
//
...
// EXCLUDED weight may be honest — Byzantine members can lapse too, but the WORST
// case for safety is that only HONEST weight lapsed, leaving Byz unchanged over
// a smaller denominator. Then the boundary quorum finalizes a fork iff Byz_old >
// ⅓·(T_old − L) — which, for Byz_old near its ⅓·T_old premise limit, holds for
// ANY honest lapse L > 0. This is the SAME fault-tolerance wall that sank the
// rejected fix (1): excluding possibly-honest weight raises the Byzantine
...
func TestModelCheck_Fix2NaiveRebasingBreaksI1(t *testing.T) {
	...
	// The field topology (run the field run): 4 anchors + 4 maturers at 64
	// MiB, 4 sybils at 1 MiB. Frozen total 516 MiB.
	...
	// SHIPPED behavior (no re-basing): the bled boundary is measured against
	// the full frozen T_old, so no fork is possible — it STALLS instead.
	// Safe. (The stall is the correct behavior; recovery is fixes (4)/(3).)
	...
	// fix (2) proof obligation DISCHARGED with a counterexample: naive
	// old∩next re-basing reopens I1 at field-realistic parameters.
	t.Logf("fix (2) naive re-basing is UNSAFE at field parameters: "+
