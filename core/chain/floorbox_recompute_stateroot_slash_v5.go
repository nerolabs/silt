...
// none of which has a committed WHOLE-SET DIGEST scalar. A slash of `culprit` (chain.go)
...
// Each digest scalar (nodeSetMTHFromInt64/Bool, statehash.go) is an RFC-6962 MTH over the
...
//
// THE CHANGED-DIGEST PRIMITIVE (stateRootDigestOp). For each touched digest scalar:
...
// the sub-question): an omitted/injected pre-member yields a different MTH ⇒ stall.
...
// load-bearing for the accept-flip). Box-fits at RegCap-era populations (kilobytes per
...
// reproducing apply's slash loop (chain.go) LEAF EFFECT:
...
func stateRootSlashWriteSet(b Block, preBonded, preQualified map[ports.NodeID]struct{}) []stateRootWrite {
	...
	for i := range b.Slashes {
		...
		// pre-state qualified leaf. Membership is taken from the anchored pre-qualified set, not a
		// witness claim.
...
// The pre-set the delta is applied to is therefore the completeness-anchored set.
//
