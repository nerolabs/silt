...
// — the archival-format golden-fixture suite.
//
// WHY: validateStructural verified attesters in era-1 form
// while era-2 signs consensusSigBytes(phase, round, hash) — so replay of ANY
// era-2 chain silently fell back to genesis, from until 2026-08-25,
...
// state root lands — consult Q5 makes this suite its RED home). If a fixture
// fails here, the fix is in the VALIDATION/DECODE path, not in the fixture.
// The generator below refuses to overwrite.
...
func buildArchivalChains() map[string][]Block {
	...
	// era2: era-1 genesis (the flip happened after launch everywhere)
...
// TestGenerateArchivalFixtures writes the fixture files. Guarded: runs
...
func TestGenerateArchivalFixtures(t *testing.T) {
...
// of an older archive or changed hash computation for committed
// bytes (a silent hard fork).
func TestArchivalFixturesReplayAtHead(t *testing.T) {
	cases := []struct {
		file     string
		blocks   int
		nextH    uint64
	...
	for _, tc := range cases {
		t.Run(tc.file, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("HEAD can no longer DECODE the committed archival bytes: %v", err)
			}
			...
			if err != nil || n != tc.blocks {
				t.Fatalf("(the class): HEAD replays only %d of %d committed blocks (err=%v) — an archived store written by an older era no longer validates; fix the era-aware validation path, NEVER the fixture", n, tc.blocks, err)
			}
			head, next := c.Head()
			if next != tc.nextH {
			...
			if got := hex.EncodeToString(head[:]); got != tc.headHash {
				t.Fatalf("head hash of the committed archive changed: got %s want %s — HEAD computes a different hash for identical committed bytes (a silent hard fork)", got, tc.headHash)
			}
			if tc.state != nil {
				tc.state(t, c)
			}
