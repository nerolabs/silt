...
// hole opens. This pin makes that hole a red test instead of something a reader must notice.
//
...
//
// PER-LITERAL, NOT UNIONED — this is the re-point (2026-09-10, owner precondition;
// D3-ANSWERDIGEST-TWO-LEVEL-HASH-DELTA-). bodyHash becomes VERSION-DEPENDENT: a v5 preimage
// substituting AnswerDigest/SlashesDigest, and the pre-v5 preimage unchanged. The earlier
// implementation accumulated every literal's keys into one flat slice, so a field folded in the
// pre-v5 literal and DROPPED from the v5 one read as covered in both — the gate would have
// stayed GREEN straight through the change it exists to police. Each literal is now checked on
// its own.
...
// TestHashLiteralPinsEveryHashCoveredField is the source gate.
//
...
func TestHashLiteralPinsEveryHashCoveredField(t *testing.T) {
	...
	// EVERY literal is judged on its own. When bodyHash becomes version-dependent for,
	// one literal per era, a hole in ANY of them is a hole in that era's signed body —
	// and a union check cannot see it.
	for i, keys := range sets {
		which := fmt.Sprintf("literal %d of %d", i+1, len(sets))
		missing, extra := hashLiteralGaps(keys, want)
		if len(missing) > 0 {
			t.Errorf("SOURCE GATE: bodyHash's `unsigned` %s does NOT fold exported Block field(s) %v — every field outside the exclusions is a HOLE in the signed body for the era that literal serves (the carrier merge must list BOTH IssuerKeys and LastCommit). Literal keys: %v", which, missing, keys)
		}
		if len(extra) > 0 {
		...
		if _, ok := reflect.TypeOf(Block{}).FieldByName("LastCommit"); ok && !contains(keys, "LastCommit") {
			t.Errorf("SOURCE GATE: Block has a LastCommit field but bodyHash's %s does not fold it — the carrier is unsigned ", which)
		}
	}
}
...
func TestHashLiteralPinHasTeeth(t *testing.T) {
	...
	// Teeth 1: drop one folded key (the shape: a merge loses IssuerKeys).
	...
	// Teeth 2: fold an exclusion (Atts) — must be reported extra. Injected at the HEAD of
	// the literal so the fixture does not depend on which field the literal happens to end
	// with (it ended with IssuerKeys before the LastCommit carrier landed, LastCommit
	// after). made bodyHash version-dependent, so the literal is spelled `unsigned = Block{`
	// inside a branch rather than `unsigned:= Block{`. Try both, and fail loudly if neither
	// is present — a teeth fixture that silently matches nothing is the decoration this file
	// exists to prevent.
	injected := string(src)
...
// v5DigestCommitted names the fields the v5 preimage commits by DIGEST rather than by value. On
// v5 these must NOT move bodyHash when mutated — the preimage folds AnswerDigest and
...
//
// RUNTIME COVER for the transitive half: the digest-consistency validity gates (*).
...
// TestHashLiteralPinRuntimePair is the RUNTIME half of the pin: for every
...
// NOT. This observes the behaviour the source gate can only read, on the live code path
// Sign/verify use, with no allowlist beyond the same five exclusions.
func TestHashLiteralPinRuntimePair(t *testing.T) {
	// EVERY ERA, not just one. re-point, 2026-09-10 (owner precondition): bodyHash becomes
	// VERSION-DEPENDENT, and this gate's fixture was `Version: 1` — so it exercised exactly
	// one branch and a hole in the v5 preimage would have been invisible to it. Each era
	// version gets the full field sweep. Add a version here the moment a new era mints one.
