...
// — the DEEP the field run genesis-fallback, made
// deterministic.
//
...
// longer mask the fallback, and the node was stranded with its frozen
// seat still counted.
//
...
// era-1 form. Era-2 attestations sign the domain-separated
// consensusSigBytes(phase, round, hash), so replay of ANY era-2 chain has always
// failed at its first non-genesis block. The defect was invisible before the
// retention prune: a restarted node silently re-fetched its whole chain from
// peers (an expensive full Reconcile per restart — a hidden load source); once
// peers pruned below the horizon, the mask fell away.
//
...
func TestReload_Era2AndPrunedBlocksReplay(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("REPRODUCED: Reload of a committed era-2 history failed at block %d: %v — validateStructural is verifying era-2 attestations with the era-1 bare-hash arithmetic, so every restart silently falls to a %d-block prefix and (behind the prune horizon) strands the validator. Use the shared era-aware verifyAtt.", n, err, n)
	}
	if n != len(persisted) {
	...
	if err != nil || n != len(prunedSet) {
		t.Fatalf("Reload of the PRUNED era-2 history: restored %d of %d, err=%v — a validator must be able to replay its own payload-pruned store ", n, len(prunedSet), err)
	}
}
