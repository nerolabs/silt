...
// Blind (Accountability) INVERTED as a regression: a genesis block must NOT be
// able to pre-emptively revoke a never-published root. AppendGenesis now rejects
// any genesis carrying takedowns (ErrGenesisTakedown), closing the path that
// bypassed the ErrRevokeUnknownRoot existence guard — so a takedown can only name
// an already-committed root, on the governed normal path (immutable #5, "never
// pre-emptive").
