...
// renewal phase-jitter: validators that all registered near genesis used to hit the
// TTL/2 renewal point together, so 5–7 heavy ~1.5 MB space-time proofs landed in
// one block — inflating the two-phase gather latency on the 1 vCPU box (the
// deep-drive crawl). BondRenewalDue now spreads each identity's renewal across the
// [TTL/4, 3·TTL/4 window by a deterministic per-identity offset, so ~1 reg lands
// per block, while keeping the renewal margin to expiry ≥ TTL/4 (a dropped renewal
// cannot lapse standing).
...
func TestBondRenewalDue_JitterSpreadsAndKeepsMargin(t *testing.T) {
	...
	// All 12 registered at genesis (bondRegHeight 0). Find the first height
	// each becomes renewal-due by scanning WITHOUT advancing the chain:
	// BondRenewalDue reads c.Head's next, so drive Head by appending plain
	// attested blocks and record, per height, how many validators are newly
	// due.
	...
	// Every validator must have become due within the window, and the margin
	// to expiry (bondRegHeight 0 + TTL) must be ≥ TTL/4.
	if len(dueAt) != 12 {
...
// — the jitter grid vs the R-rule (the field run field finding): the
...
// property — the clamp must be inert there, engaging only on colliding
// first cycles like the genesis-aligned fleet's).
func TestRenewalDueClearsRegRateBound(t *testing.T) {
	const ttl = uint64(32)
	cfg := Config{Quorum: 2, MinBond: 1 << 20, ByzantineQuorum: true,
		MatureValidators: 0, EpochBlocks: 8, BondTTLBlocks: ttl}
	c := New(cfg, func(ports.NodeID) int64 { return 0 })
	...
	for seed := int64(0); seed < 512; seed++ {
		...
		if dist := due - regH; dist < minDist {
			t.Fatalf("REPRODUCED: identity with phase %d has renewal due at +%d < R=%d — its renewal submit is refused every sweep until the chain outruns the rate bound", phase, dist, minDist)
		}
		...
		// Margin to expiry survives the clamp: due stays inside the
		// jitter window (≤ 3·TTL/4, so ≥ TTL/4 margin) — the clamp never
		// pushes past it (it only ever RAISES a too-low point to R <
		// TTL/2).
		if due > regH+3*ttl/4 {
	...
	// — the clamp is inert (R < TTL/2), the period property intact.
	for seed := int64(0); seed < 64; seed++ {
		...
		if due := c.renewalDueHeight(id); due != onGrid+period {
			t.Fatalf("on-grid reg at %d (phase %d): next due %d, want exactly one period later %d (the steady-state property)", onGrid, phase, due, onGrid+period)
		}
	}
}
