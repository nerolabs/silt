...
// =============================================================================
// OWNER CALL A — THE ERA-4 CONSENSUS SIGNATURE PREIMAGE. GATES.
// =============================================================================
//
// WHAT WAS BOUGHT, AND ON WHAT. consensusSigBytes gains HEIGHT and a CHAIN ID for era 4
// (consensusSigBytesV5, chain.go). Bought on a real defect, not on an optimisation: dropping Height is
// the SECOND occurrence of the watermark defect — the same (height, round, step) schema
// family, the same dropped field, and the dropped field is again the thing that broke. silt's own
// durable watermark already carried (height, round, step) while its own signature preimage
// carried only (round, step): the two halves of one mechanism disagreed about the slot, inside
// one codebase.
//
// EVERY GATE IN THIS FILE CARRIES ITS ABLATION IN-PROCESS: a green gate with no demonstrated
// red is decoration. Where an ablation is a source edit ("key the
// dispatch on b.Version", "drop height from the preimage"), this file DRIVES the ablated
// arithmetic beside the shipped one and asserts they DISAGREE — so the red is permanent and re-run
// on every commit, not a one-off manual edit whose evidence lives in a report.
//
...
// ---------------------------------------------------------------------------
// THE LIVENESS WEDGE AT H_era4.
// ---------------------------------------------------------------------------
//
// THE DEFECT, from era4Active is AT-OR-GREATER, so H_era4 is itself the first v5 height and ITS
// PARENT IS v4. HeadCarrier filters the parent's precommits with the v4 parent in scope, so it
// produces v2-FORM entries. validateCarrier then verifies those entries with the v5 CHILD in
// scope. Key that dispatch on the CONTAINING block's Version and every entry fails: the honest
// proposer mints a block its own replica rejects, every designated proposer at that height does
// the same, and the chain never advances past H_era4. A permanent liveness wedge at exactly one
// height.
//
// THE FIX, verifies: dispatch on the ATTESTATION's own Phase — the mechanism verifyAtt already
// used for era-1 vs era-2. Wire-additive, and what the one-dispatcher pin demands.
//
...
// unrelabelled.
func TestEra4BoundaryCarrierIsNotAWedge(t *testing.T) {
	...
	//
	// Height 1 must be v2 and not v4 for a fixture reason worth stating: a v4 block seats its
	// own attesters from b.Atts, and the proposer populates its committed roots BEFORE
	// gathering, so the first v4 block that seats anybody cannot commit at all (that is, the
	// defect the carrier exists to fix and which era 3 still carries). Height 1 seats the three
	...
	if err := c.ValidateProposal(b); err != nil {
		t.Fatalf("VIOLATED (the proposer's own replica): an honest proposer at H_era4 rejected its "+
			"OWN block — that is the permanent liveness wedge: %v", err)
	}
	if out, err := ValidateProposalV5(liveView{c}, b); out != Accept {
		t.Fatalf("VIOLATED (the ONE accept composition): %s / %v", out, err)
	}
	if err := c.Append(*b); err != nil {
		t.Fatalf("VIOLATED (commit): the boundary block must COMMIT: %v", err)
	}
	...
	if n, err := replay.Reload(blocks); err != nil || n != len(blocks) {
		t.Fatalf("VIOLATED (reload): the boundary block must replay: %d of %d, %v", n, len(blocks), err)
	}
	...
	t.Logf("ablation RED as required: container-keyed dispatch at H_era4 ⇒ %v", err)
}

// ---------------------------------------------------------------------------
// THE DERIVED SIGNING HEIGHT AT THE CARRIER SEAM.
// ---------------------------------------------------------------------------
//
...
// b.Height-1. Certified SOUND under P1 PARENT BINDING and a strict NARROWING.
//
...
func TestTheCarrierSigningHeightIsTheParents(t *testing.T) {
...
// ---------------------------------------------------------------------------
// THE PHASE CONSTANT ORDERING IS A SAFETY CONSTRAINT.
// ---------------------------------------------------------------------------
//
...
// constants reddens rather than being caught by someone reading a comment.
func TestPhaseConstantOrderingIsMonotone(t *testing.T) {
	...
	if allowedAfter(PhasePrepareV5, PhasePrecommitV5) {
		t.Fatal("VIOLATED: a prepare is signable after a precommit at one height")
	}
	...
	if !allowedAfter(swappedPrepare, swappedPrecommit) {
		t.Fatal("GATE IS DECORATION: with the constants swapped the watermark comparison must " +
	...
	t.Logf("ablation RED as required: swapping the two era-4 constants makes a prepare signable "+
		"after a precommit at one height (%d > %d)", swappedPrepare, swappedPrecommit)
}

// ---------------------------------------------------------------------------
// THE DECLARED HEIGHT BINDS THE SIGNATURE (the relabel has no expression).
// ---------------------------------------------------------------------------
//
// The cross-height forgery takes two GENUINE signatures by an honest validator at two
...
func TestDeclaredHeightMustBindTheSignature(t *testing.T) {
	k := key(97001)
	cid := ports.HashBytes([]byte("chain"))
	...
	for _, wrong := range []uint64{0, 9, 11, 1 << 40} {
		if verifyAtt(att, attScope{ChainID: cid, Height: wrong}, h) {
			t.Fatalf("VIOLATED: a signature released at height 10 verified at declared height %d — "+
				"the accuser can relabel the height and the cross-height forgery is back", wrong)
		}
	}
	...
	// the degree of freedom exploited.
	...
	for _, anyHeight := range []uint64{9, 10, 11, 1 << 40} {
		if !ed25519.Verify(k.Public().(ed25519.PublicKey), heightless(anyHeight), ablatedSig) {
			t.Fatalf("GATE IS DECORATION: without the height in the preimage one signature must "+
				"answer at EVERY declared height (that is the forgery); it failed at %d", anyHeight)
		}
	}
	t.Log("ablation RED as required: with `height` dropped from the preimage, one genuine " +
		"signature verifies at every declared height — the relabel convicts an honest validator")
}
...
// second copy of that map, which is the drift shape this whole arc is paying back.
func eraFloorAt(t *testing.T, h, era4At uint64) uint64 {
	t.Helper()
	return eraFloorOf(era4At)(h)
}

// ---------------------------------------------------------------------------
// CROSS-CHAIN: TWO HONEST SIGNATURES ON TWO NETWORKS ARE NOT EVIDENCE.
// ---------------------------------------------------------------------------
//
...
// (the network-identity binding).
func TestCrossChainHonestSignaturesAreNotEvidence(t *testing.T) {
	...
	if err := CheckEquivocation(&live, cidX, eraFloorOf(lateBoundary)); err != nil {
	...
	t.Logf("LIVE RESIDUAL, driven at height %d (BELOW the era-4 floor, asserted above): with the v2 "+
		...
		"permanently evicted. Open,; closer = the M2 era-floor rule; on an "+
		"RC genesis committing Era4ActivationHeight=1 no height above 0 is in this interval.", h)

	// --- THE M2 CLOSURE, ON THE SAME BYTES: at or above the era-4 floor the pair is REFUSED. ---
	//
	// The two arms differ in exactly ONE input — the era floor — so this is the discriminating
	// pair the residual statement above rests on. Without it, "the RC network has no reachable
	// height for this" would be prose beside a green ablation.
	if err := CheckEquivocation(&live, cidX, eraFloorOf(rcBoundary)); !errors.Is(err, ErrNotEquivocation) {
		t.Fatalf("M2 VIOLATED: the SAME cross-network pair, judged on a chain whose floor at height %d is "+
			"v%d, must be refused as below the era floor — the sub-era-4 leg binds no chain id and is "+
			"portable between silt networks; got %v", h, BlockVersionWitnessable, err)
	}
	...
	for name, cid := range map[string]ports.Hash{"network X": cidX, "network Y": cidY} {
		if err := CheckEquivocation(&proof, cid, eraFloorOf(rcBoundary)); !errors.Is(err, ErrNotEquivocation) {
			t.Fatalf("VIOLATED on %s: an honest validator running on two silt networks must NOT be "+
	...
	if err := CheckEquivocation(&real, cidX, eraFloorOf(rcBoundary)); err != nil {
		t.Fatalf("NON-VACUITY BROKEN: a genuine era-4 double-sign on ONE network must still convict; got %v", err)
	}
}

// ---------------------------------------------------------------------------
// THE HONEST EXEMPTIONS SURVIVE THE ERA-4 FORM. Each driven ONE AT A TIME.
// ---------------------------------------------------------------------------
//
// SCOPE NOTE, stated rather than implied: the C1 (a proof carries EITHER two
// bodies OR the v5 tuple) and C2 (the phase whitelist on that tuple) are conditions on the
// FIXED-SIZE EquivV5 EVIDENCE OBJECT, which route (C) introduces and this branch does not. They
// cannot be driven here and are NOT silently claimed as covered. What IS driven is every exemption
// that the preimage change itself can break.
func TestHonestExemptionsSurviveTheEra4Form(t *testing.T) {
	...
	t.Run("cross-round lock change is HONEST", func(t *testing.T) {
		...
		if err := CheckEquivocation(&e, cid, eraFloorOf(1)); !errors.Is(err, ErrNotEquivocation) {
		...
		if err := CheckEquivocation(&Equivocation{Culprit: pub, A: a, B: bb2}, cid, eraFloorOf(1)); err != nil {
	...
	t.Run("bare-hash ProposerSig is authorship, not a vote", func(t *testing.T) {
		...
		// signature to either. Re-proposing fresh after a lock-free view change is honest,
		// and the era-1-shaped ProposerSig must never be read as an era-4 vote. Structurally
		// it cannot be: Sign signs 32 bytes and the era-4 preimage is 99, so the message
		// spaces are disjoint.
	...
	t.Run("different steps at one slot are not a conflict", func(t *testing.T) {
		...
		if err := CheckEquivocation(&Equivocation{Culprit: pub, A: a, B: bb}, cid, eraFloorOf(1)); !errors.Is(err, ErrNotEquivocation) {
			t.Fatalf("a prepare and a precommit are different slots; got %v", err)
		}
	})

	t.Run("the mixed-FORM harvest is REFUSED at or above the era floor", func(t *testing.T) {
		// WHAT THIS SUBTEST USED TO DEMAND, AND WHY IT WAS WRONG. It was titled "a v2-form
		// and a v5-form signature at ONE slot DO convict" and asserted CheckEquivocation ==
		// nil, calling the refusal an ACCOUNTABILITY REGRESSION. That demand is an I5
		// VIOLATION stated as required behaviour, and it directly contradicts the property —
		// "an honest validator running on two silt networks must NOT be slashable" — on bytes
		// that are identical.
		//
		// THE DERIVATION, driven below one step at a time. AttPhase returns `step` unchanged
		// for a sub-era-4 block (chain.go), so AttestAt never enters the era-4 branch and
		// NEVER READS chainID; verifyAtt's era-2 arm ignores the scope entirely. The
		// era-2-form leg therefore carries no network, and an attacker on network X can lift
		// the victim's genuine era-2-form precommit off network Y and pair it with the
		// victim's genuine era-4-form precommit on X. One honest act on each network; a
		// conviction on X. The penalty hits the honest.
		//
		// WHAT REPLACED THE DEMAND, AND WHAT REPLACED THE RECORD. The first rewrite turned the
		// demand into a RECORD of the open face, with a trip that reddened the day it closed. M2
		// closed it, the trip fired, and this arm is now the CLOSURE it asked for: the same
		// harvested bytes are REFUSED at or above the era floor, and — the discriminating half —
		// still CONVICT below it, because the sub-era-4 form is chain-blind forever there. The
		// honest-node half of T-STEP-VS-FORM — that a validator cannot double-sign ACROSS the era
		// boundary because ports.SignMark records the canonical step and is era-independent — is
		// driven where it actually lives, in core/node:
		// TestUpgradeMustNotReinterpretTheDurableSignMark. It never needed a slash verdict.
		//
		// WHAT WAS SOLD FOR IT, and it is settled, not overlooked: the ON-NETWORK boundary
		// double-signer (a validator precommitting a v4 block and a v5 block at one height
		// near H_era4) becomes unslashable. It cannot FINALIZE either way —
		// validateEra4Version refuses a sub-era-4 block at every height at or above the
		// boundary on every disk-write path — and on an RC genesis committing
		// Era4ActivationHeight = 1 there is no such boundary to stand on at all, which the
		// scope assertions below measure rather than assert.
		...
		// (3) THE CLOSURE. At or above the era floor the harvested leg is below the form this
		// chain requires at that height, so the pair is not evidence here.
		harvest := Equivocation{Culprit: pub, A: harvested, B: bb}
		if err := CheckEquivocation(&harvest, cid, eraFloorOf(rcBoundary)); !errors.Is(err, ErrNotEquivocation) {
			t.Fatalf("M2 VIOLATED at height %d: a mixed-FORM pair whose sub-era-4 leg is BIT-IDENTICAL to one "+
				"harvested from another silt network must NOT convict on a chain whose floor here is v%d. "+
				"Convicting it is the shape — the penalty hits the honest. Got %v",
				h, BlockVersionWitnessable, err)
		}

		// (4) THE DISCRIMINATING HALF, and without it (3) is not about the floor. BELOW the era-4
		// floor the SAME bytes still convict, because consensusSigBytes is frozen and carries no
		// chain id there. The two arms differ in exactly ONE input.
		if err := CheckEquivocation(&harvest, cid, eraFloorOf(lateBoundary)); err != nil {
			t.Fatalf("THE REFUSAL ABOVE IS NOT ABOUT THE ERA FLOOR: the same pair is also refused on a chain "+
				"whose floor at height %d is BELOW v%d, where the sub-era-4 form is the REQUIRED form. Either "+
				"the residual closed for some other reason — in which case say which — or this arm can no "+
				"longer tell a floor from a constant; got %v", h, BlockVersionWitnessable, err)
		}
		t.Logf("M2 CLOSURE, driven at height %d: the harvested mixed-FORM pair is REFUSED on a chain whose "+
			"floor here is v%d and CONVICTS on one whose floor is below it. The residual at h < H_era4 is "+
			"live and bought off by Era4ActivationHeight=1, never closed (consensusSigBytes is frozen). The "+
			"price paid at h >= H_era4 is the boundary double-signer, unslashable and settled.",
			h, BlockVersionWitnessable)
	})
}

// ---------------------------------------------------------------------------
// THE v5 QUORUM DEMANDS EXACTLY ONE FORM.
// ---------------------------------------------------------------------------
//
// The carrier accepts BOTH precommit forms, by necessity (it cannot see the parent's version).
// That dual acceptance is a SEATING rule and must not leak into a quorum: a heightless v2-form
// precommit inside a v5 block's Atts must be FATAL at collectQuorumSigs, not silently ignored, or
// a signature that binds no height completes an era-4 quorum.
func TestV5QuorumIsPhaseExact(t *testing.T) {
	c, keys := era4AnchorChain(t, 1, 1)
	...
	if err := c.ValidateCommit(&mixed); !errors.Is(err, ErrBadSignature) {
		t.Fatalf("VIOLATED: a heightless v2-form precommit inside a v5 block's Atts must be FATAL "+
	...
	if out, _, err := v5CollectQuorumSigs(liveView{c}, &mixed, mixed.Atts, PhasePrecommit, mixed.CommitRound); out != nil || err == nil {
		t.Fatalf("VIOLATED (composition): the shared constructor must refuse the mixed-form set; got %v", err)
	}
	...
	if !isCarrierPrecommit(mixed.Atts[0].Phase) && !isCarrierPrecommit(PhasePrecommit) {
		t.Fatal("GATE IS DECORATION: if the carrier ALSO refused the v2 form, this gate would be " +
...
// figure must never become the citation. The preimage layout is DERIVED at 99 bytes; this MEASURES
// it. It also measures the evidence sizes that actually exist in this branch, and states plainly
// which figure it cannot measure and why.
func TestMeasureThePreimageAndTheEvidenceMaterial(t *testing.T) {
	...
	if got != want {
		t.Fatalf("MEASURED era-4 preimage = %d B, layout = %d B", got, want)
	}
	...
	// The evidence object. A ~251 B FIGURE IS NOT MEASURABLE IN THIS BRANCH and is not claimed
	// here: 251 B describes the FIXED-SIZE EquivV5 tuple that route (C) introduces, which does
	// not build. The preimage is 99 B; conflating the two is a category error and it is recorded
	// rather than papered over. What IS measurable is the body-form
	// proof this branch still uses for every height, and the junk-leaf price it sets.
	k := key(94001)
	...
	t.Log("NOT MEASURED HERE, and NOT claimed: the rule ~251 B figure is the size of the " +
		"fixed-size EquivV5 evidence tuple introduced by route (C). The signature change " +
		"PREIMAGE only, so no such object exists in this branch. The junk-leaf re-pricing that figure " +
		"feeds must be measured when route (C) lands, not before.")
}

// ---------------------------------------------------------------------------
// THE CARRIER ACCEPTS BOTH PRECOMMIT FORMS AND NOTHING ELSE.
// ---------------------------------------------------------------------------
//
...
// by lemma. What it must NOT become is a hole: every other phase — both prepare forms and the era-1
// legacy shape — must still be refused, and the CLOSED COMPLEMENT is asserted rather than a list of
// examples, so a future constant cannot be admitted by omission.
func TestCarrierAcceptsBothPrecommitFormsAndNoOther(t *testing.T) {
	...
	for p := 0; p < 256; p++ {
		...
		if got := isCarrierPrecommit(phase); got != want {
			t.Fatalf("VIOLATED: phase %d admissible=%v, want %v — the carrier's phase rule must be "+
	...
	// off-form seating held in tension, and it is asserted, not assumed.
	offForm := Attestation{
	...
	for _, a := range c.HeadCarrier() {
		if !isCarrierPrecommit(a.Phase) {
			t.Fatalf("VIOLATED: HeadCarrier emitted phase %d, which validateCarrier refuses — the "+
