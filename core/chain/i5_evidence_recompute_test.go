...
// The I5 cross-height Pruned slash-forgery fix.
// the cross-height forgery fix
//
// Root cause: VerifyEquivocation quantifies over a fact (the height two signatures were
// released at) that lives OUTSIDE the signed message, and reads that fact from two different
// sources: a struct field (equivocation.go) for the height check, and Hash
// (equivocation.go) for the signature check. For a NON-pruned block those sources agree; a
// pruned block's Hash returns the attacker-chosen b.Pruned field instead, severing the
// identity. The fix (F2-EVIDENCE-RECOMPUTE): compute both blocks' hashes by re-marshalling
// the body, NEVER by reading Block.Pruned; a pruned block is therefore never admissible
// equivocation evidence — the accept set only narrows.
//
// Every test below drives the REAL commit path (Append) — "Append is the oracle in every
// case" preamble — because VerifyEquivocation alone was the exact reason looked
// survivable: it stopped short of the write path.

// ---------------------------------------------------------------------------------------
// the forged cross-height Pruned slash must be REFUSED end-to-end through
// Append, the culprit must remain unslashed, and must remain qualified.
// ---------------------------------------------------------------------------------------

// TestSlashProofBindsHeightToTheSignedHash_Era1 is the gate. the gate verbatim, driven through
// Append. RED today: Append currently ACCEPTS the forged slash (VerifyEquivocation trusts
// Block.Pruned as if it were Hash), so the culprit is wrongly evicted.
func TestSlashProofBindsHeightToTheSignedHash_Era1(t *testing.T) {
	...
	// FORGE: keep the genuine signatures, override Hash with Pruned, declare a
	// single shared fictitious height. No key material required.
...
// TestSlashProofBindsHeightToTheSignedHash_Era2 is the gate, extended to drive Append (the
// shipped probe stopped at VerifyEquivocation). RED today for the same reason as.
...
// TestGenuineDoubleSignStillConvictsThroughAppend is the gate: guards against "fix by
...
// ---------------------------------------------------------------------------------------
// the per-block encoded-byte ceiling on Slashes.
// ---------------------------------------------------------------------------------------

// TestSlashesBytesCapEnforced is the gate's over-ceiling half. Duplicating a SINGLE genuine
...
// including BondReg.Answer where present) from accumulating without limit in a slot Prune
// never reaches. RED today: no byte ceiling exists, so this ACCEPTS.
func TestSlashesBytesCapEnforced(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("T-6 RED (expected): a block whose canonically-encoded Slashes field is %d "+
			...
			"ceiling exists on Slashes today", lastLen, SlashesBytesCap, len(slashes))
	}
...
// and the fixtures stay fast (a header-only proof is ~1 KB; ~18k of them would make
...
// TestSlashesAtCeilingAccepted is the gate's I4 liveness edge, mirroring TestRegCapAtCeilingAccepted
...
// ---------------------------------------------------------------------------------------
// The honest-path-unaffected claim: FindEquivocations never pairs a pruned block.
// ---------------------------------------------------------------------------------------

// TestFindEquivocationsNeverPairsAPrunedBlock is. FindEquivocations is what
...
// (equivocation.go, ab.Hash==bb.Hash) must use the SAME recomputing hash function as
// VerifyEquivocation — if it did not, a stale Pruned digest could still pass candidate
...
func TestFindEquivocationsNeverPairsAPrunedBlock(t *testing.T) {
	...
	for _, e := range found {
		if e.A.IsPruned() || e.B.IsPruned() {
			t.Fatalf("VIOLATION: FindEquivocations produced an Equivocation with a PRUNED "+
	...
	if len(found) != 0 {
		t.Fatalf("expected NO equivocations from a pair where one side is pruned, got %d", len(found))
	}
}

// ---------------------------------------------------------------------------------------
// The memo bypass: the recompute must not be satisfiable by a stale hashMemo.
// ---------------------------------------------------------------------------------------

// TestVerifyEquivocationNotSatisfiableByStaleMemo is the gate point 3, §11. Same underlying defect as
// a fact (Height) decoupled from what Hash actually covers — reached via the
// hashMemo cache (F5/the mechanism) instead of Pruned: a Block value can be mutated in place
// AFTER Sign without invalidating hashMemo, so Hash keeps returning a digest over content the
// struct no longer holds. RED today: VerifyEquivocation reads e.A.Hash/e.B.Hash directly, so it
// inherits this staleness exactly like Append/ blockByHash do.
func TestVerifyEquivocationNotSatisfiableByStaleMemo(t *testing.T) {
	...
	// Mutate a hash-covered field DIRECTLY (no re-Sign, no memo invalidation) so the
	// struct now DECLARES Height=2 (matching honestB) while its Hash keeps returning the
	// STALE memo computed over the ORIGINAL Height=1 content.
	stale := honestA.Hash() // captures the memo
	forgedA := honestA
	forgedA.Height = honestB.Height
	if forgedA.Hash() != stale {
		t.Fatal("precondition broken: mutating Height did not preserve the stale memo — " +
	...
	if VerifyEquivocation(&forged, ports.Hash{}, eraFloorOf(0)) {
		t.Fatal("VIOLATION (RED expected): VerifyEquivocation accepted evidence whose " +
			"declared Height (the struct field) does not match what its OWN memoized Hash() " +
			"covers — the recompute must bypass hashMemo, never trust a " +
	...
	err := w.c.Append(bs)
	if err == nil {
		t.Fatal("Append accepted a stale-memo forged slash")
	}
	if w.c.slashed[culpritID] {
		t.Fatal("culprit must not be slashed via a stale-memo forgery")
	}
}

// ---------------------------------------------------------------------------------------
// One hash function: candidate selection and verification must call the SAME
// hash-producing method.
// ---------------------------------------------------------------------------------------

// TestOneHashFunctionForEquivocationCandidatesAndVerification is the gate point 1, "ONE
// FUNCTION, THREE CALLERS", §11. FindEquivocations selects CANDIDATES by comparing block
// hashes and VerifyEquivocation VERIFIES using block hashes; if selection and
...
func TestOneHashFunctionForEquivocationCandidatesAndVerification(t *testing.T) {
	...
	// Any call of the shape `<ident>.<Method>` where Method contains "Hash" is treated
	// as a hash-producing call.
	...
	if len(verifyMethods) == 0 {
		t.Fatal("SOURCE GATE: fixture: CheckEquivocation calls no *Hash* method —  cannot see the call it must pin; update the fixture if the function was refactored")
	}
	if len(findMethods) == 0 {
		t.Fatal("SOURCE GATE: fixture: FindEquivocations calls no *Hash* method —  cannot see the call it must pin; update the fixture if the function was refactored")
	}
	if len(verifyMethods) != 1 || len(findMethods) != 1 {
		t.Fatalf("SOURCE GATE:: expected exactly ONE hash-producing method name per function, "+
			"CheckEquivocation calls %v, FindEquivocations calls %v — a second distinct hash "+
			"entry point on this path is exactly what forbids", verifyMethods, findMethods)
	}
	if verifyMethods[0] != findMethods[0] {
		t.Fatalf("SOURCE GATE: VIOLATION: CheckEquivocation's verification and FindEquivocations' "+
...
func funcBody(t *testing.T, src, marker string) string {
	...
	if i < 0 {
		t.Fatalf("could not find %q in equivocation.go —  fixture is stale, update the marker", marker)
	}
	rest := src[i:]
