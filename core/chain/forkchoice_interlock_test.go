...
// =============================================================================
// THE FORK-CHOICE INTERLOCK GATE
// =============================================================================
//
// The interlock ("never repair the blockWeight verify alone") had ZERO test enforcement:
// the forbidden one-line repair could be applied and both ./core/... and ./sim/... stayed
// green. This gate is that enforcement.
//
...
// RUNTIME GATE: TestCertificateVariantNeverRanksHeavier,
// TestForkChoiceIsCertificateIndependentWithoutFinality, TestFastSlowPathSameHead.
...
var o3tHeavierDenied = map[string]string{
	"Weight":            "the retired fork-choice weight",
	...
	"attesterQualified": "reads bonded/epochSet/launchAnchor — the oscillation mechanism",
	"launchAnchor":      "reads handedOff()",
	"cfg":               "own-config — a fork-choice input must be shared by every replica, not configured",
	"Pruned":            "attacker-choosable where no independent hash binds it ",
}
...
//
// The walk is ALIAS-AWARE. The form keyed on the literal parameter names, and the ablation D Q2 —
// `ca:= a; if ha == hb { return len(ca.bonded) > len(cb.bonded) }` — passed that pin AND every
// runtime oracle: the (b2) twins hold identical replica-local state, so a replica-local ranking
// term agrees across the fixture and is invisible at runtime. That is the oscillation class T
// exists to make unbuildable, so the SOURCE gate must see it. Rules, in the order they fire:
//
	...
	ast.Inspect(fd.Body, func(n ast.Node) bool {
		...
		case *ast.CallExpr:
			...
			case *ast.SelectorExpr:
				...
				// A tainted receiver (a.Weight, ca.Weight) is reported by the
				// selector rule.
			default:
...
// TestHeavierReadsOnlyHeightAndHeadHash is gate (b1), the fork-choice purity pin. RED on main
// (chain.go reads a.Weight/b.Weight); GREEN after T.
func TestHeavierReadsOnlyHeightAndHeadHash(t *testing.T) {
	...
	if v := o3tWalkHeavier(fset, fd); len(v) > 0 {
		t.Fatalf("SOURCE GATE: heavier reads outside {blocks[...].Height, blocks[...].Hash()} (%d site(s)):\n  %s\n\n"+
			...
			"  interlock that turned out to have no enforcement. Retire the term;\n"+
...
// TestHeavierPinHasTeeth proves the walk bites, and bites ONLY where it should. The synthetic
// heavier re-injects every shape the pin must see:
//
// RED-first injection len(a.blocks[len-1].Atts) as the first term
//
//	the retired term a.Weight / b.Weight
//	own-config + a helper given a param a.cfg.MinBond, weightOf(a)
//
// C (block alias) x:= a.blocks[len-1]; len(x.Atts)
// D (chain alias) ca, cb:= a, b; len(ca.bonded) at equal height — the class;
//
//	 passed the lexical pin AND every runtime oracle
...
func TestHeavierPinHasTeeth(t *testing.T) {
...
// TestCertificateVariantNeverRanksHeavier is gate (b2) in the FINALITY posture (roundsWorld:
...
func TestCertificateVariantNeverRanksHeavier(t *testing.T) {
	arms := []struct {
		name    string
...
// o3tNoFinalityWorld builds an OBJECTIVE chain in the documented no-finality posture P2-off:
// -byzantine-quorum=false with Quorum below bftThreshold(N): 7 anchors, Quorum 1,
// ByzantineQuorum off. Every valid genesis-rooted fork is admitted, so the RANKING itself is
// observable — the posture the research says T makes replica-independent for the first time.
...
// TestForkChoiceIsCertificateIndependentWithoutFinality is gate (b2)'s head-level face. In the
...
func TestForkChoiceIsCertificateIndependentWithoutFinality(t *testing.T) {
...
// TestFastSlowPathSameHead is gate (b3) at the chain tier. The fast path (core/node
...
// (core/node/forkchoice_fastslow_test.go).
//
...
// appends it — recorded in the memory. The cumulative len(Atts) revert does NOT redden this gate (a
// positive cumulative term is extension-monotone); the (b2) oracles carry that revert.
func TestFastSlowPathSameHead(t *testing.T) {
