...
// era-3 committed state-root validity — build step 2b of the sequence.
//
...
// block validates on its era-2 merits alone (the 2a→2b window named in the
// 2adecision). This file closes that window. It adds the v4-gated validity predicate
// so that, once 2c mints v4, a block whose committed roots do not equal the post-apply
// recompute is INVALID — an honest attester refuses it, a commit carrying it is
// rejected.
//
...
// read bug; 2adecision §3, "What 2b MUST carry" (3). Enforcing root == recompute makes
// the value encoding load-bearing: the equality check IS the encoding's enforcement,
// and cross-node byte-identity is the property the step-1 determinism oracle already
// proves (modelcheck_stateroot_determinism_test.go).
//
...
// committed state) — every honest replica computes the same verdict (I5).
var (
	// ErrEra3RootMissing is a v4 block with a nil StateRoot or LogRoot. 2a's
	// omitempty schema makes a nil-root v4 block a well-formed CBOR object, so the
	// "roots are required" intent (the format freeze condition) is enforced
	// HERE, explicitly — a nil pointer is "no root", named as such, never folded
	// into the equality test (2adecision "What 2b MUST carry" (1)).
	...
	// the silent-mis-validation era-3 exists to prevent. Build step 2c.
...
// mis-validation era-3 exists to prevent. Below the boundary this never fires
...
// deep-copied.
//
// DRIFT PROTECTION (the class): a committed field this clone forgets would make the
...
func (c *Chain) cloneForDryRun() *Chain {
	...
	// maintains the copy, not the live chain (the same drift protection as the
	...
	// per-epoch issuer-key binding: deep-copied for the same reason —
	// apply on the clone must maintain the copy, not the live chain.
...
// shared inner map into the live chain (a nested-map alias would be the bug the
