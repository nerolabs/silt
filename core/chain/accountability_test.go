...
// ADVERSARY REGRESSION — accountability corner (censor persona).
//
// The external red-team broke accountability three ways through the on-chain
// revocation path Finding 5:
...
func TestRevokeUnknownRoot_Denied(t *testing.T) {
...
func TestRevocationIsPerOperator_Denied(t *testing.T) {
	w := newWorld(DefaultConfig())
	pub := w.block(entry(7))
	w.attestAll(pub)
	if err := w.c.Append(*pub); err != nil {
