...
// The NAME guard for residual R-G; deliberation.
//
// shipped the seenRoot per-root distinct-ID dedup in validateBondRegs.
// AppendGenesis (chain.go) does NOT run validateBondRegs — it goes straight
// to apply — so the dedup does NOT cover the genesis path. Genesis apply IS
// order-dependent for two distinct-ID UNPROVEN same-root regs.
//
...
// agreed — chain.go) and it carries NO BondRegs at all
// (genesis.Build → core/genesis/genesis.go, Entries only). With no per-node
...
// silently break — the future era-3 freeze gate inherits a named check rather than
// an unstated assumption. NO change to genesis validity: this is the record-it
// half of the "close it OR record it," inside the already-envelope.
...
func TestGenesisSameRootApplyIsOrderDependent(t *testing.T) {
	...
	// build applies a genesis carrying two distinct-ID UNPROVEN regs on one
	// shared root, in the requested slice order. AppendGenesis does not validate
	// BondRegs, so both reach apply and "first in the slice wins" the root.
	...
	if ownerA == ownerB {
		t.Fatalf("PREMISE CHANGED: genesis apply() is now ORDER-INDEPENDENT for two "+
			...
			"option (b) of the design notes — a "+
			"consensus-rule change to genesis VALIDITY, which is research-gated and "+
			"human-settled. Route it; then update this named premise to match the new "+
