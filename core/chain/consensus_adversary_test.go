...
// else (TestHeavierReadsOnlyHeightAndHeadHash). The bond is objective, and it
...
// replica on the reconciling chain's own rep, so an identical chain ties under ANY
// total order: this fixture PASSES with the retired rep-view weight restored as
// heavier's first term. "Fork-choice reads only Hash-covered content" is gated by
// TestHeavierReadsOnlyHeightAndHeadHash (the alias-aware purity pin),
// TestCertificateVariantNeverRanksHeavier and
// TestForkChoiceIsCertificateIndependentWithoutFinality — cite those, never this,
// for the ranking claim. (Was …ObjectiveWeightAgreesAcrossDivergentReplicas, which
// compared the retired Weight; re-grounded on the preserved admission property.)
func TestObjectiveForkChoiceAgreesAcrossDivergentReplicas(t *testing.T) {
	prop := key(1)
	...
	for _, dir := range []struct {
		...
		if adopted {
			t.Fatalf("%s: F6 regression (admission axis): a replica ADOPTED an identical committed chain — Reconcile's no-op path is broken. NOT a ranking finding: this fixture cannot see a replica-local ranking term; the purity pin TestHeavierReadsOnlyHeightAndHeadHash is the gate for that", dir.name)
		}
	}
	if h, _ := r1.Head(); h != h1 {
...
func TestObjectiveForkChoiceConvergesByCatchUp(t *testing.T) {
	// Under the BFT model, a committed objective block is FINAL, so honest replicas never
	// diverge onto CONFLICTING committed forks — that would require validators to
	// equivocate (a slashable double-sign), and a >⅓ equivocation is out of the fault
	// model (it STALLS, prefer-stall-to-reorg, rather than silently healing by weight).
	// Convergence is therefore by CATCH-UP: a replica that is BEHIND adopts a peer's
	// longer finalized chain that EXTENDS its own — the finality gate ALLOWS a fork that
	// contains the committed head and REFUSES one that would revert it. This is the same
...
// cannot be constructed. The purity pin (TestHeavierReadsOnlyHeightAndHeadHash)
...
func TestForgedBondRegDenied(t *testing.T) {
	prop := key(1)
	vals := []ed25519.PrivateKey{key(2), key(3), key(4), key(5)}
