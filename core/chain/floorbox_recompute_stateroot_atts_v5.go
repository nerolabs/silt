...
// own uncovered b.Atts. Before the re-point the box derived class A from b.Atts, so a peer serving a
// same-hash copy of a committed block with a legitimately DIFFERENT certificate (a same-round
// superset, shape S5) made the box compute a different post-set and Reject a canonical block. With
// the carrier the derivation is a pure function of hash-covered content, so every replica and the box
// agree on the class-A write-set by construction; the prevStateRoot anchoring the screen already had
// is then exactly right, because for the child prevStateRoot IS the parent'"'"'s committed
// post-state, which is the state the chain'"'"'s applyCarrier screens against.
//
// research: floorbox-recompute-classA-classP-wholeset-
// (A: carries — assert objective-mode, stall in legacy —
// and — the mature-epoch screen reads the FROZEN epochSet, not live
...
// child'"'"'s pre-state: it sets `validatorsSeen[id] = true` (ADD-ONLY — apply never deletes
// validatorsSeen). Measured leaf diff:
...
// The digest scalar (nodeSetMTHFromBool, statehash.go) is an RFC-6962 MTH over the WHOLE
...
//
// THE QUALIFICATION SCREEN — O(payload) POINT reads, box-computed. The box computes each
...
// (— read epochSet, NOT live bonded; a bond that joins mid-epoch is not
...
// 4. legacy mode: rep(id) — NOT a committed leaf. assert objective-mode, STALL.
...
// wrong-accept", which was true of the SCREEN and false of the INPUT CARRIER-1 /, 2026-09-03.
// What is true now:
//
...
//	 satisfies (ADD direction); that slot no longer exists.
//
// The box's verdict remains a STALL-or-agree: box.Accept ⇒ node.Accept, never the biconditional.
// The box is permitted to stall where the node accepts; it must never agree where the node
// rejects.
//
// COST — HONEST. O(|b.LastCommit|) screen + O(|validatorsSeen|) digest. The DIGEST term is ≈
// O(registry) and rides R-membership (OPEN, load-bearing for the accept-flip). The SCREEN term
// does NOT: validateCarrier applies no qualification screen, so |b.LastCommit| is bounded by
// nothing but the transport frame (~1.3M entries in 132 MiB), and an earlier version of this line
// that folded it into "≈ O(registry)" was wrong. A size
// rule on hash-covered content is a v5 validity rule and needs research.
...
type StateRootAttScreen struct {
	...
	// WITNESS-SOUNDNESS ANCHORS (per-field proofs against prevStateRoot). Each predicate
	// the screen reads is anchored the same way a fold-written leaf is: by a proof the box
	// VERIFIES against prevStateRoot before the read is trusted. Without them a forged screen
	// flips qualification and emits a spurious validatorsSeen||id ADD (class-A source of the
	// class-M poisoning). The box resolves each proof and STALLS (MustStall) on a nil/forged
	...
	// EpochSetProof anchors InEpochSet: present-proof of epochSet||id at the committed frozen
	// weight (InEpochSet=true) OR non-membership proof (InEpochSet=false). Membership is all the
	// mature-epoch screen needs (weight discarded); the box requires the presence/absence proof to
	// verify and does not read the weight.
...
// stateRootAttWriteSet derives the class-A per-member committed-leaf write-set for block b, reproducing
// applyCarrier'"'"'s LEAF EFFECT (carrier.go): one validatorsSeen||id ADD (Present) per carried signer
// the box computes to be qualified, excluding the parent'"'"'s proposer. The carrier fold runs BEFORE
// this block'"'"'s bond regs / TTL / slashes in apply, which is why the screen'"'"'s prevStateRoot
// anchoring is the correct pre-state. It is ADD-ONLY (apply never deletes validatorsSeen). An attester
// already in validatorsSeen pre-state is an idempotent re-set (still emitted; the leaf value is
// unchanged, so it does not move validatorsSeenRoot — the digest emit is gated on the post-SET
// differing from the pre-SET, see stateRootAttDigestOp).
//
// The screen is computed from own-cfg over the anchored pre-slashed set + the per-attester point
// witnesses — never a witness verdict. It returns the post-validatorsSeen id-set for the
...
func (c *Chain) stateRootAttWriteSet(
	...
	// The legacy branch falls to rep(id), which is NOT a committed leaf, so the box cannot
	// reproduce it from committed state. A v5 block is objective by construction, but assert it
	// and STALL otherwise (never guess legacy qualification).
	if !c.objective() {
	...
	// The v5 seating source is the HASH-COVERED LastCommit carrier, never the block's own
	// uncovered Atts. The box is v5-only; a sub-v5 block's frozen b.Atts rule is not
	// reproduced here (it is the defect the carrier closes) — stall rather than guess.
	...
	// The excluded id is the PARENT's proposer (the carrier republishes precommits over
	// b.Prev, so the block being attested is the parent). It is BOX-OWNED (HeadRef.ProposerID,
	// class 3 —): the box door derives it from the parent block it holds, after P1 has
	// bound b.Prev to that parent's hash. It is NEVER a witness field: the earlier
	// witness-supplied anchor ("some key signed b.Prev") was satisfiable by a freshly minted
	// keypair, which un-excluded the true parent proposer and let it self-seat (the ADD
	// direction). There is no slot in StateRootWitness for it any more, so there is nothing to
	// forge.
...
// attesterQualifiedFromScreen computes attesterQualifiedAt(id, 0) (chain.go) from the
// witnessed committed pre-state inputs + the box's OWN cfg — never a witness verdict. It
// reproduces the height-0 form (height 0 is never a recovery boundary, so effectiveEpochSet(0)
...
// screen consults is now either witness-Resolved against prevStateRoot or own-cfg — NO live
// box state decides a branch or a value (pinned by the fold-file
// c.<selector> allowlist, floorbox_recompute_foldlivestate_pin_v5_test.go).
//
// Every screen predicate is ANCHORED against prevStateRoot BEFORE it is read. A forged
// Slashed/InEpochSet/BondedPresent/BondedSize flips qualification and emits a spurious
// validatorsSeen||id ADD (class-M poisoning source). Each read is trusted only after its proof
// Resolves against prevStateRoot; a nil/forged proof yields NoWitness ⇒ stall (never a
// false/absent read, C-7 §104). Returns (qualified, stall-reason).
func (c *Chain) attesterQualifiedFromScreen(prevStateRoot ports.Hash, sc StateRootAttScreen, pre stateRootHandoffPre) (bool, error) {
	...
	// THE BRANCH SELECTOR — the ANCHORED committed pre-state, never c.matureEpoch. apply's
	// attestation loop (chain.go) runs BEFORE rotateEpoch (chain.go, rotate-LAST), so
	// the value a full node screens against is the PRE-state matureEpoch committed in prevStateRoot
	// — exactly pre.matureEpoch, Resolved present by handoffPreState before this dispatch. A box
	// that replays no apply has no c.matureEpoch to read; reading it made every mature-epoch block
	// take the pre-maturity branch (wrong-accept of a mid-epoch joiner against an attacker root,
	// false stall on an honest one).
	if c.epochsEnabled() && pre.matureEpoch {
		// The mature-epoch screen is FROZEN epochSet membership, weight discarded. Anchor
		// InEpochSet: present ⇒ inclusion proof of epochSet||id at its committed weight;
		// absent ⇒ non-membership proof. A forged InEpochSet=true for a non-member (the
	...
	// is own-cfg, never a witness — no anchor needed.
	...
	// launchAnchorGiven is the SAME predicate the live node uses (chain.go), parameterized on
	// the handoff bool rather than reading c.handedOff — the non-fork rule, one function, two
	// callers. The box supplies the ANCHORED pre-state handoff (matureEpoch with epochs
	// enabled, everMature without); the live node supplies c.handedOff.
