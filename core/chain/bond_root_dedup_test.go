...
// Blind (Sybil, Critical) INVERTED as a regression: one plot (one bond ROOT) must
// NOT back N standings. The objective chain now enforces the same per-root dedup
// the credit ledger has (c.bondRootOwner) — a bond Root credits AT MOST ONE
// identity — so a colluding operator pointing N identities at one shared plot
// earns one bond's standing, not N. This is the attacker's own construction with
// the assertions flipped to DENIED.
...
// from DISTINCT identities is now REJECTED at the validity layer. This
// SUPERSEDES the earlier behavior where the block was ADMITTED and apply's
// per-root dedup credited exactly one identity: that resolution was
// order-dependent (the winner was chosen by intra-block slice order — see
// TestSameRootDistinctIDNoDivergentCommit), which the era-3 SMT root cannot
// tolerate. Rejecting at validity is strictly stronger — the divergent input
// never commits, so at most one identity is bonded off any root, and the
...
func TestSharedRootDeniedViaValidatedBlock(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("F1 regression: a block with %d distinct-ID regs on ONE root committed — "+
			"the same-root dedup must reject it at validity", n)
	}
