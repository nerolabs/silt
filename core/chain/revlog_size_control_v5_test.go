...
// =============================================================================
// the m = 1 right-spine CONTROL for tagRevLogSize, pre-registered
// =============================================================================
//
// THIS IS A ADVERSARY-STYLE CONTROL, NOT A REGRESSION GATE. It asserts that the UNSOUND construction
// is unsound, so nobody re-derives it a third time (P-table and §9 item 6: an earlier derivation produced "a
// witness-supplied m is safe under MTH size-injectivity" once, refuted it against the verifier's
// own code, and pre-registered this control). It shipped with the P13b k >= 1 STALL and TRAVELLED
// WITH tagRevLogSize when that leaf landed: the stall is now a verified extension, and this control
// asserts the precondition that lift rests on.
//
...
// reading either root; and for `m` an exact power of two the path is seeded with oldRoot and at m
// = 1 the accumulator `fr` is never updated, so `fr == oldRoot` is vacuous and the only binding
...
//
// RUNTIME GATE: TestRevocationBearingBlockVerifiesItsLogExtension — the same construction
...
// TestWitnessSuppliedLogSizeIsUnsound_Control. The parent log has THREE entries (m_true = 3,
...
func TestWitnessSuppliedLogSizeIsUnsound_Control(t *testing.T) {
	...
	// first derivation missed; it is the whole reason tagRevLogSize is a SAFETY leaf.
	...
	if verifyLogExtension(parentRoot, mTrue, forgedRoot, []ports.Hash{leaf}, forgedCons, forgedIncl) {
		t.Fatal("VIOLATED: with m AUTHENTICATED the forged LogRoot must be REFUSED")
	}
	...
	if verifyLogExtension(parentRoot, mTrue, forgedRoot, []ports.Hash{leaf}, cons, [][]ports.Hash{incl}) {
		t.Fatal("VIOLATED: the honest proofs must not verify the forged root")
	}
}
