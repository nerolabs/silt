...
// TestProductionGenesisCarriesNoAtts is the sibling of
// TestProductionGenesisCarriesNoBondRegs (genesis_sameroot_premise_test.go).
//
// The genesis attestation seating rule ships with NO era gate. That is sound only because the production
// genesis carries zero attestations: on every real network the rule "seat only the
// attestations that verify over the genesis hash" is then an identity transform, so no
// persisted chain, archival fixture, or field-test replica changes state on upgrade. This
// test is the pin that makes the no-era-gate claim hold. The archival half (each
// committed fixture's block 0 carries no Atts) lives in core/chain
// (TestArchivalFixturesGenesisCarriesNoAtts) — genesis imports chain, so a chain test
// cannot live here.
//
	...
	for i, gb := range []struct {
		...
		if gb.atts != 0 || gb.pqc != 0 {
			t.Fatalf("PREMISE CHANGED (build %d): the production genesis now carries %d Atts / %d PrepareQC. "+
				...
				"transform on production, so its no-era-gate claim must be re-derived "+
