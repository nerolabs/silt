...
// F1 — the coherence floor, and the gate that holds its SHAPE., on and its predecessor escrow-price-repair-cost-model-
//
...
// TestPriceIsOneShardOfWitnessedFetch drives floor and ceiling together over the whole
...
func TestPriceIsOneShardOfWitnessedFetch(t *testing.T) {
	// The point where floor and ceiling touch: a shard of EXACTLY one credit of
	// fetch pays exactly one credit. Below it the floor cannot be met at all (the
	// zero class); above it the price tracks the bytes.
	...
	for _, shardBytes := range []int64{1, 16, 65_552, 262_143, DeliveryBytesPerCredit, 262_160, 524_280, 1 << 20, 1<<27 + 16} {
		for _, mult := range []int{1, 2, 4, 7} {
			...
			// FLOOR: the whole witnessed price, less at most the one truncation
...
// TestPriceCarriesNoK is the shape gate and the W4 refutation, driven.
//
// W4 (REFUTED by the research): "encode c = 1/k as RepairBountyCoeffNum/Den = 1/10." It
// gives the right number at k = 10 and silently over-pays the moment k re-tunes, which it
// may — k is Evolving-tier. The counterfactual is computed here, not asserted.
//
...
func TestPriceCarriesNoK(t *testing.T) {
	const shardBytes = 262_160
	// 1. The shipped price does not move with k. k is still a parameter (it names a
	// degenerate geometry and the judge's call sites hold it, so this is the
	// gate that keeps it out of the arithmetic.
...
// TestSolvencyBandIsExact pins the D-S7 self-funding threshold as the integer relation it
...
// basis. The research report 1.20007 and 12.0007; that 0.006 % is an artifact of pricing
// income at a 262,144 B shard and outflow at a 262,160 B one, and it cannot be real — the
// ratio has no shardBytes in it.
//
...
// put back in the product, while its docstring claimed the F1 threshold. With `k`
// restored the true threshold is 12·m̄ and the old form still read green. It now calls
// repairBountyCredits, so the relation it prints is the relation the ledger would pay.
func TestSolvencyBandIsExact(t *testing.T) {
	const k = 10
...
// TestZeroClassIsTheAcceptedCost RUNS the cost the project accepted, so it cannot rot into
...
// is what a publisher chooses; a single-frame object is stored at its true length, so its
// shard is its own bytes + an 8-byte frame header + the 16-byte tag.
func TestZeroClassIsTheAcceptedCost(t *testing.T) {
