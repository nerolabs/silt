...
// byte-denominated per-increment delivery settlement: the LEDGER-tier RED-first
// gates, at the shipped price (U,
// p) = (262,144, 1), Dλ = 393,216. The ledger-tier gates live here; the harness gate
// In invariant_a_test.go; B-5 and the literal pins in cmd/silt (the one package that
// imports core/credit and core/relaypay); B-7…B-13 are the node half's.
//
// RULES: none of these call fund; every burn goes through ChargePublish against the
// fetcher's DURABLE identity holding the shipped grant. Σ_L = Σ balances + Σ escrow
// (sumConserved) over a FIXED account set registered before the baseline.
//
// ABLATIONS that must redden (six): Dλ:= U (parity tie ⇒
// margin ≤ 0); whole-lane reversal on a partial ack (⇒ B-1 partial); the
// payout reading l.fee (⇒); the retired "256 serves per block" unit (⇒);
// remainder → escrow (⇒); the burn counted per settlement (⇒).
...
// openDeliverySession is the open: the durable fetcher buys k anchors through the real
// burn on the server's ledger, the server spends them at open, and the summed face is
// the session budget.
...
// TestDeliveryAcceptStrictlyDominatesSuppressionAtEverySize — B-1 (strict at every
...
// margin is +75 credits — the break (+58,676,506 for suppression at λ = 1) closed
// structurally, not by the numéraire alone. The partial ack (j − 1) still pays for the
// acknowledged part and keeps the tail's bytes on the lane.
func TestDeliveryAcceptStrictlyDominatesSuppressionAtEverySize(t *testing.T) {
	...
	for _, B := range sizes {
		...
		if margin <= 0 {
			t.Fatalf("B=%d: bank − suppress = %d, want STRICTLY > 0 — a server prefers suppressing the receipt (broken)", B, margin)
		}
		value := j * r29P
		...
		if B == 64<<20 && margin != 75 {
			t.Fatalf("64 MiB margin %d, want +75 (224 paid − 149 reversed at PF 1.5) — the accept margin moved", margin)
		}
...
// TestProvisionalLaneEqualsCreditedBalanceAndReversesPerIncrement — B-2 (credit site
...
// TestDeliverySettlementIsBoundedByAnchorFaceOnThePayingLedger — B-3 (spend-at-open,
...
// ≤ 0 on a fully acknowledged delivery, equality iff j·p == Σ face; the payout tracks the
// budget it is handed, never l.fee.
...
// TestDeliveryFundTopUpSpendsFreshAnchorsOnce: A top-up is a second all-or-nothing
// spend of FRESH anchors: Σ face rises exactly; a re-presented anchor is refused (per
// top-up) and a refused batch records nothing (per top-up).
...
// TestPaidSerialCapDominatesBothPopulations — B-4 (UNIT half; the field half —
// guardFullRefusals == 0 per lane on a graded run, T_b and the resident cost measured —
// is still owed). The cap is derived from bytes per anchor for both populations,
// (W+1)·E, the target rate and a T_b upper bound, floored at 65,536; the derived φ = 1
// corner is pinned by independent arithmetic so the retired "256 serves per block" unit
// reddens it. It is a CORNER, not a dominance claim (T-QUANT: under spend-at-open the
// binding bound is the session count, enforced at start-up by the assertion against the
// runtime faucet capacity). A two-population fill refuses at cap and never evicts; each
// lane's refusal moves ITS counter and the live count per lane is exported.
func TestPaidSerialCapDominatesBothPopulations(t *testing.T) {
	...
	if d, r := l.GuardFullRefusalsByLane(); d != 1 || r != 1 {
		t.Fatalf("per-lane refusals (delivery %d, relay %d), want (1, 1) — the split the lanes require", d, r)
	}
...
// TestDeliveryRemainderIsNeverRoutedToEscrow checks the deposit rule. Settling j·p < Σ face: the object's escrow rises by skim(j·p) only; no account and no
// escrow receives the remainder AT CLOSE — it is booked as a pending deposit (Σ_L is down by
// exactly the remainder until the anchor expires) and returns to the fetcher's EXISTING
// account, whole, when the anchor leaves the window.
...
// TestRemainderIsAccountedOnceAtCloseNotPerSettlement. A session settles in DELTAS; the remainder is accounted once at close (booked pending —
// not burned, not routed): no account and no escrow rises at the close, Σ_L is down by
// exactly the remainder, and PendingRefundCredits rises ONCE. Ablation: account `budget −
// value` on the per-settlement path and settle in three deltas — the pending counter
// over-reports and this catches it.
...
// TestAckReversalUsesTheBudgetCappedCount. Settling count = 50,000 against budget = 1 on
// a lane holding 12 MiB: the reversal is at most one increment's worth (whole·U), never
// the raw count's, so the server is not left worse off than suppression. The pre-fix code
// IS the ablation (reversed from the raw count).
...
// ---- the settlement skim under fetcher-chosen deltas. Ablation for all three:
...
// TestSkimIsExactOverTheSessionNotPerSettlement pins the measured table as a
...
// TestSettlementSkimNeverExceedsItsOwnValue: 0 ≤ skim_i ≤ value_i, so the payout
...
// TestWitnessingNeverDefundsTheEscrow, the composed claim: serve 64 MiB (the unwitnessed
// leg deposits its skim), then witness 256 increments in deltas of 1. The escrow ends
// STRICTLY higher than after the serve alone, and the witnessed deposit is at least PF ×
// the unwitnessed one (32 vs 21). Ablation: the per-settlement floor ⇒ the serve-time
// skim is clawed back and nothing replaces it — the escrow ends at 0, net −21.
...
// ---- the deposit released at anchor expiry. Ablations: release at close
// (releaseEpoch:= watermark), credit through acct, refund `budget` instead of
// `budget − settled`, and an unbounded pending table each redden a gate below.

// TestRemainderIsConservedAcrossCloseAndRelease. Withdrawal → open → serve → settle →
...
// TestReleaseNeverRegistersAnAccount. A deposit whose fetcher has no account
...
func TestReleaseNeverRegistersAnAccount(t *testing.T) {
	...
	if len(l.Balances()) != accounts || sumConserved(l) != base {
		t.Fatalf("release registered an account (%d → %d) or moved Σ_L by %d — acct minted a grant (the phantom)", accounts, len(l.Balances()), sumConserved(l)-base)
	}
...
// TestRemainderIsNotSpendableUntilTheAnchorExpires. Close at epoch E with an
...
// TestGuardOccupancyIsBoundedByTheCreditStock — THE REFUTATION ENCODED:
...
func TestGuardOccupancyIsBoundedByTheCreditStock(t *testing.T) {
	...
	// only prove W+1 > 0. The close passes the anchor's REAL epoch.
...
// TestZeroSettleSessionReturnsItsWholeFace: a session that
// settles nothing returns its WHOLE face at close or at anchor expiry, whichever is later;
// nothing is burned. A close-only assertion must NOT pass a correct build.
...
// TestPendingRefundTableIsBoundedAndRefusesNeverEvicts (build-immutable #8). The
...
// TestNoIdentityIsJoinedToAnAnchorSerialInTheDurableStore (Don't #3). The guard
// entry and the durable store's record carry no FETCHER identity; a durable join between a
// durable identity and an anchor serial is the refused shape.
func TestNoIdentityIsJoinedToAnAnchorSerialInTheDurableStore(t *testing.T) {
	// The EXACT field sets. `lane`/`Relay` name the POPULATION an entry belongs to
	// (delivery or relay), not a party: it is derivable from the call that spent the
	// anchor and joins nothing to an identity. It is on the whitelist for that reason
	// and no other.
...
// TestRestartLosesTheRemainderAndCountsIt. A restart (a fresh ledger loading the
...
func TestRestartLosesTheRemainderAndCountsIt(t *testing.T) {
	...
	// Two more live guard entries that lose NOTHING: a relay anchor (the relay lane keeps
	// the burn) and a fully settled delivery session (remaining 0). Past N = 1 the entry
	// count and the lost-deposit count differ, so the counter's honest meaning is pinned:
	// DELIVERY-lane entries, an UPPER BOUND on lost deposits. The relay anchor is excluded
	// because the durable record now carries its lane; before that it was restored as
	// delivery and inflated the count to 3.
	...
	if d, r := l2.LivePaidSerialsByLane(); d != 2 || r != 1 {
		t.Fatalf("after restart the lanes are (%d, %d), want (2, 1) — the restore must not re-label the relay anchor as delivery ", d, r)
	}
...
// TestAssertionStillBoundsOccupancyUnderTheRelease. At the largest
...
func TestAssertionStillBoundsOccupancyUnderTheRelease(t *testing.T) {
	const fee, grant = int64(50_000), int64(500_000)
	l := New(fee, grant)
	src := &mockEpochSource{}
	l.SetEpochSource(src)
	...
	for round := 0; round < 3; round++ { // "as fast as the loop allows": three passes inside the window
		...
		if live := len(l.paidSerial); live > MaxPaidSerial/4 {
			t.Fatalf("round %d: %d live guard entries > MaxPaidSerial/4 = %d — the assertion is vacuous (deposits re-spent inside the window)", round, live, MaxPaidSerial/4)
		}
	}
