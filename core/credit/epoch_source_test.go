...
// The epoch source is monotone from the ledger's point of view: a source that FALLS
// lowers nothing and re-admits nothing. The ledger reads the source ONCE at entry,
// raises its watermark by max, and sweeps and screens against the WATERMARK — never
// against the live source. Both arms are covered: delivery and relay.
//
// Worked example. A serial S issued at source 5 with window W = 4 is PAID at source 9
// (in-window, 5 + 4 = 9) and SWEPT when the source reaches 10 (floor 6). At source 10
// it is ReasonBackdated, not paid.
...
// TestFallingSourceLowersNothingAndReadmitsNothing_Delivery is the delivery arm.
func TestFallingSourceLowersNothingAndReadmitsNothing_Delivery(t *testing.T) {
	const fee = 50_000
	...
	if l.epochWatermark != 10 {
		t.Fatalf("(i) a source that fell 10 → 6 lowered the watermark to %d, want 10 ( "+
...
// TestFallingSourceLowersNothingAndReadmitsNothing_Relay is the relay arm: the
...
func TestFallingSourceLowersNothingAndReadmitsNothing_Relay(t *testing.T) {
	const fee = 50_000
	src := &mockEpochSource{e: 9}
	l := New(fee, 0)
	l.SetEpochSource(src)
	...
	if l.epochWatermark != 10 {
		t.Fatalf("(i) a source that fell 10 → 6 lowered the watermark to %d, want 10 ", l.epochWatermark)
	}
...
// TestNilSourceReadsAsEpochZero pins the epoch-0 permissive core: a ledger that
// never set a source keeps today's epoch-0 behaviour, so every in-process fixture
// that never sets one is untouched.
func TestNilSourceReadsAsEpochZero(t *testing.T) {
	const fee = 50_000
	wantPay := int64(fee) - int64(fee)*SkimNum/SkimDen
	l := New(fee, 0)
