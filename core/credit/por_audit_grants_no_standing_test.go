...
// INVERTED as a regression. The red team broke the Sybil corner
// (over our own G2 merge) through the PoR-audit standing press: a data-less identity
// RELAYS an honest holder's aggregated proof, passes the audit, and — under the old
// formula `+ auditsPassed*25` — reached propose/attest eligibility (100 rep) with ZERO
// storage. The fix: PoR audits no longer mint Sybil-resistant standing; standing rests on
// the bond alone. See §2 (Invariant A), §4 (S2), memo 03.

// piling on PoR audit passes must NOT lift a bondless identity to standing. This
// is the exact "4 passes = 100 rep = eligibility with zero disk" claim, denied:
// with no bond, no number of audit passes clears the bar.
func TestAuditPassesGrantNoStandingWithoutBond(t *testing.T) {
	l := New(50_000, 0)
	liar := id(1)
	...
	if got := l.Reputation(liar); got >= attesterBar {
		t.Fatalf("regression: %d audit passes lifted a BONDLESS identity to %d ≥ %d — a disk-less Sybil earns consensus standing", 40, got, attesterBar)
	}
	if got := l.Reputation(liar); got > 0 {
		t.Fatalf("PoR audits must grant ZERO standing without a bond, got %d", got)
	}
}

// Invariant A §2: no POSITIVE standing without a verified, identity-bound, bond-gated
// proof. Property form: for ANY identity holding no bond, Reputation ≤ 0 regardless of
// how many audits it passed — so the only press that GRANTS standing is the bond. A
// future press that mints standing without a bond would trip this.
...
// the mint.
func TestFailedAuditStillPenalizes(t *testing.T) {
	l := New(50_000, 0)
	n := id(1)
