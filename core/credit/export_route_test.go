...
// The export route, closed at the package boundary.
//
//
...
// while its failure text claimed a whole-tree property. It was ablated past in
...
// cannot: a SECOND exported reader added inside this package. It parses EVERY non-test.go
// file of core/credit and requires that the only exported functions returning a
// BBootstrapHistogram are the sanctioned two. Unlike its predecessor it checks exactly the
// scope its failure text names — one package, every file in it, found by reading the
...
// TestBBootstrapHasOneExportedRoute is the package-scope source gate behind the
// minimum-requester floor's structural claim.
//
// RUNTIME GATE: cmd/silt's TestBelowFloorBlockIsAFunctionOfTheClockAlone observes the
// resulting property on the published bytes. This gate closes only the "is there a
...
// some other type is the stated residual, not covered here.
func TestBBootstrapHasOneExportedRoute(t *testing.T) {
...
// TestExportRouteGateHasTeeth is the positive control: the same predicate, run over a
// synthetic declaration of the exact shape that ablated the old gate. If
// the walk's predicate could not flag it, the gate above would be decoration.
//
// RUNTIME GATE: TestBBootstrapHasOneExportedRoute.
func TestExportRouteGateHasTeeth(t *testing.T) {
