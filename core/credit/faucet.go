...
// The faucet rate limit: a lazily, CONTINUOUSLY accruing token bucket over an INJECTED
// monotonic time source, gating the ledger's starter grant. It is LOCAL ADMISSION CONTROL,
// not a consensus rule: no other node's validity depends on this node's grant rate, and two
// honest nodes may run different rates forever without diverging.
//
...
//
// CONTINUOUS ACCRUAL, NOT A PERIODIC TOP-UP. Tokens accrue in
// proportion to elapsed time at `refill` per `interval`, with the sub-token remainder
// carried in integer nanoseconds so nothing is lost to truncation. A cohort of 120 arriving
// at a 100-token bucket waits minutes for the next tokens, not until the next hour boundary;
// the sustained rate and the Sybil price are the same either way, the honest experience is
// not.
//
...
// — the gate bounds dN/dt, the RATE of fresh grants, never N. It is a soft, disclosed
// deterrent standing in for the unbuilt structural cost of identity /§5, and it is labelled
// as such.
type faucet struct {
...
// refill or interval. A zero interval or refill would otherwise drain once and never
// refill, which is the permanent denial the whole design exists to avoid; refusing the
// configuration belongs to the caller that owns the operator's flags (cmd/silt), and the
// constructor's part is to never build a bucket that cannot refill.
