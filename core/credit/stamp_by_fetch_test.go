...
// `bbootstrap` build tag: untagged, stampFirstFetch is an empty body
...
//
// THE DEFECT THIS PINS. The B_bootstrap age axis is specified as time since first FETCH. The
// stamp used to be written in Register, and Register is reached through acct by every ledger
// path, so it actually recorded first ledger touch by ANY path: bond audit
// (core/node/bondaudit.go -> RecordBondChallenge), PoR grading (core/node/por.go ->
// RecordAudit), bounty payment (PayBounty) and the false-repair slash. An identity that is
// also a DHT participant therefore published an age over-stated by however long it had been a
// peer before it first fetched, unbounded above by the ledger's uptime, on the input to a
// security parameter. Certified in
// section 4.
...
// TestNonFetchLedgerTouchesLeaveNoStamp is the gate. Every non-fetch path is
...
func TestNonFetchLedgerTouchesLeaveNoStamp(t *testing.T) {
...
// TestAgeIsMeasuredFromTheFetchNotTheBondChallenge is the composed gate, and it
...
func TestAgeIsMeasuredFromTheFetchNotTheBondChallenge(t *testing.T) {
	const (
		hour = int64(3600) * 1e9
		day  = 24 * hour
	)
	...
	if h.MaxOccupiedAgeEdgeNanos >= bbAgeEdgeNanos[4] {
		t.Fatalf("highest occupied age bucket has lower edge %d ns; every identity fetched %d ns ago, so nothing may reach the one-hour edge %d. The age axis is measuring first ledger TOUCH (the bond challenge a day earlier), not first FETCH — ", h.MaxOccupiedAgeEdgeNanos, 30*60*int64(1e9), bbAgeEdgeNanos[4])
	}
	...
	// ONLY tick a bond challenge writes since deleted the first-seen stamp.
	if got := l.accounts[peer].lastBondTick; got != 7 {
		t.Fatalf("lastBondTick = %d, want the bond auditor's tick 7 — the fetch stamp must not disturb the retention tick", got)
	}
}
