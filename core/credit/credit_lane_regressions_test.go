...
// ---------------------------------------------------------------------------
// CLOSED. A restart is not an eviction: the same serial, presented to a
...
// ---------------------------------------------------------------------------
func TestRestartDoesNotEvictTheGuard(t *testing.T) {
	const fee = 50_000
	...
	// Boot 1. The ledger reads its own epoch, and an anchor above that
	// clock is refused as future-dated — so the clock has to be at the anchor's epoch,
	// exactly as the daemon's chainEpoch is.
	l1 := New(fee, 500_000)
	l1.SetEpochSource(&mockEpochSource{e: 3})
	l1.SetPaidSerialStore(store)
	if err := l1.LoadPaidSerials(); err != nil {
		t.Fatal(err)
	}
	if got := paidOnLane(l1, srv, fetcher, obj, s, 3); got != wantPay {
		t.Fatalf("setup: first redeem must pay %d, got %d", wantPay, got)
	}
	if got := paidOnLane(l1, srv, fetcher, obj, s, 3); got != 0 {
...
func TestRedeemBeforeLoadIsRefusedNotPaid(t *testing.T) {
	...
	if paid, why := settleOnLane(l, id(1), id(2), id(7), testSerial(4), 1); paid != 0 || why != ReasonAlreadyPaid {
		t.Fatalf("after the load the guard must remember the serial, got %d (%s)", paid, why)
	}
}

// TestGuardEntryIsDurableBeforeTheCreditMoves: the ORDER is the property. If the
...
func TestGuardEntryIsDurableBeforeTheCreditMoves(t *testing.T) {
	l := New(50_000, 500_000)
	l.SetPaidSerialStore(&failingStore{mem: &memStore{}})
	if err := l.LoadPaidSerials(); err != nil {
		t.Fatal(err)
	}
	// Register both accounts first: acct auto-registers with the grant, which
	// would otherwise show up in the conservation delta as a faucet, not a payout.
	l.Register(id(1))
	l.Register(id(2))
	before := sumConserved(l)
	paid, why := settleOnLane(l, id(1), id(2), id(7), testSerial(5), 0)
...
// ---------------------------------------------------------------------------
// CLOSED. "Evicted ⇒ expired" now holds PER TOKEN. The probe's attack: one
...
// ---------------------------------------------------------------------------
func TestGuardEntryExpiresOnItsOwnIssueEpoch(t *testing.T) {
	const fee = 50_000
	skim := int64(fee) * SkimNum / SkimDen
	wantPay := int64(fee) - skim
	...
	l := New(fee, 0)
	l.SetEpochSource(&mockEpochSource{e: 4}) // the redeemer's clock: both tokens are in window at 4
	// Token A, issued epoch 0, redeemed at epoch 4 (in window, W=4).
	if got := paidOnLane(l, srvA, fetcher, obj, s, 0); got != wantPay {
	...
	if got := paidOnLane(l, srvB, fetcher, obj, s, 4); got != wantPay {
	...
	if _, live := l.paidSerial[paidKey(4, s)]; !live {
		t.Fatalf("BREAK REOPENED: the sweep at epoch 5 forgot the entry for a token " +
	...
	for cur := uint64(5); cur <= 8; cur++ {
		...
		if paid, why := settleOnLane(l, srvC, fetcher, obj, s, 4); paid != 0 {
			t.Fatalf("epoch %d: server C re-collected %d (%s) off token B", cur, paid, why)
		}
	}
}

// TestEveryEvictedEntryIsExpired is the coupling condition stated directly over
...
func TestEveryEvictedEntryIsExpired(t *testing.T) {
	l := New(50_000, 0)
	src := &mockEpochSource{}
	l.SetEpochSource(src)
	s := testSerial(11)
	// One serial, a token at every epoch in 0.8 — the shape the old MIN-epoch keying
	// collapsed into a single entry. The clock walks with them: an anchor above the
	// ledger's own epoch is future-dated, and one below it by more than W is
	// backdated.
	for e := uint64(0); e <= 8; e++ {
		src.e = e
		paidOnLane(l, id(byte(e)+20), id(2), id(7), s, e)
	}
...
// ---------------------------------------------------------------------------
// (control, must keep HOLDING). Two withdrawals, two fees, two payouts is
// CONSERVED. The re-key must not have turned the under-pay it removed into a mint.
// ---------------------------------------------------------------------------
func TestSameSerialTwoEpochsIsStillConserved(t *testing.T) {
	const fee = 50_000
	srvA, srvB, fetcher, obj := id(1), id(2), id(3), id(7)
	s := testSerial(9)
	l := New(fee, 0)
	l.SetEpochSource(&mockEpochSource{e: 4}) // both tokens in window at the redeemer's clock
	l.Register(fetcher)
	...
	paidOnLane(l, srvA, fetcher, obj, s, 0)
	l.sweepExpiredSerials(5)
	paidOnLane(l, srvB, fetcher, obj, s, 4)
...
// ---------------------------------------------------------------------------
// (DISCLOSED, quantified — no fix, by design). Cap griefing costs the skim, and the
// skim funds the griefer's own escrow. This gate PINS the disclosed number so the
// disclosure cannot silently go stale: if a change makes the grief cheaper or the
// refusal different, it reddens. Held-in-tension; the production close is grant = 0.
// ---------------------------------------------------------------------------
func TestCapGriefCostsExactlyTheSkim(t *testing.T) {
	const fee = 50_000
	skim := int64(fee) * SkimNum / SkimDen
	...
	for i := 0; i < maxPaidSerial; i++ {
		...
		if paid := paidOnLane(l, griefer, fetcher, grieferRoot, testSerial(i), 0); paid == 0 {
	...
	paid, why := settleOnLane(l, victim, fetcher, victimRoot, testSerial(999999), 0)
