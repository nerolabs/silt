...
// The relay-lane prepayment anchor: the LEDGER-tier RED-first gates. (conservation re-derived: Δ Σ_L = settled − Σ
// face ≤ 0, equality iff fully consumed — C-1 withdraws the 2026-09-03 "unchanged" corollary), §2.4 (the six doors), §5 (guard window
// == keyset window is THE security property), §9. Build shape: step 3
// (SpendRelayAnchors(anchors []RelayAnchor{Epoch, Serial}, current) (face, reason): verify-none, guard-check all, reserve k, durable
// append all, record all, return k × l.fee; RedeemRelayCredit(relay, ephID, chainValue, budget) pays min(chainValue, budget) to
// acct(relay) only, never acct(ephID)).
//
// RULES: none of these call fund; T-2/T-4/T-5 burn through the REAL
...
// pair total (closed by).
//
// ABLATIONS that must redden: remove the all-or-nothing check (all_or_nothing /
// restore budget:= S × inc; touch acct(ephID).
...
const (
	shippedFee   = int64(50_000)  // the shipped --fee; face = Fee
	shippedGrant = int64(500_000) // the shipped faucet grant (cmd/silt/daemon.go) — the durable buyer's honest source
	// relayChainMaxCredit is relaypay.MaxChainLength × RelayIncrementCredit: the value of a
	...
	relayChainMaxCredit = int64(50_000)
)
...
// tokenChargeFor → ChargePublish takes on the wire). It returns the k anchors the
// relay would have blind-signed for those fees.
...
// TestRelaySettlementRefusesUnanchoredSession is the gate; GREEN by the interim, kept
// as the ablation guard for "touch acct(ephID)": shipped grant, no anchors —
// budget 0 because Σ face of zero spent anchors is 0 — ⇒ paid == 0, the relay's
// balance is unchanged, the ephemeral is never registered (white-box l.accounts),
// and the ledger total does not move.
func TestRelaySettlementRefusesUnanchoredSession(t *testing.T) {
	...
	paid := l.RedeemRelayCredit(relay, eph, relayChainMaxCredit, 0)
	if paid != 0 {
		t.Fatalf("an unanchored session (budget = Σ face = 0) settled %d, want 0 — 's per-session mint", paid)
	}
...
// TestRelayLaneConservesTotalSupplyOnOnePerNodeLedger is the gate, CORRECTED per:
// on ONE per-node ledger, through the REAL ChargePublish burn × k, then open
// (SpendRelayAnchors) → pay c → settle:
//
//	after ≤ before
...
// oracle, not extended. RED on main: the interim pays 0, so the fully-consumed
// case has after == before − k·fee.
func TestRelayLaneConservesTotalSupplyOnOnePerNodeLedger(t *testing.T) {
	for _, k := range []int{1, 3, 6} {
		kFee := int64(k) * shippedFee
		for _, c := range []int64{0, 1, kFee - 1, kFee, kFee + 1, relayChainMaxCredit} {
			t.Run(fmt.Sprintf("k=%d/c=%d", k, c), func(t *testing.T) {
				...
				if reason != "" || face != kFee {
					t.Fatalf("SpendRelayAnchors(k=%d) = (face %d, reason %q), want (k·fee = %d, \"\") — face is an identity with the burn", k, face, reason, kFee)
				}
				...
				if after-before != settled-kFee {
					t.Fatalf("Δ Σ_L = %d, want settled − k·fee = %d − %d = %d", after-before, settled, kFee, settled-kFee)
				}
	...
	t.Run("unanchored total unchanged", func(t *testing.T) {
		...
		l.Register(relay)
		before := sumConserved(l)
		if settled := l.RedeemRelayCredit(relay, eph, relayChainMaxCredit, 0); settled != 0 {
...
// TestRelayCredentialIsSpentOncePerLedger is the gate, ledger tier: a second spend of
// the same (epoch, serial) is refused; a batch containing one spent anchor is
// refused ALL-OR-NOTHING with nothing recorded (T-10's ledger half); and the guard
// at its cap of LIVE entries REFUSES, never evicts (— the lesson: eviction of
// a live entry is a second payout). The two-relay variant (relay B refuses relay
// A's anchor) is a node-tier property and lives in core/node.
func TestRelayCredentialIsSpentOncePerLedger(t *testing.T) {
	t.Run("second_spend_refused", func(t *testing.T) {
		l := New(shippedFee, 0)
		clock := &mockEpochSource{}
		l.SetEpochSource(clock) // the ledger's clock
		a := anchorsAt(0, 0, 1)
		if face, reason := l.SpendRelayAnchors(a); face != shippedFee || reason != "" {
			t.Fatalf("first spend: (face %d, reason %q), want (%d, \"\")", face, reason, shippedFee)
		}
	...
	// presented twice ⇒ face 2·fee ⇒ Δ Σ_L = +fee, a (k−1)·fee mint per burn and
	// was unpinned — removing it left every gate green.
	t.Run("duplicate_within_one_batch_refused", func(t *testing.T) {
		...
		if face, reason := l.SpendRelayAnchors([]RelayAnchor{a}); face != shippedFee || reason != "" {
	...
	t.Run("all_or_nothing", func(t *testing.T) {
		...
		if face, _ := l.SpendRelayAnchors([]RelayAnchor{a2}); face != shippedFee {
			t.Fatalf("setup: spending a2 alone gave face %d, want %d", face, shippedFee)
		}
		...
		if face, reason := l.SpendRelayAnchors([]RelayAnchor{a1}); face != shippedFee {
			t.Fatalf("a1 is no longer spendable after a REFUSED batch (face %d, reason %q) — the refused open recorded an anchor; the fetcher lost anchor 1 because anchor 2 was spent", face, reason)
		}
	})
	...
	t.Run("cap_refuses_never_evicts", func(t *testing.T) {
		...
		l.SetEpochSource(capClock) // the epoch advance below is the LEDGER's
		...
		for _, idx := range []int{0, len(accepted) / 2, len(accepted) - 1} {
			if face, _ := l.SpendRelayAnchors([]RelayAnchor{accepted[idx]}); face != 0 {
				t.Fatalf("at cap, previously spent anchor #%d became spendable again (face %d) — the guard EVICTED a live entry to make room (the eviction pump)", idx, face)
			}
		}
		...
		// (65,536 mod 6 = 4). Fill them one anchor at
		...
		// The epoch advance is the LEDGER's, not an argument: without moving the
		// clock the epoch-1 anchor is refused as future-dated and this arm proves
		// nothing.
		capClock.e = 1
...
// TestRelaySettlementIgnoresForwardedBytesIsBoundedByAnchor is the gate, ledger
// tier: Count = S_max (the WHOLE chain revealed), forwarded = 0 (the ledger
// cannot see forwarding — that is the whole reason the bound must come from
// the anchor), one anchor ⇒ settled ≤ face, and the ledger total moves at
...
func TestRelaySettlementIgnoresForwardedBytesIsBoundedByAnchor(t *testing.T) {
	...
	if face != shippedFee || reason != "" {
		t.Fatalf("SpendRelayAnchors(k=1) = (%d, %q), want (%d, \"\")", face, reason, shippedFee)
	}
	afterSpend := sumConserved(l)

	settled := l.RedeemRelayCredit(relay, eph, relayChainMaxCredit, face)
	if settled > face {
		t.Fatalf("settled %d > face %d for a fully revealed S_max chain — settlement is bounded by S, not by the anchor (-3 re-opened)", settled, face)
	}
	if settled != face {
		t.Fatalf("settled %d, want min(S_max = %d, face = %d) = %d", settled, relayChainMaxCredit, face, face)
	}
...
// TestRelaySettlementNeverLeavesAnAccountNegative is the gate: a zero-budget settle pays
// 0; a zero-balance buyer cannot burn below zero (ChargePublish refuses, the burn is
// REFUSABLE — (iii)); and after the full flow no account on L is below 0, the settle
// debits NOBODY (the burn already happened at issuance), and the ephemeral is never
// registered.
func TestRelaySettlementNeverLeavesAnAccountNegative(t *testing.T) {
	...
	t.Run("full_flow_debits_nobody_at_settle", func(t *testing.T) {
		...
		if buyerAfterBurn != shippedGrant-k*shippedFee {
			t.Fatalf("buyer balance after k burns is %d, want %d", buyerAfterBurn, shippedGrant-k*shippedFee)
		}
		face, _ := l.SpendRelayAnchors(anchors)
		relayBefore := l.Balance(relay)
		settled := l.RedeemRelayCredit(relay, eph, 1<<40, face)
		if settled != k*shippedFee {
			t.Fatalf("settled %d against budget %d for an over-long chain, want the budget %d", settled, face, k*shippedFee)
		}
...
// TestRelayAnchorGuardSurvivesRestart is the gate, restart is not an eviction; the F-4
// creditSpent lesson: a spent anchor is in the durable PaidSerialStore BEFORE
// SpendRelayAnchors returns; a new ledger loaded from the same store refuses the
// re-spend; an attached-but-unloaded store refuses EVERY spend (the ledger does
// not yet know what it already accepted); a store whose Append fails refuses the
// spend (an under-pay, never an over-pay).
func TestRelayAnchorGuardSurvivesRestart(t *testing.T) {
	store := &memStore{}
	l1 := New(shippedFee, shippedGrant)
	l1.SetPaidSerialStore(store)
	if err := l1.LoadPaidSerials(); err != nil {
		t.Fatal(err)
	}
	a := anchorsAt(0, 0, 1)
	if face, reason := l1.SpendRelayAnchors(a); face != shippedFee {
		t.Fatalf("first spend: (face %d, reason %q), want (%d, \"\")", face, reason, shippedFee)
	}
	found := false
	...
	l2 := New(shippedFee, shippedGrant)
	...
	l3 := New(shippedFee, shippedGrant)
	...
	if face, _ := l3.SpendRelayAnchors(b); face != shippedFee {
	...
	l4 := New(shippedFee, shippedGrant)
...
// TestRelayAnchorGuardWindowMatchesKeysetWindow is the gate, the
...
func TestRelayAnchorGuardWindowMatchesKeysetWindow(t *testing.T) {
	...
	l := New(shippedFee, 0)
	src := &mockEpochSource{} // the ledger reads its clock; the loop moves it
	l.SetEpochSource(src)
	a0 := []RelayAnchor{{Epoch: 0, Serial: serial}}
	if face, reason := l.SpendRelayAnchors(a0); face != shippedFee {
