...
// rule; the research is the authority on why this
// shape. Two rules, both load-bearing:
//
...
// wash bound follows: a colluding fetcher+server pair pays fee F for the token
// and redeems F − skim, a strict loss of F·SkimNum/SkimDen per loop.
//
// SUPERSEDE: a delivery paid by a redeemed receipt is never ALSO self-credited. The
// serve path self-records one credit per Dλ bytes as it serves (RecordServeToObject, two
// floors over the lane's byte accumulator since) — an unfunded self-mint that predates
// PoD and is exactly the "per-receipt subsidy" conservation bans, if it stacked with the
// receipt credit. So every object-aware serve is tracked as PROVISIONAL per (requester,
// root), and a redeem for that delivery reverses the provisional self-credit before
// applying the conserved one (the form (i) — robust when the server cannot know at serve
// time whether a receipt will follow). A serve never redeemed keeps its self-record:
// that is the legitimate unwitnessed bilateral fallback, unchanged.
//
...
// to reverse a re-served lane (the eviction-reversal fix;
...
// laneFor below), so an evicted lane is left in the
...
// The paid-serial guard's DERIVED cap (economist , residual; re-derived for,
// and later corrected for quantization). The guard
// holds TWO populations, both spent at session OPEN: delivery anchors
// and relay anchors. The cap must DOMINATE the honest live set so that EXPIRY — not the
// cap — does the eviction work. Two bounds on that live set, and which one binds:
//
...
// core/node/chainrole.go — never on a clock; still owed, with), so this corner
// takes an upper bound on it. At 125 MiB/s and a one-hour interval it is 2,160
// entries; 4× headroom is 8,640. This term bounds ONLY the φ = 1 corner
// (T-QUANT): under spend-at-open an anchor is consumed per SESSION, and the
// per-session face consumption on the real fetch path is φ ≈ 5×10⁻⁴ … 8×10⁻³,
// which puts the byte-rate form 2.7×–43× OVER the floor. It is kept as the
// derivation's lower corner, not as a dominance claim.
//
// 2. The SESSION-COUNT bound, which is what actually caps occupancy
...
// fresh-identity arrival rate A the faucet bounds. That relation is
// enforced at start-up — `capacity × (grant/fee) × (W+1) ≤ MaxPaidSerial/4`,
// cmd/silt/daemon.go, gated by cmd/silt/faucet_test.go — against the
// RUNTIME faucet capacity, which no compile-time constant here can see. The delivery
// remainder is refunded at ANCHOR EXPIRY — the same window this guard sweeps on —
// so a face is accounting-neutral and the (g/f) term keeps bounding CONCURRENCY:
// the assertion survives verbatim (T-DEPOSIT, refund–4). The
...
// (re-priced; the residual stays open).
//
...
//
// A liveness-only residual: a faucet-funded flood of opens can fill the
// shared guard for ≤ W+1 epochs and both lanes REFUSE, never evict. The faucet re-prices it
// through bound 2; the composed relation `C ≤ maxPaidSerial · (f/r) / ((W+1) · B_floor)`
// is what closes it. Per-lane refusal counters and live counts (GuardFullRefusalsByLane,
// LivePaidSerialsByLane) show an operator WHICH population fills the guard.
const (
	...
	// (relaypay.RelayIncrementBytes/RelayIncrementCredit). Both are duplicated
	// literals — core/credit imports neither core/relaypay nor cmd/silt — pinned to
	// their sources by TestPaidSerialCapLiteralsMatchTheirSources in cmd/silt, the one
	// package that can import all three.
	...
	// capHeadroom is the multiple of the honest live set the cap must clear: ≥ 4.
	capHeadroom = 4
)

// DeliveryBytesPerAnchor is ⌊f/p⌋·U: the bytes ONE delivery anchor funds at one server
// (12.21 GiB at the shipped price). RelayBytesPerAnchor is the relay twin, f × the
...
// MaxPaidSerial and PaidSerialWindow are exported READ-ONLY for the start-up assertion in
// cmd/silt: `capacity × (grant/fee) × (W+1) ≤ MaxPaidSerial/4` ties the faucet's burst,
// the grant, the fee and the guard cap — four constants in three packages re-tuned by
// three different processes — so raising any one of them past the guard's cliff refuses
// to start instead of silently opening a hole (economist §2.3).
const (
...
// the 2026-09-04 requires. Never read by an accounting rule.
type guardLane uint8

const (
	laneDelivery guardLane = iota // a paid delivery serial (flat redeem) or a delivery anchor
	laneRelay                     // a relay anchor
)
...
// requester) into ONE ledger get distinct lanes. Per-node prod has
...
type provisionalServe struct {
	server ports.NodeID
	bytes  int64 // the lane's byte accumulator: every serve on this lane, lifetime of the lane
...
// the flat redeem-in-window and eviction. The settlement reverses PER INCREMENT
// through reverseLane directly (deliveryanchor.go). Keeping one implementation is what
// makes the escrow floor identical at every site.
...
func (l *Ledger) reverseLane(server ports.NodeID, root ports.Hash, net, skim int64) {
	...
	if e, eok := l.escrow[root]; eok {
		...
		e.balance -= r
		// a reversal only ever claws back the object's own auto-SKIM, never an
...
//
// EVICTION REVERSAL (the money-pump fix): when the map is at cap and
// the oldest lane is FIFO-evicted, its eager self-mint is REVERSED before the
// lane is forgotten — not left on the server's balance. This closes the
// double-pay: after eviction an evicted lane is in the same accounting state as
// "never served" (mint gone, skim floored-reversed, entry gone), so a later
// redeem for that lane pays the conserved leg ONLY and mints nothing. Rule (b):
// one delivery, one payment. The cost is the unwitnessed bilateral fallback — an
// evicted, never-redeemed serve loses its self-record — an under-pay at the
// >maxProvisional tail, never an over-pay, never a denial. Verified by
// TestMoneyPumpConservation.
...
// point that closes the eviction-reversal break: the map and the live entries of provOrder
...
// misleading because nothing was banked.
//
...
const (
	...
	ReasonNoFee = "no-fee"
	// ReasonGuardUnloaded: a durable guard store is attached but its contents
	// have not been loaded yet, so this ledger does not know what it already
	// paid. Refuse rather than pay — a redeem before load completes is exactly
	// the restart window the store exists to close.
	//
	// NOT AN OPERATOR SIGNAL — it is UNREACHABLE on the shipped daemon. cmd/silt
	// opens and loads the store BEFORE the node exists and returns a start-up
	// error if the load fails, so no receipt can arrive with the guard
	// attached-but-unloaded. It is correct defence in depth for an EMBEDDER that
	// wires the ledger itself, and the record must not count it as something an
...
// Delivery = paid delivery serials and delivery anchors,
// relay = relay anchors. Observability only.
...
// can see WHICH lane fills a shared guard. Entries restored from
...
// rule requires for "evicted ⇒ expired ⇒ un-redeemable".
//
...
// nothing). Pinned RED-before/GREEN-after by
...
func (l *Ledger) sweepExpiredSerials(current uint64) {
	...
	//
	// TWO ERROR CLASSES, NOT ONE. A failed compaction is not an accounting error: the port contract (ports.PaidSerialStore, the
	// handle clause) guarantees the store is then EITHER still appendable with the log a superset of the live set — which only ever
	// refuses more — OR failing every Append. So a Compact error is recorded here (the WARN; this package has no logger, counters are its
	// observability surface) and never refuses a payout by itself. A store that is actually broken reports it the one way the anchor spend
	// already refuses on: Append fails, and spendAnchors returns ReasonGuardStore. Refusing on every Compact error instead was REFUSED by
	// because it is a self-inflicted liveness break at exactly the load where a benign compaction fails. Two gates cover it:
	// a benign failure still pays, and a broken store pays 0.
...
// batch must fit or none is recorded (SpendRelayAnchors). Same rule: sweep
...
// sweepIfEpochAdvanced runs sweepExpiredSerials AT MOST ONCE PER EPOCH.
...
// that early.
//
// Two callers drive it: the epoch-watermark advance on the redeem path (the retention bound) and reservePaidSerial at the cap (the liveness bound).
func (l *Ledger) sweepIfEpochAdvanced(current uint64) {
	if current > l.sweptEpoch {
		...
		l.releaseDueRefunds() // Deposits whose anchors just left the window return now
	}
}
...
//
// WHY THE EPOCH IS IN THE KEY, 2026-09-03. Keyed by the serial alone, the entry's
// expiry epoch was whatever the FIRST redeem supplied and later redeems returned early
// — i.e. the MINIMUM epoch over the tokens sharing a serial. The withdrawer picks both
// the serial and the epoch, so it can hold two valid tokens on ONE serial at two
// epochs; the low-epoch entry then expired and the guard forgot a serial for which a
// still-in-window token existed. "Evicted ⇒ expired" was false. Keyed by the token, an
// entry is removed only once ITS OWN issue epoch is outside the band, which is the
// coupling condition the rule requires. The pump the guard closes is
// unaffected: the same TOKEN redeemed at two servers is one key either way, and a
// second token on the same serial costs a second withdrawal fee.
...
func (l *Ledger) LoadPaidSerials() error {
	...
	l.paidSerial = fresh
	// Each restored DELIVERY entry is an anchor whose session state did not
	// survive the restart: sessions and deposits are ephemeral; the guard is
	// durable so nothing is re-spent, so the count is an upper bound on lost
	// deposits — surfaced at boot and on /api/status so the loss is
	// operator-visible, never silent. Relay anchors are EXCLUDED: they keep the
	// burn and never had a deposit, and the durable record now carries the lane
	// that says which is which (closed; the pre-bump restore counted both).
