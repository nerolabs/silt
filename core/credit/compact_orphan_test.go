...
//Gates. the gate lives at adapters/guardstore/compact_orphan_test.go — this file
//is the ledger-tier half.
//
// AS LITERALLY STATED IS ALREADY GREEN ON MAIN, AND STAYS THAT WAY, NOT A NEW
// GATE HERE. Thedecision's simplest contract — "a store whose Append returns a
// non-nil error" ⇒ RedeemDeliveryCreditReason returns ReasonGuardStore, 0 paid — is
// already covered by TestGuardEntryIsDurableBeforeTheCreditMoves
// (core/credit/credit_lane_regressions_test.go, via failingStore, which fails
// Append unconditionally). Re-verified green as part of this work: `go test -run
// TestGuardEntryIsDurableBeforeTheCreditMoves` PASS. It is unconditional on Compact's
// own outcome — delivery.go's addPaidSerial already refuses on any Append error,
// regardless of why the store is in that state — so it does not exercise that shape
// specifically and is not repeated here.
//
// What IS missing, and is the actual ledger-side gap: sweepExpiredSerials
// discards Compact's error UNCONDITIONALLY (delivery.go, `_ =
// l.paidStore.Compact(.)`), for every class of failure, benign or broken. So today:
//
// - (a BENIGN, pre-rename Compact failure must not refuse payouts) holds only
// Because ALL Compact errors are ignored — not because the ledger tells the two classes apart.
// Written first, per decision ("write it before the ledger-side change"), so the
// anti-over-correction property is pinned before anyone teaches the ledger to read Compact's
// error at all. Redirected (the literal form above is already green): a BROKEN store — one left
// in the orphaned state described above, where Append keeps returning nil while its records
// become unrecoverable — is NOT observable by the ledger today. The redeem that follows reports
// ReasonPaid while the guard entry it believes it just wrote can never be restored. This is the
// gap the ledger-side error-class split (decision §1, "stop discarding the error... only refuse
// payouts when the store reports itself broken") exists to close.
...
// benignCompactFailure models a PRE-rename Compact failure (decision §1: temp-file
...
func (b *benignCompactFailure) Compact([]ports.PaidSerial) error {
	...
	// over-refuses" case (decision §1's table) that
	// fail-closed-on-any-Compact-error was REFUSED for conflating with a broken
	// store.
	return errors.New("injected: pre-rename compaction failure (disk full)")
}

// TestBenignCompactionFailureDoesNotRefusePayouts is, the
...
func TestBenignCompactionFailureDoesNotRefusePayouts(t *testing.T) {
	const fee = 50_000
	...
	if paid != wantPay || reason != ReasonPaid {
		t.Fatalf("a BENIGN (pre-rename) compaction failure must not refuse payouts: "+
			...
			"by the decision precisely for this case: a pre-rename failure leaves a superset "+
...
// brokenAfterCompact models the orphaning failure itself at the ledger's
...
//
// WHAT "REPORTS ITSELF BROKEN" MEANS, AS SHIPPED. The port contract's handle
// clause (ports.PaidSerialStore.Compact) gives a store exactly two legal states after
// a failed Compact: still appendable-and-reachable, or failing every Append. A store
// in the orphaned state therefore FAILS Append — that is
...
// sound against a store that lies about durability, and the redirected gate below was
// RED against it precisely because the ledger had no class signal. Under the shipped
// contract the gate is GREEN through the existing ReasonGuardStore path, with no new
// ledger machinery. `lost` is kept so the gate can still assert that nothing was
// silently written past the break.
...
func (b *brokenAfterCompact) Compact(live []ports.PaidSerial) error {
	...
	return errors.New("injected: post-rename re-open failed (decision P7)")
}

// TestBrokenStoreMustBeObservableByTheLedger is redirected (see file header: the
// literal form is already green via TestGuardEntryIsDurableBeforeTheCreditMoves). It
// drives the real orphaning sequence through the ledger — a band advance that sweeps and compacts, where the compact
// fails in the orphaning way — and asserts the invariant the missing port clause exists to
// state: a redeem the ledger reports as PAID must have a guard entry a fresh Load can
// recover. It was RED on main against a double whose Append returned nil past the break
// (the ledger had no signal beyond Append's own return value). Under the shipped contract
// the double fails Append past the break and the gate closes through ReasonGuardStore — see
// the brokenAfterCompact comment. Ablation: with the
// double's Append restored to return nil, the gate is RED again, which is the port clause's
// own statement that a lying store is unsound.
func TestBrokenStoreMustBeObservableByTheLedger(t *testing.T) {
	const fee = 50_000
	...
	// The invariant (decision §1's missing port clause, restated at the ledger): a
	// redeem the LEDGER reports as PAID must have left a guard entry a FRESH Load
	// can recover. If the store paid nothing (refused), that's a different,
	// acceptable outcome (an under-pay) and this check does not apply — but the
	// refusal must be the store's own class, and nothing may have been paid on a
	// record past the break.
	...
	if paid > 0 && reason == ReasonPaid {
		...
		if !visible {
			t.Fatalf("at the ledger tier: RedeemDeliveryCreditReason reported "+
				...
				"(TestGuardEntryIsDurableBeforeTheCreditMoves) catches this too.",
				paid, testSerial(2))
		}
	}
}
