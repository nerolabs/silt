...
// detector A4 — escrow laundering by self-repair, the LEDGER half.
//
// funded splits into a PREPAY leg and a SKIM leg, because the wash loop's
// recoverable money is the skim.: bounties released to a repairer that had already
// fetched from this node — a SHAPE, never a detection, never a disbursement input.
//
// lives in core/node (the ledger does not know its own node id): see
// TestBountyPaidToSelfCountsTheJudgeAsHolder.
//
// ABLATIONS run RED before this shipped:
