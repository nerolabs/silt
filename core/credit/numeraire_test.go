...
// numéraire gates inside core/credit.

// TestChunkingInvariance: serving N bytes in M calls mints ⌊N/Dλ⌋ (plain path)
// and the same two floors (object path) independent of M. Ablation: a per-call
...
// TestSupersedeLeavesNoRemainder: serve a lane to just under a mint boundary,
// redeem a witnessed receipt (the lane dies), serve again — the post-redeem lane starts
// at ZERO. Ablation: keep the remainder on the account ⇒ one extra credit appears,
// the only double-pay path this mechanism has.
func TestSupersedeLeavesNoRemainder(t *testing.T) {
	const fee = 50_000
	l := New(fee, 0)
...
// TestRepairBountyBaseIsPricedInTheFetchPrice: base == c·shardBytes/(U/p) (re-based by F1
// on 2026-09-12): a 64 MiB shard pays 256 credits; a shard one byte short of a credit pays
// 0. Ablation: leave the base at shardBytes ⇒ RED (67 million).
func TestRepairBountyBaseIsPricedInTheFetchPrice(t *testing.T) {
...
// two geometries the record argues about. This gate shipped
...
//
// The 2026-09-06 geometry correction still holds and is what makes the numbers computable: a
// shard is a WHOLE ciphertext chunk, so the former default's shard is 65,552 B and the
// shipped default's is 262,160 B. Only the coefficient's basis changed. The publish
// threshold derives from the same arithmetic and rises 26,199 → 262,128 B, which IS the
// accepted cost of the re-pricing.
...
// TestSkimAccumulatesAcrossServes: the escrow leg floors the LANE's byte
// accumulator, so eight serves of one Dλ each fund one credit of durability.
// Ablation: a per-call skim floor ⇒ RED (0).
func TestSkimAccumulatesAcrossServes(t *testing.T) {
	l := New(50_000, 0)
	server, req := id(1), id(2)
...
// TestByteObservablesStayBytes: ServedBytes / FetchedBytes are bytes at any Dλ —
// the census estimand. Ablation: route either through the mint ⇒ RED.
func TestByteObservablesStayBytes(t *testing.T) {
	l := New(50_000, 0)
	server, req := id(1), id(2)
...
// TestGLambdaServeMintTelemetry: the instrument the research asked for.
func TestGLambdaServeMintTelemetry(t *testing.T) {
	...
	// Both legs report their own waiting bytes: one Dλ on a fresh lane mints
