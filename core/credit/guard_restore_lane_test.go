...
// The guard's per-lane live counts must survive a restart.claude/ §4 measured it: "lanes before restart (delivery, relay) = (2, 1);
// after restore = (3, 0)" — LoadPaidSerials rebuilt every entry as laneDelivery because the durable record carried no lane, so
// RestoredGuardEntries and LivePaidSerialsByLane both conflated the two populations. The lane is observability only (no accounting
// rule reads it), which is exactly why nothing else went red.
//
...
func TestRestoredGuardEntriesKeepTheirLane(t *testing.T) {
	...
	if face, reason := l1.SpendDeliveryAnchors(server, anchorsAt(0, 0, 2)); face != 2*shippedFee {
		t.Fatalf("delivery open: (face %d, reason %q), want (%d, \"\")", face, reason, 2*shippedFee)
	}
	if face, reason := l1.SpendRelayAnchors(anchorsAt(0, 2, 1)); face != shippedFee {
		t.Fatalf("relay open: (face %d, reason %q), want (%d, \"\")", face, reason, shippedFee)
	}
	...
	if d, r := l2.LivePaidSerialsByLane(); d != 2 || r != 1 {
		t.Fatalf("after restart (%d, %d), want (2, 1) — the restore conflates the two populations ", d, r)
	}
