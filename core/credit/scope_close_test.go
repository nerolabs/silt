...
// The scope-close pin for. The credit ledger stays EPHEMERAL through the
// release candidate. That is a POSTURE, not an accident, and this is
// what tests it.
//
// The decision re-arms all three scope items automatically on the
...
//	T2 · the economy-ON default flip (cmd/silt -economy defaults true)
...
// the FP-2 obligation set (§4.7 of the research) in the SAME PR: provisional
// + provOrder/provIndex/provHead; epochWatermark + sweptEpoch under T-W; l.order;
// FP-1 Bank.spent; a account/escrow cap and eviction rule (build-immutable #8);
// and the exclusion of the standing fields.
//
//
...
//
// RUNTIME GATE: TestCrashBetweenTheGuardAppendAndThePayBurnsTheReceipt
// (crashwindow_test.go) is the behavioural cover — it MEASURES the
...
// TestTheLedgerStaysEphemeralUnderDFP2Scope is arms T1 and T3.
func TestTheLedgerStaysEphemeralUnderDFP2Scope(t *testing.T) {
	...
	// The ONE durable store permits: the paid-serial guard. It stores SERIALS (a
	// spend guard), never a balance, so it does not persist ledger state.
	...
	if got := strings.Join(all.storeFields, ","); got != wantField {
		t.Fatalf("SOURCE GATE: T1 — credit.Ledger's durable-store FIELD set is %q, want exactly %q.\n"+
			"keeps the ledger EPHEMERAL through the RC. A new store field re-arms FP-1, FP-2 "+
			"and. Do NOT narrow this gate: land the FP-2 obligation set in the SAME PR "+
			"(provisional + provOrder/provIndex/provHead; epochWatermark + sweptEpoch under T-W; l.order; "+
			"FP-1 Bank.spent; a account/escrow cap and eviction rule; the standing fields EXCLUDED). "+
			"Route the change to the research — the eviction rule is build-immutable #8 and must be "+
			"first. Brief: FP-2-redeem-atom-and-ledger-durability-OWNE.md",
			got, wantField)
	}
	if got := strings.Join(all.storeSetters, ","); got != wantSetter {
		t.Fatalf("SOURCE GATE: T1 — the *Ledger store-ATTACH method set is %q, want exactly %q. "+
			"Same obligation set as the field arm above; see ", got, wantSetter)
	}
...
// TestTheEconomyDefaultStaysOff is arm T2. The economy-ON default flip is
// the first named re-arm trigger in
func TestTheEconomyDefaultStaysOff(t *testing.T) {
	root := gfp20RepoRoot(t)
	...
	if bad != "" {
		t.Fatalf("SOURCE GATE: T2 — the -economy flag no longer defaults to false (%s). That is the  "+
			"economy-ON default flip, the FIRST re-arm trigger named by: FP-1, FP-2 and "+
			"are live again and the ledger's ephemerality is no longer an accepted cost. "+
			"Land the FP-2 obligation set before flipping this default.", bad)
	}
}

// TestScopeScannerHasTeeth injects a synthetic file carrying each violation and
// asserts the scanner flags all three. Without this, a scanner that silently
// stopped matching would read as a green posture.
func TestScopeScannerHasTeeth(t *testing.T) {
