...
// Invariant A for the B_bootstrap instrument's two exported ledger methods.
//
// THEY ARE CLASSIFIED HERE RATHER THAN IN invariant_a_test.go BECAUSE THEY ONLY EXIST
// HERE. The instrument compiles only under the `bbootstrap` build tag, and
// TestInvariantA_EveryLedgerMethodClassified checks standingClassification in both
// directions — every *Ledger method needs an entry, and every entry needs a method —
// so an untagged entry would fail the reverse check in a default build. This init adds
// them exactly when the methods are present.
//
...
// one: TestBBootstrapSnapshotWritesNothing deep-compares the account map across it,
