...
// The minimum-requester floor and the dead discriminator.
//
...
// objects and the difference is what this gate is about.
...
// TestFloorWithholdsEveryCensusCountBelowRMin is the gate at the core tier: at
...
// than over the grid.
func TestFloorWithholdsEveryCensusCountBelowRMin(t *testing.T) {
	if BBootstrapMinRequesters < 10 {
		t.Fatalf("R_min = %d, below the floor of 10", BBootstrapMinRequesters)
	}
	...
	t.Logf("R_min = %d, derived as ceil(1/(1-q)) at the edge q >= 0.90; suppressed at %d, published at %d",
		BBootstrapMinRequesters, BBootstrapMinRequesters-1, BBootstrapMinRequesters)
}

// TestFloorIsDerivedNotChosen pins the DERIVATION, not just the number. R_min is
// ceil(1/(1-q)) evaluated at the lower edge of the range the research covers (q >=
// 0.90), which is 10; the census must carry far more than one cell's requirement, so a
// census-wide floor of 10 is strictly dominated by the fit's own need and costs it
...
func TestFloorIsDerivedNotChosen(t *testing.T) {
	...
	if want < 10 {
		t.Fatalf("the derived floor is %d, below the minimum of 10: the derivation was re-run at a q below the edge (q >= 0.90) without re-deriving it", want)
	}
}

// --- the dead discriminator ---------------------------------------------------

// TestServedBytesSelectsTheEmptySet is. The splitting the census by `servedBytes > 0`
// to separate infrastructure fetchers from viewers: RecordServe and RecordServeToObject
// credit the SERVER, and on a serving node the server is always that node itself, so
// the predicate holds for exactly one account — the node's own — and that account is
// never in the census, because self-serving returns early and leaves its fetchedBytes
// at zero. The split partitions the census into everyone and nobody.
//
...
func TestServedBytesSelectsTheEmptySet(t *testing.T) {
	...
	t.Logf("servedBytes > 0 selects %d of %d accounts, and that one is not a census member — the proposed split is everyone vs nobody",
