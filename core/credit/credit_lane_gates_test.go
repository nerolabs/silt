...
// C3 close — the credit-layer gates for the confirmed breaks and for
// the measured residuals. Each names its ablation.
//
// Gate G TestSharedKeyRotationDoesNotReopenThePump (break 1, probe G)
// Gate D TestGuardHealsUnderASharedKey (probe D)
// Gate E TestSweepRunsAtMostOncePerEpoch (measured 1.32 ms/redeem)
// — TestRefusedRedeemLeavesOnlyTheBilateralFallback
// — TestCapFullRefusalIsObservable
...
// TestSharedKeyRotationDoesNotReopenThePump is GATE G.
//
...
func TestGuardHealsUnderASharedKey(t *testing.T) {
	s := newSharedKeyScene(t)
	l := New(50_000, 0)
	src := &mockEpochSource{} // the ledger's own clock walks with the keyset's
	l.SetEpochSource(src)
...
// TestSweepRunsAtMostOncePerEpoch is GATE E. At a full cap of STILL-LIVE
...
func TestSweepRunsAtMostOncePerEpoch(t *testing.T) {
	...
	src := &mockEpochSource{e: epoch} // the ledger reads its clock; the test moves it
	l.SetEpochSource(src)
...
// TestCapFullRefusalIsObservable pins the FINDING that a cap-full refusal returned a
// bare 0, indistinguishable from self-delivery, an already-paid serial, or a zero fee
// — and surfaced at the node inside a log line reading "delivery receipt banked".
// Each non-paying path must now name itself.
func TestCapFullRefusalIsObservable(t *testing.T) {
	l := New(50_000, 0)
	src := &mockEpochSource{} // the backdated case moves the ledger's clock
	l.SetEpochSource(src)
	srv, fetcher := id(1), id(2)
	obj := id(7)
...
// TestRefusedRedeemLeavesOnlyTheBilateralFallback bounds the combination the
// research measured and no shipped gate exercised: a PROVISIONAL serve lane plus a
// REFUSED redeem. The refusal returns before the supersede block, so the serving
// node keeps its unreversed 1-credit/byte self-mint and Σ is up by exactly `bytes`.
//
// THE BOUND (why this is disclosed and gated rather than fixed): the leak EQUALS
