...
// Expired guard entries must be retired on
...
func TestExpiredGuardEntriesAreRetiredOnTheBandAdvanceNotOnlyAtTheCap(t *testing.T) {
	const fee = 50_000
	store := &memStore{}
	l := New(fee, 500_000)
...
// TestTheSweepStaysAtMostOncePerEpoch: the new trigger must not turn every redeem
...
func TestTheSweepStaysAtMostOncePerEpoch(t *testing.T) {
	l := New(50_000, 500_000)
	src := &mockEpochSource{e: 10} // the ledger reads its clock; the test moves it
	l.SetEpochSource(src)
	srv, fetcher, obj := id(1), id(2), id(7)
	l.Register(srv)
	l.Register(fetcher)
