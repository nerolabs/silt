...
// The gossip stamp's read must not WRITE.
//
// MECHANISM. Node.send stamps every outbound message with this node's two work counters
// (core/node, rows 8-9). The obvious readers, ServedBytes and RepairsDone, go through
// acct, and acct calls Register, and Register CREATES an account: on an unconfigured
// ledger it hands out the starter grant, and on an faucet-configured one it sets
// grantPending and increments grantsPending. So reading a counter to fill in a
// FindNode's gossip fields would mint an account and move faucet accounting as a side
...
// TestWorkSampleDoesNotRegisterAnAccount is the gate. ABLATION (run before shipping):
...
func TestWorkSampleDoesNotRegisterAnAccount(t *testing.T) {
