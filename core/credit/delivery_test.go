...
func TestDeliveryCreditNeverTouchesStanding(t *testing.T) {
	...
	for i := 0; i < 1_000; i++ { // a heavy delivery volume, one anchor each
		if paid := paidOnLane(l, server, fetcher, obj, testSerial(501_000+i), 0); paid <= 0 {
...
func TestDeliveryCreditIsConserved(t *testing.T) {
	...
	paid := paidOnLane(l, server, fetcher, obj, testSerial(502), 0)
...
func TestSelfDeliveryPaysNothing(t *testing.T) {
	l := New(50_000, 0)
	n := id(1)
	if paid := paidOnLane(l, n, n, id(7), testSerial(520), 0); paid != 0 {
...
func TestWitnessedReceiptSupersedesServeSelfRecord(t *testing.T) {
	...
	paid := paidOnLane(l, server, fetcher, obj, testSerial(503), 0)
...
func TestWashLoopIsAStrictLoss(t *testing.T) {
	...
	paidOnLane(l, server, fetcher, obj, testSerial(504), 0)
...
func TestPaidBountyIsNotRecoverableBySupersede(t *testing.T) {
	...
	paidOnLane(l, server, fetcher, obj, testSerial(505), 0)
...
func TestProvisionalCapIsBoundedAndDeterministic(t *testing.T) {
	...
	paid := paidOnLane(l, server, first, obj, testSerial(506), 0)
	...
	refPaid := paidOnLane(ref, server, first, obj, testSerial(507), 0)
