...
//ON THE ANCHORED SESSION LANE: a server that is NOT PAID must never end better
//off than one that is. Research, item 4.
//
// THE LEVER THESE GATES CLOSE. RecordServeToObject self-credits — the unfunded
// self-mint the conservation rule bans as a standalone subsidy — and only
// a settlement reverses it. If not-being-paid can be worth more than being paid,
// the whole witnessed lane is optional for the server, and the server can trigger
// the no-payment branch itself (fill your own guard with junk anchors; the red team
// measured the grief cost at exactly the skim).
//
// C1 — WHAT MOVED, AND WHAT DID NOT. On the retired FLAT leg the property was stated
// as an ORDERING inside one call: every guard refusal below the supersede, so a
// refused receipt "gives up" its self-mint. That ordering does not exist on the
...
// the unwitnessed bilateral fallback — where keeping the mint is the
// behaviour (gates). The economic question is unchanged and is asserted here
// in the form that is true on the lane:
//
...
// (B-1 /), which pins the same +75 margin at 64 MiB from the other direction.
//
// The root cause of the original break was the FLAT fee against a byte-proportional
// mint (residual). The lane's payment is byte-proportional too (min(count·p, budget)),
// which is why the lever is closed structurally now and not by an ordering rule.
//
// ABLATION (run RED 2026-09-08): re-price the self-mint to 1 credit per byte — the
// earlier shape the break was measured on — by setting ServeMintBytesPerCredit to
// 1 in numeraire.go. The mint then dwarfs the settlement at every size and
// TestNotBeingPaidIsNeverBetterThanBeingPaid goes RED at the first size with
...
// TestNotBeingPaidIsNeverBetterThanBeingPaid is gate (i)+(iv) in its lane form, at
...
func TestNotBeingPaidIsNeverBetterThanBeingPaid(t *testing.T) {
	const fee = 50_000
	for _, b := range []int64{mintUnit, 8 * mintUnit, 64 << 20} { // sizes that mint (below 8·Dλ/7 the lane mints nothing yet)
		...
		if notPaid >= paid {
			t.Fatalf("B=%d: NOT being paid nets the server %d and being paid nets %d — "+
				...
				"operator-triggerable opt-out of the conservation rule.",
				b, notPaid, paid)
		}
		...
		// not run — the conservation walk over the whole cycle is the gate's.)
...
// TestSettlingBeatsRefusingAtTheProductionChunk pins the exact arithmetic the lever turned on, so a
...
func TestSettlingBeatsRefusingAtTheProductionChunk(t *testing.T) {
	const fee = 50_000
	const b = int64(64 << 20)
	const wantMint = 149  // ⌊7·b/(8·Dλ)⌋ at Dλ = 393,216 (58,720,256 at λ = 1)
	const wantPaid = 224  // ⌈b/U⌉ increments settled, gross less the 1/8 skim
	const wantMargin = 75 // the accept margin (B-1 / this gate pins the same number)
...
// TestOneSettlementReversesExactlyOnce is gate (ii) in its lane form. The reversal
...
func TestOneSettlementReversesExactlyOnce(t *testing.T) {
	const fee = 50_000
	l := New(fee, 0)
...
// TestRecordedTokenDoesNotReverseAgain is the ordering constraint a naive fix breaks: a
// token already recorded in the guard must be refused BEFORE anything is reversed. The
// provisional lane key is (server, requester, root) — a lane, not a delivery — so a
// receipt cannot say which serve it acknowledges. If a recorded anchor could reach a
// settlement it would reverse a RE-SERVED lane's fresh self-mint, for free, on every
// re-presentation. On the session lane the refusal is at the OPEN (ReasonAlreadyPaid),
// which is strictly earlier than the flat leg's screen.
func TestRecordedTokenDoesNotReverseAgain(t *testing.T) {
	const fee = 50_000
	l := New(fee, 0)
