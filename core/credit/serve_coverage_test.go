...
// detector A2 — supersede suppression.
//
...
//
// ABLATIONS run RED before this shipped, one per test:
...
func TestServedByteSplitIsExactAcrossEveryTerminalState(t *testing.T) {
	...
	// THE JUSTIFYING CLAUSE, driven. The whole reason coverage's denominator is
	// object-aware bytes rather than total served bytes is that a node serving
	// MANIFEST chunks — no proof-anchored root, no lane, never witnessable (core/node
	// chooses RecordServe for them) — must not be scored as if it were suppressing.
	// Until now no test drove the plain path, so the clause was the untested half of
	// the design. A plain serve must move the unwitnessable floor and leave a
...
// and the suppressing server ends STRICTLY poorer.
func TestSuppressionShowsAsCoverageBelowOne(t *testing.T) {
	...
	if margin := bank.Balance(server) - suppress.Balance(server); margin <= 0 {
		t.Fatalf("bank − suppress = %d, want STRICTLY > 0: suppression must never pay better", margin)
	}
}
