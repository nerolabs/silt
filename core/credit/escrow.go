...
// auto-skim of the object's own serving revenue, with a rarest-shard multiplier
// that pays most to repair the stripe closest to data loss.
//
...
type objectEscrow struct {
	...
	// The two funding legs. Lifetime credits deposited, SPLIT
	// by where they came from, because the wash loop's recoverable money is
	// the skim and not the prepay: with one combined `funded` there is no
	// denominator for "escrow recovered by self-repair" and the S5 qualifier
...
// paths is exactly the drift the split exists to remove.
//
...
// price `base = c × shardBytes`: the base is RELATIVE to the shard the payee moves,
...
// reconstruction as such earns nothing by design repair.md §8b; C-5 G1 2026-08-27. The
// holder's marginal act is ONE shard moved inbound, so the coherent basis is one shard
// of witnessed fetch price:
//
...
//
// THAT DETERMINACY IS THE BASE'S, and nothing above it. The disbursed price is base ×
// RarestShardMultiplier, which runs to n−k+1: measured against the bytes the payee itself
// moved, 7× at the shipped k = 10, n = 16 and 33× at k = 1, n = 33. F1 makes it strictly
// better — the same path reached 70× at the default before — but the multiplier is
// separately settled and unchanged here, so "no value is chosen" must never be published
// without this qualifier.
//
// ★ THE HOLDER IS NOT ALWAYS A DIFFERENT NODE, AND F1 DOES NOT HOLD WHERE IT IS NOT. The
// paramedic KEEPS the shard it rebuilt and names ITSELF the payee whenever core/node
// (*Node).selfHoldEligible passes — the repair economy on, its own failure domain
// non-zero, and that domain unused by this stripe — and that path is tried BEFORE remote
// placement (core/node/repair.go, the (a-domain-fresh) block). `-domain` is a shipped
// flag and is set in integration/sybil/docker-compose.yml and
...
//
// It is FILED, NOT SETTLED HERE:, coupled to, which is why the meters cannot tell the
// two payees apart today. The price is deliberately NOT changed for it — a two-rate
// price, a self-hold exclusion and an accepted under-pay are all economic-mechanism
// changes under the research gate (D-S7 escrow/skim/bounty), and a code comment must not
// settle one by assertion. Nothing disburses on main today because `-economy` defaults
// OFF. That is a WEAKER reason than "by design" and is written as such on purpose: the
// gap is held inert by a FLAG DEFAULT, not by a proof, and flipping that flag is a
...
// 2026-08-19 research made re-deriving `c` off the payee's own basis a CONDITION of
// keeping the split; the split was settled and the re-derivation was never performed.
// Measured on the shipped code it over-paid the reconstruction 2.31×–36.0× over the
// admissible loss range and the payee's own basis 10×–60×.
//
...
// price would silently over-pay 20 %, measured in TestPriceCarriesNoK. (The record
...
// its own.) Gates: TestPriceIsOneShardOfWitnessedFetch, TestPriceCarriesNoK.
//
...
//
// — 3.60 stripe-retrievals per shard-repair at m̄ = 3, against 12·m̄ = 36.00 before. Both are exact: the
// 1.20007 and 12.0007 carry a spurious 0.006 % from pricing income at a 262,144 B shard and outflow at a 262,160 B one.
// The cancellation is what makes the threshold dimensionless, which is the same property that clears build-immutable #3's steerable-estimand rule. Pinned in TestSolvencyBandIsExact. The prior published single number 36 is WITHDRAWN as a point: `reachable` is the judge's own
// POST-repair count, so m̄ is bracketed [1, m] and the honest band was [12.0, 36.0] and is now [1.20, 3.60]. Hot data self-funds inside the band; cold data stays prepay-dependent (D-S7 finite horizon), the mechanism's honest scope. Evolving-tier: re-tune only on field g. Certs, in
// order:
const (
...
// RepairBountyBase is the per-shard base bounty in CREDITS: c × shardBytes / (U/p) — ONE
// shard of witnessed delivery price, which is the act the payee performs (F1),
// denominated in the witnessed delivery price (T-NUMERAIRE; before 2026-09-06 the base
// was implicitly 1 credit per byte, which would have moved D-S7's self-funding threshold
// by 12.6 million the moment λ moved). This replaces the old absolute
// Config.RepairBountyBase so re-tuning shardBytes (Evolving-tier) re-prices repair
// automatically.
//
...
// RepairBountyCoeffNum/Den on why, and TestPriceIsOneShardOfWitnessedFetch, which
...
// "off", and the judge names a zero base loudly (core/node/repairclaim.go). That zero
// class WIDENED 10.008× with F1 — an accepted, disclosed cost; the publish-time warning
// is what discloses it, and cmd/silt refuses to start if the shipped default ever falls
// into it.
...
// exactly once, at the END — (BOULDER2 residual-closures research, 2026-09-07,
// §2.6). Flooring the base FIRST and multiplying after threw away mult × frac(x)
// instead of frac(x·mult): for integer mult ≥ 1 and real a ≥ 0, ⌊a⌋·mult ≤
// ⌊a·mult⌋ ≤ a·mult, so dividing last is never an over-pay and recovers up to
// (n−k+1)−1 credits — most on the stripe nearest data loss, which is exactly the
// stripe the multiplier exists to prioritise.
//
...
//
// It is what the publish warning says out loud on its TRUNCATES arm — and since F1 that
// arm is UNREACHABLE through cmd/silt bountyPriceWarning, because the warning's
// threshold is the shipped default's own base, which is now 1, so "below the threshold"
// means "zero". The arithmetic stays and is driven directly here: the threshold is
// DERIVED and both U/p and the publish default are Evolving-tier, so a re-tune that
// lifts the shipped base above 1 revives the arm. NARROWS records the cost of the gap
// in the meantime.
...
// c·shardBytes·(lost+1) divided into credits ONCE, at the end. It is the
...
// cannot mask a geometry that pays nothing on a healthy stripe.
...
// 10.008× widening of the zero class that F1 costs.
//
...
// shard was 65,552 B and now pays 0, where under the stripe basis it paid 2. (The build and
// review stated the geometry as shard = chunk/k and filed a residual on it; the
// review of the default chunk size caught the error.)
...
func (l *Ledger) FundEscrow(root ports.Hash, funder ports.NodeID, amount int64) error {
	...
	a := l.acct(funder)
	if l.faucet != nil {
		l.applyGrant(a) // FundEscrow is the third SPEND GATE
	}
	if a.balance < amount {
		l.noteSpendRefused(a) // symmetric with ChargePublish's refusal
		return ports.ErrInsufficientCredit
	}
	a.balance -= amount
	e := l.escrowFor(root)
	e.balance += amount
	e.fundedPrepay += amount // the PREPAY leg — never clawed back by a reversal
	return nil
}
...
func (l *Ledger) RecordServeToObject(server, requester ports.NodeID, root ports.Hash, id ports.ChunkID, bytes int64) int64 {
	...
	// The self-credit is PROVISIONAL against a later witnessed receipt for this
	// same delivery lane, which supersedes it (delivery.go — the PoD conservation
	// rule). An unwitnessed serve keeps it: the bilateral fallback. the mint is TWO
	// FLOORS off the lane's byte accumulator — ⌊(SkimDen−SkimNum)·acc/(SkimDen·Dλ)⌋
	// to the server and ⌊SkimNum·acc/(SkimDen·Dλ)⌋ to the escrow — each leg an
	// under-pay with its own remainder, so the skim accumulates across serves
	// instead of flooring to zero per call, and both recorded amounts stay exact
	// for reverseProvisional. The remainder lives on the LANE and dies with it at
	// both terminal sites (redeem, eviction): a remainder on the account would
	// survive a witnessed supersede and later mint for bytes the receipt already
	// paid.
	...
	s := l.acct(server)
	s.balance += net
	s.servedBytes += bytes // the byte observables stay BYTES
	l.recordFetched(requester, bytes)
	e := l.escrowFor(root)
	e.balance += skim
	e.fundedSkim += skim // the auto-SKIM leg — the only leg a reversal claws back
...
func (l *Ledger) PayBounty(root ports.Hash, repairer ports.NodeID, amount int64) int64 {
	...
	if r.fetchedBytes > 0 {
		// (credit.go): the repairer had already fetched from this node when the
...
// BountyToPriorFetcher reports the wash SHAPE: how many repair bounties this
