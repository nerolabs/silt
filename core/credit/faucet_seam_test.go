...
// The seam gates. A CONFIGURED ledger hands out a zero balance at Register and applies the
// starter grant at the first ADMITTED SPEND (CanPublish / ChargePublish / FundEscrow); a
// denial is retryable, never permanent; the project is unmetered; the non-spend paths
// consume no token; the deny floor degrades instead of denying when set. The UNCONFIGURED
// ledger is byte-for-byte the earlier faucet — every other test file in this package is
// that gate.
...
// TestRegisterIsZeroAndTheGrantLandsAtTheFirstSpend: under a configured faucet a fresh
...
func TestRegisterIsZeroAndTheGrantLandsAtTheFirstSpend(t *testing.T) {
...
// TestDenialIsPendingNotPermanent: with the bucket empty a fresh identity is refused at
...
func TestDenialIsPendingNotPermanent(t *testing.T) {
...
// TestOwnerIsGrantedUnmetered: the node's own identity receives its grant without
// touching the bucket, once, whether the bucket is full or empty.
func TestOwnerIsGrantedUnmetered(t *testing.T) {
	...
	if st := l.FaucetStats(); st.Level != 0 || st.GrantsIssued != 2 || st.GrantsPending != 0 {
		t.Fatalf("stats: %+v — the project grant must not consume a token and must count as issued", st)
	}
...
// TestNonSpendPathsConsumeNoToken: a fetcher credited fetched bytes, a bond-challenged
// peer, a PoR prover and a bounty-paid repairer are registered PENDING and take nothing
// from the bucket.
func TestNonSpendPathsConsumeNoToken(t *testing.T) {
...
// TestDenyFloorIsAnAdvanceNotASettlement: with a floor set, an identity that
...
func TestDenyFloorIsAnAdvanceNotASettlement(t *testing.T) {
...
// TestUnconfiguredLedgerIsTheUnmeteredFaucet pins the default: with no SetFaucet, Register
// grants at first touch, no account is ever pending, and FaucetStats reports unconfigured.
func TestUnconfiguredLedgerIsTheUnmeteredFaucet(t *testing.T) {
	l := New(50_000, 500_000)
	l.Register(r212ID(1))
...
// TestGrantsDeniedCountsDistinctRefusedIdentitiesNotRegistrations: a sweep that
// registers thousands of identities through read-side paths moves grantsPending and NOT
// grantsDenied; a real refusal at a spend gate moves grantsDenied once per identity
// however many times it retries; an advance is not a denial.
func TestGrantsDeniedCountsDistinctRefusedIdentitiesNotRegistrations(t *testing.T) {
...
// TestTopUpIsGrantMinusTheAmountAdvanced: the completion tops up by grant − the amount
// actually advanced, recorded on the account, never grant − the floor in force at
// completion time.
func TestTopUpIsGrantMinusTheAmountAdvanced(t *testing.T) {
	l, now := r212Ledger(t, 1, 1, 1_000, 50_000)
	l.CanPublish(r212ID(1))
	d := r212ID(2)
