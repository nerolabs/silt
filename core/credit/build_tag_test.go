...
// — the ledger-tier gates for "a default silt build contains no
// part of the B_bootstrap instrument".
//
...
// build_tag_absent_test.go. The two bond-path gates (the inversion and the
...
// TestDefaultBuildStampsNoFirstTouchOnRegister is the regression gate for the finding
// that made this decision: before it, cmd/silt/daemon.go injected the observability
// clock UNCONDITIONALLY, so every default-flags silt node recorded (identity,
// cumulative bytes, first-seen wall-clock nanosecond) for every requester, and no
// flag disabled it. the genuine addition to the RECORD was the `when`.
//
...
//
// THE FIXTURE DRIVES RecordServe, AND THAT IS THE STAMPING PATH. The stamp moved off
// Register onto recordFetched, the one write path for fetchedBytes, so the field this
// gate reads is firstFetchTick — the census's own, and since the ONLY
// first-touch field on an account. This test keeps its name because the property it
// pins is unchanged and bbootstrap_off.go cites it.
//
...
func TestDefaultBuildStampsNoFirstTouchOnRegister(t *testing.T) {
	l := New(1_000, 500_000)
	...
	for i := 0; i < 3; i++ {
		...
		if a.firstFetchTick != 0 {
			t.Fatalf("requester %d: firstFetchTick = %d with NO observability clock injected, want 0. A default silt build must record no first-fetch time for a fetcher at all — the tuple is (identity, bytes), never (identity, bytes, WHEN). See ", i, a.firstFetchTick)
		}
	}
}
...
// TestBondChallengeStampsNoFirstTouch is the INVERSION of the gate that used to
// stand here, and the other half of the
// gate above: a default build records no `when` for a fetcher, and the BOND path
// records no `when` either.
//
// WHAT WAS WRONG BEFORE. RecordBondChallenge wrote account.firstSeenTick — the wall-clock nanosecond of the first challenge an identity
// answered — and the old gate here pinned that write as "something else's mechanism" that removing would break. There was no other
// mechanism. Nothing read the field in any build configuration: DecayStale reads lastBondTick, Reputation reads neither, the census reads
// firstFetchTick. A retained `when` that no decided function needs is SURPLUS under T-DONT3 prong (a), so the write and the field are
// deleted and residual this gate is CLOSED.
//
...
// liveness reading rather than a `when`. TestRetentionReadsLastBondTickInNanoseconds
// pins its unit.
//
// RUNTIME GATE: core/node's TestBondAuditStampsAWallClockNanosecondNotACounter
// measures what the auditor actually passes; this gate covers what the ledger does with
// it.
func TestBondChallengeStampsNoFirstTouch(t *testing.T) {
	l := New(1_000, 500_000)
	...
	if a.firstFetchTick != 0 {
		t.Fatalf("firstFetchTick = %d after a bond challenge, want 0. A bond challenge is not a fetch and must not place an identity on the census age axis ", a.firstFetchTick)
	}
	// THE STRUCTURAL HALF. The field itself is gone, and this reads the TYPE
	// rather than the source: the set of tick-typed fields on account is CLOSED,
	// exactly {firstFetchTick, lastBondTick}, so a tick added under ANY name
	// fails here with that name in the message. It is a whitelist on the field's
	// type, not a match on its name. The earlier form of this check matched
	// names containing "firstseen"; the stamp was re-added as
	// `bondSeenTick` with the identical unset-guarded write and every gate
	// stayed green, Blocker 1. Under this form that ablation is RED, as is
	// `bondSeenAt ports.Time`.
	//
...
// assertAccountTickSetIsClosed is the closed-set gate TestBondChallengeStampsNoFirstTouch
...
func assertAccountTickSetIsClosed(t *testing.T) {
	...
	for i := 0; i < at.NumField(); i++ {
		...
		if !wantTicks[f.Name] {
			t.Fatalf("account has a tick-typed field %q (%s) outside the closed set {firstFetchTick, lastBondTick}. The bond-path first-touch stamp was deleted under (T-DONT3 prong (a): a `when` no decided function reads is SURPLUS); a bond challenge writes lastBondTick and nothing else, and a new `when` under any name needs its reader named and the set here re-derived, not a quiet field", f.Name, f.Type)
		}
		seen[f.Name] = true
	}
...
// TestRetentionReadsLastBondTickInNanoseconds is the third item: the ablation that
// proves the deletion above was surgical. lastBondTick must still advance on a passing
// challenge, and DecayStale must still zero a lapsed bond at BondMaxAge — and it must
// do so in NANOSECONDS, because that is the unit the daemon feeds it.
//
...
func TestRetentionReadsLastBondTickInNanoseconds(t *testing.T) {
	...
	if got := l.accounts[prover].bondedBytes; got != 0 {
		t.Fatalf("bondedBytes = %d one nanosecond past BondMaxAge, want 0 — a bond that stops being re-proven must lapse, and this is the retention surface the deletion had to leave intact", got)
	}
...
// TestLedgerStateIsEmptyInADefaultBuild pins the SIZE claim: untagged, the
...
func TestLedgerStateIsEmptyInADefaultBuild(t *testing.T) {
	...
	if !hasBBootstrap && size != 0 {
		t.Fatalf("bbootstrapState occupies %d bytes in a DEFAULT build, want 0. A default silt binary must carry no observability clock, no monotone source and no injection origin ", size)
	}
}
