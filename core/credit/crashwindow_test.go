...
// C3 — the OTHER half of the supersede/append crash window, MEASURED.
//
// The research ruled the FIRST half safe: a crash after the supersede and before the
// guard append is an under-pay in every arm. This file measures the half nothing had
// measured — the append LANDS and the pay is lost — because that half is NOT
// symmetric, and the asymmetry is a second precondition on the FP-2 persisted-ledger
// flip.
//
...
// TestCrashBetweenTheGuardAppendAndThePayBurnsTheReceipt measures the arm and pins
...
func TestCrashBetweenTheGuardAppendAndThePayBurnsTheReceipt(t *testing.T) {
	const fee = 50_000
	const b = int64(64 << 20)
	const wantMint = 149         // ⌊7·64 MiB/(8·Dλ)⌋ — the eager self-mint since (was 58,720,256 at λ = 1)
	...
	if paid, why := settleOnLane(crashed, srv, fetcher, obj, serial, 0); paid != wantPaid || why != ReasonPaid {
	...
	paid, why := settleOnLane(survivor, srv, fetcher, obj, serial, 0)

	// (1) THE HARD INVARIANT — no double-pay, ever. ReasonAlreadyPaid sits ABOVE the
	// supersede, so the durable guard entry is what burns the receipt.
	...
	// (2) THE MEASURED RESIDUAL. ReasonAlreadyPaid returning above the supersede
	// is exactly what stops the reversal, so the eager self-mint stays. That is
	// the lever's shape reached through a crash instead of a refusal.
	...
	if residual != wantMint {
		t.Fatalf("the crash-window residual is %+d, was measured at %+d on 2026-09-03. "+
			...
			"the design notes §9 and route it to the "+
			"The fix is not ledger-local (it needs the guard append and the "+
			"ledger write to share one durable transaction, or an idempotent re-pay keyed "+
			"on the guard entry, which re-opens the bound).",
			residual, wantMint)
	}
