...
// The credit NUMÉRAIRE — every byte→credit conversion in the balance economy, derived from ONE
// price pair.
//
...
// settled at 512 KiB per credit. The left inequality is STRICT paritydecision 3: a
// witnessed delivery must pay more than the self-mint for the same bytes; the right one
// is Don't #7 (reward tracks value: a relay that stores nothing may not out-earn the
// node that served the bytes). Both are derived here, never pinned twice; the constants
// gates in cmd/silt (which can import core/relaypay) hold the ordering.
//
// The fee f = 50,000 and the grant g = 500,000 do NOT move (ONE FACE): the 64 GiB
// grant/r pin is realized in the PRICE, and λ and the bounty base follow it.

// DeliveryIncrementBytes (U) and DeliveryIncrementCredit (p) are the per-increment
// delivery price: one credit per 256 KiB witnessed. The smallest power of two clearing
// (U/p ≥ 186,268 bytes per credit, the delivery lane's share of the 64 GiB pin after
// the relay lane's), with a 27 % pin margin: one grant buys 81.38 GiB across the SUM of
// the two prices. consumes these; until it lands the witnessed leg is the flat fee
// − skim and parity is vacuous. The provisional (4,096, 4,097) was REFUTED outright
// (186,313× short of the pin).
const (
...
// the ONE shard the bounty's payee moves, since F1; it was
...
// 100 % of serves, so the accumulator
...
// GrantOverRPinBytes is the 64 GiB grant/r pin: one starter grant must buy at least
// this many bytes across the SUM of the prices a NAT'd fetcher pays at once
// (delivery + relay). Read by the priced-lane start-up refusal in cmd/silt. The
// structural floor beneath it, S_max × N/K ≈ 44.7 GiB, is derived in that gate from
// erasure.DefaultParams, never transcribed.
const GrantOverRPinBytes int64 = 64 << 30

// ServeMintStats is the serve-mint telemetry: what the unwitnessed lane minted against
// what it served, and how much served work is still waiting below a mint boundary.
// Node-wide aggregates only — never joined to an identity axis (Don't #3).
type ServeMintStats struct {
	...
	// A2 supersede-suppression. ObjectAwareBytes / WitnessedBytes /
	...
	//
	// The identity has THREE terminal terms, not four. The spec named a fourth,
	// serveBytesSupersededFlat, on the flat delivery leg's supersede block and
	// said in the same sentence that it "is deleted with the leg". this lane
	// deleted the leg (RedeemDeliveryCreditReason and the core/demand v2
	// primitive are gone), so there is no site left to count and a fourth
	// counter would be a permanent zero.
