...
// (BOULDER2 residual-closures research 2026-09-07 §2.6): the repair price divides
// into credits ONCE, at the end — ⌊c·shardBytes·(lost+1)/(U/p)⌋ — instead of flooring
// the base first and multiplying a whole number after. These gates RUN the claim;
...
//
// The geometries moved 10× on 2026-09-12 (F1): the price is one SHARD of witnessed fetch,
// not k of them, so the shard that carries an exact 1.99996 credits is now 524,280 B
// rather than 52,428 B. Every credit figure below is unchanged, which is the point — this
// gate is a property of the division order, not of the price basis.
...
func TestRepairBountyDividesAfterTheMultiplier(t *testing.T) {
	...
	if floorFirst != 4 {
		t.Fatalf("fixture: the pre-fix price was %d, want 4 (a base of 1 times a 4× multiplier)", floorFirst)
	}
...
// division is never WORSE than flooring first (that is why this gate is free), and
// never pays more than the exact price (that is why it is safe — PayBounty caps at the
// escrow balance regardless, but conservation must not lean on the cap).
	...
	for _, shardBytes := range []int64{1, 16, 20_000, 26_215, 52_428, 65_552, 262_143, 262_144, 262_160, 524_280, 1 << 20, 1<<27 + 16} {
		for reachable := n + 2; reachable >= 0; reachable-- {
			...
			if floorFirst := RepairBountyBase(k, shardBytes) * mult; got < floorFirst {
				t.Fatalf("shard %d, reachable %d: RepairBounty %d < the floor-first price %d —  must be dominant", shardBytes, reachable, got, floorFirst)
			}
...
// is ZERO to a positive price, so the zero-signal would go VACUOUS if it read the paid
// price instead of the base. A 200,000-byte shard is below one credit of fetch — yet a
// stripe at the k-floor pays 5 credits for it.
...
// warning speaks: they are computed, never typed, and the money path uses no
...
// stated the wrong way round. The claim was that a sub-frame object, whose repair bounty
// base is now ZERO, is "kept alive by the serve economy instead". Measured, it is not:
// the auto-skim accumulates on a per-(server, requester, root) LANE, so a small object
// served once each to many DIFFERENT fetchers never reaches one credit on any lane. Its
// durability is prepay-only.
//
