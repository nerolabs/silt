...
func TestRoundTripLoadDelete(t *testing.T) {
	...
	mk := func(b byte, col int) (ports.ChunkID, ports.StorageProof) {
		...
		// The spot-check leaf width must persist too, or a restarted host
		// re-announces a shard it can no longer open a challenge over and is
		// failed for holding bytes it genuinely has.
		return id, ports.StorageProof{Root: root, Index: int(b), Total: 8,
			Path: []ports.Hash{p0, p1}, Column: col, LeafBytes: int(b) + 64}
	}
	...
	if g := got[id1]; g.LeafBytes != pr1.LeafBytes {
		t.Fatalf("id1 spot-check leaf width did not survive restart: got %d want %d", g.LeafBytes, pr1.LeafBytes)
	}
