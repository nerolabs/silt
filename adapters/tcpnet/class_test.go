...
// The transport half of observed-address keying. The transport is the ONLY place an IP exists; it
// exports an opaque per-process-salted (class, group) through ports.PeerClassifier. Gates (address
// book never classifies), (relayed conns present the relay's group in ONE namespace), (DIRECT never
// downgraded), (exemption scope: loopback + link-local only; RFC1918 CLASSIFIED — the cloudtest plan
// is 10.20.0.x trap), (no group on the wire, on disk or in a log line; per-process salt).
// and-observed-address-keying-–§8.
...
// TestExemptionScopeIsLoopbackAndLinkLocalOnly. Loopback, link-local and unspecified
// are exempt (group 0); EVERYTHING else is classified, RFC1918 included — a
// geth-style LAN exemption would classify every cloudtest node as exempt and the
// shadow run would measure nothing. v4 keys by /24, v6 by /32.
func TestExemptionScopeIsLoopbackAndLinkLocalOnly(t *testing.T) {
	var salt [16]byte
	...
	for _, ip := range []string{"127.0.0.1", "127.0.0.9", "::1", "169.254.1.1", "fe80::1", "0.0.0.0", "::"} {
		if g, exempt := group(ip); !exempt || g != 0 {
			t.Fatalf("RED: %s classified (group %#x, exempt %v); loopback / link-local / unspecified must be exempt with group 0", ip, g, exempt)
		}
	}
	for _, ip := range []string{"10.20.0.11", "10.30.0.11", "192.168.1.1", "172.16.0.1", "203.0.113.5", "2001:db8::1", "100.64.0.1"} {
		if g, exempt := group(ip); exempt || g == 0 {
			t.Fatalf("RED: %s EXEMPT (group %#x, exempt %v) — RFC1918/CGNAT must be CLASSIFIED: the cloudtest plan is 10.20.0.x/10.30.0.x and an exempt private range measures nothing", ip, g, exempt)
		}
	}
	...
	if !same("10.20.0.11", "10.20.0.99") {
		t.Fatalf("RED: 10.20.0.11 and 10.20.0.99 are in different groups; v4 keys by /24")
	}
	if same("10.20.0.11", "10.20.1.11") || same("10.20.0.11", "10.30.0.11") {
		t.Fatalf("RED: distinct /24s (10.20.0.x vs 10.20.1.x / 10.30.0.x) share a group; v4 keys by /24")
	}
	if !same("2001:db8:1::1", "2001:db8:ffff::1") {
		t.Fatalf("RED: two addresses in one v6 /32 are in different groups; v6 keys by /32 (an IPv6 /64 is free — red-team)")
	}
	if same("2001:db8::1", "2001:db9::1") {
		t.Fatalf("RED: distinct v6 /32s share a group")
	}
	if same("10.20.0.11", "2001:db8::1") {
		t.Fatalf("RED: a v4 /24 and a v6 /32 collide")
	}
	...
	if g16a, _ := addrGroup(salt, net.ParseIP("10.20.0.11"), 16, 32); g16a != func() uint64 { g, _ := addrGroup(salt, net.ParseIP("10.20.1.11"), 16, 32); return g }() {
		t.Fatalf("RED: width 16 does not merge 10.20.0.x and 10.20.1.x")
	}
}

// TestAddressBookNeverClassifies: at the transport: an address learned by
...
func TestAddressBookNeverClassifies(t *testing.T) {
	...
	if _, _, known := tr.ClassOf(peer); known {
		t.Fatalf("RED: AddPeer classified a peer that never answered — the address book is a declared value, not an observation")
	}
	tr.learnGossip(peer, "198.51.100.7:4001", true)
	if _, _, known := tr.ClassOf(peer); known {
		t.Fatalf("RED: envelope gossip classified a peer that never answered")
	}
	tr.observeConn(peer, tcpAddr("203.0.113.5"), false)
	c, g, known := tr.ClassOf(peer)
	if !known || c != ports.ClassDirect || g == 0 {
		t.Fatalf("RED: after a completed direct conversation ClassOf = (class %d, group %#x, known %v); want (DIRECT, non-zero, true)", c, g, known)
	}
}

// TestRelayedConnPresentsTheRelaysGroupInOneNamespace: A relayed
...
func TestRelayedConnPresentsTheRelaysGroupInOneNamespace(t *testing.T) {
	...
	if !rknown || rc != ports.ClassDirect || rg == 0 {
		t.Fatalf("fixture: the relay's own direct conversation is (class %d, group %#x, known %v)", rc, rg, rknown)
	}
	for i := int64(65); i < 70; i++ {
		...
		if !known || c != ports.ClassRelayed {
			t.Fatalf("RED: a relay-spliced conversation classified as (class %d, known %v); want RELAYED — the relay's address is being read as the peer's own (rule (c), REFUTED) or exempted (rule (b), REFUTED)", c, known)
		}
		if g != rg {
			t.Fatalf("RED (C-1): relayed client group %#x ≠ the relay's own direct group %#x — the group namespace is split by class, so a relay /24 yields cap_direct + cap_relay slots", g, rg)
		}
	}
}

// TestTransportNeverDowngradesDirect: at the transport: once DIRECT at
// a /24, a later relayed conversation leaves the class alone; a later DIRECT conversation
// at another /24 re-keys.
func TestTransportNeverDowngradesDirect(t *testing.T) {
	...
	if !known || c != ports.ClassDirect || g != gA {
		t.Fatalf("RED (C-3): after a later relayed conversation ClassOf = (class %d, group %#x); want the earlier (DIRECT, %#x) kept — the adopt-newest-wins oscillation flips honest ponies into the reserve", c, g, gA)
	}
	tr.observeConn(pony, tcpAddr("198.51.101.20"), false) // a new /24, directly
	c, g, _ = tr.ClassOf(pony)
	if c != ports.ClassDirect || g == gA || g == 0 {
		t.Fatalf("RED: a later DIRECT conversation at a new /24 did not re-key: (class %d, group %#x), old %#x", c, g, gA)
	}
}

// TestLoopbackConversationsClassifyExempt: on REAL sockets: two on-host
...
func TestLoopbackConversationsClassifyExempt(t *testing.T) {
	...
	for _, side := range []struct {
		...
		if !known || c != ports.ClassDirect || g != 0 {
			t.Fatalf("RED (%s): a completed loopback conversation is (class %d, group %#x, known %v); want (DIRECT, 0 = exempt, true) — an on-host swarm must classify as exempt, and the hook must fire on both a dial and an accept", side.name, c, g, known)
		}
	}
}

// TestRelayedRoundTripClassifiesRelayedOnBothEnds: on REAL sockets through a
...
func TestRelayedRoundTripClassifiesRelayedOnBothEnds(t *testing.T) {
	...
	if c, _, known := trA.ClassOf(identB.NodeID()); !known || c != ports.ClassRelayed {
		t.Fatalf("RED (dialer side): a relay-form dial classified (class %d, known %v); want RELAYED", c, known)
	}
	if c, _, known := trB.ClassOf(identA.NodeID()); !known || c != ports.ClassRelayed {
		t.Fatalf("RED (RelayInbound side): a registrant's spliced inbound classified (class %d, known %v); want RELAYED", c, known)
	}
}

// TestSaltIsPerProcessAndGroupNeverLeavesTheProcess. Two transports classify the same
// /24 to different groups (the salt is per transport instance, drawn at New); the wire
// envelope and the persisted peer record carry no class/group field (a positive pin of
// the CURRENT key sets: envelope keys 1.5, wireMsg keys 1.28, tcpnet.Peer = {ID,
// Addr}).
func TestSaltIsPerProcessAndGroupNeverLeavesTheProcess(t *testing.T) {
	...
	if gA == gB {
		t.Fatalf("RED: two transports classify 203.0.113.0/24 to the SAME group %#x — the salt is not per process (a group would be comparable across nodes; leak 4)", gA)
	}
	...
	for _, typ := range []reflect.Type{reflect.TypeOf(envelope{}), reflect.TypeOf(wireMsg{}), reflect.TypeOf(Peer{})} {
		for i := 0; i < typ.NumField(); i++ {
			...
			if strings.Contains(name, "class") || strings.Contains(name, "group") || strings.Contains(name, "salt") {
				t.Fatalf("RED: %s has a field %q — no class/group/salt may exist on the wire or on disk", typ.Name(), typ.Field(i).Name)
			}
		}
	}
	if got := reflect.TypeOf(Peer{}).NumField(); got != 2 {
		t.Fatalf("RED: tcpnet.Peer has %d fields; the persisted peers file carries exactly {ID, Addr}", got)
	}
	...
	for k := range top {
		if k < 1 || k > 5 {
			t.Fatalf("RED: envelope carries CBOR key %d outside the pinned set 1..5 — a new wire field; if intentional, re-pin here AND prove it is not a class/group", k)
		}
	}
	...
	for k := range msg {
		if k < 1 || k > 28 {
			t.Fatalf("RED: wireMsg carries CBOR key %d outside the pinned set 1..28", k)
		}
	}
}

// TestNoLogLineCarriesAGroup is a SOURCE gate: no logging call in the transport, the
// table or the node names a "group" key. Checked: text only — every `logf(`/`dlog(`/
// `Printf(` call in adapters/tcpnet, core/dht, core/node non-test sources, for a string
// literal "group" argument. UNGATED: that no OTHER output path (the /api/status JSON, a
// metrics line) prints a raw group value — series E is a density census by design and
// must aggregate, not list.
func TestNoLogLineCarriesAGroup(t *testing.T) {
	var files []string
	...
	for _, f := range files {
		...
		for _, m := range call.FindAllString(string(src), -1) {
			if strings.Contains(m, `"group"`) || strings.Contains(m, `"groups"`) {
				t.Fatalf("SOURCE GATE: %s has a logging call with a \"group\" key: %s — a salted group must never appear in a log line ", f, strings.Join(strings.Fields(m), " "))
			}
		}
	}
}
