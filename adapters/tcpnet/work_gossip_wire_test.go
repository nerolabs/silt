...
func TestWorkGossipSurvivesWire(t *testing.T) {
...
// TestWorkGossipIsAdditiveForAnOldPeer: the two new slots are omitempty and a decoder that
// does not know keys 29/30 skips them, so a node that has served nothing adds zero bytes
// to every packet and an un-upgraded peer keeps parsing. This is DHT gossip, not a frozen
// consensus format, so additive is the correct posture — but it has to be true.
// TestWorkGossipIsAdditiveForAnOldPeer is ALSO the wire half of the research's condition
// (C3-GOSSIP-DISCLOSURE-vs-): "with privacy on, an exact scan of the raw CBOR map
// finds neither key 29 nor key 30 on a node with non-zero work."
//
...
// NEITHER key 29 nor key 30, so its frame is byte-identical to a earlier node's. THERE:
// core/node's TestWorkCountersAreGossipedOnlyWhenTheOperatorPublishesThem proves that
...
// survives. The research names that vacuity by reference.
func TestWorkGossipIsAdditiveForAnOldPeer(t *testing.T) {
