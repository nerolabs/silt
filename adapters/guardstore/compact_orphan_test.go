...
//The gate.
//
// THE MECHANISM (decision §1, "the one measurement I ran", P7) — as it was BEFORE the
// fix. Compact renamed the temp file onto d.path, THEN re-opened the append handle on
// the new path. If that re-open failed, Compact returned the error — the caller was told
// — but d.f still points at the file's PREVIOUS inode, which the rename has just
...
// return is the only signal, and today's one caller (core/credit/delivery.go)
// discards it unconditionally.
//
// SOURCE SEAM rule 8 — a gate may only claim what it measures, so the mechanism
// creating the injection point is named here, not hidden in a helper. guardstore.go now
// routes both OpenFile calls that open/re-open the append handle (Open, and the tail of
// Compact) through the package var `openAppend = os.OpenFile`. That is the ONE non-test
// line this gate required; it is behaviour-preserving in production (the var's value is
// os.OpenFile unless a test overrides it) and touches no other call site — `realign`'s
// OpenFile (a O_WRONLY truncate-only open, never the append handle) is deliberately
// left alone.
...
func TestPostRenameOpenFailureOrphansTheAppendHandle(t *testing.T) {
	...
	// ran AFTER the rename (the handle swap broke, decision's measurement); on the
	// fixed tree it runs BEFORE the rename. The gate asserts the PORT PROPERTY either
	// way: after a failed Compact, Append is non-nil OR its record is visible to a
	// fresh Load — never nil-and-invisible.
	orig := openAppend
	...
	if appendErr == nil && !visible {
		t.Fatalf("Compact failed post-rename (%v), then Append(serial=%q) "+
			"returned nil AND a fresh Load of the same path never saw it. The missing port "+
			"contract clause (decision §1): after a Compact that returns an error, the store "+
