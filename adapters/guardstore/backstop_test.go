...
// The sticky backstop behind open-before-rename; re-keyed per finding 1. The backstop
// keys on ONE signal — after the swap the append handle's inode must be the inode at
// d.path — and on nothing else. Two pins: it FIRES when that signal is false (sticky
// ErrStoreBroken on every later Append and Compact), and it does NOT fire when only
// the retired handle's Close fails (a healthy store must never refuse payouts).
...
// TestBackstopFiresWhenTheHandleDoesNotReachThePath: the seam opens the append
...
func TestBackstopFiresWhenTheHandleDoesNotReachThePath(t *testing.T) {
	dir := t.TempDir()
...
// TestRetiredHandleCloseFailureDoesNotBreakTheStore: closing the RETIRED handle out
// from under Compact makes old.Close fail. That inode is already unlinked; the live
// handle is fine. The store must NOT be marked broken — marking it broken would
// refuse every payout on a healthy store until restart.
func TestRetiredHandleCloseFailureDoesNotBreakTheStore(t *testing.T) {
...
// A pre-rename open failure (the injection) leaves the store UNCHANGED and healthy:
// not broken, still appending to the live path. This pins the benign class at the
// adapter, the other half of the port clause.
func TestPreRenameOpenFailureLeavesTheStoreHealthy(t *testing.T) {
	p := filepath.Join(t.TempDir(), "paidserials.log")
	d, err := Open(p)
	if err != nil {
		t.Fatal(err)
