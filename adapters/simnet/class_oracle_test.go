...
// The simnet class oracle: the deterministic mirror of the tcpnet classifier, so the
// node-tier liveness sims (core/node) can run under the class rule. SetClass(id,
// class, group) is what an endpoint reports for id after a DIRECT delivery from id; a
// relay-spliced delivery reports (RELAYED, the relay's group); an endpoint that never
// heard from id reports known=false; DIRECT is never downgraded.
...
func TestSimnetClassOracleMirrorsTheTransport(t *testing.T) {
	s := simclock.New()
	...
	if c, g, known := n.Endpoint(pub).ClassOf(ponyA); !known || c != ports.ClassDirect || g != 0x201 {
		t.Fatalf("oracle RED: after a direct delivery pub reports pony A as (class %d, group %#x, known %v); want (DIRECT, 0x201, true) — a pony that dials out is DIRECT at its own NAT /24", c, g, known)
	}
	...
	// A later relayed delivery never downgrades pub's DIRECT view: pub→ponyA is
