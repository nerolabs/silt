...
//
// Mechanism 2 (F6, band-level prune, 2026-09-03 — the research found this one still live
...
// TestAnEarlierRotationDoesNotPruneALaterRotationsPrePublishedKey is the F6
// mechanism stated deterministically, with no goroutines: the sequence the
// research measured. Ablate the maxEpoch term in ensureBand's prune and this is
// RED with "LOST=[15]".
func TestAnEarlierRotationDoesNotPruneALaterRotationsPrePublishedKey(t *testing.T) {
	dir := t.TempDir()
...
// TestEpochStoreRotationsNeverLoseAnInstalledKey drives the daemon's own launch
...
// band may legitimately have pruned it"; the research showed the pruning turn is the
...
func TestEpochStoreRotationsNeverLoseAnInstalledKey(t *testing.T) {
	// 80 is the count at which the research observed the F6 loss on 80/80 runs. It
	// costs ~40s of RSA keygen, so -short takes a smaller sample; the deterministic
	// gate above carries the mechanism when this one is sampled down.
	attempts := 80
	if testing.Short() {
		attempts = 5
	}
...
func assertEveryInstalledKeySurvived(t *testing.T, installed, onDisk map[uint64]*rsa.PrivateKey, topology string) {
	...
	if len(lost) != 0 {
		...
		t.Fatalf("BREAK REOPENED (%s): epochs %v were installed and staged "+
...
// TestEnsureBandIsAtomicUnderConcurrency is the same property stated on the
...
func TestEnsureBandIsAtomicUnderConcurrency(t *testing.T) {
	dir := t.TempDir()
	s, err := OpenEpochs(dir)
	if err != nil {
		t.Fatal(err)
...
// ---------------------------------------------------------------------------
// (the cliff itself, UNCHANGED and deliberately so). Once fp(key_E) is committed and
// key_E is gone from the store, EnsureBand regenerates a DIFFERENT key and the
// append-only chain will never accept its fingerprint. The store cannot see the
...
// recoverable one.
// ---------------------------------------------------------------------------
func TestLostKeyAfterCommitIsStillAPermanentlyDeadEpoch(t *testing.T) {
	dir := t.TempDir()
	s, err := OpenEpochs(dir)
	if err != nil {
		t.Fatal(err)
...
// ---------------------------------------------------------------------------
// CLOSED at the daemon (the blast radius) and PINNED here (the store).
...
// lane-off instead of dying; see cmd/silt/laneoff_test.go.
// ---------------------------------------------------------------------------
func TestCorruptStoreErrorsAndIsNeverRewritten(t *testing.T) {
	dir := t.TempDir()
	s, err := OpenEpochs(dir)
	if err != nil {
		t.Fatal(err)
...
// ---------------------------------------------------------------------------
// (documented, unchanged): a FRESH store generates only forward, so the [cur-W, cur
// past band is absent until it has been running that long. Pinned so the
// boundary-race guarantee is stated with its one exception rather than absolutely.
// ---------------------------------------------------------------------------
func TestFreshStoreGeneratesOnlyForward(t *testing.T) {
	dir := t.TempDir()
	s, err := OpenEpochs(dir)
	if err != nil {
		t.Fatal(err)
