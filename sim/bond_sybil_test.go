...
// Integration/sim tier for the M0 Sybil fix G2. The unit tests
// (core/bond/prefix_plot_sybil_test.go) prove the crypto: a plot is bound to ONE
// (identity, size), so a prefix or a foreign plot fails the labeling recompute.
// This drives the IDENTITY half of that binding THROUGH THE LIVE AUDIT WIRE.
//
...
func TestSharedPlotSybilEarnsNoStandingOverTheNetwork(t *testing.T) {
	const (
		seed     = int64(23)
