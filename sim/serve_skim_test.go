...
func TestServeAutoSkimFundsObjectEscrow(t *testing.T) {
	...
	// 32 MiB in 512 KiB chunks: a column holder serves ~64 shards of 52 KiB to the
	// retriever on ONE lane, so the lane's byte accumulator crosses the 8·Dλ = 3 MiB
	// escrow boundary across many calls — the seam this sim is the only integration cover
	// for. At the old 256 KiB / 4 KiB geometry every lane would now skim zero.
