...
// On the wire: the UI consumer==provider promise, end to end. A daemon that
// fetches a link through its own /api/fetch must come out the other side as a
// DISCOVERABLE provider of what it consumed — visible to `swarm holders` — not
// a silent hoarder whose bytes count against the pledge while no fetcher can
// find them. The sim tier (core/node/netget_retention_test.go) proves the
// drop/retain semantics and serve-after-holder-death; this proves the wire
...
func TestUIFetchMakesTheConsumerAProvider(t *testing.T) {
	...
	// The promise: B now appears in the swarm's provider view. The
	...
	for {
		holdersOut = runClient(t, "swarm", "holders", pub.Link, "-peers", bootstrapA, "-registry", regRef)
		if strings.Contains(holdersOut, idB) {
			t.Logf("consumer B is a discoverable provider (consumer==provider wired,)")
			return
		}
		if time.Now().After(deadline) {
			t.Fatalf("consumer B never became a discoverable provider after /api/fetch:\n%s", holdersOut)
		}
