...
//
// This was written to reproduce (on GCP this same shape did NOT commit — chain
// stuck at height 0). It does NOT reproduce locally: the consensus code + exact GCP
// params commit fine over real TCP in <2s. So is GCP-ENVIRONMENT-specific (the
...
func TestObjectiveColdStartCommitsGenesis(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		...
		args := []string{
			...
			// addresses it needs to gather quorum. Required now that raised the launch
			...
			// is the shipped fix, so the regression guard should exercise it.
	...
	if !committed {
		...
		t.Fatalf("objective 4-validator simultaneous cold start (quorum 2, mature 4, Byzantine on, bond-audit %s) did NOT commit a block within %ds — the anchor training-wheels bootstrap has regressed (cf.)", bondAudit, windowS)
	}
}

// hypothesis 2: the full cloudtest topology has an EXTRA objective validator
...
func TestObjectiveColdStartWithSatelliteValidator(t *testing.T) {
	...
	launch := func(i int, extra ...string) *daemon {
		...
		// tier (network-durability §8 / the cloudtest precedent, Layer 2): consensus
		...
		// strict anchor majority (3 of 4) then can't be reached — a CONFOUND (mesh
		// convergence) masquerading as the variable this test isolates (does the SATELLITE
		// block commit?). Earlier the lower 2-quorum was reachable on a half-formed mesh,
	...
	if !committed {
		...
		t.Fatal("REPRODUCED: a 5th bonded non-anchor (adversary-style) validator prevented the 4 anchors from committing genesis")
	}
}
