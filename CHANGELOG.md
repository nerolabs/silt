...
### Added
- **Carrier seating agreement is model-checked — the owed property `R-CARRIER-MODELCHECK`, freeze manifest item 14** (2026-09-11). A blind audit found that none of the 22 `modelcheck_*` files covered the LastCommit carrier's seating rule, so the claim that *every honest replica seats the same identity set from a carrier* had never been checked anywhere. Two new files close it at both consensus tiers. **Tier 1** (`core/chain/modelcheck_carrier_seating_test.go`) drives the reducer over a population that is **non-uniform by construction** — two qualified identities at different weights, and identities unqualified for four *different* reasons (below `MinBond`, unbonded, slashed, outside the frozen epoch set) — because a uniform population lets *"seat everything carried"* and *"seat the qualified"* agree on every input. It enumerates all 2^8 carrier contents in each regime and requires the seated set to equal the screen evaluated on the **parent's committed post-state**, then holds that set fixed while varying everything that must be irrelevant: 32 orderings, all four proposers, duplicated entries and entries declared at another round — comparing the committed v5 state root, which is the quantity a replica actually compares. **The ordering property is DRIVEN, not pinned.** `TestCarrierFoldPrecedesBondRegsInApply` asserts the statement has not moved; this asserts what moving it would cost, by re-folding the same carrier over the same block's post-bond/TTL/slash state and showing the seated set diverges in all three directions (an in-block registration that newly qualifies, an in-block slash, an in-block TTL lapse). **Tier 2** (`core/node/modelcheck_carrier_agreement_test.go`) drives six real replicas through the real propose → gather → commit → broadcast loop over held delivery under five schedules, where acceptance *is* the witness: `validatorsSeenRoot` is a committed v5 leaf, so a replica that seated a different set recomputes a different root and refuses the block. Its **positive control** injects a replica whose only difference is one unrecognised launch anchor — invisible in committed state, since an unbonded anchor appears in no leaf — and requires that replica to fall behind; a confound guard demands it agree at height 1, whose carrier is empty by rule, so the divergence is attributable to the carrier and nothing else. **Six ablations were watched RED before any green was accepted**: deleting the qualification screen, excluding this block's proposer instead of the parent's, moving the fold below the bond loop, moving it below the slash loop, seating from the uncovered `Atts`, and un-gating the frozen sub-v5 `Atts` loop — the last two reproducing the original `R-BOX-ATTESTS` *"commit rejected by own replica"* stall at the node tier. The era seam is covered from both sides: an identity present only in a **v4 parent's** `Atts` is seated, one present only in the **v5 child's** `Atts` is not, and nothing the parent seated is lost across the boundary. NOT a format item: no block field, no committed leaf, no validity rule.
