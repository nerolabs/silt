...
- **The delivery idle window is DERIVED and SHIPPED: default 24m, floor `430 s × 4/3` = 9m33.33s (Lane C2,
  ...
  fetcher gapped by a stall; the constants carry compile-time proofs of their own three inequalities. 24m over the tighter
  ...
  wall-clock STEP half stands and is now driven (a session that settled one second before the step is reaped, and the cost
  is a deposit, never bytes); the derived number survives the correction, since 430 s is still the right envelope for how
  long an honest fetcher may be gapped. Gates: `TestC2ShippedFloorIsDerivedFromTheBound`,
  ...
  `cmd/silt/c2_idle_window_default_test.go`). **Not fixed here, and re-priced worse:** the relay lane settles once at
