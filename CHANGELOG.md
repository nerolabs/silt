...
### Changed
- **M3 — the chain-bound token domains: a credit, a demand token or a relay anchor minted on one network no longer verifies on another** (2026-09-11, research certification `NETWORK-IDENTITY-BINDING-THREE-LAYER-RESEARCH-CERTIFICATION-2026-09-11.md` §3, GATED, owner-ratified). `demandMsg = epoch ‖ serial` named no network and the credit domain signed a bare serial, so under a shared issuer key a token bought on X redeemed on Y — EIP-155's unlearned lesson. The chain id (the genesis hash, 32 B) now LEADS the blind-signed message and the three domains are retired rather than extended: `silt/blindcredit/fdh/v1`→`v2`, `silt/blinddemand/fdh/v2`→`v3`, `silt/blindrelay/fdh/v1`→`v2`, so an old signature is REFUSED, not re-read. A per-network domain CONSTANT was refuted (it destroys the byte-exact pins and walks toward the open `R0.4b-FDH` residual); binding it in the message is the schema `demandDomain` already proved for the epoch, and RFC 9578's own move. The chain id is a REQUIRED PARAMETER of every bound call — a missed site is a compile error — and never a field of the token, the request or the wire (`core/demand/TestTokenCarriesNoChainIDField` fails if `demand.Token` ever grows one); the D3 ephemeral withdrawal, which holds no chain by construction, receives it from its durable parent beside `issuerPub` and `epoch`. A ZERO chain id refuses at both ends: every bound entry point returns `ErrZeroChainID`, and `answerDemandTokenRequest` declines to sign before any charge when `(*Node).chainID()` is zero — network zero is not a network. PayWord is NOT a target (`payword.go` has no signature and no domain, only a hash); the leaf session domains INHERIT (no session opens without a chain-bound anchor, driven by `TestDeliveryAnchorFromAnotherNetworkIsRefused`). **The publish-token domain `silt/blindtoken/fdh/v1` is deliberately NOT bound:** `publishtoken.Verify` runs inside `ValidateEntry`/`v5ValidateEntry`, so binding it changes block validity — a consensus-rule change the certification's landing table misclassifies as economic. Named in `fdhDomain`'s doc with its open cross-network consequence — not registered as a residual, because a register row needs an owner and a closer this change cannot assign. Deliberation: `docs/thinking/2026-09-11-m3-network-binding-of-the-token-domains.md`.
