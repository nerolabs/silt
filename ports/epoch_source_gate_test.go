...
// The SOURCE gate for the epoch rule: "No ports.CreditLedger method takes an epoch."
// It covers SpendRelayAnchors and the core/node deliveryReasoner twin.
//
...
// TestFallingSourceLowersNothingAndReadmitsNothing_Delivery and _Relay (the
// ledger follows an injected source), core/node
// TestLedgerFollowsItsSourceNotTheCaller (the node-tier dual).
//
...
// and cannot build until that lands.
...
// TestNoPortMethodCarriesAnEpoch is. RED when it was written: all four signatures
// ended in an epoch parameter the CALLER supplies (ports.go currentEpoch / current;
// delivery.go currentEpoch; relayanchor.go current; demandrole.go currentEpoch). The
// sites move as lanes retire; the RULE does not.
//
// RUNTIME GATE: core/credit TestFallingSourceLowersNothingAndReadmitsNothing_Delivery.
func TestNoPortMethodCarriesAnEpoch(t *testing.T) {
	type site struct {
	...
	sites := []site{
		...
		// C1: RedeemDeliveryCredit / RedeemDeliveryCreditReason were the
		...
		// the flat receipt's deliveryReasoner seam is retired; the session
	...
	for _, s := range sites {
		...
		for _, p := range ps {
			if p[0] == "currentEpoch" || p[0] == "current" {
				t.Errorf("SOURCE GATE: %s %s%s still takes the caller-supplied epoch %q. Checked: "+
					"parameter NAMES of one signature.: the ledger READS its epoch from "+
		...
		if len(ps) != s.wantN {
			t.Errorf("SOURCE GATE: %s %s has %d parameters %s, want %d ending in %q. Checked: "+
				...
				"observed by core/credit TestFallingSourceLowersNothingAndReadmitsNothing_Delivery",
