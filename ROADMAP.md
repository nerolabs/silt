...
| F9 | The three website pages stop being committed and are generated at deploy | **BUILT 2026-09-12, CONDITION DISCHARGE PENDING ON THE PR'S OWN DEPLOY PREVIEW** (`D-WEBSITE-HTML-AT-DEPLOY-2026-09-12`), scope **all three together** — `website/changelog.html`, `website/roadmap.html`, `website/buildlog.html`. `netlify.toml`'s build command runs all three generators, the three files are untracked and gitignored, and the three *"Fail if … page is stale"* CI steps are retired. **Those three steps had to go in the same commit: measured 2026-09-12, `git diff --quiet -- website/roadmap.html` exits 0 against an untracked, gitignored file holding unrelated content — they would have gone VACUOUS, not RED.** They are replaced by a step asserting none of the three is tracked (driven RED by `git add -f`) plus `check_links.py`, which exits 1 naming nine broken links if the pages are absent, because `index.html`, `docs.html` and `node.html` all link to them | **THE DISCHARGE ROUTE CHANGED, AND IT IS STRONGER THAN THE ONE THIS ROW ORIGINALLY NAMED.** The row asked for a Netlify **dashboard** check. A dashboard reading is a claim about settings; **the PR's own deploy preview is the deploy itself**, and it discharges both halves at once — that the repo's build command is the one that runs, and that one deploy produces all three pages. Fetch the three pages from the preview URL (it is in the PR's own check data, `gh pr view --json statusCheckRollup`) and compare them byte-for-byte against locally generated output. **Any page missing or different ⇒ STOP, do not merge.** Production is untouched either way, which is why this order was chosen: deleting first and checking after is how a public site goes blank. `ROADMAP.md`, `CHANGELOG.md` and `docs/buildlog/*.md` stay the single sources of truth — this removes a copy, not a source | Builder |
