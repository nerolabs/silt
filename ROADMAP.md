...
- **R1.8 the accept-flip** lands only after the simpler design proves out, and only if it is still
  needed (a consensus-rule change, I1: Researcher re-certifies, owner ratifies). **HARD PRECONDITION added
  2026-09-13: `R-DIGEST-OP-COLLISION` must be CLOSED before the flip** — a reproduced `box.Accept ∧
  node.Reject` on the compound-block digest path (#853), with `R-DIGEST-COLLISION-AT-EPOCH-BOUNDARY` as its
  uncovered worst instance. The R1.8 downgrade is today the only barrier between that finding and damage,
  and the pins that hold it redden when the DEFECT moves while staying GREEN if the flip lands with the
  defect live — so the flip is gated here, in the plan, and not by a test.
...
7. **ANSWERED 2026-09-12 — YES, and a chainless client carries 32 BYTES**
   ...
   **SUPPLY ROUTE BUILT 2026-09-13 — route (a) only, and the CONSUMER is still #828.** `swarm add`
   ...
   it yet** — the credit lane binds it in #828 — so the INSTALL SITE is held by a SOURCE gate
   (`cmd/silt TestSwarmAddCallsDeclareNetworkIdentity`) that retires the day a behavioural arm
   exists. **What that gate proves is now stated exactly, because it used to over-claim:** a blind
   review left the install call written in `swarmAdd` and made its guard impossible, and the whole
   `cmd/silt` suite stayed GREEN — the defect the gate's own message said it caught. The GUARD
   therefore moved into `declareNetworkIdentity`, which
   `cmd/silt TestDeclareNetworkIdentityIsBehaviouralAndOnTheLoop` drives for real. **Moving the guard
   MOVED the hole, and the fold-in review measured it:** discarding `parseDeclaredChainID`'s DECLARED
   result and passing a literal `false` to the still-written call left the whole `cmd/silt` package
   GREEN. The gate now walks `ast.CallExpr.Args` and binds arguments 3 and 4 to the identifiers the
   parse bound in the same body, so the ARGUMENT class is gated; the residual that no source gate of
   this shape can close — the call made UNREACHABLE — is filed as `R-CHAINID-INSTALL-SOURCE-GATED`.
   `(*Node).HasNetworkIdentity` lands with it so #828 can REFUSE an undeclared client rather than
   blind under the zero hash (`core/node TestHasNetworkIdentityDistinguishesAbsentFromZero`). The
   `D-TD-3` disclosure is closed in the same change: a credit-lane refusal now NAMES its cause
   (`node.ErrCreditIssuerKeyUnknown`, its own sentinel) instead of surfacing as a bare
   `ErrTokenAcquire`, and the gate carries an anti-tautology arm.
...
| `R-GRADED-RUN-CHAIN-EVIDENCE-UNCAPTURED` | ACTIONABLE | **FILED 2026-09-13 out of the `869ad9a-deep` graded run, and it is RECORDED, NOT FIXED — the harness is explicitly out of this PR's scope.** The run's one failing row, `12b-deep-prune`, read a uniform zero on all four validators, and the artifact that would have settled it independently — the 84 MB `chain.cbor` — was destroyed: a seat staged it on a VM, the download was classifier-blocked, and teardown then took the VM. **The capture must PRECEDE the report**, because the report is what makes anyone want the file, and by then teardown has run. **Scoped honestly against the fix that shipped with this filing:** `cmd/silt/chainstatus.go` now counts `HeavyProofsShed()` rather than the v5-retired `IsPruned()`, which removes the STRUCTURAL zero — a `0` on a v5 chain is no longer uninformative by construction. That does not close this row. `chain-status` still reports no block-version-by-height map, no retention horizon, and no count of registrations carrying a heavy proof below it, so an honest zero remains unattributable from the scrape alone. Closer: the graded harness pulls `chain.cbor` from every validator into the run's artifact directory before the report is generated — **a harness decision, routed to the Tester, not taken here** | silt-agent-memory/tester/evidence/2026-09-13-launch-869ad9a/final/12b-deep-prune-attribution.md (the tester's root cause; the same directory holds the run's report and results, and NO chain.cbor — which is the finding) · ROADMAP.md L1479 (the filing prose, `Test / harness debt`) | — |
| `R-ANSWERDIGEST-DOC-FALSE` | ACTIONABLE | **FILED 2026-09-13 as a RECORDED-NOT-FIXED finding of the `chain-status` prune-observable repair; keeping the code change out of that PR is the correct scoping call, but CHANGELOG prose is not a filing — this lint reads `ROADMAP.md` only.** Lane TAIL (LOW) · Builder: `core/chain/chain.go`, `answerDigestOf` documents itself as returning *"the zero hash when the registration carries no heavy proof"*; its body is `sha256.Sum256(answer)`, so a nil answer yields `e3b0c442…`, not `Hash{}`. **Every caller is correct today** — `HeavyProofsShed` compares against `answerDigestOf(nil)` rather than against a zero hash — so this is a false sentence, not a defect. **It is filed rather than shrugged at because that sentence is now load-bearing for an operator observable:** `cmd/silt/chainstatus.go`'s `pruned:` counter reads `HeavyProofsShed()`, and a future reader who trusts the doc and writes `*r.AnswerDigest != (ports.Hash{})` turns the predicate true for every digest-only registration — an over-count in the exact counter the repair installed. Closer: correct the comment (one line, `core/chain`, no behaviour) in any PR that already touches that file; do NOT open a `core/chain` PR for it alone | silt-agent-memory/principal-engineer/reviews/ruling-pr854-chainstatus-pruned-v5-observable.md (F-2; the body read at source) · ROADMAP.md L1595 (the filing prose, `Polish & latent wins`) | — |
...
| `R-CHAINID-INSTALL-SOURCE-GATED` | ACTIONABLE | **FILED 2026-09-13 by blind review of #852 (F-3 + F-4), and it is TWO facts about one mechanism: the requester-side network identity SHIPS INERT, and the only thing holding its install site is a SOURCE gate that cannot prove the install runs.** Lane D2 · Builder; closer is the **#828 behavioural arm**, and its companion is the RETIREMENT of the source gate. **(1) INERT, measured with `go tool nm` on a throttled `go build ./cmd/silt` at this branch:** `(*Node).RequesterChainID` and `(*Node).HasNetworkIdentity` are **ABSENT from the linked `./cmd/silt`** — nothing calls either, and the flag's 32 bytes are carried and never read. `(*Node).SetNetworkIdentity` and `node.ErrCreditIssuerKeyUnknown` ARE present, and `main.swarmAdd.declareNetworkIdentity.func2` is present, so the SUPPLY route links and only the CONSUMER is missing. This is the `scar:mechanism-shipped-inert-2026-09-10` class named in `scripts/reachability_lanes.txt`'s own header; the ROADMAP narrative disclosed it in prose, and prose is not a row the residual lint reads. **No lane record is added and nothing is wired to make a gate green** — the `R-INGEST-WITNESSES-INERT` precedent. **(2) SOURCE-GATED, and the blind spot is MEASURED, not asserted:** ablation A9 left `swarmAdd`'s install call written and made its guard impossible (`declaredChainID != zero && declaredChainID == zero`); the flag parsed, refused malformed input, silently dropped the value, and **the entire `cmd/silt` suite stayed GREEN** — verbatim the defect the gate's own `t.Fatal` claimed to catch. An `ast.Inspect` collecting call names sees a call regardless of reachability. **A source gate proves a call is WRITTEN, never that it RUNS.** What was done: the GUARD moved out of `swarmAdd` into `declareNetworkIdentity`, so A9's defect class is RED at the unit tier (`cmd/silt TestDeclareNetworkIdentityIsBehaviouralAndOnTheLoop`, four arms, four ablations); the gate was renamed to `TestSwarmAddCallsDeclareNetworkIdentity` and its message now states its reach exactly instead of over-claiming. **A SECOND hole of the same class was found by the fold-in review (`silt-agent-memory/principal-engineer/reviews/RULING-PR852-foldin-verification-dff6b6a-2026-09-13.md`), and it is CLOSED here rather than enumerated:** moving the guard into a callee relocated the un-gated surface to the CALL'S ARGUMENTS. Ablation B-ARG — discard `parseDeclaredChainID`'s DECLARED result into `_` and pass a literal `false`, with the install call still written — left the **whole `cmd/silt` package GREEN**, which is A9 verbatim one argument to the right: the flag parses, a malformed value is still refused, the value is silently dropped. Neither instrument followed the guard, because the four behavioural arms call `declareNetworkIdentity` with their OWN arguments and an `ast.Inspect` collecting call NAMES never reads `ast.CallExpr.Args`. **The gate now walks the arguments** and binds positions 3 and 4 to the identifiers the `parseDeclaredChainID` assignment bound in the same body; `_` and a literal are both refused; five ablations drive five arms RED one at a time (the discard, a hardcoded wrong hash at argument 3, a hardcoded `false` at argument 4, a changed arity, and the parse assignment rewritten as a `var` declaration), and B-ARG now reddens the package it used to pass. Arguments 1 and 2 (`run` and the node) are NOT pinned and the docstring says so — `run`'s posting behaviour is gated on the callee side. **What is left un-gated, and it is why this row is still open:** `swarmAdd`'s unconditional call being made UNREACHABLE — wrapped in a dead branch, or preceded by an early return. That is not closable by any source gate of this shape, and it has no observable at ANY tier including e2e, because `swarmAdd` builds its client node internally and exposes it to no caller while no production reader of `RequesterChainID` exists. **Do not close this row by broadening the source gate** — the argument widening above did NOT close it and was not filed as closing it; reachability is the residual, and only the #828 behavioural arm retires it | silt-agent-memory/principal-engineer/reviews/RULING-PR852-chainless-client-chain-id-2026-09-13.md | — |
...
| `R-DIGEST-OP-COLLISION` | ACTIONABLE | **FILED 2026-09-13 — a REPRODUCED `box.Accept ∧ node.Reject`, pinned by #853 and NOT fixed** · Lane TAIL (POST-RC, with Boulder 1) · **Researcher** certifies the remedy (the v5 state-root fold is a consensus-rule surface, so it is behind the research gate) → **Builder** builds it → **OWNER** ratifies. **THE CLOSER IS A HARD PRECONDITION OF THE R1.8 ACCEPT-FLIP (L302), NOT OF THE FREEZE** — the freeze locks the repair, not the bug. **Mechanism:** `assembleStateRootRecomputeOps` appends every class's digest ops into ONE slice in the order S → B → T → A → M → P, and classes S, B and T each compute `tagBondedRoot` / `tagQualifiedRoot` from the anchored pre-set plus THEIR OWN CLASS DELTA ONLY. All three land on the identical `statehash.Key(tag, nil)` carrying a byte-identical `OldValue` and the same wrapped witness, so ALL of them verify at step (1), and `FoldChangedPaths` step (5) replays them in slice order with no dedup and no duplicate-key guard: the last op on a key wins. **MEASURED on an S+B block:** S emits `a7bd89ee…`, B emits `6bc0de14…` (the fold takes B's) and real `apply()` commits `c92028bf…`, so **the honest value is NEITHER candidate and a dedup cannot close it** — the direction is one cross-class post-set threaded in `apply()` order, which the tree already computes in `reconstructPostQualifiedWithWrites` for the class-P freeze and never generalised. **CONTAINMENT: `(*Box).Validate` applies the R1.8 downgrade, so the box never Accepts on this path today**, and that downgrade is the only barrier. **This row is the forcing function and the pins are not:** every pin reddens when the DEFECT moves and stays GREEN if the flip lands with the defect live. | silt-agent-memory/researcher/reviews/research-outcome/anchoredPreSet-not-anchored-REMEDY-RESEARCH-CERTIFICATION-2026-09-13.md | — |
| `R-DIGEST-COLLISION-AT-EPOCH-BOUNDARY` | ACTIONABLE | **FILED 2026-09-13 by the blind review of #853. UNCOVERED — no test drives it** · same lane, same seats and the SAME closer as the row above (it is that break's worst instance, not a second remedy), plus a boundary fixture that drives `isBoundary` true. **At an epoch boundary the box derives TWO MUTUALLY INCONSISTENT QUALIFIED SETS INSIDE ONE BLOCK:** class P's `reconstructPostQualifiedWithWrites` computes the CORRECT cross-class post-qualified set (B → T → S over the anchored pre-set) and freezes it into `epochSetRoot`, while in the same block the `qualifiedRoot` digest takes the last-write-wins WRONG value — two consensus-visible digests disagreeing about who is qualified inside one accepted root, which is strictly worse than a digest disagreeing with its own leaves. Every fixture in the #853 gates runs `EpochBlocks: 1024` at a low height, so `isBoundary` is false and class P never dispatches. **It is also the existence proof that the remedy is ALREADY WRITTEN in this package** and was never generalised. | silt-agent-memory/principal-engineer/reviews/ruling-pr853-digest-op-collision-pins-2026-09-13.md | — |
| `R-ANCHORED-PRE-SET` | ACTIONABLE | **FILED 2026-09-13** — a CONFIRMED WRONG-ACCEPT, found by the Tester and independently re-derived by a blind PE, which had no register row while its gates were already in the tree · **Lane TAIL (POST-RC); the DEADLINE is the accept flip (R1.8 / #657)** · Researcher certifies, then Builder builds — it is a consensus-adjacent verification rule and is RESEARCH-GATED, so no seat builds it alone. `anchoredPreSet` (`core/chain/floorbox_recompute_stateroot_slash_v5.go`) checks only that a digest witness exists and that its `Proof` is non-nil: it computes no `nodeSetMTH`, calls no `Resolve`, and takes no `prevStateRoot`. Anchoring is a SIDE EFFECT of a later `digestFoldOp`, and `FoldChangedPaths` verifies only the ops it is handed, so a tag whose fold op is never emitted has its `PreIDs` and `Proof` consumed as ATTACKER DATA. **Four such reads:** class B `slashedRoot` (no op for that tag exists at all, and it is the sole carrier of the F2 bar), class B `bonded`/`qualifiedRoot` (guarded by `!idSetsEqual`, an equality the forged pre-set itself steers), class P `qualifiedRoot` (the boundary freeze source), class A `validatorsSeenRoot` (suppressed when post == pre). Contrast `provenView.members`, which Resolves and checks unconditionally. **CONTAINMENT: the R1.8 downgrade in `(*Box).Validate` is the only barrier** — this row is what happens on the day R1.8 flips, which is why the deadline is that flip and not a date. **Gates already in the tree:** `RT-ANCHOR-0..6` in `core/chain/rt_anchor_preset_gates_test.go`, `PINNED_DEFECT` under `D-REPAIR-CLAIM-GATES-PINNED-2026-09-12` — they assert the CURRENT BROKEN behaviour and redden when the remedy lands (driven: all seven RED under a simulated remedy). The record carries the classification too: `foldInputCoverageTable`'s `StateRootDigestWitness.PreIDs` row is `FIX-OPEN`. **One OPEN OWNER CALL rides on this row:** the *expected 0 `FIX-OPEN` rows* assertion walks `r12CoverageTable` only, so it does not see that row and stays GREEN; widening it would park a RED gate on `main` for as long as the research gate holds the remedy. Measured 2026-09-13: the same disposition inside `r12CoverageTable` takes `TestAdversarialRootCoverageIsComplete` RED, so the walk is alive and the blindness is about the TABLE. **Closer:** a research certification of the anchoring rule, then the build, then retire the seven pins and restore the `PreIDs` row — the remedy reddens NINE tests, not seven (the two extra are named in `rtAnchorPin`'s FIX CASE text) | silt-agent-memory/principal-engineer/reviews/RULING-pr851-rt-anchor-preset-pins-2026-09-13.md | — |
| `R-HASH-VALUE-PRINTED-VIA-STRINGER` | ACTIONABLE | **FILED 2026-09-13 by the blind PE on PR #851 (required fix 4, production half), which that PR deliberately does NOT touch — different risk class, pre-existing at its base** · Lane TAIL (LOW) · Builder. `ports.Hash` is `[32]byte` with a **value-receiver** `String()`, and `fmt` routes `%x` through `Stringer`, so `%x` on a **value** hex-encodes the 64-char hex string a second time and prints **128 chars**. Measured live, not inferred: a run of PR #851's own pin text printed a root of `3030303030…`, the hex of the ASCII `"000…"`. **Four production sites, all consensus-diagnostic operator text an operator reads when a node REJECTS a block** — `recomputeStateRootEntriesRevocations` (`core/chain/floorbox_recompute_stateroot_v5.go`, `ErrRecomputeStateRootMismatch`, args `postRoot` and `committedStateRoot`); `core/chain/era3validity.go` **twice** (`ErrEra3StateRootMismatch` with `*b.StateRoot`/`wantState`, and the `ErrEra3LogRootMismatch` twin below it with `*b.LogRoot`/`wantLog`); `core/chain/stateview_proven_v5.go` (`ErrEra3LogRootMismatch`, `*b.LogRoot` and `*v.head.LogRoot`). **`go vet` does not and will not flag this** — `%x` is a legal verb for a `Stringer` — so nothing stands between this and the next occurrence; the rest of the tree mostly slices (`root[:]`, `id[:]`), which is why it reads as a slip rather than a convention. **Closer:** a PR slicing the four sites, with a gate that has teeth on at least one of them (the in-PR half is gated by `TestRTAnchor_PinTextPrintsA64CharRootAndNeverTheZeroHash`, driven RED both ways). Consider whether the general instrument is a lint over `%x`-on-a-`Stringer`-value rather than four edits | silt-agent-memory/principal-engineer/reviews/RULING-pr851-rt-anchor-preset-pins-2026-09-13.md | — |
...
**Test / harness debt:**
- **A graded run must capture `chain.cbor` BEFORE the report is written — `R-GRADED-RUN-CHAIN-EVIDENCE-UNCAPTURED`.**
  On `869ad9a-deep`, row `12b-deep-prune` failed with a uniform zero across all four validators and the
  one artifact that would have settled it independently — the 84 MB `chain.cbor` — was lost: a seat
  staged it on a VM, the download was classifier-blocked, and teardown then destroyed the VM. The row's
  scraped number and the on-disk byte count are both DERIVED observations; neither can distinguish
  "the prune did not engage" from "the counter cannot see it", and that is exactly the discrimination
  that was needed. **This is NOT made moot by the `chain-status` fix**
  (`TestChainStatusPrunedCountIsSoundOnAV5Chain`), which removes one confound — the structurally-zero
  counter — and leaves the rest: `chain-status` reports no block-version-by-height map, no horizon,
  and no count of registrations carrying a heavy proof below it, so a future honest zero is still
  unattributable without the file. The
  capture must precede the report, not follow it, because the report is what makes anyone want the
  file. Sizing, from the same run: 84 MB per validator.
...
- **`answerDigestOf`'s doc comment is false — `R-ANSWERDIGEST-DOC-FALSE`.** It says *"the zero
  hash"* for a nil answer and returns `sha256("")`. No caller is wrong; the sentence is. It is
  recorded rather than ignored because `chain-status`'s `pruned:` observable now reads
  `HeavyProofsShed()`, which is defined against `answerDigestOf(nil)` — believing the comment and
  comparing against `ports.Hash{}` instead would over-count every digest-only registration.
