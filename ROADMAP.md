...
**▶ TWO MORE CALLS OPENED 2026-09-11 (the freeze-manifest re-audit). FOUR OPEN, under the cap of five.**
...
3. **Manifest item 8 (`R-CARRIER-BYTES`) — in the RC, or explicitly declined and disclosed to B8?** THE
...
- **PLACED 2026-09-11, so the plan carries them somewhere:** **M3** (the FDH bindings) is STAMP-RAISE
  ...
  **G-3**, the witness id-list size gate, is now `R-membership`'s closer and is UNBUILT; it is not a
  ...
  belongs inside D1 at all is a question for the Researcher rather than an assumption.
...
|---|---|---|---|---|
| `R-H43-ROUND-LADDER-DESYNC` | ACTIONABLE | Lane A3 · Tester: the fix is merged (#772) and field-confirmed (`2633a11-deep`); the row closes when the next graded run grades `6-fault-tolerance` at the re-priced 190 / 380 s tiers with the head-reading wait (#774) | silt-agent-memory/researcher/reviews/research-outcome/CONSENSUS-LIVENESS-h43-COMPOSED-DIFF-c2a476a-RESEARCH-CERTIFICATION-2026-09-07.md | — |
...
| `R-CARRIER-MODELCHECK` | ACTIONABLE | **BUILT 2026-09-11 (#816); the VERDICT is UNSETTLED and that is why the row is open.** Lane D2 · Tester; freeze manifest item 14. Six model-checks exist with no `t.Skip` and no `testing.Short` gate — `core/chain TestModelCheck_CarrierSeating_AgreementOverCarrierVariation`, `…_ScreenReadsTheParentPostState`, `…_EraSeamV4ParentV5Child`, `…_SetupIsNonUniform` (the non-uniformity control) and `core/node TestModelCheck_CarrierAgreement_HonestReplicasSeatTheSameSet`, `…_ControlADivergentScreenFallsBehind` — and they were re-signed under the chain id at the M1 rebase. **No seat has run them at HEAD.** A test that exists is not a property that holds, and a classification reached by reading is not a verdict. **Closer:** run the six by name under `taskpolicy -c background nice -n 19 go test ./core/chain/ ./core/node/ -run <name> -v`, all GREEN, before the stamp raise | silt-agent-memory/researcher/reviews/research-outcome/ERA4-V5-FREEZE-MANIFEST-RESEARCH-CERTIFICATION-2026-09-07.md | — |
