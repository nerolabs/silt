...
12. **`B_bootstrap` run preconditions (R2.9a) — what is OPEN for the owner as of 2026-09-05, each with
    ...
    - **RE-PIN BEFORE ANY RUN · the provisional `grant/r`.** The provisional is `λ = 1` ⇒
      `grant/r = 500,000 bytes = 488 KiB`, which is 134× below one 64 MiB production chunk
      (`core/pipeline/pipeline.go`). Geometry alone refutes it before any measurement:
    ...
    - **RATIFIED, no longer owed:** the 5-second snapshot interval (`D-STATUS-SNAPSHOT-INTERVAL`,
      ...
      (`D-DONT3-READING`, 2026-09-05). G-BB-13′ (the Don't #3 veto before exposure on a routable
      interface) is moot for a default build under the tag and live only for a tagged node bound
      off loopback. `R-BB-BOND-STAMP-TUPLE` is CLOSED (G-BB-28, 2026-09-05: the bond-path
      first-seen stamp is deleted; nothing read it).
