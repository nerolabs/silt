...
| A2 | `#380` objective-mode `Config.Quorum` floor divergence — `RequiredQuorum()` returns the LOCAL `cfg.Quorum` verbatim in the mature regime (I1), and `SupportMeetsQuorum` gates `newViewFor`, so a divergent floor is also a permanently dead designee (liveness) | CERTIFIED 2026-09-07 (same certification as A1); direction (1) — ignore the local floor in `ValidateCommit`, keep `Quorum` as a proposer-side gather target — **RATIFIED 2026-09-07** (owner call 20, `D-CONSENSUS-ARMING`) | Tester encodes G-H43-8 RED first; Builder lands direction (1) in one PR; a consensus-rule change (I1), so the Researcher re-certifies the diff (rule 9: one cert, for the rule change) | Tester → Builder → Researcher |
...
the shadow run), **12** (commission the external B8 seat, at D3), and the `#558` refusal-surface
ratify-or-narrow (the rule refuses on ANY structural-verification failure, a larger surface than
