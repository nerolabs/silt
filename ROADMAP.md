...
| # | Item | Status | What is left | Gate / seat |
|---|---|---|---|---|
| C1 | R2.9 tail — delete the `core/demand` v2 primitive and the ledger's flat leg (`RedeemDeliveryCreditReason`, ~25 tests re-homed); `R-GUARD-RESTORE-LANE-UNKNOWN` (persist the lane byte in the guard record — a store-format bump) rides with it | B-9 retired the flat path at the node (#764); the primitive + flat leg remain | one attended PR (defined in `docs/thinking/2026-09-07-b9-flat-path-retirement.md`) | Builder; blind PE |
...
| C3 | R2.2 full observability set (serve-work AND repair-work Gini, per-tier margin, live `g`, funded-horizon, wash detection; network panels via the DHT crowd-estimator with knowability tiers) | DEFINED (`docs/thinking/2026-09-01-economy-observability-design.md` §3–§4) | one builder session on the Economist's 17-row build list (`/Users/andrewedmond/Claude/claude/silt-reviews/economist/ADVISORY-boulder2-telemetry-spec-R2.4-checklist-and-RC-scope-2026-09-07.md` §2): `/api/economy/flows`, `/g`, `/concentration`, `/network`, the two gossip fields (`servedBytes`, `repairsDone`; tier class DERIVED from gossiped `CapTotal`), the four dashboard panels; the design doc's network repair-Gini gate is VACUOUS by construction, so the Tester asserts `serveGini ≤ 0.15` ∧ pony serve-share ≥ 0.50, `repairGini ≤ 0.40` within the capable sample, caretaker set ≥ 3 — each ablation RED first | Builder + Tester |
| C4 | R2.7 blocking telemetry — the A2 (supersede-suppression) and A4 (money-pump) detectors | SPECIFIED 2026-09-07 (the same advisory, §1): seven new counters + one split; `bountyPaidToEscrowFunder` is DEGENERATE (every escrow funder on a ledger is `n.id`) and is replaced by the prepay/skim split of `funded`, `Stats.BountyPaidToSelf` and `bountyPaidToPriorFetcherCredits`; the third detector is `spendRefusedInsufficientCredit` / `spendRefusersDistinct` (the build-immutable-#4 affordability floor); no `(fetcher × object)` join anywhere (Don't #3) | one small Builder PR with a unit test per detector (fixture shapes in the advisory) | Economist → Builder |
| C5 | Pre-flip closers: `R-BOUNTY-TRUNCATION` (G-BT-1 a truncation arm on the publish warning + G-BT-2 divide AFTER the rarest-shard multiplier) · `R-SHORT-FINAL-STRIPE` (parity at the shard's true length — a content-addressing break, so before economy-on) · `R-ANCHOR-BEARER-TRANSFER` (a demand anchor is a bearer instrument; red-team pass on the transfer surface) | the family CERTIFIED 2026-09-07 (`/Users/andrewedmond/Claude/claude/silt-reviews/research/research-outcome/BOULDER2-residual-closures-bounty-truncation-lambda-dust-relay-anon-refuse-self-spend-parity-estimand-RESEARCH-CERTIFICATION-2026-09-07.md`): the λ-dust bound CLOSED (24.00 GiB escrow leg, exact); parity amplification DISCHARGED (the per-server floor corrects DOWN to 27.94 GiB, so the 64 GiB `grant/r` pin gains margin 1.43× → 2.29×); the refuse-and-self-spend and relay-anonymity faces are held in tension (`m0.md` §10); the price-level and relay-wash positions DECIDED 2026-09-07 (owner call 13) and carried INTO the flip — re-confirmed at R2.4 only if C7 contradicts them | Builder: G-BT-1 + G-BT-2 and the stale `swarm.go` burn comment (one small PR); Builder: the short final stripe; red-team: the bearer-anchor pass | Builder / red-team |
| C6 | R2.4 the economy-ON default flip — phased: correctness gates → economy-OFF baselines over ≥ 3 epochs → canary (window = 1 epoch ≈ 5.9 min at the measured `T_b`, hysteresis 3 windows; HARD aborts: any `guardFullRefusals`, caretakers < 3, `BountyPaidToSelf` > 0, any rise in `spendRefusersDistinct`; soft aborts: coverage < 0.75, pony serve-share < 0.50 or −20 pp, `serveGini` > baseline + 0.10, `repairGini` > 0.40, funded horizon < 600 s, evicted/object-aware > 0.01) → default | DEFINED as a CHECKLIST 2026-09-07 (the advisory, §3) | after C1–C5; the flip is the FIRST of the three `D-FP2-SCOPE` re-arm triggers (the ephemeral-ledger items re-open with it); OUTSIDE the RC by `D-RC-SCOPE-S1` — a `0.9.x` release after C7's clean verdict, before `1.0.0` | Builder; Economist |
...
| # | Item | Status | What is left | Gate / seat |
|---|---|---|---|---|
| D0 | **The cold auditor — the RC's ONLY floor-box requirement** (owner call 2, RATIFIED 2026-09-07, `D-TRUE-UP-CALLS-2026-09-07` (2)): the floor box is a COLD AUDITOR — unconditional loud stall, never Accept | CERTIFIED (a′), 4 conditions; NOT built. Replaces the R1.x ladder (R1.1–R1.6 + the structure rounds) as the freeze's floor-box gate; R1.7 = R4.4 stays as E4 | one PR: unconditional loud stall; delete `RecoveryDirective.Heights` + `LiveFollower`; refuse pruned blocks; `trustFloor` off the contract surface; the `-ws-checkpoint` re-anchor documented as irrecoverable-if-unreachable — PLUS one DRIVEN never-Accept suite: for every v5 block class, forge it, commit the divergent root, assert a stall (simplicity rule 7 — an un-driven "safe" row fails the completeness meta-test) | Builder; blind PE; Tester (every gate RED first) |
...
- **OPEN:** A2 `#380` (direction (1), gate G-H43-8) · A3 the graded run at the re-priced tiers.
...
>
> Pruned 2026-09-08 from 125 rows to the actionable set: 60 closed rows deleted (verbatim in
...
| `R-GUARD-RESTORE-LANE-UNKNOWN` | ACTIONABLE | Lane C1 · Builder: `LoadPaidSerials` rebuilds every guard entry as lane DELIVERY because `ports.PaidSerial` carries no lane, so the per-lane live counts conflate the two populations after a restart; persist the lane byte in the durable record (a store-format bump) in the C1 PR | silt-reviews/principle-engineer/RULING-R2.9-deposit-at-anchor-expiry-4d4a90c-2026-09-07.md | — |
...
| `R-BOUNTY-TRUNCATION` | ACTIONABLE | Lane C5 · Builder: bounded at the shipped 256 KiB (0.006 %) but `-chunk-size 52412` under-pays 50 % silently; one small PR — G-BT-1 a truncation arm on the existing publish warning + G-BT-2 divide AFTER the rarest-shard multiplier (free, strictly dominant); the accumulator is REFUTED on build-immutable #8 | silt-reviews/research/research-outcome/BOULDER2-residual-closures-bounty-truncation-lambda-dust-relay-anon-refuse-self-spend-parity-estimand-RESEARCH-CERTIFICATION-2026-09-07.md | — |
| `R-SHORT-FINAL-STRIPE` | ACTIONABLE | Lane C5 · Builder: compute a single-chunk object's parity at the shard's TRUE length (a 1 KB object stores 2,097,264 B at the 256 KiB default; the erasure floor is K × chunkSize); a content-addressing break, so it lands before economy-on | silt-reviews/economist/ADVISORY-default-chunk-size-256KiB-2026-09-06.md | — |
