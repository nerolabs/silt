...
12. **G-BB-1 — pin `W` and `q` for the `B_bootstrap` run (R2.9a). NEW 2026-09-04, and it BLOCKS the
    ...
    **Two more owner gates join it (DELTA certification, 2026-09-04): G-BB-9**, pin the POPULATION
    `P` the estimand is over — viewers-only, or all honest fetchers including repairing/judging
    peers, since a repairing caretaker fetches over the same priced path and pays the same `r`; the
    choice trades M0 (viewers-only ⇒ lower `B` ⇒ Sybil-dearer, repair may starve) against D-S7
    (all-fetchers ⇒ higher `B` ⇒ Sybil-cheaper, repair funded), and the Researcher declined to take
    it. **G-BB-13**, a VETO GATE on Don't #3 (`docs/TENETS.md` Part VI) before the block is ever
    exposed on a routable interface: with the minimum-R floor built and the run on loopback, does any
    shipped configuration expose an anonymity set small enough to attribute? Source:
