...
| `R-GENESIS-HASH-FREEZE-SURFACE` | ACTIONABLE | Researcher: is the height-0 block hash inside the era-3/4 freeze surface (R3.4), and what is the migration rule for a network that already holds persisted chains? Filed by the blind PE on 4′ (`silt-reviews/principle-engineer/RULING-default-chunk-256k-manifest-framing-b365f10-2026-09-07.md` item 1): the manifest framing change alone moved the genesis hash (7becf754…32ce → f428d0a8…0951) with no refusal and no log line. MEANWHILE `core/genesis` pins the padded 64 KiB manifest frame (`pipeline.Options.ManifestFrameBytes`) and `TestGenesisBlockHashIsPinned` holds the literal, so the hash does not move until the question is answered; Boulder 2 / pre-freeze carry-list | silt-reviews/principle-engineer/RULING-default-chunk-256k-manifest-framing-b365f10-2026-09-07.md | — |
