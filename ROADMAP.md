...
**The RC is ONE release: the era-4/v5 stamp-raising release, with the consensus format FROZEN and
the floor box still never-Accept.** The external B8 pass (R1.7 = R4.4, one engagement) attacks that
...
|---|---|---|---|---|
| A1 | `R-H43-ROUND-LADDER-DESYNC` — under f=1 down, per-validator #432 round ladders drifted ~77 s apart and block 43 took 17 min 20 s (run `c450985-deep`; evidence `integration/cloudtest/h43-stall-evidence-c450985-deep/`) | field-discovered; ⟨SEAT:A1-TESTER⟩ | ⟨SEAT:A1-LEFT⟩ | model-check RED first (Tester); fix direction certified (Researcher); consensus-rule change → OWNER ratifies |
| A2 | `#441` mature-regime PUBLISH starvation (I4, entry path) and `#380` objective-mode `Config.Quorum` floor divergence (I1) — the two tracked consensus-touching residuals | ⟨SEAT:A2-STATUS⟩ | ⟨SEAT:A2-LEFT⟩ | Researcher (same certification as A1) |
| A3 | The next graded GCP deep run (the RC field-test gate needs a green pass on the frozen artifact) | BLOCKED on A1 | run only after the model-check reproduces A1 and the fix is field-confirmable; `RUN_LOCAL_PROOF` green first | Tester; owner's explicit go (billable) |

#### Lane B — Boulder 1: the floor box (the accept-flip's preconditions)

| # | Item | Status | What is left | Gate / seat |
|---|---|---|---|---|
| B1 | `R-STRUCTURE-REDERIVATION` — ONE accept composition `ValidateCommitV5(view, block)` over a three-valued `StateView`, all box doors unexported, `box.Accept ⇒ node.Accept` | RATIFIED 2026-09-03; build plan CERTIFIED (`FLOORBOX-STRUCTURE-BUILD-PLAN-CERTIFICATION-2026-09-03.md`, gates BG-1…BG-4, eleven items); ⟨SEAT:B1-PE⟩ | ONE builder session per the build brief; closes by construction `R-CARRIER-PARENT-BINDING`, `R-CARRIER-PARENTPROPOSER`, `R-LOGROOT-FORMAT-SCOPE`, `R-BOXENTRY-RESIDUALS` (N1–N8), `R-SUPPORT-SLASHED-SCREEN`, `R-MALFORMED-DIVERGENCE`, `R-PIN-LABEL-ESCAPE`, `R-PIN-REANCHOR-POSITION`, `R-PIN-BUDGET-ESCAPE`, `R-FENCE-TABLE-DRIFT`, `R-BOUNDARY-PREDICATE-COVERAGE`, `R-INVENTORY-HAND-LIST`, and R3.2 (the class-A probes re-pointed to `LastCommit` and naming A1/A2/A3) | Builder; blind PE on the diff; Tester ablations (every gate RED first) |
| B2 | `R-membership` — retire `slashedRoot` and `validatorsSeenRoot` from the v5 digest set (D-V5-WHOLESET-ROOTS five → three) instead of capping identities; G-1 the explicit `objective()` guard | GATED (7 gates), certified 2026-09-03 | **OWNER** sentence (owner call 1 below); then one PR: the `objective()` guard + the digest-set change (a hard fork at activation, free while era-4 is dark) — PRE-FREEZE | Researcher gates G-1…G-7; Tester RED-first |
| B3 | Recovery boundary (formerly #535) — the floor box is a COLD AUDITOR | CERTIFIED (a′), 4 conditions | **OWNER** sentence (owner call 2); then one PR: unconditional loud stall, delete `RecoveryDirective.Heights` + `LiveFollower`, refuse pruned blocks, `trustFloor` off the contract surface, the `-ws-checkpoint` re-anchor documented as irrecoverable-if-unreachable | Builder; PE |
| B4 | `R-CARRIER-BYTES` — the box witness/frame byte ceiling as a v5 validity rule | principle + formula CERTIFIED; VALUE gated on a pony-class measurement (none exists); BG-3 stalls the box above its own budget meanwhile | Tester: measure the honest-carrier LOWER bound and the box witness cost per carried entry on the floor box (1 CPU / 2 GB); **OWNER** ratifies the value; the rule lands in the stamp-raising release (Lane D) | Tester measurement → owner → Builder |
...
| B6 | Small owed items: `R-CARRIER-DOUBLESIGN-SLOT` (LOW; the evidence producer lifts a carried precommit onto the parent's `Atts`, with the SILENT-GREEN and FALSE-SLASH traps as gates) · `R-CARRIER-ORDER-ORACLE` (Tester rewrites the call-site gate on the behavioural oracle in the pre-maturity branch) · `R-SWARM-NOTBANKED-DEAD` (reachability argument or removal) · `R-CARRIER-CREDIT-DENIAL` (**OWNER** call 3: buy the multi-block inclusion window or decline; plus the doc fix) | OPEN, all LOW | one small PR each, any order | Builder / Tester |
...
| E2 | R4.2 the A-axis — re-scope to measure / publish / hand to B8 as-is; do NOT wire A3 | measure/publish SHIPPED (#715); the re-scope sentence is un-ratified | **OWNER** sentence (owner call 11); the bonded-adversary domain-collision finding goes in the R4.4 brief | owner |
...
| E5 | The RC field grade — a green multi-machine GCP deep run of the frozen artifact (the standing release gate) | last green: `c450985-deep` 29 pass / 2 gap / 0 fail (the stall is one of the gaps) | after A1 and D3 | Tester |
| E6 | R1.8 the flip (B7) → `1.0.0` | — | after E4 + E5 | owner |

**Owner calls open (2026-09-07) — each is one sentence to ratify; the recommendation is the seat's, the call is the owner's.**
1. **R-membership (B2):** close it by retiring `slashedRoot` and `validatorsSeenRoot` from the v5 committed digest set (five → three) rather than by capping seated identities; a hard fork at activation; not certified until the `objective()` guard lands. *Recommended: ratify.*
2. **Recovery boundary (B3):** the floor box is a COLD AUDITOR — unconditional loud stall, the two directive knobs deleted, pruned blocks refused, `trustFloor` off the contract surface, recovery by a fresh `-ws-checkpoint`-class anchor at H+1, irrecoverable if unreachable. *Recommended: ratify.*
3. **`R-CARRIER-CREDIT-DENIAL` (B6):** decline the multi-block inclusion window for v1 (the carrier already narrows the pre-existing denial to one proposer; the window is a format change). *Recommended: decline, doc fix only.*
4. **The delivery idle window (C2):** set `-delivery-idle-window` to 2 epochs (≈ 12 min at the measured 44 s/height) as the shipped default. *Recommended: 2 epochs; refuse-until-set stays for the flag's absence.*
5. **`R-SESSION-WALLCLOCK-STEP` (C2):** accept as a disclosed v1 residual (a forward wall-clock step reaps every live session and the deposit returns at anchor expiry, so nothing is lost but latency). *Recommended: accept.*
6. **`R-CREDITSPENT-UNBOUNDED` (C2):** accept the 65,536 cap as an OPERATOR-MANAGED ceiling (rotate the publish key AND clear `creditspent.log` together) until the epoch-bind lands with the freeze. *Recommended: accept (PE).*
7. **`R-ANCHOR-STALL` (C2):** accept ≤ 300,000 credits per 1 GiB relay session as a disclosed v1 residual; R2.14b `MsgRelayFund` is the follow-on. *Recommended: accept.*
8. **Cloud row 13b (C2):** SKIP (not GAP) while era-4 is dark, behind the era probe. *Recommended: SKIP (PE).*
9. **R3.4 the freeze (D3):** freeze on the D1 manifest, buying `tagRevLogSize` as the one liveness leaf. *Recommended per D1's certification.*
...
11. **R4.2 (E2):** re-scope the A-axis to measure / publish / hand to B8 as-is; do not wire A3. *Recommended: ratify.*
12. **R4.4 (E4):** commission the external B8 seat at the RC. *Owed at D3.*
13. **`R-DEMAND-PRICE-LEVEL` and `R-RELAY-WASH-ZERO-LOSS` (C5):** decided AT the R2.4 flip, not before. *Owed at C6.*
14. **`R-ISSUERKEY-POP` and `R-E2E-ERA4-FIXTURE`:** reserve the PoP slot inert at the stamp raise; accept the e2e cost. *Folded into D1/D2; recommended: reserve-only, accept.*
15. **The `-grant-capacity` help note:** leave as is. *Recommended: leave.*

**Scope calls the owner has not yet made — these move the RC date more than any build.**
- **S1 — Is the economy-ON default flip (C6) inside the RC?** Today the roadmap implies yes ("an economy-off HEAD certifies a network nobody runs"; R2.7 feeds the B8 brief) but no sentence says so. *Recommendation: the RC (`0.9.0`, the frozen stamp-raising release) ships with the economy DEFAULT-OFF and every paid lane BUILT, gated and dark; the B8 pass attacks the frozen consensus + the economy under `-economy` ON in the harness; the default flip is a `0.9.x` release inside the RC line once C7's verdict is clean, before `1.0.0`. Reason: the flip re-arms FP-2/FP-1 and is the one economic mechanism with no field data behind it; freezing the format does not depend on it.*
- **S2 — Is the operational floor (packaging, signed installers, R4 self-update, the S6 O(delta) maturation and reprovide dirty-tracking) inside the RC?** The archived Phase 5 says "RC-path, needs scoping"; the flixz handoff says post-RC (Boulder 5). *Recommendation: post-RC for packaging/self-update; the two S6 scaling kills are `1.0.0` gates because they price out the honest operator (build-immutable #4/#8). Define Boulder 5 accordingly.*
- **S3 — `#558` torn `chain.cbor` → silent genesis fallback.** A real silent-loss residual against the no-silent-loss floor. *Recommendation: RC-blocking; the fix direction is in the repro doc (atomic chain-store write + refuse-to-start on a torn tail); one builder PR, no research gate.*
- **S4 — `#437` transport authentication (TLS/Noise).** Certified NOT a safety break; residual = censorship over a controlled link. *Recommendation: post-RC (`1.x`), research-gate the crypto choice then.*

### The Boulders — status ledger (trued up 2026-09-07)

The five Boulders are unchanged as the organizing spine; each Rock is one line here. DONE means
...
- **OPEN (Lane B):** B1 `R-STRUCTURE-REDERIVATION` (+ `R-BOXENTRY-RESIDUALS`, the `HeadRef` trio, `R-INVENTORY-HAND-LIST`, R3.2) · B2 `R-membership` · B3 recovery boundary · B4 `R-CARRIER-BYTES` · B5 O-2 probe · B6 the four small items · B7 R1.8 the flip. Stamp-raise deliverables `R-CARRIER-PRUNED-HASH`, `R-CARRIER-MODELCHECK`, `R-CARRIER-ROLLOUT-SIGNAL`, `R-E2E-ERA4-FIXTURE`, `R-ISSUERKEY-POP` sit in Lane D. Lane A (the h43 stall) is consensus liveness and is homed here.
...
- **OPEN (Lane C):** C1 the v2 primitive deletion · C2 five owner calls · C3 R2.2 · C4 the R2.7 telemetry · C5 the pre-flip closers · C6 R2.4 · C7 R2.7 · C8 R2.8 · C9 measurements · C10 small PRs.
...
| `R-BOUNTY-TRUNCATION` | ACTIONABLE | Lane C5 · REOPENED 2026-09-07 (blind PE on 4′, `silt-reviews/principle-engineer/RULING-default-chunk-256k-manifest-framing-b365f10-2026-09-07.md` item 3): the 256 KiB default pays exactly 10 (truncation 0.006 %), but the MECHANISM — a floor with no accumulator, unlike the serve lane — is unfixed, and `-chunk-size 65536` still under-pays 20 % (exact 2.5006 → 2) with no warning (`bountyChunkWarning` speaks only at base 0). Researcher-owned per the Economist advisory; closer: the Researcher's call between an accumulator and a truncation warning at the geometry; Boulder 2 / R2.4 pre-flip | L497, silt-reviews/economist/ADVISORY-default-chunk-size-256KiB-2026-09-06.md | — |
| `R-MANIFEST-PADDING` | CLOSED-BY-BOUND | CLOSED 2026-09-07 (4′ BUILT and MERGED, #765, under the NEW genesis the owner accepted): `pipeline.ManifestFrameSize` frames a manifest that fits in one chunk at its own length + 8; larger manifests keep the chunk size. `chunk.Split` pads every frame to the chunk size and manifests carry no parity (87.6 % of the flixz store is 1.4 KB manifests padded to 65,536 B); closes with the same PR as the 262,144 B default — one content-addressing break (D-R2.9-NODE-HALF-CALLS 4′, Boulder 2) | L497, silt-reviews/economist/ADVISORY-default-chunk-size-256KiB-2026-09-06.md | — |
...
| `R-STRUCTURE-REDERIVATION` | ACTIONABLE | Lane B1 · archive:L561: "RATIFIED 2026-09-03; BUILD OPEN, after R-STATEVIEW-ENUMERATION." Closer: the structure build PR (Boulder 1, own session). | archive:L506, archive:L561–611, PE/RULING-floorbox-predicate-rederivation-structure-2026-09-03.md | — |
...
| `R-CARRIER-PARENTPROPOSER` | ACTIONABLE | Lane B1 · archive:L654: "RE-PRICED: NOT a leaf, NOT pre-freeze … Remains a flip precondition." Closer: row 4's `HeadRef`. | archive:L511, archive:L654–701 | — |
| `R-LOGROOT-FORMAT-SCOPE` | ACTIONABLE | Lane B1 · archive:L760: "a flip-gated SAFETY item; closes with ZERO format change" — verify-not-recompute with `parentLogRoot` in the head record. Closer: row 4; liveness half needs `tagRevLogSize` (R3.4, archive:L1253). | archive:L646, archive:L760–809 | — |
| `R-BOXENTRY-RESIDUALS` | ACTIONABLE | Lane B1 · archive:L593: "OPEN (box-entry round A, HELD unmerged; the HOLD is RATIFIED)"; "Live findings owed with it". Closer: the structure build (part B). [note: Contains rows 10–16 and the G-F clause.] | archive:L512, archive:L593–648, silt-reviews/research/research-outcome/floorbox-box-entry-round-A-fee43ba-DELTA-CERTIFICATION-2026-09-03.md | — |
| `R-SUPPORT-SLASHED-SCREEN` | ACTIONABLE | Lane B1 · Closer: structure build — the build-plan cert deletes `BoxQuorumSupport` (`RO/FLOORBOX-STRUCTURE-BUILD-PLAN-CERTIFICATION-2026-09-03.md` residual table). | archive:L601 | — |
| `R-MALFORMED-DIVERGENCE` | ACTIONABLE | Lane B1 · Build-plan cert: "CLOSED — `BoxQuorumSupport` is deleted, not fenced" — in the PLAN; ROADMAP archive:L601 still lists it open. Closer: row 4. | archive:L601 | — |
| `R-PIN-LABEL-ESCAPE` | ACTIONABLE | Lane B1 · Closer: row 4 (driver-supplied inputs become box-owned). | archive:L602 | — |
| `R-PIN-REANCHOR-POSITION` | ACTIONABLE | Lane B1 · Closer: row 4. | archive:L602 | — |
| `R-PIN-BUDGET-ESCAPE` | ACTIONABLE | Lane B1 · Build-plan cert: "RE-PRICED — now buildable (`s.budget` is box-owned). Not owed this round." Closer: row 4. | archive:L602 | — |
| `R-FENCE-TABLE-DRIFT` | ACTIONABLE | Lane B1 · Build-plan cert: "CLOSED by NG-5 + the one-door surface" (plan). Closer: row 4. [note: Instance of row 17 (archive:L869: "mirrored by R-FENCE-TABLE-DRIFT").] | archive:L602, archive:L869 | — |
...
| `R-CARRIER-CREDIT-DENIAL` | ACTIONABLE | Lane B6 · archive:L693: "GATED (window); minimum REFUTED; vector REFUTED"; archive:L700: "Only dated item: a doc fix." Closer: owner call #4 (archive:L70–115, un-ticked) + the doc fix. [note: Spawns row 26.] | archive:L73, archive:L693–743 | — |
...
| `R-CARRIER-ROLLOUT-SIGNAL` | ACTIONABLE | Lane D1 · archive:L749: "CERTIFIED minimum; no part is a consensus rule; the override is LATENT"; archive:L753: "Release-runbook precondition: any future `Era4ActivationHeight` flag lands with the named-upgrade check." Closer: the stamp-raising release (R3.4). | archive:L635, archive:L749–798 | — |
...
| `R-CARRIER-PRUNED-HASH` | ACTIONABLE | Lane D2 · archive:L672: "OPEN, owed BEFORE the stamp raise (not a flip gate)." Closer: the two proofs/tests before the stamp raise (Boulder 3 carry-list). | archive:L672–722, archive:L1253 | — |
...
| `R-E2E-ERA4-FIXTURE` | ACTIONABLE | Lane D2 · archive:L892: "NOT independently schedulable; a deliverable OF the stamp raise"; archive:L897: "Owner: accept the e2e cost increase at the stamp raise" (archive:L85, un-ticked). Closer: owner call + fixture upgrade in the stamp-raising release (R3.4). | archive:L85, archive:L892–941, archive:L1253 | — |
| `R-ISSUERKEY-POP` | ACTIONABLE | Lane D1 · archive:L900: "RE-DIRECTED … reserve the format slot at the stamp raise"; archive:L906: "Owner: build the slot now vs reserve-only. Until then, correct the two comments." Closer: owner call #8 (archive:L83, un-ticked) + research-gated D-DEMAND change + slot at R3.4. [note: Canonical for the L359 entry "R0.4b-PoP" (same DSKS fact, older statement).] | archive:L83, archive:L900–950, archive:L1253 | — |
...
| `R-LATE-REVEAL` | ACTIONABLE | Lane D1 · archive:L1253: the (d-3) two-level block hash is "the ONLY close for the cap's completeness face and for R-LATE-REVEAL; free while v5 is dark". Closer: (d-3) designed once with the carrier's `Hash()` pin at the R3.4 freeze. | archive:L1253, silt-reviews/research/research-outcome/I5-cross-height-pruned-slash-forgery-FIX-DIRECTION-…-2026-09-03.md | — |
| `R-AAXIS-TAG-RESERVE` | ACTIONABLE | Lane D1 · archive:L1253: on the R3.4 pre-freeze carry-list ("one line"). Closer: the R3.4 freeze PR. | archive:L1253, archive:L1277, silt-reviews/research/research-outcome/R4.2-A-axis-…-2026-09-03.md | — |
...
| `R-BB-ESTIMAND-MISSPECIFIED` | ACTIONABLE | Lane C9 · archive:L1221: "NOT built and carried forward: G-BB-21 (re-specify the estimand … this BLOCKS THE RUN)". Closer: G-BB-21 restatement in the `B_bootstrap` run/handoff record (Researcher/Planner; R2.9a). | archive:L129 | — |
...
| `R-BB-EXPORT-SCALAR-BYPASS` | UNCLEAR | since:2026-09-06 — ROADMAP carries no sentence and no closer; the RECERT's unexported-snapshot + package-scope export gate (archive:L1030) looks like its close but no source says so. Needs the Researcher to state closed-or-open. | archive:L1030, docs/thinking/2026-09-04-r29a-min-r-floor.md | — |
...
| `R-BB-TOKEN-MODE-STARTUP-ONLY` | UNCLEAR | since:2026-09-06 — No disposition or closer stated anywhere. Owner/Tester call: accept as operator posture, or add a periodic re-check gate. | archive:L1036, docs/thinking/2026-09-05-r29a-g12-reader-is-operator.md | — |
...
| `R-DEMAND-PRICE-LEVEL` | ACTIONABLE | Lane C5 · `docs/decisions.md` D-R2.9-NODE-HALF-CALLS (2): "a separate item for the economy-on decision; the bonded-fetcher credential is its lever." Closer: owner decision at R2.4 (Boulder 2). [note: Canonical for row 77.] | archive:L1006 | — |
...
| `R-V2-V3-DEMAND-DILUTION` | CLOSED-BY-BOUND | CLOSED 2026-09-07 by B-9: the flat receipt is refused at the node, so one face has no second surface to buy a v2 demand unit on. archive:L1006: "closed by B-9's retirement PR, where `SubmitDeliveryReceipt` now has zero production callers". Closer: the B-9 flat-path retirement PR (R2.9). | archive:L1006 | — |
| `R-SESSION-WALLCLOCK-STEP` | ACTIONABLE | Lane C2 · `docs/decisions.md` :2045: "Decide the wall-clock-step exposure (`R-SESSION-WALLCLOCK-STEP`) with (1)" — i.e. with the G-6 refund certification; the idle window itself waits on `T_b` (call 5). Closer: G-6 refund cert (+ `T_b`). | archive:L1006 | — |
...
| `R-LAMBDA-DUST′` | ACTIONABLE | Lane C5 · L356: "RESEARCH-GATED … the Researcher owes the correction." Closer: Researcher correction to `RO/G-R212-7-lambda-redenomination-RESEARCH-CERTIFICATION-2026-09-06.md` §6.2. [note: One row with `R-LAMBDA-DUST` (decisions.md :1970, unprimed).] | L356 | — |
| `R-RELAY-ANON-SET′` | CLOSED-BY-BOUND | merged 2026-09-07 into `R-RELAY-ANON-SET` (triage §3: same fact, one closer) | L358 | `R-RELAY-ANON-SET` |
| `R-ANCHOR-STALL` | ACTIONABLE | Lane C2 · archive:L64: "Two owner calls carried, BOTH OPEN … (1) R-ANCHOR-STALL for v1, proposed as a disclosed residual"; archive:L1213: "Owner sentence still unsaid". Closer: the owner sentence at the re-priced numbers + follow-on R2.14b `MsgRelayFund` (R2.14). [note: Canonical for row 97.] | archive:L64–107, archive:L1168–1212, archive:L1213, silt-reviews/research/research-outcome/G-R212-2-relay-lane-reprice-…-2026-09-06.md | — |
...
| `R-RELAY-WASH-ZERO-LOSS` | ACTIONABLE | Lane C5 · archive:L1172: "DECIDED 2026-09-04 with R2.9 sentence 6: NO relay skim in v1; re-opens with R2.12"; R2.12 is BUILT (archive:L1076), so the re-open trigger fired; archive:L66: "(2) a relay skim before R2.4 — OPEN." Closer: owner call before R2.4. | archive:L1006, archive:L1172 | — |
...
| `R-REAPER-FORFEIT` | CLOSED-BY-BOUND | CLOSED for the delivery lane 2026-09-07 (the idle close returns the deposit; a reaped session forfeits nothing but latency — refund cert §3.8); stands for the relay lane, which keeps the burn. archive:L1207: "Tester must MEASURE whether a maximum session settles inside one epoch before the flag is ever enabled" — needs `T_b`. `T_b` MEASURED 2026-09-07 (44 s/height, one epoch ≈ 5.9 min); the relay-lane check that a maximum 24.4 GiB session settles inside one epoch is a Tester measurement owed before `-accept-relay-payments` is ever enabled (Lane C2). | archive:L1206–1250 | — |
| `R-REFUSE-AND-SELF-SPEND` | ACTIONABLE | Lane C5 · archive:L1211: "inert while relay balance is private bookkeeping, load-bearing at FP-2/R2.10 — on that carry-list; direction (serial bound to root, or accept-and-gate) research-gated before R2.9." The research direction is not recorded anywhere, and R2.9's ledger and node halves have since shipped (archive:L1006). Closer: the Researcher's direction; owner to confirm whether the FP-2 inert posture covers it. [note: Inert half → FP-2 carry-list.] | archive:L1208–1254 | — |
| `R-CREDITSPENT-UNBOUNDED` | ACTIONABLE | Lane C2/D1 · archive:L1139: "Owner call (PE recommends accept): the cap is an OPERATOR-MANAGED ceiling … until R-CREDITSPENT-UNBOUNDED (… a credit-format change …; research-gated) lands." Closer: owner accepts the interim posture; research-gated credit-format change (a stamp-raise-class item). | archive:L1127, archive:L1139–1185 | — |
...
| `R-PARITY-AMPLIFICATION` | ACTIONABLE | Lane C5 · L543: per-stripe BUILT, "NOT claimed discharged (research-gated; a corrupting provider still forces `S·N/K` because `fetchFrom` transfers before it verifies)". Closer: a Researcher discharge certification (or an explicit acceptance of the worst case under the 64 GiB pin). | L543, docs/decisions.md | — |
| `R-SPARSE-COLUMN-PROVIDER` | UNCLEAR | since:2026-09-06 — "Filed, not fixed" (`docs/thinking/2026-09-06-parity-fetch-per-stripe.md` :78); no closer named. Belongs with the #500/#501/#502 repair-sweep family (L558). | L546–1868 | — |
...
| `R-PS-LOCAL-ROT-NOT-HEALED` | UNCLEAR | since:2026-09-06 — No closer named. Candidate home: the repair-sweep family (#277/#500). | L552–1875 | — |
...
| `R-GENESIS-HASH-FREEZE-SURFACE` | ACTIONABLE | Lane D1 · Is the height-0 block hash inside the era-3/4 freeze surface (R3.4), and what is the migration rule once a network holds persisted chains? Filed by the blind PE on 4′ (`silt-reviews/principle-engineer/RULING-default-chunk-256k-manifest-framing-b365f10-2026-09-07.md` item 1): the manifest framing moved the genesis hash (7becf754…32ce → f428d0a8…0951) with no refusal and no log line. OWNER RULED 2026-09-07: accept the new genesis now (no live network; development chains are wiped on upgrade); `TestGenesisBlockHashIsPinned` holds the literal so every further move is explicit. Closer: R3.4 — the freeze stamps height-0 identity and answers the migration rule (Researcher); Boulder 2 / pre-freeze carry-list | silt-reviews/principle-engineer/RULING-default-chunk-256k-manifest-framing-b365f10-2026-09-07.md | — |
