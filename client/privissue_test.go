...
// demandIssueEpoch is the issue epoch the D3 tests withdraw under. A non-zero value so a
...
func TestPrivateWithdrawalUsesEphemeralIdentity(t *testing.T) {
	...
	tok, ephID, err := WithdrawDemandTokenPrivately(rng, issuerID, tr.Addr(), issuerPub, demandIssueEpoch, credit, 10*time.Second)
	if err != nil {
		t.Fatalf("private withdrawal: %v", err)
	}

	// The token is a real, verifiable demand token.
	if !demand.VerifyToken(issuerPub, demandIssueEpoch, tok) {
...
func TestPrivateWithdrawalRefusedWithoutCredit(t *testing.T) {
	...
	_, _, err = WithdrawDemandTokenPrivately(rng, issuerIdent.NodeID(), tr.Addr(), issuerPub, demandIssueEpoch, bogus, 5*time.Second)
	if err == nil {
...
func TestPrivateWithdrawalThroughRelay(t *testing.T) {
	...
	tok, ephID, err := WithdrawDemandTokenPrivately(rng, issuerID, issuerRelayAddr, issuerPub, demandIssueEpoch, credit, 15*time.Second)
	if err != nil {
