...
func mintCredit(t *testing.T, issuer *blindtoken.Issuer) ports.PublishCredit {
	...
	blinded, secret, err := blindtoken.BlindCredit(rand.Reader, pub, demandChainID, serial)
	if err != nil {
	...
	csig, uerr := blindtoken.UnblindCredit(pub, demandChainID, serial, sig, secret)
	if uerr != nil {
...
// demandIssueEpoch is the issue epoch these tests withdraw under. A non-zero value so a
// dropped epoch (the (b1) ablation) cannot pass by coinciding with the zero value.
const demandIssueEpoch = uint64(7)

// demandChainID is the network these tests withdraw on — the durable parent's chain id,
// stood in for here. It is NON-ZERO and non-uniform for the same reason demandIssueEpoch is
// non-zero: a dropped chain id must not pass by coinciding with the zero value, and a
// truncated one must not pass by coinciding with a repeated byte.
var demandChainID = ports.Hash{
...
//
// The D3 withdrawal moved off the publish-token lane onto the per-epoch demand lane,
// because the publish key never enters the demand keyset — a demand token bought on
// MsgTokenRequest is one no bank will ever honour. echoEpoch lets a test make the
...
func mockIssuerHandlerEpoch(tr *tcpnet.Transport, issuer *blindtoken.Issuer, sawFrom chan ports.NodeID,
	...
	return func(from ports.NodeID, msg ports.Message) {
		...
		if msg.Credit == nil || !blindtoken.VerifyCredit(pub, demandChainID, msg.Credit.Serial, msg.Credit.Sig) || spent[string(msg.Credit.Serial)] {
...
func TestPrivateWithdrawalUsesEphemeralIdentity(t *testing.T) {
	...
	tok, ephID, err := WithdrawDemandTokenPrivately(rng, issuerID, tr.Addr(), issuerPub, demandChainID, demandIssueEpoch, credit, 10*time.Second)
	if err != nil {
		t.Fatalf("private withdrawal: %v", err)
	}

	// The token is a real, verifiable demand token.
	if !demand.VerifyToken(issuerPub, demandChainID, demandIssueEpoch, tok) {
...
func TestPrivateWithdrawalRefusedWithoutCredit(t *testing.T) {
	...
	tr.SetHandler(func(from ports.NodeID, msg ports.Message) {
		...
		if msg.Credit == nil || !blindtoken.VerifyCredit(issuerPub, demandChainID, msg.Credit.Serial, msg.Credit.Sig) {
	...
	_, _, err = WithdrawDemandTokenPrivately(rng, issuerIdent.NodeID(), tr.Addr(), issuerPub, demandChainID, demandIssueEpoch, bogus, 5*time.Second)
	if err == nil {
...
func TestPrivateWithdrawalThroughRelay(t *testing.T) {
	...
	tok, ephID, err := WithdrawDemandTokenPrivately(rng, issuerID, issuerRelayAddr, issuerPub, demandChainID, demandIssueEpoch, credit, 15*time.Second)
	if err != nil {
