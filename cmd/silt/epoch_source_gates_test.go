...
// The two cmd/silt SOURCE gates, in the laneoff_test.go style: cmdDaemon's wiring sits
// inside the flag-driven block with no callable seam, so these see literals and their
...
// TestDaemonRefusesPaidLanesWithoutAnEpochClock_Source is the text half of.
//
// RUNTIME GATE: e2e TestPaidLanesRefuseToStartWithoutAnEpochClock (an OS-process
...
// assertion.
func TestDaemonRefusesPaidLanesWithoutAnEpochClock_Source(t *testing.T) {
	...
	if found < 0 {
		t.Fatalf("SOURCE GATE: no `%s` line in daemon.go names %s, %s AND %s together. Checked: "+
			"three literals on one line, per occurrence of the marker.: with effective "+
			...
			"observed by e2e TestPaidLanesRefuseToStartWithoutAnEpochClock", marker, receipts, relay, epochs)
	}
...
// TestDaemonWiresTheLedgerEpochSourceToTheNode is the text half of.
//
// RUNTIME GATE: TestLedgerEpochFollowsTheChainThroughTheDaemonSeam (this package;
...
func TestDaemonWiresTheLedgerEpochSourceToTheNode(t *testing.T) {
	src, err := os.ReadFile("daemon.go")
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(hits) != 1 {
		t.Fatalf("SOURCE GATE: daemon.go calls `wireLedgerEpochSource(ledger, nd)` %d times, want exactly 1. "+
			...
			"and anchor spend runs at epoch 0 and the paid lanes brick at the guard cap ( "+
			"; more than one means the ordering below no longer decides which source wins. "+
