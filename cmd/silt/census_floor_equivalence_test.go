...
// THE EQUIVALENCE GATE, asserted AT THE WIRE.
//
// THE PROPERTY, and why it replaced a field list. An early enumeration named three
// fields; the build widened it to five; a two-file source gate claimed the rest. All three enumerations
// missed a field, in one change — `ageExceedsUptime` (census-derived, published
...
// the anonymity set: a disclosure, carried in the project brief rather than hidden.
//
...
//
// FIXTURE RULES. Every arm of a group replays the
// SAME clock script, so clock state is equal BY CONSTRUCTION — unlike the byte-identity
// arm, which holds only because its fixture pins clk.now and must not be cited as a
// production property. And r29aServer sets uiServer.peerCount, a nil func in the
// economyServer fixture that segfaults /api/status without it.
...
func bb20Render(t *testing.T, g bb20Group, c bb20Census) (string, map[string]any) {
	...
	if !present {
		t.Fatalf("fixture %s/%s: the bBootstrap block is absent from /api/status", g.name, c.name)
	}
	if block["clockSource"] != "injected" || block["monotonicSource"] != "injected" {
		t.Fatalf("fixture %s/%s did not inject both clock sources (%v / %v): the comparison would be between two dead instruments",
...
// TestBelowFloorBlockIsAFunctionOfTheClockAlone is.
//
// RUNTIME GATE: this test IS the runtime cover for. It observes published bytes
// and reads no source text.
func TestBelowFloorBlockIsAFunctionOfTheClockAlone(t *testing.T) {
	for _, g := range bb20Groups() {
		...
		t.Run(g.name, func(t *testing.T) {
			...
			for _, c := range g.census[1:] {
				...
				t.Fatalf("FAILED in group %q, which discriminates %s.\n"+
...
// TestBelowFloorEquivalenceHasTeeth is the POSITIVE CONTROL, and it is the reason a green means
...
// not, the arms compared by this gate were identical before the floor ever ran.
//
// RUNTIME GATE: TestBelowFloorBlockIsAFunctionOfTheClockAlone.
func TestBelowFloorEquivalenceHasTeeth(t *testing.T) {
	...
	for _, g := range bb20Groups() {
		...
		t.Run(g.name, func(t *testing.T) {
			...
			if a == b {
				t.Fatalf("POSITIVE CONTROL FAILED for group %q: two DIFFERENT censuses published byte-identical blocks ABOVE the floor, so 's byte-identity below the floor is a property of the fixture and not of the floor.\n  %s\n  %s", g.name, a, b)
			}
		})
	}
}
