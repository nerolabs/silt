...
// This lane — the SHIPPED half of the delivery idle window: the constants the daemon
// runs on, pinned to the derivation the RED-first gates in
// c2_idle_window_default_test.go judge. Those gates ask "does the shipped value clear
// the bound"; these ask "is the shipped value the DERIVED one, and is it the one the
// flag actually hands the node".
//
...
// The shipped constants ARE the derivation. Every input is cross-pinned to the
// independently-written copy, so a change to either side is a RED test and not a silent
// divergence: the divisor to core/node's (via c2StampDivisor, itself pinned by core/node
// TestC2StampDivisorIsFour), the bound to (21), the floor to the predicate, and the
// default to the flag's own source literal.
func TestC2ShippedFloorIsDerivedFromTheBound(t *testing.T) {
	...
	if deliveryIdleBound != c2GoverningBound {
		t.Fatalf("deliveryIdleBound = %v, want %v (the workless-designee bound: (N+2)·ChainSyncInterval + G = 14·30 + 10 at N = 12)",
	...
	if deliveryIdleFloor/2 <= 0 {
		t.Fatal("deliveryIdleFloor/2 is not positive — the delivery-sweep ticker would panic (a review item 3)")
	}
}

// The shipped DEFAULT, and the reason it is 24m rather than the tighter 10m that also
// clears the floor. The margin datum is the ONE stall the field produced (1040 s, the
// a field run); the survival claim about it is driven in core/node
...
func TestC2ShippedDefaultClearsTheFloorAndTheObservedFieldStall(t *testing.T) {
	...
	if deliveryIdleFieldStall != 1040*time.Second {
		t.Fatalf("deliveryIdleFieldStall = %v, want 1040 s = 17m20s — block 43 committed 17 min 20 s after block 42 on run c450985-deep "+
			"(evidence: integration/cloudtest/-stall-evidence-c450985-deep/README.md; the same figure is core/node's c2FieldStallObserved, "+
...
// The daemon's periodic sweep cadence, derived from the INSTALLED window. The interval
// must stay strictly positive at every window the daemon can accept: at the old 1 s
// floor `idle/2` was 500 ms and at anything sub-second it rounds toward zero, which
// panics time.NewTicker. UNGATED: this pins the
// arithmetic, not the firing. No tier observes the goroutine in cmd/silt/daemon.go
// actually posting a sweep; at the shipped 24m window it fires at 12m, longer than any
// graded cloud flow lives.
...
// The flag hands the node the DERIVED CONSTANT, not a literal that happens to match it
// today. This gate sees STRINGS ONLY: it reads daemon.go and checks the expression
// inside the fs.Duration call, so it can tell `deliveryIdleDefault` from
// `24*time.Minute` but cannot tell you what either evaluates to at run time.
// RUNTIME GATE: e2e TestDeliveryIdleWindowDefaultBootsThePaidLane — the daemon booted with
// -accept-delivery-receipts and NO -delivery-idle-window announces `idle window 24m0s`
// On its affordability line and is not refused.
