...
// — the daemon tier's default-build gates. Each one asserts an ABSENCE, so
// the file compiles only without the `bbootstrap` tag.
...
// TestDefaultBuildHasNoBBootstrapFlag asserts a default silt binary does not
...
// OWN flag set: an earlier build put `fs.Bool("bbootstrap", false, …)` straight into the
...
// ordinary test job.
func TestDefaultBuildHasNoBBootstrapFlag(t *testing.T) {
	fs := flag.NewFlagSet("silt", flag.ContinueOnError)
	fs.SetOutput(new(strings.Builder))
	on := registerBBootstrapFlag(fs)
	if f := fs.Lookup("bbootstrap"); f != nil {
		t.Fatalf("-bbootstrap is declared in a DEFAULT build (default %q). The instrument compiles only under the `bbootstrap` build tag ", f.DefValue)
	}
	if on() {
	...
	for _, f := range files {
		...
		for _, decl := range flagNamesDeclaredIn(t, f) {
			if strings.Contains(strings.ToLower(decl.name), "bbootstrap") {
				t.Fatalf("SOURCE GATE: %s declares the flag %q (via %s) and compiles in a DEFAULT build. That puts -bbootstrap — and every reference the instrument drags in behind it — into every shipped silt binary, which is exactly what removed. Declare it in a file carrying //go:build bbootstrap", filepath.Base(f), decl.name, decl.method)
			}
		}
	}
	if !sawDaemon {
...
// TestDaemonHandsTheNodeAWallClock pins the fact the texts got BACKWARDS. Four
// sites said RecordBondChallenge's tick was "stamped from the bond auditor's own
// request counter rather than from a wall clock". It is a wall clock:
...
//
// The first-seen stamp that fact exposed (residual) has since been DELETED under
// — nothing read it. The unit still matters after the deletion: the same tick is kept as
// lastBondTick and DecayStale compares it against a nanosecond BondMaxAge, so a node
// clock that was not a wall clock would silently disable retention (core/credit's
...
//
// RUNTIME GATE: TestWalltimeAdapterIsAWallClock (below) runs the adapter this reads
// the name of; core/node's TestBondAuditStampsAWallClockNanosecondNotACounter
// observes the stamp the auditor derives from it.
func TestDaemonHandsTheNodeAWallClock(t *testing.T) {
...
// TestDefaultBuildStatusHasNoBBootstrapKey asserts on the BYTES /api/status emits,
// which is the boundary that matters — the same boundary this gate asserts the
// floor's property on, one build over.
//
...
func TestDefaultBuildStatusHasNoBBootstrapKey(t *testing.T) {
	...
	if n := reflect.TypeOf(statusExtras{}).NumField(); n != 0 {
		t.Fatalf("statusExtras has %d field(s) in a DEFAULT build, want 0. It is embedded in the /api/status payload, so any field here is a key a default silt binary can publish ", n)
	}
...
// TestWalltimeAdapterIsAWallClock is the runtime half: the adapter named in the
...
func TestWalltimeAdapterIsAWallClock(t *testing.T) {
	...
	if got < 1_500_000_000_000_000_000 {
		t.Fatalf("walltime.Clock.Now = %d, below Unix-nanosecond magnitude — a tick of this size would read as a counter, which is exactly the misreading that put a false fact in four texts ", got)
	}
}
