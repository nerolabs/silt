...
// THE WIRING, as a SOURCE gate, with the limit stated rather than glossed.
//
...
// This is the identical shape and the identical limit as for the era pair.
//
...
func TestDaemonPrintsTheNetworkIdentityAtStartUp(t *testing.T) {
	...
	if !strings.Contains(s, call) {
		t.Fatalf("SOURCE GATE: RED — the literal %q is no longer in daemon.go's text, so the daemon "+
			"no longer reports WHICH NETWORK it is on. The renderer is then decoration, and the project "+
	...
	for _, before := range []string{"chainstore.Recover(chainPath, ch, *acceptChainLoss)", "genesis.Build(store, &gp)"} {
		if i := strings.Index(s, before); i < 0 || i > at {
			t.Fatalf("SOURCE GATE: RED — in daemon.go's TEXT the network-identity call appears BEFORE "+
...
// THE THREE GENESIS FLAGS EXIST, CARRY THE SHIPPED DEFAULTS, AND REACH chain.Config.
//
...
func TestTheThreeGenesisFlagsAreDeclaredAndWired(t *testing.T) {
	...
	for _, f := range []struct{ decl, assign, why string }{
		...
		if !strings.Contains(s, f.decl) {
			t.Fatalf("RED: daemon.go no longer declares the flag %q — %s. A committed genesis field "+
		...
		if !strings.Contains(s, f.assign) {
			t.Fatalf("RED: daemon.go declares a flag but never assigns %q into the chain.Config literal "+
		...
		if d := strings.Index(s, f.decl); d > strings.Index(s, f.assign) {
			t.Fatalf("RED: %q is declared AFTER it is assigned in daemon.go's text", f.decl)
		}
	}
}

// THE RC POSTURE, DRIVEN: Era4ActivationHeight = 1 empties the sub-era-4 interval above the
// genesis, and the height-0 residual is named rather than hidden.
//
// This is the measurement the flag default rests on. It does NOT assert that the daemon passes 1 —
// does that on the source — it asserts what committing 1 MEANS, which is the half a source
// gate cannot see.
func TestEra4ActivationOneEmptiesTheSubEra4Interval(t *testing.T) {
	...
	for _, h := range []uint64{1, 2, 3, 8, 1 << 20} {
		if got := c.MintVersion(h); got != chain.BlockVersionWitnessable {
			t.Fatalf("RED: with Era4ActivationHeight=1, height %d mints v%d, not era 4 (v%d). The "+
