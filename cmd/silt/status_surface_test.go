...
// The /api/status surface: the cached fixed-interval snapshot (research
// the gate) and the object-level attribution leak. Both are properties of the WIRE, so
// both are asserted on the raw JSON rather than on a struct that would silently drop a
...
// that assert absence-versus-presence (the three wire states).
...
// TestStaleIsIdentifiableAsStale. A cache that quietly serves an old number is
...
// an analyst can price the staleness without reading the source.
func TestStaleIsIdentifiableAsStale(t *testing.T) {
	s, led := statusServer(t)
	// One served byte so the document is not degenerate. Untagged this records
	// (identity, cumulative bytes) and no `when` at all — the point of
...
// TestOperatorsOwnWriteIsNotHiddenByTheCache. The cache must not hide the
...
func TestOperatorsOwnWriteIsNotHiddenByTheCache(t *testing.T) {
	s, led := statusServer(t)
...
func r29aCaredObject(t *testing.T, s *uiServer, led *credit.Ledger) ports.Hash {
	...
	s.onLoop(func() {
		...
		led.RecordServeToObject(s.nd.ID(), fetcher, root, ports.ChunkID{0x1}, 1024*econMintUnit) // 1024 units → skim 1024
	})
	return root
}

// TestStatusWithholdsPerObjectDetailWithoutAToken is the gate on the object half
// of who-fetched-what.
//
...
func TestStatusWithholdsPerObjectDetailWithoutAToken(t *testing.T) {
	s, led := statusServer(t)
	root := r29aCaredObject(t, s, led)
...
// TestEconomySelfWithholdsPerObjectDetailWithoutAToken closes the twin surface.
...
// says must be withheld, and /api/roots names the root with no token. The.funded and
// the open selfFunding.skimIn as the same number on a live daemon and recovered 131,104
// bytes of a named root from a 16,388 step, at 330 ms. The gate did not miss the leak;
// it pinned it in place. Now it asserts the property that matters: withheld untokened,
// present tokened.
func TestEconomySelfWithholdsPerObjectDetailWithoutAToken(t *testing.T) {
	s, led := statusServer(t)
	root := r29aCaredObject(t, s, led)
...
// and the scan in escrow_wire_gate_test.go), so adding a route reddens every one of
// them until the route has been examined against each scan's property.
const r29aWholeSurfaceGETRoutes = 11

// RAISED 7 -> 11 on 2026-09-08 (this lane), and the examination was REWRITTEN on
// 2026-09-09 after a by measurement. The original text is preserved in the git history
// and; it argued that self's own bytes being one term inside the sample made the two
// gossip-estimated routes safe to leave OPEN. That was false. Keeping the refutation here
// rather than quietly replacing the sentence is the point of a comment that a count is
// anchored to.
//
...
// whatever the token. TestFlowsAndGAreTokenGatedAcrossAMeasuredWindow is the
...
// Node.EconomySample; a reconstruction recovered 987,654,321 with error 0. The recovered
...
// TestAPublishedGiniNeverReconstructsAPrivacyWithheldCounter (which asserts the
// RECOVERY fails, not that a named field is absent) and
// TestTheTwoGossipRoutesHonourThePrivacyClauseOverHTTP.
...
// pinned by TestTheOpenCrowdEstimateIsTheOneStatusAlreadyPublishes), and the C2
...
const r29aWholeSurfaceBytes = 1_234_567 * econMintUnit // 1,234,567 mint units → funded (skim) = 1,234,567
...
// TestNoUnauthenticatedResponseOnTheWholeSurfaceCarriesTheWithheldCounter is the gated
// object, no unauthenticated response on ANY route may carry objects[0].funded, under
// any name. It walks apiRoutes — the list serve registers — so a route added later is
// examined by construction, and it asks "what reconstructs the quantity", not "where
// is the field called funded": the scan is over every number in every body.
//
...
// its aliases reserve, net and per-object skimIn). It does NOT close the single-root
// inference: revenue.servedBytes is 8x funded and balance is 7x funded here, and on a
// node holding one root those node-wide totals are that root's. Those stay open
// because the cross-origin observatory reads stats.BytesServed with no token by
// design; gating them is the project trade, surfaced in the decisions correction. The
// gate logs the open aliases so the residual is measured, not assumed.
func TestNoUnauthenticatedResponseOnTheWholeSurfaceCarriesTheWithheldCounter(t *testing.T) {
	s, led := statusServer(t)
	root := ports.Hash{0xF2, 0x0B, 0x1E}
	fetcher := ports.NodeID{0xC3}
	s.onLoop(func() {
	...
	const funded = int64(r29aWholeSurfaceBytes / econMintUnit) // one skim credit per mint unit
	at := s.started
	...
	t.Logf("F2 whole-surface: %d GET routes carry no %d unauthenticated. OPEN by decision, not by omission: revenue.servedBytes = %d (8x) and balance = %d (7x) on a node whose one held root /api/roots would name",
		walked, funded, full.Revenue.ServedBytes, full.Revenue.Balance)
}

// --- one cache, two views -------------------------------------------------------------

// TestOneCacheTwoViewsAnAnonymousReadDoesNotStripTheOperatorsView. The cache holds ONE
// document and serves it at two privilege levels, so the withholding must be applied
// to a COPY. The.Durability = …` with `doc.Durability = …` — the one-token change a
// future "avoid the copy" optimisation takes — and the whole suite stayed green in
// both builds: on a live daemon one anonymous GET then stripped the operator's own
// solvency panel for the rest of the interval, intermittently, with no error.
// Silent-loss shape (Don't #4), reintroduced on a new axis and ungated. This is the
// gate: tokened, untokened, tokened again, all INSIDE one interval, and the third read
// must be byte-identical to the first and still carry the objects. Both documents,
// because both are served off that one cache.
func TestOneCacheTwoViewsAnAnonymousReadDoesNotStripTheOperatorsView(t *testing.T) {
	s, led := statusServer(t)
	root := r29aCaredObject(t, s, led)
...
// TestEconomySelfIsServedFromTheStatusSnapshot. The floor(uptime/T) bound the claims
// is only true if every endpoint that republishes the ledger aggregates reads the same
// snapshot. As first shipped /api/economy/self recomputed per request, and. This gate:
// a serve inside the interval is invisible on economy/self until the interval turns
// over, the two documents carry the SAME taken-at stamp, and the age moves so a reader
// can tell the cached value from a live one.
func TestEconomySelfIsServedFromTheStatusSnapshot(t *testing.T) {
	s, led := statusServer(t)
	root := r29aCaredObject(t, s, led)
	base := s.started
	...
	s.onLoop(func() {
		led.RecordServeToObject(s.nd.ID(), ports.NodeID{0xC4}, root, ports.ChunkID{0x2}, 2048*econMintUnit) // +2048 skim
	})
