...
// C3 final round — the S5 announced-marker gate for `swarm receipt`.
//
// THE FINDING. `TestDeliveryReceiptRefusedWhenLaneOff` asserts a CLIENT-LEGIBILITY
// contract: a daemon started without -accept-delivery-receipts must refuse, and the
// client must say "NOT banked". The daemon refused correctly; the client did not say
// it. The withdrawal lane added a key-resolution step ABOVE the submit, so a
// lane-off peer — which serves no issuer key at all — returns at that step, and the
// announced marker, which lives only below the submit, became unreachable on the one
// path a user actually hits. That is the announced-string class (the `freeload: ON` rename):
// an announced string is an observable contract, and moving the control flow around
// it breaks the contract just as surely as renaming it.
//
