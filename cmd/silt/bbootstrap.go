...
// The B_bootstrap instrument's WHOLE cmd/silt surface: the -bbootstrap flag, the clock
// injection it gates, the /api/status renderer, and the one extra status key.
//
// THIS FILE COMPILES ONLY UNDER THE `bbootstrap` BUILD TAG. A default `go build` gets
// bbootstrap_off.go instead: no flag, no injection, no renderer, no histogram type.
// `silt daemon -bbootstrap` on a default binary fails at flag parse with "flag provided
// but not defined", which is the intended answer — the mechanism is not there to
// enable.
//
...
// account and its ledger holds exactly what it held before — (identity,
...
// one key; untagged (bbootstrap_off.go) it is empty and contributes nothing.
type statusExtras struct {
	// ABSENT unless -bbootstrap is set. Absent and empty are different objects: a
	// reader that sees no key knows the instrument is off, and a reader that sees
	// the key with zero requesters knows the instrument is on and the ledger is
	// idle.
	BBootstrap *bBootstrapInfo `json:"bBootstrap,omitempty"`
	// BBootstrapWithheld is the marker: the instrument is ON and this reader is
	// NOT the operator, so the block is not served to it. It is a SIBLING key,
	// never a field inside the block — a zero-valued bBootstrapInfo would emit
	// sixteen keys of false facts, including ageAxisLive:false + cells:null,
	// which is byte-identical to the documented "no clock injected" state. Three
	// wire states, each a distinct key set: neither key (default build, or
	// tagged with the flag off); this key alone (on; not the operator);
	// bBootstrap alone (on; the operator).
	BBootstrapWithheld bool `json:"bBootstrapWithheld,omitempty"`
}

// withholdBBootstrap is the clause of uiServer.readerView, the (b) half of: the
// B_bootstrap block is served only to a reader that presented the API token in the
// Authorization header. Anyone else — a reverse proxy or port-forward arriving from
// loopback, a co-tenant process that cannot read the 0600 token file, a cross-origin
// page (the observatory, or any http://localhost:* origin the guard reflects), a ?token=
// query that could have come from a log — gets the withheld marker and no block.
// Together with bbootstrapRefuseRoutableBind (which keeps the token from ever needing to
// leave the host) this is the code establishing that the reader can read
// <store>/ui-token, mode 0600 — the one checkable correlate of "is the operator".
//
...
func registerBBootstrapFlag(fs *flag.FlagSet) func() bool {
	on := fs.Bool("bbootstrap", false, "RECORD AND PUBLISH the B_bootstrap histogram on GET /api/status. A full-census 2-D COUNT histogram over (identity age × log2 fetched bytes) — 8 age buckets × 41 log2 byte bins (one per doubling, 1 byte … 1 TiB), 328 counters, about 1.3 KiB on the wire, constant in the requester count. Counts ONLY: no requester id, no salted label, no per-identity row, no exact age, and no per-cell byte SUM (a cell sum with count 1 is that identity's exact byte total in disguise). The age axis is the boot-relative elapsed tick from the node's injected clock, right-censored at the ledger's uptime — a restart destroys every account, so no window longer than the longest clean uptime is measurable at all. It is the instrument sentence 4 makes a precondition of pinning grant/r, and it is INSTRUMENTATION ONLY: no conservation rule, no standing calculation and no economic rule reads it. This FLAG IS ONLY PRESENT IN A BINARY BUILT WITH -tags bbootstrap; a default silt binary rejects it, because the mechanism is not compiled in. DEFAULT OFF, and off means NOT RECORDED, not merely unpublished: no observability clock is injected, so no account carries a first-touch time. Turning it on takes a restart and then a wait for the population to re-stamp. THE READER MUST BE THE OPERATOR: the daemon refuses to start unless -ui is a loopback bind and <store>/ui-token is owner-only, and the block is served only to a request carrying that token in the Authorization header — read it with: curl -H \"Authorization: Bearer $(cat <store>/ui-token)\" http://127.0.0.1:<port>/api/status. Any other reader sees bBootstrapWithheld:true and no block")
	return func() bool { return *on }
}

// bbootstrapRefuseRoutableBind is the gate′ Part A ("refuse at startup" item 4), and the
// (a) half of: a tagged daemon REFUSES TO START when -bbootstrap is set and -ui
// names anything but a loopback address. The predicate is isLocalHost — the same one the
// request guard uses for the Host header — applied here to the OPERATOR'S OWN flag string
// before anything is bound, so "127.0.0.1:8081", "localhost:8081", "[:1]:8081" and any
// 127/8 literal pass, and "0.0.0.0:8081", "[:]:8081", ":8081" (all interfaces) and any
// LAN or public literal or hostname are refused with both flag names in the message.
//
...
// the absent-vs-empty ambiguity this file forbids everywhere else (the own words for
// form (a)). The shape is the -dht-address-reserve precedent in daemon.go: a
// security-relevant flag combination the code will not run, stated as a constraint.
//
...
// forward, a port-forward and a co-tenant process all arrive from loopback, and Tor's man
// page names the same residual for its own MetricsPortPolicy ("allowing localhost, every
// user on the server will be able to access it"). The reader-is-the- operator half of
// this gate is the token requirement on the block, decided separately.
func bbootstrapRefuseRoutableBind(uiAddr string, on bool) error {
	if !on || uiAddr == "" || isLocalHost(uiAddr) {
		return nil
	}
	return fmt.Errorf("-bbootstrap with -ui %q refused: the B_bootstrap histogram is published on GET /api/status and may only be served on a loopback bind (127.0.0.1, ::1, localhost); bind -ui to loopback and read it locally (curl, ssh -L, docker exec) —  Part A, settled 2026-09-05", uiAddr)
}

// bbootstrapRefuseInsecureTokenFile is the second startup refusal the gate needs, and it
...
// silently hand every local user the operator predicate with every gate green. With
// -bbootstrap set, a token file readable by group or other refuses the start and names
// The fix. A missing file is fine: it is about to be created 0600. Tagged build only; a
// default daemon is untouched.
func bbootstrapRefuseInsecureTokenFile(storeDir string, on bool) error {
	...
	if why := bbootstrapTokenFileIssue(fi, os.Geteuid()); why != "" {
		return fmt.Errorf("-bbootstrap refused: %s %s; the B_bootstrap block is served to whoever presents this token, so it must be the daemon user's and owner-only — run `chown $(id -u) %s && chmod 0600 %s` ", path, why, path, path)
	}
	return nil
}
...
// On a platform whose FileInfo carries no owner the second clause is skipped, stated here
// so the skip is a known gap and not a silent one.
...
// window of measurable uptime (wants monotone uptime >= 2x the read bucket's upper
// edge, so the wait is the run's own precondition and not an extra cost on top).
//
...
//
// TWO SOURCES, ONE CALL. clk is a wall clock — adapters/walltime returns
...
// on that by TestDaemonInjectsBothClocksBeforeTheLedgerIsReachable.
...
// bBootstrapInfo is the published B_bootstrap histogram: a full-census 2-D COUNT
// histogram over (identity age × log2 fetched bytes), the instrument sentence 4 requires
...
// WHERE grant/r LANDS, so the reader does not inherit the earlier error: too low is
// build-immutable #4 (the honest-participation floor); too high is Don't #7 / T-AR /
// build-immutable #8 (unpaid real work at the edge tier, a free-resource surface). Never
// M0's Sybil corner — the grant mints balance and standing is bond-only (Invariant A).
//
...
//
// DEFAULT OFF (-bbootstrap), and served ONLY TO THE OPERATOR: a loopback bind is refused
// at startup otherwise, and the block itself is withheld from any request that does not
// carry the project-only API token in the Authorization header (withholdBBootstrap).
// Reversing a default is cheap now and expensive after adoption, and the measurement
// needs exactly one deployment.
type bBootstrapInfo struct {
	ClockSource string `json:"clockSource"` // "injected" | "none" — the age axis self-report (H-1)
	// PublishDefaultChunkSize is this binary's compiled publish default
	// (pipeline.DefaultChunkSize), the geometry the byte axis is padded to. INSTRUMENT class: a
	// compile-time constant, a function of nothing the ledger holds. The census is a time series
	// and the 4′ change moved its byte axis (64 KiB → 256 KiB), so a future analyst must be able
	// to tell a bin shift that is traffic from one that is geometry. Content published with an
	// explicit -chunk-size is not distinguished here (that per-publish counter is the caller's
	// §5 item 5, not built).
	...
	// THE MINIMUM-REQUESTER FLOOR. suppressed is true when the census held
	...
	// nothing.
	//
	// WHAT THE FLOOR IS NOT. IT BOUNDS THE PUBLISHED
	...
	// admission control — so an observer that can FETCH lifts the floor for nine
	// keypairs and one chunk each, permanently for the process's lifetime. The floor is
	// a FIT PRECONDITION and a defence against a reader that CANNOT fetch. It is not a
	// privacy mitigation against a capable adversary, and the Don't #3 question is not
	// answered by it (both open). Nor does it close the delta trajectory of a POLLED
	// series (open, bounded by poll rate). And `suppressed: true` is itself a
	// disclosure: a published upper bound of R_min − 1 on the anonymity set.
	//
	// THE RULE BEHIND WHICH KEYS ARE ABSENT IS A PROPERTY, NOT A LIST: below the floor
	// this block is a function of the INSTRUMENT fields alone — the injected clock
	// sources, their injection instants and the compiled axis constants — plus this
	// one bit. Every key below that is a pointer is CENSUS class. this gate asserts
	// the property on these exact bytes.
	...
	AgeExceedsUptime        *bool  `json:"ageExceedsUptime,omitempty"`        // the censoring assertion failed — the run is suspect. CENSUS class (a threshold on maxOccupiedAgeEdgeNanos, which the floor withholds): ABSENT below the floor

	// The clock cross-check. uptimeNanos and every age come off ONE
	...
	BinsPerOctave int     `json:"binsPerOctave"` // 1 — one log2 byte bin per doubling
...
// down, unrecorded, because the same flag gates the clock injection.
