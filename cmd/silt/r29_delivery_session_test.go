...
func TestR29DaemonRefusalsAreWiredAtStartup_Source(t *testing.T) {
	...
	if deliveryIdleFloor < time.Second {
		t.Fatalf("SOURCE GATE: deliveryIdleFloor %s below one second — the idle/2 wall-clock ticker interval would round to zero and panic (PE item 3); the runtime cover is e2e TestDeliveryIdleWindowFloorIsEnforcedAtStartUp", deliveryIdleFloor)
	}
}
