...
//
// The property, in the own words: under `-privacy` ON, no open GET route may publish
// numbers from which a sample-stuffing caller recovers a node-wide counter the privacy
// clause withholds. It asserts the RECOVERY fails, not that some named field is absent,
// because the break is a reconstruction and not a direct field.
//
...
//
// recoverUnknown is LIFTED from the proof of concept; PoC
// adversary_gini_test.go rather than reimplemented, so
// the gate solves the attack the adversarial seat actually ran and not a weaker one
// this seat invented.
//
...
// TestGossipSampleIsExactlyGiniOverSampledValues pins that EconomySample computes exactly
...
// solveFromDocument runs the solve against the SERVED document — and it is
// VALUE-SHAPED, not name-shaped, which is the whole point of this rewrite.
//
// WHY IT HAD TO CHANGE. The first version decoded exactly two keys by name, `sample` and
// `serveGini`, while its own docstring claimed "a future edit that republishes the same
// information under a different name must redden here." False. The review republished the
// identical Gini on the same unauthenticated document under the key `workConcentration`,
// changed nothing else, and solved the secret exactly:
//
...
//
// The whole suite in this package stayed **ok**, and the document still said
...
// sibling jsonNumbers (status_surface_test.go) keeps only values that are exact int64s,
...
func TestAPublishedGiniNeverReconstructsAPrivacyWithheldCounter(t *testing.T) {
	...
	if leaf, got, ok := solveFromAnyLeaf(t, r22Serve(t, renamed)); !ok || got != r22Secret {
		t.Fatalf("the same Gini republished under the key \"workConcentration\" was NOT recovered (leaf %v got %d ok %v). The solve has gone back to reading key names, and a rename is exactly what the review walked through this gate green while the unauthenticated document handed out the secret and still said countersWithheld:true", leaf, got, ok)
	}

	// ---- THE SELF ARM (the measurement behind "dropping self is not a fix"). A document
	// built from a sample that EXCLUDES self is still solvable, because the recovered
	// term need not be self: any node with an open route is an oracle for its PEERS'
	// withheld counters. This ran as a hand-driven revert in the first round and passed
	// GREEN, which is what refuted the narrowest of the three fixes decision offered; it
	// is encoded here so the refutation survives without anyone re-running it.
	noSelf := sample
...
// TestNoUnauthenticatedRouteReconstructsTheWorkCounterOnTheWholeSurface widens the arm
...
// TestAPublishedGiniNeverReconstructsAPrivacyWithheldCounter does, on a stuffed sample,
...
func TestNoUnauthenticatedRouteReconstructsTheWorkCounterOnTheWholeSurface(t *testing.T) {
...
// TestTheOpenCrowdEstimateIsTheOneStatusAlreadyPublishes pins the exceptions above, so
...
//
// IT PINS TWO QUANTITIES, and the second was added later (register
// row). The withhold drops `sample.size` on the stated ground that "size is half of the
// equation the Gini is the other half of" — and that half is ALREADY OPEN two routes over:
// /api/status's network block is untouched by the privacy clause and carries KnownPeers =
// len(n.peerCaps) (core/node/capacity.go). Every peerCaps entry has CapTotal
// > 0 and is therefore classifiable, so EconomySample.Size == KnownPeers + (1 if self
// pledges.
//
...
func TestTheOpenCrowdEstimateIsTheOneStatusAlreadyPublishes(t *testing.T) {
	s, _ := statusServer(t)
	s.privacy = privacyDefaultWithheld
	at := s.started
	...
	//The withheld sample SIZE is open on /api/status as KnownPeers. That is a decision,
	//so it is pinned: if it is ever withheld, the "size is half the equation" sentence
	//in ui_economy.go must be re-read, and if the sentence is ever taken to mean the
	//size is secret, this names where it is not.
...
// TestTheTwoGossipRoutesHonourThePrivacyClauseOverHTTP is the wiring half: the withhold
...
func TestTheTwoGossipRoutesHonourThePrivacyClauseOverHTTP(t *testing.T) {
	s, _ := statusServer(t)
	s.privacy = privacyDefaultWithheld
	at := s.started
