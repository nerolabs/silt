...
// The affordability floor reaches the operator. The counters live on the ledger
// (core/credit) and this is the wire half: they ride the faucetInfo block on GET
// /api/status beside grantsDenied, and they ship WHETHER OR NOT a faucet bucket is
...
//
// ABLATION run RED before this shipped: dropping the two counters
...
func TestAffordabilityFloorReachesTheStatusSurface(t *testing.T) {
	...
	if !strings.Contains(base.SpendRefusalNote, "zero proves NOTHING") {
