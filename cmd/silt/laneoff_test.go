...
//
// Why that discipline: the research reintroduced the exact F7 regression with every
// string below intact — a DIFFERENT early return above the switch — and this file stayed
// GREEN with `go vet` clean
// rule fired 2026-09-03. The behaviour now has its own runtime gate, named per test,
// and scripts/check_source_gates.py enforces both halves repo-wide.
...
//
// RUNTIME GATE: core/node TestRestartDoesNotRePayTheSameWireReceipt observes the
...
func TestDaemonLoadsThePaidSerialGuardBeforeTheNodeExists(t *testing.T) {
	...
	case iAttach < 0 || iLoad < 0:
		t.Fatal("SOURCE GATE: `SetPaidSerialStore(` or `LoadPaidSerials()` is absent from " +
			...
			"observed by core/node TestRestartDoesNotRePayTheSameWireReceipt")
	case iBank < 0:
...
// TestDaemonArmsTheRotatorOnlyAfterABootInstall is the H-2 structure gate.
// `rotateDemandKeys` used to be assigned ABOVE the boot install, so the
// boot-rotation-failure branch printed "LANE OFF" and `break`ed out with the scheduler
// still armed: the OnCommit hook kept rotating, and the daemon went on staging
// IssuerKeyReg commitments into consensus and charging withdrawal fees for tokens a
// nil demandBank would deny forever.
//
...
//
// UNGATED: that no rotation goroutine actually RUNS after
...
func TestDaemonArmsTheRotatorOnlyAfterABootInstall(t *testing.T) {
	...
	case iArm < iInstall:
		t.Fatalf("SOURCE GATE: daemon.go assigns rotateDemandKeys at byte %d, ABOVE the boot "+
			...
			"for a bank that will deny every receipt (a review) — is UNGATED at runtime "+
			"", iArm, iInstall)
	case iCount != 1:
