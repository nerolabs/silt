...
// silt binary built without the `bbootstrap` tag. There is no -bbootstrap flag, no
// observability clock injection, no histogram type and no /api/status renderer in the
// binary.
//
...
// exactly as before, because there is no histogram on the surface to contain.
