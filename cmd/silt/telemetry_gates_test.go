...
// This lane / — THE THREE TESTABLE-TELEMETRY GATES
// re-pointed.
//
// WHY THESE GATES EXIST AT ALL. The observability design doc's own §6 gate asserted a
// NETWORK-WIDE repair Gini under a threshold. That gate is vacuous: under the transient
// pony tier serves and relays but does no durability work, so at the vision ratio the
// network-wide repair Gini reads 0.9902 with repair spread PERFECTLY EVENLY over every
// capable node. A threshold below it fails on a healthy network; one above it passes on
// total capture. Three assertions replaced it. This file is two of them;
// the third is named at the bottom with the product surface it needs.
//
...
//
// The published Gini is computed over a SELF-SELECTED sample: under the rule
...
// const block below and pinned by TestIndeterminacyBoundaryIsATheoremOfTheTolerance.
...
// serveGini from 0.0000 to 0.8696 (core/node/economysample.go, the comment). These gates
// may ABORT a canary. They may never PROVE federation.
...
//
// THESE ARE FIXTURE CONSTANTS FOR NAMED SYNTHETIC DISTRIBUTIONS. They are NOT honest bands and they are NOT field thresholds. Both
// of their original provenances were withdrawn by the research after this file's first round measured against them:
//
...
// TestServeGiniHonestNullIsMixDependentNotSampleSizeDependent.
//
...
// TestRepairNullIsHoldingsProportionalSoTheConstantIsNotAFieldThreshold.
//
...
// relative-to-holdings (repair). THE PER-TIER WORK TOTALS THEY NEEDED HAVE LANDED (: node.EconomySample carries ServeBytesByTier / RepairsByTier /
...
// is pertier_work_gates_test.go. The two constants below are unchanged and still
...
type c3ConcentrationWire struct {
	...
	// The two fields the per-tier work totals added (pertier_work_gates_test.go).
...
// c3ServeGate is assertion 1 over the served document. Population = the whole
...
// c3RepairGate is assertion 2. Population = the repair-CAPABLE subset, because
...
// `capableSize` itself , and
// TestCapableSizeClosesTheCrossDocumentRepairCoverageJoin asserts it equals this function's
// nw.capable on one snapshot. Re-pointing this gate at c.CapableSize is the call, not the
// Builder's.
...
// ---- the shapes ----------------------------------------------------------------------
//
// THE HEALTHY SHAPE is the vision ratio, "ponies: horses: archival ~ 10000: 100: 1" at
// an instance that fits maxPeerInfo, with serve work weighted CPU-wise 1: 7:
// 24. Every peer reports, ponies do no durability work.
//
...
// TestRepairNullIsHoldingsProportionalSoTheConstantIsNotAFieldThreshold, which
...
// TestServeWorkFederationOnTheOperatorPanel is assertion 1, re-pointed.
//
...
// TestIndeterminacyBoundaryIsATheoremOfTheTolerance.
//
...
func TestServeWorkFederationOnTheOperatorPanel(t *testing.T) {
	...
	// neither work key, so the rule excludes all of them and the serve series is
	...
	// giniOver renders known:false when the series SUMMED TO ZERO, but the route
	// only publishes the series at all once it holds minGossipSample entries — so
	// the wholly silent sample above never reaches that branch. The reachable shape
	// is a sample whose peers DO report (so they are in the series) but report zero
	// SERVE work: the discriminator is the PAIR, so a peer emitting repairs and
	// no serve is a reporting peer whose serve zero counts. credit.Gini over [0,0,.]
	// returns 0 ("universal poverty is technically equality") and a gate that read
	// the VALUE — or fed it into G_adj as if it were seen — would score it as
	// perfect federation at full coverage.
	repairOnly := []c3Peer{
	...
	// rather than a little. Under those peers are EXCLUDED, so the published figure is
	...
	if concA3.ServeGini.val() > c3ServeGiniMax {
		t.Fatalf("ABLATION A3: the BARE published figure now exceeds %.2f (%.4f). When this arm was written the panel published 0.0000 known:true on total capture. The exclusion rule changed — re-derive the adjustment rather than deleting it.", c3ServeGiniMax, concA3.ServeGini.val())
	}
...
// TestIndeterminacyBoundaryIsATheoremOfTheTolerance pins the reason the gates carry
...
func TestIndeterminacyBoundaryIsATheoremOfTheTolerance(t *testing.T) {
	...
	// And the separate-clause composite really does admit what decision says it
	// admits.
...
// TestRepairWorkWithinTheCapableSubsetOnTheOperatorPanel is assertion 2,
...
func TestRepairWorkWithinTheCapableSubsetOnTheOperatorPanel(t *testing.T) {
	// --- HEALTHY: repair spread evenly over 10 horses + 1 archival, 1000 ponies serving
	// And doing none of it. This is the shape whose NETWORK-WIDE repair Gini is
	// ~0.99. READ c3HealthyPeers's comment first: "evenly" is UNIFORM PER NODE,
	// which is an assumption, and it is the assumption c3RepairGiniMax rests on.
...
// TestRepairNullIsHoldingsProportionalSoTheConstantIsNotAFieldThreshold is the
...
// needs the per-tier work totals .
func TestRepairNullIsHoldingsProportionalSoTheConstantIsNotAFieldThreshold(t *testing.T) {
...
// TestServeGiniHonestNullIsMixDependentNotSampleSizeDependent records the MEASURED
// reason c3ServeGiniMax is a fixture constant and not a field threshold.
//
// RENAMED. The earlier name stated the
// SYMPTOM first measured — the honest disk-weighted Gini falls as n grows and does not
// reach 0.15 until ~5,600 nodes, above the maxPeerInfo bound of 4,096 — and the
...
//
// The entire falling curve is one artifact: below 10,101 nodes the vision ratio 10000: 100:
// 1 cannot be held with an INTEGER archival node, so forcing exactly one over-weights the
// 50 TB tier by 10101/n — 2.47x at n=4,096, 9.99x at n=1,011, 99x at n=102. Sample size was
// a proxy for the real variable, the sampled COMPOSITION, and it is a proxy that fails: two
// samples of the same honest network at the SAME size, differing by one node, read 0.1726
// and 0.0800 — a factor of 2.16.
//
...
//
// AND THE GINI CANNOT CARRY T-AR AT ALL. T-AR is a TIER-SHARE statement; the Gini is a per-node DISPERSION
// statistic. On a network whose settled tier design spans a 500:1 capacity dispersion the two disagree by
// construction: at the ceiling sample below, the Gini reads 0.1726 (over the constant) while the pony tier still
// does 81.8 % of the bytes (far above the tenet floor). This test asserts that disagreement, because it is the
// whole argument for moving the T-AR gate to the tier share (TestTierMixShareIsNodeCountNotServedBytes, and the
// the per-tier work totals).
//
...
// pertier_work_gates_test.go. What this pin still records is unchanged and still true: the
// GINI's honest null moves with the sampled mix, so a size-based alarm on the Gini is
// unsound. Whether that retires this test is the call.
func TestServeGiniHonestNullIsMixDependentNotSampleSizeDependent(t *testing.T) {
	if testing.Short() {
	...
	// TestEdgeMajorityOfServeWorkIsTheTenetNotTheNodeShare drives the product's
...
// TestTierMixShareIsNodeCountNotServedBytes exists because assertion 1
...
//
// THE PRODUCT NOW PUBLISHES IT : node.EconomySample carries
...
// TestEdgeMajorityOfServeWorkIsTheTenetNotTheNodeShare.
//
...
func TestTierMixShareIsNodeCountNotServedBytes(t *testing.T) {
