...
//
// Why this exists: three times an upstream change broke an announced string while every
...
// that defect to the unit tier: TestObservableContractStringsAreStillEmitted asserts each
...
// (the second instance) is invisible here. Therefore: keep the e2e assertion for every
...
// ObservableContract — seed entries verified present at origin/main 748594f.
var ObservableContract = []ContractedString{
	...
	{"delivery receipt banked", "core/node/deliverysession.go", " the session lane emits the SAME marker per settled receipt; e2e TestPaidDeliverySessionEndToEnd finds it", "TestPaidDeliverySessionEndToEnd"},
	{"delivery session closed", "core/node/deliverysession.go", " the close line (reason idle/exhausted/disabled, per-session numbers only — no identity, no object; M0 log audit)", "TestPaidDeliverySessionEndToEnd"},
	{"delivery settlement: p=", "cmd/silt/numeraire.go", "the S5 affordability line, every number computed from the constants", "TestAffordabilityLineIsAnnounced"},
	...
	{"need an epoch clock", "cmd/silt/daemon.go", "the paid-lane refusal at effective EpochBlocks == 0 must name both lane flags and -epoch-blocks; e2e reRefuseLine", "TestPaidLanesRefuseToStartWithoutAnEpochClock"},
	...
	{"re-bootstrapped: recovered from an empty routing table", "cmd/silt/daemon.go", "e2e bootstrap self-heal ", "TestBootstrapRetryRecoversColdStartRace"},
	...
	{"fetch posture: ", "cmd/silt/swarm.go", "the deadline structure a retrieval ran under; the field harness DERIVES its cross-region fetch bound from this line instead of from a harness constant, so deleting it replaces a derived bound with a typed one", "TestPublishCommitFetchOverTCP"},
	{"operation ceiling ", "core/node/swarmclient.go", "the outer bound inside the posture line — the cap the client enforces on itself, reported from the same symbol it is enforced from", "TestPublishCommitFetchOverTCP"},
	{"fetch elapsed: ", "cmd/silt/swarm.go", "what a retrieval SPENT against the ceiling it was held to; the bound above grades nothing without it", "TestPublishCommitFetchOverTCP"},
	...
	{"anchored", "core/node/relaytransport.go", "The settlement line's reason field says the paid session was anchored (min(count, Σ face) settled); the interim's no-anchor value is retired — an unanchored open is refused and never settles", "TestRelayAnchorsAreBoughtOnTheRelaysOwnLedger"},
	// The era observable (freeze manifest item 19). These are the strings the cloud sheet's
	// 13b-delivery-settlement row reads to tell "era-4 dark" from "keys off-commitment". Both
	// the ACTIVE and the NOT-ON-THIS-CHAIN form are registered: the row's whole content is
	// that the two RENDER DIFFERENTLY, so deleting either half to make a build green would
	// restore exactly the ambiguity the row closes.
	{"head version: v%d", "cmd/silt/chainstatus.go", "the head block's rule era; an operator reading a live net's chain.cbor could not see it before this row", "TestChainStatusDistinguishesAnEra4ChainFromADarkOne"},
	...
	{"max atts:     %d, first at height %d", "cmd/silt/chainstatus.go", "max_h len(blocks[h].Atts), the live attestation-carrier width is unmeasured", "TestCensusMeasuresMaxAttsOnANonUniformChain"},
	...
	{"the genesis hash is the IDENTITY and the name is a LABEL", "core/chain/networkidentity.go", "the rule that stops an operator trusting a name two networks can both choose", "TestTheNameNeverRendersWithoutTheTag"},
	{"NO network identity yet", "core/chain/networkidentity.go", "a node with no genesis has no network — the narrated zero that keeps it from rendering a zero hash as a tag", "TestAnEmptyChainHasNoIdentity"},
	...
	// rename was exactly the shape `` exists to stop and the
