...
var ObservableContract = []ContractedString{
	...
	{"paid-serial guard compaction failed", "core/node/deliverysession.go", "R2.13 / R-COMPACT-ORPHAN owed-after: the operator's WARN for the BENIGN compaction-failure class (the durable paid-serial store refused to compact on an expiry sweep; payouts continue, the file stops shrinking); emitted once per NEW failure, carries compact_failures and error; the same numbers ride /api/status deliverySettlement.compactFailures / lastCompactError", "TestBenignCompactionFailureLogsTheWarnMarkerOncePerFailure"},
