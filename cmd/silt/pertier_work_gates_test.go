...
// THE PER-TIER WORK TOTALS, and the T-AR gate they re-found.
//
// WHY THIS FILE EXISTS. T-AR is a TIER-SHARE statement: "the edge tier that does the
// MAJORITY of the work must remain a net-positive place to do it" Part IX. Until the
// per-tier totals landed, nothing published carried a per-tier work quantity, so the tenet
// had no source and the two Ginis beside it answer a different question — they are per-NODE
// dispersion over a tier design that spans a 500:1 capacity ratio. The sibling file
// measures the two statistics DISAGREEING on one sample: G_adj 0.1726 (CONCENTRATED) while
// the pony tier serves 0.8184 of the bytes. Only one of them is the tenet.
//
// AND THE NEAREST-LOOKING FIELD IS THE TRAP. mix[].share is a share of NODE COUNT and reads
// ~0.99 by construction under the target 10000:100:1 vision ratio.
// TestTierMixShareIsNodeCountNotServedBytes measured 0.9891 against a true byte
...
// figure here is a NAMED ABSENCE. That is expected: it grades decentralization in
// the HARNESS, where the operator sets the counters, and claims no production concentration
// alarm. These gates are harness gates. They may abort a canary. They may never prove
// federation.
...
const (
	...
	// threshold in this lane was RETAINED while both Ginis were withdrawn,
	tarEdgeMajorityFloor = 0.50
	// ptValidityMarginNum / ptValidityMarginDen is the 0.8 factor on the
	// mix-conditioned null, and it is [ASSUMPTION] in the own labelling — the
	// still owed a re-derivation on the first field data.
	//
	...
	// by a rounding mode". MEASURED, and re-measured independently before
	...
	// They do differ, by one ulp, OFF the fixed point -- measured over 240,000 mixes of
	// this family, 83,512 of them (34.8 %), including the k = 4 row of the table in
	// TestTheValidityBoundaryIsMeasuredNotDerivedFromTheComparison, where min differs too
	// (0.34042553191489361 against 0.34042553191489366). The table is written in the
	// ratio's values, so the code should be too. NOTHING DEPENDS ON IT: the min clamp
	// absorbs an upward ulp at 0.50, and a downward ulp only loosens the floor, which is
	// the false-PASS direction and never a false RED.
	ptValidityMarginNum = 4.0
	ptValidityMarginDen = 5.0
)

// The disk weighting, in TB, from the published tier table.
//
// WHY THE DISK WEIGHTING AND NOT THE CPU WEIGHTING, given that the same derivation WITHDREW
...
// the Finding 2 forced: the honest null is MIX-dependent, not sample-size
...
// ptEdgeMajorityGate is assertion: ponyShareOfServedBytes >= 0.50, with
...
// c3ServeReportingMin (0.85), measured structurally incapable of the
// job it was standing in for: see
// TestTheConcentratingTierMustBuyDecoysToBuyAPass, where five nodes of 1,011
// -- 0.49 % of the sample -- turn a measured EDGE-MINORITY of 0.1998 into a PASS of 0.9940
// while the sample-wide fraction stays at 0.9951. Under the target 10000:100:1 target the
...
// TestEdgeSilenceDepressesTheEdgesOwnShare asserts it: if only ponies go silent
...
// TestThePerTierCoverageFloorIsATheoremOfTheTenetFloor.
//
...
//
// THE UNIFYING RULE, stated once because two seats rediscovered it separately: every
// concentration statistic carries a coverage refusal at its OWN granularity, and is never
...
// TestEdgeMajorityOfServeWorkIsTheTenetNotTheNodeShare is assertion 1's
...
//
// CONTROLLED REVERT: point ponyServeShare at the node count — `sample.Mix[node.TierPony]`
// over `sample.Size`. MEASURED at both reddening points, because "the concentrated arm
// passes" is a claim and only the run says so: the gate stops FIRST on the healthy arm's
// literal check (published 0.989119683, want 0.914076782), and with that one literal muted
// it reaches the concentrated arm and reports `CONCENTRATED ARM PASS:. the edge tier serves
// 0.9891` — total serve capture read as a PASS, which is the substitution this whole file is
// about.
func TestEdgeMajorityOfServeWorkIsTheTenetNotTheNodeShare(t *testing.T) {
	// --- HEALTHY: the target vision ratio, CPU-weighted, everybody reporting.
	concH, nwH := c3Fixture(t, 41, c3HealthyPeers())
	rH := ptEdgeMajorityGate(concH, nwH)
	if rH.Verdict != ptPass {
		t.Fatalf("HEALTHY ARM %s: %s\n  The settled vision shape must clear its own tenet, or this gate fires on the network silt is trying to build.", rH.Verdict, rH.Why)
	}
	...
	// THE HARNESS DISCIPLINE, asserted rather than assumed: on a GRADED arm the
	// conditioned branch must be inert (the floor IS the flat tenet floor) and coverage
	// must be 1.0. Anything less is a fixture defect, not a finding.
	...
	if rC.WorstTierCoverage != 1 {
		t.Fatalf("CONCENTRATED ARM: worst-tier coverage %.4f, want 1.0000 — a graded arm short of full coverage is a fixture defect, and the silence case has its own gate (TestTheConcentratingTierMustBuyDecoysToBuyAPass)", rC.WorstTierCoverage)
	}
	...
	// CONTROLLED REVERT: set PonyShareOfServedBytes and CapableSize BEFORE the
...
// TestATierWithNoReportingPeerIsANamedAbsenceNeverAZero closes the exclusion hole
// ONE LEVEL DOWN, and it is the reason tierShareOf checks reporters BEFORE the denominator.
//
// THE MECHANISM. Under the rule (core/node/economysample.go) a peer that reported
// neither counter is excluded from the work series rather than counted as a zero. So a tier
// ALL of whose peers are silent contributes 0 to every numerator while OTHER tiers keep the
// denominator positive — and 0/positive is a perfectly well-formed 0.0. That number says
// "this tier does none of the work". The truth is "no peer of this tier told me anything".
// On the shipped -privacy default that is EVERY tier, every time.
//
// CONTROLLED REVERT: DELETE the reporters test from tierShareOf. The silent tier
...
// drives it. The first draft of tierShareOf's comment claimed the ORDER was the
// rule; running it refuted that in one command.
func TestATierWithNoReportingPeerIsANamedAbsenceNeverAZero(t *testing.T) {
	...
	// The two absences are DIFFERENT absences and must not collapse: serve/pledged are
	// "no reporter", repair is "outside the population by construction" — a pony is not
	// repair-capable under coupling (b) even when every pony reports.
	...
	// THE ORDERING ARM. Now nobody at all reports, so BOTH refusals in tierShareOf are
	// live: the tier has no reporter AND the denominator is zero. This is the ONLY input
	// on which the order of the two tests is observable, and it is the shipped -privacy
	// default's own shape: no node gossips its counters, so every peer is excluded and
	// both work series are empty. The reason must be the one an operator can act on.
	allSilent := []c3Peer{
		{capTotal: c3PonyCap, served: 0, repairs: 0, n: 100},
...
// TestEdgeSilenceDepressesTheEdgesOwnShare measures the ONE claim about this
...
//
// CONTROLLED REVERT (shared with TestThePerTierCoverageFloorIsATheoremOfTheTenetFloor and TestTheConcentratingTierMustBuyDecoysToBuyAPass): gate the raw
// observation instead of the interval's lower bound. MEASURED: arm B goes to PASS at 0.8449.
//
// CONTROLLED REVERT: scale the pony numerator by population/reporting — the obvious
...
func TestEdgeSilenceDepressesTheEdgesOwnShare(t *testing.T) {
	// THE MIX IS THE VISION FAMILY AT k = 11, DELIBERATELY: at k >= 11 the min clamp
	// is strictly active, so the conditioned floor IS the flat
	// 0.50 tenet floor and no arm here depends on the conditioned branch. That branch belongs
	// to TestTheValidityBoundaryIsMeasuredNotDerivedFromTheComparison, which is where a boundary belongs. An earlier draft
	// used a 100: 10: 1 mix, where the honest disk-weighted null is 0.1429 and the conditioned floor drops to 0.1143 -- and
	// arm B then read PASS, correctly against THAT floor, which told the reader nothing about edge silence.
	full := []c3Peer{
	...
	// The fixture's OWN assigned shares: in a harness the ground truth is known, so
	// assert against it and not against a model. 1100 pony-units of 1100 + 11*7 + 24 =
	// 1201 with everyone reporting; 550 of 550 + 77 + 24 = 651 with half the edge
	// silent.
	...
	if rA.WorstTierCoverage != 1 {
		t.Fatalf("the control arm's worst-tier coverage is %.4f, not 1.0000. On a graded harness arm anything less is a fixture defect.", rA.WorstTierCoverage)
	}
...
// TestTheValidityBoundaryIsMeasuredNotDerivedFromTheComparison publishes the two
// boundaries this floor has, WITH the runs behind them, and it corrects one
// of them.
//
...
// itself the finding. change ptDiskWeightArchival from 50 to 49. MEASURED, the table reddens
// on its FIRST row — `k=4 (n=405, 400:4:1): null 0.430108 floor 0.344086, want
...
// itself, so the gate must BE the table and never `want:= theFunctionUnderTest(.)`. change
// tarEdgeMajorityFloor from 0.50 to 0.51 — the k = 11 row and the fixed-point assertion both
// redden, so the flat floor is pinned in both directions. AND THE ONE THAT DOES NOT:
// substituting the literal 0.80 for ptValidityMarginNum/Den leaves every arm GREEN. Measured,
// recorded in the const block, and the reason the margin's FORM carries no claim in this
// file.
func TestTheValidityBoundaryIsMeasuredNotDerivedFromTheComparison(t *testing.T) {
	type row struct {
		k                   int
		wantNull, wantFloor float64
	}
	// The literal table. NEVER `want:= theFunctionUnderTest(.)`: a fixed point
	// has no mutation to ablate, so the gate must BE the expected values.
	table := []row{
		{4, 0.425532, 0.340426},
		{5, 0.476190, 0.380952},
		{6, 0.517241, 0.413793},
	...
	// every other arm in the file GREEN (measured independently, twice),
	...
	// honest null realised. n = 556, below the k=10 boundary, and the two floors
	// disagree: the conditioned floor is 0.4000 while the bare tenet floor is 0.5000.
	honestSmall := []c3Peer{
		{capTotal: c3PonyCap, served: 1 * c3Unit, repairs: 0, n: 550},
	...
	// exercised only where it forgives. Every other violation arm in this file sits at
	// k = 10, where the conditioned floor IS the flat 0.50, so none of them reaches
	// EDGE-MINORITY through the min. This one does: a 500: 5: 1 mix has a null of
...
// TestRepairShareIsRelativeToHoldingsWhichIsWhatTheWithdrawnConstantCouldNotBe
// builds the replacement named when repairGini <= 0.40 was withdrawn outright.
//
...
//
// THE REPLACEMENT, from Finding 2b, and every term is measured from the sample:
//
...
//
// The margin 0.20 is the [ASSUMPTION], labelled as such there and owed a
// re-derivation on the first field data. It is not this seat's number and it is not
// asserted as a field alarm — this is a harness gate under
//
// CONTROLLED REVERT: accumulate PledgedBytesByTier over every CLASSIFIABLE peer
// instead of every REPORTING peer (move the line above the return in
...
// core/node's TestPerTierTotalsFollowTheExclusionRuleAndNameTheirAbsences reddens on
// `PledgedBytesByTier[pony] = 12884901888, want 8589934592`.
func TestRepairShareIsRelativeToHoldingsWhichIsWhatTheWithdrawnConstantCouldNotBe(t *testing.T) {
	// HONEST, holdings-proportional: a 4 TiB archival node holds 64x a 64 GiB horse, so
	// it repairs 64x as much. Ponies serve but do no durability work coupling (b).
	//
	// FULL CAPABLE COVERAGE, and it is now a REQUIREMENT rather than an accident: the
	// coverage refusal in ptWorstRepairExcess declines to grade this arm otherwise. An
	// earlier draft carried five SILENT horses here so the revert would be visible at this
	// tier; that made the arm undecidable under the refusal, and the honest reading is that
	// a partly silent capable tier cannot be graded at all. this gate is a core/node-tier
	// defect and core/node's own gate reddens on it -- an ablation belongs at the tier the
	// defect lives at, not at whichever tier can be contorted to see it.
	//
	// THE ARCHIVAL TIER HAS THREE NODES, not the vision ratio's one, and that is
	// forced rather than chosen: a tier share is published only over minGossipSample
	// reporters (TestATierShareWithOneReporterIsThatPeersCounter), so a one-node
	// archival tier publishes NO share and drops out of the renormalisation entirely.
	// Measured on the one-node version, the honest excess reads -0.8649 instead of 0.
	// That is the honest consequence and not a fixture convenience: at the target
	// 10000: 100: 1 target three archival nodes need 30,000 ponies and maxPeerInfo is
	// 4,096, so THE REPAIR ALARM IS STRUCTURALLY DARK ON A VISION-RATIO SAMPLE. It
	// grades a harness topology, which is what asks of it.
	honest := []c3Peer{
	...
	t.Logf("CAPTURED: max(observed - expected) = %+.4f on %q, against the [ASSUMPTION] margin of 0.20", worstCap, worstCapTier)
	// WITHDRAWN as a margin and kept only as the constant the two-arm straddle is asserted
	// against. The NAME says so, because the promotion this file most has to survive is a
	// later seat reading it as a threshold.
	...
	// ADVERSARIAL SILENCE. The SAME capture, with the capturing horse withholding. It
	// leaves BOTH terms and the remaining tiers renormalise, so the alarm reads clean
	// unless the coverage refusal declines to grade it.
	capturedMute := []c3Peer{
		{capTotal: c3PonyCap, served: 1 * c3Unit, repairs: 0, n: 1000},
	...
	// THE CEILING, in the same breath as the separation.1. Item 1 of the doc block,
...
// the SAME mix rows of the SAME document, so there is no join. WHAT THIS STATISTIC DOES NOT
// CATCH — the doc block requires before this merges, because the promotion path is short and
// inviting: a later seat reads a passing test and adds an "archival capture" arm to it.
// Every item is a MEASURED limit of the statistic, not of the fixtures.
//
...
// weak reading, it is no reading -- and it sits at ~59.5 % of honest
...
//
// 4. THE 0.20 IS WITHDRAWN by -- withdrawn OUTRIGHT as an
...
// The substitution is withdrawn on that ground: the raw repair Gini asked "is
...
// threshold, there being no honest null for a pledged distribution.
//
// THE COVERAGE REFUSAL. The same hole that defeats the edge share defeats this alarm and by
// the same mechanism: a wholly or partly silent capable tier leaves BOTH the observed
// numerator and the expected denominator, the remaining tiers renormalise, and the offending
// tier's own silence buys it a clean bill. Measured in the adversarial arm below: the horse
// that does 202 of 232 repairs withholds and the excess reads +0.0448, no alarm, where the
// truth is +0.8259.
//
...
// tolerance would inherit the assumption rather than answer it. Under this is harness
// apparatus and the harness answer to coverage is 1.0 -- anything less is a fixture defect,
// the same discipline arms A1a and B3 already apply to the Gini. It is strictly stronger
// than any floor and costs no parameter.
...
// TestCapableSizeClosesTheCrossDocumentRepairCoverageJoin is the
// item 3.
//
...
//
//	delete `out.CapableSize = &capable` from economyConcentrationDoc -> the join
//	 assertion reddens on `CapableSize:<nil>`.
//	delete the node.RepairCapable test from tierWorkFor ONLY -> the reporting pony
//	 tier renders `&{Known:true Value:<nil>}` (a measured 0.0 repair share over a
//	 denominator it is not in). TestATierWithNoReportingPeerIsANamedAbsenceNeverAZero
//	 reddens too, because the pony's repair refusal collapses into the
//	 no-reporters reason.
//	key RepairsByTier for EVERY tier in core/node ONLY -> this gate stays GREEN,
//	 correctly: the renderer's own predicate still defends. core/node's
//	 TestPerTierTotalsFollowTheExclusionRuleAndNameTheirAbsences is what reddens.
//	 Two guards, and the isolated runs show each covers a different tier.
func TestCapableSizeClosesTheCrossDocumentRepairCoverageJoin(t *testing.T) {
	// A pony that reports repairs. It must never acquire a repair share: under
	// coupling (b) durability is the persistent tiers' work, so the repair series'
	// denominator excludes it and a share of a denominator it is not in is not a
	// ratio.
	peers := []c3Peer{
...
// TestThePerTierCoverageFloorIsATheoremOfTheTenetFloor pins the coverage floor the
...
// repairing, so they are REPORTING peers under and contribute a measured zero to the
...
//	6 0.6 0.6 0.50 PASS
//
// CONTROLLED REVERT (shared with TestTheConcentratingTierMustBuyDecoysToBuyAPass and TestEdgeSilenceDepressesTheEdgesOwnShare): change the PASS test in
// ptEdgeMajorityGate from `r.Lower >= r.Floor` to `r.Observed >= r.Floor`, dropping the interval and gating the raw observation. MEASURED, the m = 4 row
// then PASSES.
func TestThePerTierCoverageFloorIsATheoremOfTheTenetFloor(t *testing.T) {
	type row struct {
		reporting int
		wantPhi   float64
		want      ptVerdict
...
// TestTheConcentratingTierMustBuyDecoysToBuyAPass encodes the addendum Part 1 §4 attack
// permanently, AND the price of defeating the fix — because the fix does not close the
// attack, it prices it, and an earlier name for this gate ("CannotBuyAPass") asserted a
// universal that one node defeats.
//
// THE ATTACK COSTS NOTHING AT FIRST. The party doing the concentrating simply stops
// reporting, which is the compiled default. No Sybils, no forged
...
// coverage clause can ever reach it. Under the target 10000:100:1 target the non-edge tiers
...
// claim one.
//
// CONTROLLED REVERT: see
// TestThePerTierCoverageFloorIsATheoremOfTheTenetFloor.
func TestTheConcentratingTierMustBuyDecoysToBuyAPass(t *testing.T) {
...
// TestATierShareWithOneReporterIsThatPeersCounter is the second floor, and it is a
// DIFFERENT quantity from the coverage refusal above.
//
...
// (gini_reconstruction_test.go). Reconstruction is closed by gossipWithheld. What this
...
//
// CONTROLLED REVERT: delete the `reporters < minGossipSample` branch from
// tierShareOf and this gate reddens on the inversion above, with the recovered 300 in the
// failure message.
func TestATierShareWithOneReporterIsThatPeersCounter(t *testing.T) {
