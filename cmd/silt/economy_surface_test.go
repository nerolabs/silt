...
// The four new economy routes, on the wire.
//
// The two properties the whole-surface scans (status_surface_test.go,
// escrow_wire_gate_test.go) CANNOT carry for these routes: those fixtures pin ONE
...
// the who is TestAPublishedGiniNeverReconstructsAPrivacyWithheldCounter's subject.
...
// TestFlowsAndGAreTokenGatedAcrossAMeasuredWindow is the non-vacuous half of the
...
func TestFlowsAndGAreTokenGatedAcrossAMeasuredWindow(t *testing.T) {
	...
	if flows.Pooled.Consec != flowDrainSamples || !flows.Pooled.Draining {
		t.Fatalf("pooled drain = %d consecutive / draining %v, want %d and true — %d consecutive negative samples is the drain signal",
...
// TestWindowNotYetMeasuredIsNotAZero: before two samples exist there is no delta, and
...
func TestWindowNotYetMeasuredIsNotAZero(t *testing.T) {
	s, led := statusServer(t)
...
// TestGossipEstimatedFieldsNeverRenderWithoutTheirSample drives the PURE renderings
...
func TestGossipEstimatedFieldsNeverRenderWithoutTheirSample(t *testing.T) {
	...
	for name, gv := range map[string]*giniValue{"serveGini": conc.ServeGini, "repairGini": conc.RepairGini} {
		if !strings.Contains(gv.Epoch, "NOT lifetime") {
			t.Fatalf("%s carries epoch %q. The counters behind it reset at every restart, so it is not a lifetime figure and a concentration measure over it partly reflects uptime — publishing the number without that is the same error as publishing a gossip figure without its sample size", name, gv.Epoch)
		}
	}
	if !strings.Contains(conc.RepairGini.Scope, "REPAI") {
	...
	// And symmetrically, the serve series carries its own: a big classifiable
	// sample in which almost nobody REPORTED work publishes no serve Gini ('s
	// population is the reporting subset, not the sample).
...
// TestAnAllZeroWorkSampleIsNotAMeasuredEquality is blocker B2.
//
...
func TestAnAllZeroWorkSampleIsNotAMeasuredEquality(t *testing.T) {
	// The fixture: eight peers in the series, all reporting zero work.
	//
	// (the gossip research) now EXCLUDES a zero from the series upstream, so a live
	// EconomySample can no longer produce this shape — a zero-summed series would have
	// size 0 and fall below the floor. This gate therefore drives the document DIRECTLY
	// with the shape the gate forbids, which is exactly what makes it a regression gate: if a
	// later edit stops excluding zeros, credit.Gini's 0 must still not be published as a
	// measurement. Defence in depth, deliberately, and `known` is required on
	// giniValue by name.
	...
	for name, gv := range map[string]*giniValue{"serveGini": zero.ServeGini, "repairGini": zero.RepairGini} {
		...
		if gv.Known {
			t.Fatalf("%s = %+v over a sample that reported no work at all. A 0 here reads as perfect equality across %d nodes; the truth is that %d nodes said nothing (a review B2)", name, gv, gv.SampleSize, gv.SampleSize)
		}
...
// TestPublishedBandsMatchTheClassifierIsUsing: the bands on the wire are what an
...
func TestPublishedBandsMatchTheClassifierIsUsing(t *testing.T) {
	...
	for _, b := range bands {
		if !strings.Contains(b.Source, "Tier table:") {
	...
	if r := targetTierRatio(); r[0].Ratio != 10000 || r[1].Ratio != 100 || r[2].Ratio != 1 {
		t.Fatalf("target ratio = %+v, want 10000:100:1", r)
	}
}

// TestConcentrationCarriesC2WhenAChainExists closes the read-through half of row 12
...
func TestConcentrationCarriesC2WhenAChainExists(t *testing.T) {
