...
func cmdDaemon(args []string) error {
	...
	deliveryIdle := fs.Duration("delivery-idle-window", 0, "R2.9 paid delivery sessions (G-R212-8 certification 2026-09-06): close a session idle for this long since its LAST settlement, measured on this node's monotonic clock, and account its unsettled face remainder once (burned under G-6 as ratified). REFUSE-UNTIL-SET: no shipped default, because the value depends on the unmeasured block interval T_b (R-REAPER-FORFEIT) — a daemon with -accept-delivery-receipts must set it. A session is one durable fetcher's prepayment at this server (one demand-domain anchor = 50,000 credits = 12.21 GiB of delivery), spans objects, settles incrementally on cumulative-count receipts, and is topped up with fresh anchors")
	...
	if *acceptReceipts {
		...
		if *deliveryIdle <= 0 {
			return fmt.Errorf("-accept-delivery-receipts refused: set -delivery-idle-window (R2.9 paid delivery sessions close on idleness measured from the last settlement; the window is refuse-until-set until the block interval T_b is measured — R-REAPER-FORFEIT)")
		}
		...
		if need, have, ok := grantFundsThePinInWholeFaces(ledger.Grant(), ledger.Fee(), relaypay.RelayIncrementBytes/relaypay.RelayIncrementCredit); !ok {
			return fmt.Errorf("priced delivery refused: the 64 GiB grant/r pin needs %d whole anchor faces across delivery + relay but one grant (%d credits) holds only %d faces of %d — re-derive the prices or the face before enabling the lane (G-λ-8-2, G-R212-8)", need, ledger.Grant(), have, ledger.Fee())
		}
	}
