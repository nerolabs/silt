...
func cmdDaemon(args []string) error {
	...
	if *acceptReceipts {
		...
		if *deliveryIdle < deliveryIdleFloor {
			return fmt.Errorf("-accept-delivery-receipts: refusing to start — set -delivery-idle-window to at least %s (R2.9 paid delivery sessions close on idleness measured from the last settlement; the window is refuse-until-set until the block interval T_b is measured — R-REAPER-FORFEIT; got %s)", deliveryIdleFloor, *deliveryIdle)
		}
