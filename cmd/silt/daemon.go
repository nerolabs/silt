...
func cmdDaemon(args []string) error {
	...
	maxEntryBytes := fs.Int64("max-entry-bytes-per-block", 64<<10, "byte budget for mempool publish ENTRIES folded into ONE block (#441) — SEPARATE from -max-bondreg-bytes-per-block by design: a single ~1.5 MB bond reg fills the whole reg cap, so a shared budget would leave the tens-of-bytes entry no room (the publish starvation one layer down), and the dual — an entry flood must never crowd out consensus-critical renewals. Each stream is guaranteed its own slice; their SUM must stay WAN-gatherable (#286 L2b). At least one entry always folds. 0 = unbounded BOUND TO CONSENSUS (2026-09-09, owner call 'close the route'): this budget is an input to the derivation of chain.SlashesBytesCap, a validity rule every validator enforces, so the daemon REFUSES to start when 2*(bondreg budget + entry budget + overhead) exceeds that cap -- past the boundary a REAL double-signer's evidence no longer fits and the equivocator keeps its seat. 0 (unbounded) is therefore refused too: an unbounded body defeats the invariant at any cap.")
	...
	// cmd/silt TestG_SLASHCAP_3_DaemonRefusesBudgetsThatDefeatSlashEvidence drives this
