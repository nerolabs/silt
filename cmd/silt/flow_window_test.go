...
// /.
//
// THE DEFECT, measured on the real fixture: pooled differenced two whole-sample
...
func TestPooledFlowIsTheSumOfTheRowsBeneathIt(t *testing.T) {
	...
	t.Run("entry", func(t *testing.T) {
		...
		// This is the measured case, in its own numbers.
		entry := b1Ring(
		...
		if pooled.Net != sum {
			t.Fatalf("pooled net %d but the rows beneath it sum to %d. A root cared for mid-window put its whole PRE-window lifetime inflow into one window (a review B1). A pooled figure that is not the sum of the rows printed beneath it is a lie whatever the arithmetic behind it", pooled.Net, sum)
		}
	...
	t.Run("departure", func(t *testing.T) {
		...
		// So the old pooled arithmetic saw a large negative step that no row
		// explains. Driven here because the node has no uncare API.
		departure := b1Ring(
...
// TestPooledFlowOverTheRealCarePathOnAMidWindowEntry is arm 1 end to end: the real
...
// the window — exactly the measured shape.
func TestPooledFlowOverTheRealCarePathOnAMidWindowEntry(t *testing.T) {
