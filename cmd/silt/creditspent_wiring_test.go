...
// creditSpent durability, the gate: the DAEMON wires the credit-spent store
// beside the paid-serial store — same store dir, a DISTINCT file (creditspent.log),
// attached then loaded, refuse-to-start on a load error.; deliberation
//
// A SOURCE gate, in the laneoff_test.go style: the wiring sits inside
...
func TestDaemonWiresTheCreditSpentStoreBesideThePaidSerialStore(t *testing.T) {
	...
	if iOpenCS < 0 {
		t.Fatal("SOURCE GATE: the literal `" + openCS + "` is absent from daemon.go. Checked: " +
			...
			"every held credit again, a review) is observed by core/node " +
