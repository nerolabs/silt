...
// TestShippedSlashCapFlagDefaultsAndTheRefusalText drives the daemon's OWN flag default CONSTANTS through the predicate, so
// that changing a default in daemon.go changes this result. It does NOT boot the daemon: the wiring and order are covered by the
// source gate below, and core/node this gate drives the boundary itself. Named for what it checks — the shipped defaults and the
// operator-facing refusal text — rather than for a daemon run it does not perform.
func TestShippedSlashCapFlagDefaultsAndTheRefusalText(t *testing.T) {
	const mib = 1 << 20

	// The daemon's ACTUAL flag defaults, by the same constants cmdDaemon passes to
	// fs.Int64 — not a literal restated here. Moving a default in daemon.go therefore
	// moves this assertion, instead of leaving a green test beside a binary that will
	// not start.
	shipped := node.Config{
	...
	// The operator route the project ordered closed: raise the local bond-reg
	// budget and a real double-signer's evidence no longer fits the consensus cap.
...
//
// RUNTIME GATE: TestShippedSlashCapFlagDefaultsAndTheRefusalText (above)
// observes the refusal itself, and core/node TestConfigCannotDefeatSlashEvidence
...
func TestTheSlashCapRefusalIsWiredAtStartup_Source(t *testing.T) {
	...
	if call < 0 {
		t.Fatal("SOURCE GATE: the string \"node.CheckSlashEvidenceHeadroom(cfg)\" is absent from daemon.go, so the consensus cap's derivation " +
			"is once again unbound to the configuration in force, which is the route the project ordered closed")
	}
