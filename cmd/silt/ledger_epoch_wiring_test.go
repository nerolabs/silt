...
// The RUNTIME half: "the daemon wires the ledger's source to the node's
// chainEpoch. never to a constant or a clock. After boot, advancing the chain by one epoch advances ledger.Epoch."
//
...
func TestLedgerEpochFollowsTheChainThroughTheDaemonSeam(t *testing.T) {
	const EB = 2
	sched := simclock.New()
	...
	for epoch := uint64(1); epoch <= 2; epoch++ {
		...
		if got := ledger.Epoch(); got != epoch {
			t.Fatalf("the chain advanced to epoch %d but ledger.Epoch() = %d — the ledger's source is not the "+
				"node's chainEpoch (a constant, a clock, or nothing:)", epoch, got)
		}
	}
}
