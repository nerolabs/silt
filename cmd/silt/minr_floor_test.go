...
// The WIRE tier of the minimum-requester floor and the polling oracle.
...
// --- the floor, on the wire ---------------------------------------------------

// TestWirePayloadWithholdsTheCensusBelowTheFloor is the gate on the artifact a
...
func TestWirePayloadWithholdsTheCensusBelowTheFloor(t *testing.T) {
...
//
// This is the measured refutation, mechanised. Against a polled series the counts-only
// argument fails at ANY R — R governs ATTRIBUTION, not extraction — so the oracle is
// deliberately blind to R and reads only the deltas.
...
// TestPolledSeriesRevealsNoSingleIdentityTrajectory is the gate: the refutation,
// encoded as a permanent gate.
//
// THE MECHANISM IT GUARDS. The prior research claimed "deltas are cell counts; nothing to
// match". Measured, that is false: six polls of ONE requester produced the bin sequence
// 80 → 84 → 86 → 88 → 89 → 90, a per-identity byte trajectory read straight off a
// counts-only histogram. The floor does not make that pattern unextractable — nothing in
// this instrument can, and the trajectory leak stays OPEN and bounded by poll rate — but
// at a degenerate anonymity set it withholds the grid entirely, so the singleton's
// trajectory is not published at all.
//
// THE POSITIVE CONTROL IS IN THE TEST, AND IT MOVED — it is now stronger and it is on
// the wire. The raw census no longer leaves core/credit, so the control can no
...
// With the floor lifted, the one moving identity's trajectory is readable straight off
// the PUBLISHED series. That is the measured refutation, encoded: it proves
// the oracle can fire on this exact wire path, and it records in a permanent gate that
// the floor is bought out by a fetching adversary.
func TestPolledSeriesRevealsNoSingleIdentityTrajectory(t *testing.T) {
	s, led, clk := r29aServer(t, true)
	...
	// each time (1, 2, 4, 8, 16, 32 MiB), so on the one-bin-per-doubling axis every
	// fetch crosses exactly one bin edge — the regime in which a polled series
	...
	// each is the whole price, so the pad is a fixture in eight
	...
	if got := bbTrajectorySteps(wire); len(got) != 0 {
		t.Fatalf("the polled wire sequence discloses %d single-identity bin transitions %v: one requester's byte trajectory is readable off the published cell deltas. The minimum-requester floor must withhold the grid at a degenerate anonymity set", len(got), got)
	}
	...
	t.Logf("at one requester the published series discloses 0 single-identity bin transitions and is byte-identical across all six polls; with the census padded over the floor by %d identities the SAME published path discloses %d — the floor is bought out by an observer that can fetch ",
