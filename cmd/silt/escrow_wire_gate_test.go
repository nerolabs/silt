...
// the wire gate on withheldEconomySelf's revenue rebuild.
//
// WHY THIS IS A SIBLING AND NOT AN EDIT TO THE EXISTING WHOLE-SURFACE SCAN.
// TestNoUnauthenticatedResponseOnTheWholeSurfaceCarriesTheWithheldCounter scans
...
//
// Do not read the split as forced; it is a CHOICE, and the measurement is the
...
// single-scan fix WAS available — is node-wide while the per-object walk covers
...
//
// ABLATION: reverting withheldEconomySelf's rebuild to `rev:= full.Revenue`
// must redden this, with the leaked credits figure in the text.
...
func TestPriorFetcherCreditsAreTokenGatedOnTheWholeSurface(t *testing.T) {
	...
	s.onLoop(func() {
		...
		// round-trip shape exists to make visible.
...
// r27Revenue is the slice of the revenue block. Decoded on its own so a change to
