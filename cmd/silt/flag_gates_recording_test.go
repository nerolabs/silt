...
// Second half — INSIDE a tagged build, -bbootstrap gates the
// RECORDING, not just the publication.
//
...
// TestTheFlagGatesTheRecordingNotJustThePublication is the gate.
func TestTheFlagGatesTheRecordingNotJustThePublication(t *testing.T) {
	...
	// aged AND unstamped are both ZERO here, and that is the DEAD-CLOCK payload, not
	// a third failure: with no clock the snapshot counts a requester in the census
	// and places it in NEITHER age counter (pins exactly this shape). `unstamped` is
	// the different, LIVE-clock reading — "the age axis works and this account
	// predates it" — which is what the mid-life flip in the next test produces.
	// Asserting the wrong one of those two is how this gate would go vacuous, so
	// both are pinned.
	if h.Aged != 0 || h.Unstamped != 0 {
		t.Fatalf("arm A: aged = %d, unstamped = %d, want 0 / 0. With -bbootstrap unset no observability clock was injected, so Register wrote no first-touch time, there is no `when` on any identity, and the census is age-free rather than age-zero ", h.Aged, h.Unstamped)
	}
	...
	if h2.MonotonicSource != "injected" {
		t.Fatalf("arm B: monotonicSource = %q, want \"injected\" — bbootstrapInject must still wire BOTH sources in one call ", h2.MonotonicSource)
	}
...
// TestFlippingTheFlagOnDoesNotRecoverThePastIsTheACCEPTEDCOST states the trade in a
...
// So the operator's real recovery path is a restart plus a wait, which is what accepts.
func TestFlippingTheFlagOnDoesNotRecoverThePastIsTheACCEPTEDCOST(t *testing.T) {
...
func bbRawStatus(t *testing.T, s *uiServer) string {
	t.Helper()
	// Reads AS THE OPERATOR: the block is served only to the Authorization- header
	// token. Arm B below is the positive control that the block reaches the wire when
	// the flag is on; read untokened it would observe the withheld marker and call it
	// a pass.
