...
func TestLambdaHRateOverWindow(t *testing.T) {
	...
	// A climbs 0→1→2→3→4→5 over heights 1.6: 5 distinct arrivals across 5 heights =
	// 1.0.
...
func TestLambdaHWindowEvictsOldSamples(t *testing.T) {
	...
	// Heights 1.10, A climbing by 1 each. With a width-3 window the rate is still
	// 1.0 but measured over the recent span, not the whole history.
	for i := 1; i <= 10; i++ {
