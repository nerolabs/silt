...
// receiptCoverage 0 is ambiguous three ways, and the third is the DEFAULT
// posture (the research as-built(a)).
//
...
// C6 makes coverage < 0.75 a machine-read canary abort, and the RC ships default-OFF, so
// without a lane-state field EVERY honest RC-default node trips it. The shape is the one
// economySelfFunding.bountyOn already uses for -economy.
//
// ABLATION run RED before this shipped: pinning LaneOn true (the
