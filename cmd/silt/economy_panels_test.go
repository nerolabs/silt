...
// rows 14-17 — the four panels, and the honesty rules they must render.
//
...
// So the panels are PURE functions run here under node against fixtures, exactly as the
// status cards are.
//
...
func TestEconomyPanelsRenderTheHonestyRules(t *testing.T) {
...
// B2: a sample that summed to zero. The wire sends known:false and NO value, so the cell
// is driven exactly as the endpoint would send it — and, separately, with an explicit 0
// present, because a consumer must branch on known and never on the number (a value of 0
// is falsy, which is how the old bare-number signature shipped this).
...
// B3 / decision: on the shipped -privacy default the two Gini figures are a NAMED
// absence. The block still carries its note, and the cell must not fall through to "no
// sample", which would read as a node that knows nobody.
...
// The PER-TIER work cells. A tier that reported nothing must NOT render 0 %: its share
// computes to a well-formed 0.0 because the exclusion rule keeps it out of the numerator
// while other tiers hold the denominator up, and 0 % on the panel says the tier does none
// of the work.
...
console.log(JSON.stringify(out));`
	...
	// B4: and its EPOCH. The counters reset at every restart, so a
	...
	// And a REAL measured 0 (nine nodes that all did identical work) still
	// renders as a number, or the branch above is just a blanket refusal to
	// publish zeros.
	...
	// THE PER-TIER CELLS. The rule is the SAME one as the Gini's and it has a second
	// edge here: a tier's share of the work is 0.0 both when the tier did nothing and
	// when the tier SAID nothing, and on the shipped -privacy default it is always the
	// second.
	//
	// TWO CONTROLLED REVERTS. make tierShareCell format an UNKNOWN share as a
	// percentage — measured, the silent tier renders "0.0%%". stop edgeShareCell
	// delegating to gossipCell — measured, a privacy-withheld document renders `{Text:—
	// Sub:no sample Withheld:false}`, which reads as a node that knows no peers rather
	// than as a withhold, and that is the shape that made the delegation the design
	...
	if out.TierUnknown.Text != "not reported" || !out.TierUnknown.Unknown {
		t.Fatalf("a tier with no reporting peer renders %q. 0 %% would say the tier does none of the work; the truth is that no peer of that tier said anything, and under the exclusion rule a silent peer is out of the numerator AND the denominator", out.TierUnknown.Text)
	}
	...
	if !out.EdgeUnknown.Unknown || out.EdgeUnknown.Text == "0.0%" {
		t.Fatalf("an UNKNOWN edge share renders %+v. This is the reading on every node running the shipped -privacy default, so rendering it as 0 %% would tell every operator the edge tier does none of the work", out.EdgeUnknown)
	}
