...
THE DEFECT
  ...
  of the same repository. A machine that keeps a dozen worktrees live at once then gets
  hundreds of findings that are other branches' files, judged against this branch's
  workflows and allowlists. The verdict is meaningless, and a lint that reports hundreds
  of false problems is a lint nobody runs — an unrun lint is not a gate.

  A CI checkout has no worktrees, so the lints are green there and stay green. The whole
  cost is local, and it is the cost that matters.

THE RULE, AND WHY IT IS NOT A PATH
  Excluding one directory name would leave the class open — a clone dropped anywhere
  under the root reopens it, and the next lint written would have to remember the same
  path by hand. That is how this defect exists at all: two skip-lists that disagreed
  about one directory name.
...
def repo_files(root, skip_dirs=(), suffix=None):
    ...
    a gitignored symlink cannot pull an out-of-tree store into the
...
def _self_test() -> int:
    ...
    if failures:
        print(f"FAIL — the shared repo walk is wrong:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        return 1
    print(f"OK — the shared walk skips a nested worktree and a nested clone, "
