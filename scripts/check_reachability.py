...
THE ONE CAVEAT, AND WHY THIS GATE REFUSES THIN SYMBOLS
  ...
  So an entry must name a SUBSTANTIAL symbol, and this gate enforces that mechanically
  against the declaration in source:
    - the body is at least MIN_BODY_LINES real lines (far past the inliner's cost
      budget), AND
    - the body contains a function literal, which surfaces in the symbol table as
      `<symbol>.funcN`.
  When the symbol is PRESENT, the gate additionally requires that `.funcN` witness, so
  a hollowed-out stub that kept its name cannot pass.

  DO NOT WEAKEN THIS. If a lane has no substantial client entry point, that is a finding
  about the lane, not a reason to point the entry at a wrapper.
...
EVERY ENTRY CARRIES A WRITTEN REASON, AND EVERY REASON HAS A MECHANICAL COMPANION
  ...
  against the measured body. A record missing either field FAILS.
...
# A body this long is far past the inliner's cost budget, so its ABSENCE from the
# symbol table means the linker dropped it, not that it was inlined into a caller.
MIN_BODY_LINES = 8
...
def find_declaration(pkg_dir, recv, fname):
    ...
    pkg_dir. Returns (relpath, line_no, (body_raw, body_mask)) or None."""
    if recv:
    ...
    for path in sorted(pkg_dir.glob("*.go")):
        ...
        while i < len(mask):
            ...
            return None
        # Brace-match the body on the mask; slice the same span from the real source.
        bdepth, j = 0, i
        while j < len(mask):
            if mask[j] == "{":
                bdepth += 1
        ...
        return rel, src[:m.start()].count("\n") + 1, (src[i + 1:j], mask[i + 1:j])
    return None


CLOSURE_RE = re.compile(r"\bfunc\s*\(")
DELIMS_ONLY = re.compile(r"^[)}\],;]*$")


def measure_body(body_mask):
    """(real line count, has a function literal). Blank, comment-only and
    closing-delimiter-only lines do not count as body."""
    lines = 0
    for ln in body_mask.splitlines():
        s = ln.strip()
        if not s or DELIMS_ONLY.match(s):
            continue
        lines += 1
    return lines, bool(CLOSURE_RE.search(body_mask))


# --------------------------------------------------------------------------- binary

def build_binary(out_path):
    """Build ./cmd/silt. On the shared dev box every heavy `go` command runs under the
    background QoS class (nice alone does not throttle on macOS); CI has no taskpolicy
    and wants full speed."""
    env = dict(os.environ)
    cmd = ["go", "build", "-o", str(out_path), MAIN_PKG]
...
def check_substantial(records, lanes_path, mod):
    """Refuse any entry pointing at a symbol thin enough to be inlined away. Returns
    (declarations, errors)."""
    decls, errors = {}, []
    for line_no, rec in records:
        where = f"{lanes_path.name}:{line_no}"
        ...
        if not found:
            ...
            continue
        rel, dline, (_, bmask) = found
        nlines, has_closure = measure_body(bmask)
        if nlines < MIN_BODY_LINES or not has_closure:
            errors.append(
                f"{where}: REFUSING this entry — {sym} is not a SUBSTANTIAL symbol "
                f"({rel}:{dline}: {nlines} body line(s), closure={has_closure}; need "
                f">= {MIN_BODY_LINES} lines AND a function literal).\n"
                f"    A thin symbol can be INLINED AWAY, so its absence from the symbol "
                f"table proves nothing — the gate would report a live lane as inert.\n"
                f"    Name the substantial client entry point for this lane instead. Do "
                f"NOT lower the threshold: that is how this gate stops meaning anything.")
            continue
        decls[(rec["lane"], sym)] = (rel, dline)
    return decls, errors


def check_binary(records, lanes_path, decls, syms, checklist, checklist_name):
    ...
    for line_no, rec in records:
        ...
        if label_err:
            errors.append(
                ...
                f"    (symbol {sym} is "
                f"{'PRESENT in' if sym in syms else 'ABSENT from'} the binary.)")
            continue
        ...
        if sym in syms:
            if not any(s.startswith(sym + ".func") for s in syms):
                errors.append(
                    f"{where}: {lane}: {sym} is in the binary but NONE of its `.funcN` "
                    f"closures are ({rel}:{dline} declares one).\n"
                    f"    The name survived without the body this gate is about. Confirm "
                    f"the symbol still carries the mechanism.")
                continue
            if says_unreachable or ambiguous_about_symbol:
                errors.append(
                    ...
                    f"(with its `.funcN` witness). Re-label to whatever is now true "
...
def main():
    ...
    print(f"OK [{SCAR_ID}] — {len(records)} lane entr(ies) checked against the linked "
          f"{MAIN_PKG} binary: {present} present (each with a `.funcN` witness), "
