...
//
// The state-root makes this the gate on the hot path:
//
...
//	is the same discipline that closed the earlier one.
//
// The defect it exists to stop is specific and already paid for once: was
...
// run-to-run variance (measured on the floor box —), so a timing budget would
// be either too loose to catch a regression or flaky enough to be disabled.
// Digest COUNT is exact, deterministic, and machine-independent: the same
// assertion holds on a laptop and on a 1 vCPU box. smt's digestData is
...
// digestBudget is the explicit budget the research asks for.
//
...
func TestIncrementalCostIsChangedTimesLogN(t *testing.T) {
	...
	for i, n := range scales {
		...
		if got > budget {
			t.Errorf("n=%d: %d digests exceeds the budget of %d for %d changed keys.\n"+
				"The update is not O(changed·log n) — this is the scar "+
	...
	if growth > maxGrowth {
		t.Errorf("digest count grew %.2fx while state grew 100x (limit %.1fx).\n"+
			...
			"CHANGED keys — the O(state) failure the rule Q4 forbids.",
			growth, maxGrowth)
	}
}
...
// what did — rebuild the whole tree per block — and asserts it fails the
// same budget the incremental path passes.
func TestFullRecomputeBlowsTheBudget(t *testing.T) {
	const n, changed = 10_000, 64

	// The forbidden shape: rebuild every key, as a per-block
	// Hash-the-world implementation would.
	var counter int
	...
	if counter <= budget {
		t.Fatalf("a full-tree recompute (%d digests) fits inside the per-block "+
			"budget (%d). The budget is too loose to catch the scar, so the "+
